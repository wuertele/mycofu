...
module "dns1" {
  ...
  replication_policy_on = var.dns1_replication_policy_on

  ssh_host_key_private = var.dns1_ssh_host_key_private
  ssh_host_key_public  = var.dns1_ssh_host_key_public
...
module "dns2" {
  ...
  start_vms             = var.start_vms
  register_ha           = var.register_ha
  replication_policy_on = var.dns2_replication_policy_on

  ssh_host_key_private = var.dns2_ssh_host_key_private
  ssh_host_key_public  = var.dns2_ssh_host_key_public
...
# --- DNS pair anti-affinity (preferential negative resource-affinity; PVE 9 harule) ---
# Sprint 045 / #513. The dns-pair boundary owns the dns1/dns2 relation: keep the
# two authoritative DNS VMs on different nodes when >=2 eligible nodes are
# healthy; strict=false yields to co-location under single-survivor (availability
# beats separation - UC7). One rule per pair, referencing both resources.
# Created only when HA is registered (register_ha).
resource "proxmox_virtual_environment_harule" "dns_antiaffinity" {
  count = var.register_ha ? 1 : 0

  rule      = "dns-${var.environment}-antiaffinity"
  type      = "resource-affinity"
  affinity  = "negative"
  resources = ["vm:${var.dns1_vm_id}", "vm:${var.dns2_vm_id}"]
  strict    = false
  comment   = "Managed by Mycofu (Sprint 045) - dns pair anti-affinity (preferential)"

  depends_on = [module.dns1, module.dns2]
}
