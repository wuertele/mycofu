...
variable "replication_policy_on" {
  description = "See framework/tofu/modules/proxmox-vm/variables.tf — Sprint 047 #691 interim HA-retry damage limiter."
  type        = bool
  default     = null
}
...
variable "ram_floating_mb" {
  description = "Balloon floor in megabytes. Null keeps ballooning disabled."
  type        = number
  default     = null
  nullable    = true
}
...
variable "root_disk_gb" {
  description = "Root disk size in GB for the runner /nix build scratch space"
  type        = number
  default     = 256
}
...
variable "tailscale_auth_key" {
  description = "Tailscale pre-auth key (empty = no Tailscale). When non-empty, delivered via CIDATA to /run/secrets/tailscale/auth-key and triggers tailscale-join."
  type        = string
  sensitive   = true
  default     = ""
}
