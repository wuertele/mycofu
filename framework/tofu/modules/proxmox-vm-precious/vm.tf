# --- VM (precious state variant) ---
#
# Identical to proxmox-vm/vm.tf EXCEPT: prevent_destroy = true.
# This prevents accidental destruction of VMs with irreplaceable data on vdb.
# To intentionally recreate: tofu apply -replace=<resource address>
#
# Used by wrappers that explicitly opt into prevent_destroy. Sprint 022 keeps
# that set unchanged because widening it changes lifecycle semantics and DR
# expectations separately from the pool/tagging contract fixed here.
# See report-prevent-destroy-analysis.md for design rationale.
...
resource "proxmox_virtual_environment_vm" "vm" {
  ...
  lifecycle {
    # PRECIOUS STATE: prevent accidental destruction.
    # VMs using this module have irreplaceable data on vdb (Vault Raft,
    # GitLab repos, InfluxDB time-series, Roon library). Destroying the
    # VM destroys the vdb. To intentionally recreate, the operator must
    # use: tofu apply -replace=<resource>
    ...
    ignore_changes = [
      ...
      # recreation when HA moves VMs to a different node.
      initialization,
    ]
    # Force VM recreation when CIDATA content changes.
    replace_triggered_by = [
      terraform_data.cidata_hash[0],
    ]
  }
}
