...
variable "replication_policy_on" {
  description = <<-EOT
    Whether this VM is policy-on for ZFS replication (Sprint 047).

    Consumed by ha.tf as the interim #691 damage limiter:
      - true (policy-on: has a replicated peer zvol on ≥1 other node) →
        max_restart/max_relocate left unset; PVE defaults apply.
      - false (policy-off: recreate-on-failover) → max_restart=0 and
        max_relocate=0 so a failed-start does not burn four 300s zvol-link
        waits (see docs/reports/rca-2026-07-20-drt005-policyoff-start-hang.md).
      - null (unset by caller) → treated as policy-on; existing haresources
        that predate this variable produce a ZERO plan diff.

    Callers MUST derive this from the same authority as
    framework/scripts/list-replicated-vmids.sh (Sprint 047 V5.1 single
    authority). framework/tofu/root/main.tf routes through the helper via
    data.external so the HCL never reads the replicate field from YAML
    directly.

    Removed by FI-2 (Failover Manifest sprint) when policy-off VMs stop
    registering HA at all — keep changes to this variable minimal.
  EOT
  type        = bool
  default     = null
}
