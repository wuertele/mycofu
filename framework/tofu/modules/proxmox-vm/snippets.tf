...
locals {
  ...
  all_write_files = concat(
    ...
    var.ssh_host_key_private != "" ? [
      {
        path        = "/run/secrets/ssh/ssh_host_ed25519_key"
        content     = var.ssh_host_key_private
        permissions = "0600"
      },
      {
        path        = "/run/secrets/ssh/ssh_host_ed25519_key.pub"
        content     = var.ssh_host_key_public
        permissions = "0644"
      }
    ] : [],
    var.write_files
