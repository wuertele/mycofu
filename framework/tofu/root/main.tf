...
resource "terraform_data" "precious_sizing_roon" {
  ...
  lifecycle {
    precondition {
      ...
      error_message = "Precious-state app 'roon' is enabled with backup: true but 'ram' is not set in site/applications.yaml. Remove backup: true or set applications.roon.ram explicitly. A silent catalog default would change the deployed RAM size on a fresh deploy."
    }
    precondition {
