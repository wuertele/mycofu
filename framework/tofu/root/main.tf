...
module "vault_prod" {
  ...
  storage_pool         = local.config.proxmox.storage_pool
  vdb_size_gb          = 10
  vdb_restore_expected = try(local.config.vms.vault_prod.backup, false)
  pdns_api_key         = var.pdns_api_key
  acme_server_url      = local.prod_acme_url
  extra_ca_cert        = ""
...
module "vault_dev" {
  ...
  storage_pool         = local.config.proxmox.storage_pool
  vdb_size_gb          = 10
  vdb_restore_expected = try(local.config.vms.vault_dev.backup, false)
  pdns_api_key         = var.pdns_api_key
...
module "gitlab" {
  ...
  storage_pool         = local.config.proxmox.storage_pool
  vdb_size_gb          = 50
  vdb_restore_expected = try(local.config.vms.gitlab.backup, false)
...
module "testapp_dev" {
  ...
  storage_pool         = local.config.proxmox.storage_pool
  pdns_api_key         = var.pdns_api_key
  vdb_restore_expected = try(local.config.vms.testapp_dev.backup, false)
  acme_server_url      = local.dev_acme_url
  extra_ca_cert        = local.dev_extra_ca_cert
  pdns_api_servers     = "${local.config.vms.dns1_dev.ip}\n${local.config.vms.dns2_dev.ip}\n"
  domain               = local.domain
...
module "testapp_prod" {
  ...
  storage_pool         = local.config.proxmox.storage_pool
  pdns_api_key         = var.pdns_api_key
  vdb_restore_expected = try(local.config.vms.testapp_prod.backup, false)
  acme_server_url      = local.prod_acme_url
  extra_ca_cert        = ""
  pdns_api_servers     = "${local.config.vms.dns1_prod.ip}\n${local.config.vms.dns2_prod.ip}\n"
  domain               = local.domain
...
module "influxdb_dev" {
  ...
  vdb_restore_expected = try(local.app_influxdb.backup, false)
  pdns_api_key         = var.pdns_api_key
  acme_server_url      = local.dev_acme_url
  extra_ca_cert        = local.dev_extra_ca_cert
  pdns_api_servers     = "${local.config.vms.dns1_dev.ip}\n${local.config.vms.dns2_dev.ip}\n"
...
module "grafana_dev" {
  ...
  vdb_restore_expected   = try(local.app_grafana.backup, false)
  pdns_api_key           = var.pdns_api_key
  acme_server_url        = local.dev_acme_url
  extra_ca_cert          = local.dev_extra_ca_cert
  pdns_api_servers       = "${local.config.vms.dns1_dev.ip}\n${local.config.vms.dns2_dev.ip}\n"
...
module "roon_dev" {
  ...
  vdb_restore_expected = try(local.app_roon.backup, false)
  domain               = local.domain
  extra_ca_cert        = local.dev_extra_ca_cert
