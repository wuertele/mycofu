...
module "gatus" {
  ...
  # site/gatus/config.yaml is a generated artifact (generate-gatus-config.sh),
  # produced in-path by CI (build:merge), rebuild-cluster.sh, and safe-apply.sh
  # before plan/apply. The fileexists() guard keeps `tofu validate` green in the
  # pre-build validate stage where the artifact is absent (a bare file() errors
  # at validate time). Fail-closed is enforced at PLAN time by the gatus module's
  # terraform_data.config_guard precondition, which rejects an empty / zero-
  # endpoint config so a missing generation step can never ship empty prod
  # monitoring (#690). Do NOT "simplify" this to file() alone — that reddens the
  # validate stage — nor drop the precondition — that reopens the fail-open gap.
...
module "roon_dev" {
  ...
  ram_mb                = coalesce(try(local.app_roon.ram_dev, null), try(local.app_roon.ram, null), 4096)
...
module "roon_prod" {
  ...
  ram_mb                = coalesce(try(local.app_roon.ram_prod, null), try(local.app_roon.ram, null), 4096)
  vda_size_gb           = coalesce(try(local.app_roon.disk_size, null), 4)
  vdb_size_gb           = coalesce(try(local.app_roon.data_disk_size, null), 50)
  start_vms             = var.start_vms
  register_ha           = var.register_ha
...
# `cpus_dev` / `cpus_prod` / `data_disk_size_dev` / `data_disk_size_prod`)
# with a fallback to the base field (`ram` / `cores` / `data_disk_size`);
# the guards accept EITHER form
# per environment, matching the coalesce chain in the workstation
# module blocks above. `roon` uses the same env-suffixed convention for
# RAM alone (`ram_dev` / `ram_prod`, base fallback `ram`) since #1007
# gave roon-prod 12 GB while roon-dev stayed at 8 GB; its `disk_size`
# and `data_disk_size` remain single-valued and are guarded as such.
# On the workstation guard, `disk_size` (which
...
resource "terraform_data" "precious_sizing_roon" {
  ...
  lifecycle {
    # roon uses env-suffixed RAM with a base-field fallback (see the coalesce
    # chains in module.roon_dev / roon_prod above). The preconditions mirror
    # that chain: at least one non-null value must be reachable per
    # environment.
    precondition {
      condition     = try(local.app_roon.ram_dev, null) != null || try(local.app_roon.ram, null) != null
      error_message = "Precious-state app 'roon' is enabled with backup: true but neither 'ram_dev' nor 'ram' is set in site/applications.yaml. Set applications.roon.ram_dev (env-specific) or applications.roon.ram (base fallback) explicitly. A silent catalog default would change the deployed RAM size on a fresh deploy."
    }
    precondition {
      condition     = try(local.app_roon.ram_prod, null) != null || try(local.app_roon.ram, null) != null
      error_message = "Precious-state app 'roon' is enabled with backup: true but neither 'ram_prod' nor 'ram' is set in site/applications.yaml. Set applications.roon.ram_prod (env-specific) or applications.roon.ram (base fallback) explicitly. A silent catalog default would change the deployed RAM size on a fresh deploy."
    }
    precondition {
...
resource "terraform_data" "precious_sizing_workstation" {
  ...
  lifecycle {
    ...
    precondition {
      condition     = try(local.app_workstation.cpus_dev, null) != null || try(local.app_workstation.cores, null) != null
      error_message = "Precious-state app 'workstation' is enabled with backup: true but neither 'cpus_dev' nor 'cores' is set in site/applications.yaml. Set applications.workstation.cpus_dev (env-specific) or applications.workstation.cores (base fallback) explicitly."
    }
    precondition {
      condition     = try(local.app_workstation.cpus_prod, null) != null || try(local.app_workstation.cores, null) != null
      error_message = "Precious-state app 'workstation' is enabled with backup: true but neither 'cpus_prod' nor 'cores' is set in site/applications.yaml. Set applications.workstation.cpus_prod (env-specific) or applications.workstation.cores (base fallback) explicitly."
    }
    precondition {
    ...
    precondition {
      ...
      error_message = "Precious-state app 'workstation' is enabled with backup: true but neither 'data_disk_size_prod' nor 'data_disk_size' is set in site/applications.yaml. Set applications.workstation.data_disk_size_prod (env-specific) or applications.workstation.data_disk_size (base fallback) explicitly — a silent catalog default would allocate a smaller vdb than intended on a fresh deploy."
    }
  }
}
