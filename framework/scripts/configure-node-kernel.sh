...
if [[ $VERIFY_ONLY -eq 0 && $reboot_needed_count -gt 0 && $overall_rc -eq 0 ]]; then
  ...
  echo "    HA-aware rolling reboot procedure. Run the guarded wrapper from"
  echo "    this operator workstation, once per node:"
  echo "      framework/scripts/reboot-node-rolling.sh <target-node> [<surviving-node>]"
  echo "    The wrapper picks or uses a reachable+quorate surviving node,"
  echo "    stages the cidata guard beside the inner script, and runs the"
  echo "    Step-a0/Step-z guarded maintenance flow."
