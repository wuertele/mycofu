...
# --- Run tofu ---
# PVE gives each per-storage flock acquisition a fixed 10 s deadline
# (PVE::Tools::lock_file_full, `$timeout = 10 if !$timeout`). The worst measured
# critical section was 6.322 s (pve02, 2026-09-06 00:39:14-20Z, one `zfs create`
# ioctl). N=1 is the only value contributing zero self-inflicted lock competition,
# reserving the full deadline for storage work outside this apply; per-minute
# replication zvol create/destroy was active during the incident (#1013).
# N=2/N=3 arithmetic is only an illustrative lower bound on risk: one/two such
# holds consume ~6.3/~12.6 s. flock is not FIFO and one VM resource takes several
# acquisitions, so these measurements establish no positive margin for N>=2.
# Raising the escape hatch to N=2 on the 6.322 s figure is not justified. The
# provider maintainer recommends 1 or 2 for this symptom class:
# https://github.com/bpg/terraform-provider-proxmox/issues/186#issuecomment-1355741946
#
# apply/destroy are bounded because they mutate; read-only plan/refresh are not.
# Whether Phase 2 start/register takes this lock is unestablished: the in-repo
# storage-failure-fence.md §2 says `qm start` regenerates the cloud-init zvol,
# while adversarial review disputes that it reaches PVE::Storage::vdisk_alloc.
# Both safe-apply phases are `apply`, so the uniform bound is an accepted fail-safe
# simplification, not a proven requirement.
#
# Cost: UNMEASURED beyond the eight prod Tier-1 VM resources, and deliberately
# not estimated further. Those eight summed ~145 s (2026-09-06) / ~157 s
# (2026-08-30) under concurrency, versus a ~31 s concurrent create storm -- but
# they are only 8 of that Phase-1 plan's 33 operations (11 add / 2 change /
# 20 destroy), and this module emits 9 resources per VM (vm, ha, cidata_hash,
# plus user_data + meta_data for each of the 3 nodes), so a CIDATA-changing plan
# serializes up to 6 SSH snippet uploads per VM as well. rebuild-cluster.sh
# applies the whole fleet and is proportionally larger. Phase 2's cost is
# likewise unmeasured and is NOT lock-shaped: proxmox-vm/vm.tf:14-17 sets
# agent timeout = "2m", so every start carries a guest-agent wait that today
# overlaps and now sums.
#
# The operationally important part: Phase 2 is the phase that starts the DNS
# pair, so serializing it lengthens the window in which the environment has no
# DNS -- on EVERY deploy, not only failing ones. That trade is accepted to fail
# safe on the unestablished Phase-2 lock question above. Measure a real run and,
# if it hurts, bound only the allocation-bearing apply (#1017).
# As the universal choke point this also serializes safe-apply.sh dev phases 1/2,
# rebuild-cluster.sh applies (:2020,2201,2221,2269,2294), converge-cluster.sh:480,
# lib/converge-incomplete-vm.sh:143,156, and validate.sh:2186. Pass
# -parallelism=N explicitly to escape; OpenTofu has no per-resource-group control.
TOFU_APPLY_PARALLELISM=1
...
if [[ -f "$IMAGE_VERSIONS" ]] && [[ "$FIRST_ARG" == "plan" || "$FIRST_ARG" == "apply" || "$FIRST_ARG" == "destroy" || "$FIRST_ARG" == "refresh" ]]; then
  INJECTED_ARGS+=("-var-file=${IMAGE_VERSIONS}")
fi

HAS_EXPLICIT_PARALLELISM=0
for arg in "${TOFU_ARGS[@]}"; do
  case "$arg" in
    -parallelism=*|--parallelism=*|-parallelism|--parallelism) HAS_EXPLICIT_PARALLELISM=1 ;;
  esac
done
if [[ "$FIRST_ARG" == "apply" || "$FIRST_ARG" == "destroy" ]] && [[ "$HAS_EXPLICIT_PARALLELISM" -eq 0 ]]; then
  INJECTED_ARGS+=("-parallelism=${TOFU_APPLY_PARALLELISM}")
fi

exec "$TOFU_BIN" "${TOFU_ARGS[0]}" "${INJECTED_ARGS[@]+"${INJECTED_ARGS[@]}"}" "${TOFU_ARGS[@]:1}"
