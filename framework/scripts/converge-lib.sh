...
converge_target_envs() {
  # NOTE: hil_boot is a control-plane VM (vms.hil_boot.pool == control-plane,
  # enforced by validate-site-config.sh). It has no env-specific certs to
  # seed and no env-scoped state, so it belongs in recognized_envless
  # alongside cicd and pbs. Mirrors the bucketing fix in
  # framework/scripts/git-deploy-context.sh:260 (issue #429).
  local recognized_envless=(cicd pbs hil_boot)
