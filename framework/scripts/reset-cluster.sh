...
#   framework/scripts/reset-cluster.sh --recover --restore-pin-file build/pins.json
#
# Options:
#   --node <n>  Limit to a single node (e.g., --node pve01)
#   --restore-pin-file <path>  Pin file for --recover / rebuild handoff
#   --confirm   Execute (without this, dry-run only)
#
# Cluster track (cumulative):
#   --vms       Destroy VMs, HA, replication, tofu state
#               (CIDATA snippets are managed by tofu and intentionally
#               preserved — see do_reset_vms() rationale, #347)
#   --storage   + ZFS data pools, data NVMe partition tables
#   --test      + Verified backup, then full cluster destruction
#   --recover   + Pin validation, then full cluster destruction
...
RESET_RESTORE_PIN="${REPO_DIR}/build/restore-pin-reset.json"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --vms|--storage|--test|--recover|$LEGACY_CLUSTER_FLAG|--nas|--builds|--secrets|--distclean|--forensic|--level5)
      [[ -n "$LEVEL" ]] && { echo "ERROR: Only one level flag allowed" >&2; exit 1; }
      LEVEL="${1#--}"
      shift ;;
    --cold-builds) COLD_BUILDS=1; shift ;;
    --node) SINGLE_NODE="$2"; shift 2 ;;
    --restore-pin-file) RESTORE_PIN_FILE="$2"; shift 2 ;;
    $REMOVED_BACKUP_FLAG) REMOVED_BACKUP_REQUESTED=1; shift ;;
...
print_cluster_deprecation_error() {
  echo "ERROR: --cluster has been removed." >&2
  echo "Choose one of:" >&2
  echo "  --test    Healthy-cluster DR rehearsal. Takes a verified backup before destruction." >&2
  echo "  --recover Escape corrupt state. Skips backup and requires --restore-pin-file." >&2
}

if [[ -z "$LEVEL" ]]; then
  echo "Usage: $(basename "$0") --<level> [--confirm] [--node <name>] [--restore-pin-file <path>] [--cold-builds]" >&2
  echo "Levels: --vms --storage --test --recover --nas --builds --secrets --distclean --level5 --forensic" >&2
  exit 1
fi
...
if [[ "${REMOVED_BACKUP_REQUESTED:-0}" -eq 1 ]]; then
  echo "ERROR: --backup has been removed. Use framework/scripts/backup-now.sh directly, or use --test for a full cluster DR rehearsal." >&2
  exit 1
fi

if [[ "$LEVEL" == "recover" && -z "$RESTORE_PIN_FILE" ]]; then
  echo "ERROR: --recover requires --restore-pin-file <path>" >&2
  exit 1
fi

if [[ "$LEVEL" != "recover" && -n "$RESTORE_PIN_FILE" ]]; then
  echo "ERROR: --restore-pin-file is only valid with --recover" >&2
  exit 1
fi
...
pbs_content_json() {
  ...
  ssh_node "$node_ip" "pvesh get /nodes/\$(hostname)/storage/pbs-nas/content --output-format json 2>/dev/null"
}

collect_precious_state_vm_records() {
  local vm_key=""
  local app_key=""
  local env_key=""
  local vmid=""
  ...
  while IFS= read -r vm_key; do
    ...
    printf '%s\t%s\n' "$vm_key" "$vmid"
  done < <(yq -r '.vms | to_entries[] | select(.value.backup == true) | .key' "$CONFIG" 2>/dev/null)

  while IFS= read -r app_key; do
    [[ -z "$app_key" ]] && continue
    while IFS= read -r env_key; do
      [[ -z "$env_key" ]] && continue
      vmid=$(yq -r ".applications.${app_key}.environments.${env_key}.vmid // \"\"" "$APPS_CONFIG" 2>/dev/null)
      [[ -z "$vmid" || "$vmid" == "null" ]] && continue
      printf '%s\t%s\n' "${app_key}_${env_key}" "$vmid"
...
restore_pin_has_vmid() {
  local vmid="$1"

  jq -e --arg vmid "$vmid" '
    (.pins[$vmid] // "") |
    if type == "object" then ((.volid // "") != "") else . != "" end
  ' "$RESTORE_PIN_FILE" >/dev/null 2>&1
}

copy_restore_pin_to_reset_location() {
  mkdir -p "$(dirname "$RESET_RESTORE_PIN")"
  cp "$RESTORE_PIN_FILE" "$RESET_RESTORE_PIN"
  echo "Saved restore pins to ${RESET_RESTORE_PIN}"
}

run_test_preflight() {
  mkdir -p "$(dirname "$RESET_RESTORE_PIN")"

  echo "=== Taking verified backup before cluster destruction ==="
  if ! "${SCRIPT_DIR}/backup-now.sh" --env all --verify --pin-out "$RESET_RESTORE_PIN"; then
    echo "ERROR: backup-now.sh --verify failed. Aborting before destruction." >&2
    exit 1
  fi

  echo ""
  echo "=== Reset restore pins ==="
  jq . "$RESET_RESTORE_PIN"
  echo ""
}

run_recover_preflight() {
  local pbs_node=""
  local pbs_json=""
  local pin_entries=""
  local entry_rc=0
  ...
  if [[ ! -f "$RESTORE_PIN_FILE" ]]; then
    echo "ERROR: restore pin file not found: $RESTORE_PIN_FILE" >&2
    return 1
  fi
  ...
  pin_entries="$(jq -r '
    ...
    elif (.pins | type) != "object" then
      error("missing pins object")
    else
      .pins
      | to_entries[]?
      | if (.key | test("^[0-9]+$")) and (.value | type == "string") and (.value | length > 0) then
          "\(.key)\t\(.value)"
        elif (.key | test("^[0-9]+$")) and (.value | type == "object") and (.value.volid | type == "string") and (.value.volid | length > 0) then
          "\(.key)\t\(.value.volid)"
        else
          error("invalid pin entry")
        end
    end
  ' "$RESTORE_PIN_FILE" 2>&1)"
  entry_rc=$?
  set -e
  if [[ "$entry_rc" -ne 0 ]]; then
    echo "ERROR: invalid restore pin file: $RESTORE_PIN_FILE" >&2
    printf '%s\n' "$pin_entries" | sed 's/^/  /' >&2
    return 1
  fi

  if ! pbs_node="$(resolve_pbs_node)"; then
    echo "ERROR: PBS storage (pbs-nas) is not registered on any reachable node." >&2
    return 1
  fi

  if ! pbs_json="$(pbs_content_json "$pbs_node")"; then
    echo "ERROR: could not query PBS content from ${pbs_node}" >&2
    return 1
  fi

  echo "=== Validating restore pins against PBS ==="
  while IFS=$'\t' read -r vmid volid; do
    [[ -z "$vmid" || -z "$volid" ]] && continue

    if [[ "$volid" =~ /vm/([0-9]+)/ ]] && [[ "${BASH_REMATCH[1]}" != "$vmid" ]]; then
      echo "  FAILED: VMID ${vmid} pin points at backup for VMID ${BASH_REMATCH[1]}"
      failures=$((failures + 1))
      continue
    fi

    if jq -e --arg volid "$volid" 'any(.[]?; .volid == $volid)' >/dev/null 2>&1 <<< "$pbs_json"; then
      echo "  OK: VMID ${vmid} — ${volid}"
    else
      echo "  FAILED: VMID ${vmid} — ${volid} not found in PBS"
      failures=$((failures + 1))
    fi
  done <<< "$pin_entries"

  if [[ "$failures" -gt 0 ]]; then
    echo "ERROR: restore pin validation failed for ${failures} entrie(s)." >&2
    return 1
  fi

  echo ""
  echo "=== Checking uncovered precious-state VMs ==="
  while IFS=$'\t' read -r label vmid; do
    [[ -z "$label" || -z "$vmid" ]] && continue
    if restore_pin_has_vmid "$vmid"; then
      continue
    fi
    echo "WARNING: ${label} (VMID ${vmid}) has no restore pin — vdb will be EMPTY after rebuild"
    uncovered=$((uncovered + 1))
  done < <(collect_precious_state_vm_records)

  if [[ "$uncovered" -gt 0 ]]; then
    if [[ "$CONFIRM" -eq 1 ]]; then
      echo ""
      echo "WARNING: ${uncovered} precious-state VM(s) will be recreated with empty vdb."
      read -rp "Type 'EMPTY VDB' to proceed: " ack
      if [[ "$ack" != "EMPTY VDB" ]]; then
        echo "Aborted: incomplete restore pin file was not acknowledged." >&2
        return 1
      fi
    else
      echo "ERROR: ${uncovered} precious-state VM(s) have no restore pin. Re-run with --confirm and acknowledge the warning to proceed." >&2
      return 2
    fi
  else
    echo "  All precious-state VMs are covered by restore pins"
  fi
...
do_reset_vms() {
  ...
  for ip in "${NODE_IPS[@]}"; do
    echo "  ${ip}:"
    ssh_node "$ip" '
      for zvol in $(zfs list -o name -H 2>/dev/null | grep -E "vmstore/data/(vm-|mycofu-park-)"); do
  ...
  # NOTE (#347): CIDATA snippets are NOT cleared here. They are managed by
  # tofu's proxmox_virtual_environment_file resource, which re-uploads the
  # current content on every apply that touches the owning VM. Clearing them
  # here created divergence in the documented invariant from
  # .claude/rules/proxmox-tofu.md ("CIDATA snippets must be deployed to ALL
  # Proxmox nodes"): after `reset-cluster.sh --vms --node N` followed by any
  # scoped rebuild, node N ended up missing snippets for every VM not in the
  # rebuild's scope, silently breaking HA migration to that node. The
  # historical "safety net" was guarding against stale snippet content from
  # destroyed VMs, but tofu's snippet upload is keyed by filename and
  # overwrites stale content on the next apply — no actual scenario where
  # leaving them in place causes harm.
  for ip in "${NODE_IPS[@]}"; do
    ssh_node "$ip" '
  ...
  ASSERT_FAIL=0
  for ip in "${NODE_IPS[@]}"; do
    # Skip unreachable nodes (already wiped at the full-reset level)
    if ! ssh_node "$ip" "true" 2>/dev/null; then
    ...
    ZVOL_COUNT=$(ssh_node "$ip" "zfs list -o name -H 2>/dev/null | grep -Ec 'vmstore/data/(vm-|mycofu-park-)' || true")
...
do_reset_storage() {
  ...
  # At the full-reset level, some nodes may already be wiped (e.g., Phase 4a
  # wiped pve01 before Phase 4c runs the reset on all nodes). Use
...
do_reset_cluster() {
  # Some nodes may already be wiped from a prior single-node full reset
  ...
  if [[ -n "$SINGLE_NODE" ]]; then
    ...
    git -C "$REPO_DIR" commit -m "reset-cluster full reset: erase secrets" --allow-empty 2>/dev/null || true
...
needs_cluster() {
  [[ "$LEVEL" == "vms" || "$LEVEL" == "storage" || "$LEVEL" == "test" || "$LEVEL" == "recover" || "$LEVEL" == "nas" || "$LEVEL" == "distclean" || "$LEVEL" == "level5" ]]
}

needs_storage() {
  [[ "$LEVEL" == "storage" || "$LEVEL" == "test" || "$LEVEL" == "recover" || "$LEVEL" == "nas" || "$LEVEL" == "distclean" || "$LEVEL" == "level5" ]]
}

needs_boot_destroy() {
  [[ "$LEVEL" == "test" || "$LEVEL" == "recover" || "$LEVEL" == "nas" || "$LEVEL" == "distclean" || "$LEVEL" == "level5" ]]
}
...
print_dry_run() {
  ...
  if [[ "$LEVEL" == "test" ]]; then
    echo ""
    echo "  Preflight:"
    echo "    Verified backup: ${SCRIPT_DIR}/backup-now.sh --env all --verify --pin-out ${RESET_RESTORE_PIN}"
    echo "    Abort rule:      If backup or verification fails, destruction does not begin"
  fi

  if [[ "$LEVEL" == "recover" ]]; then
    echo ""
    echo "  Preflight:"
    echo "    Restore pins:    Validate ${RESTORE_PIN_FILE} against PBS metadata"
    echo "    Missing pins:    Any precious-state VM without a pin requires EMPTY VDB acknowledgment"
    echo "    Normalized pin:  ${RESET_RESTORE_PIN}"
  fi
  ...
  if needs_cluster; then
    ...
    echo "    Snippets:       PRESERVED (managed by tofu, #347)"
  ...
  if needs_boot_destroy; then
    echo ""
    echo "  Full cluster reset core (--test/--recover, inherited by --nas/--level5):"
  ...
  if [[ "$LEVEL" == "nas" || "$LEVEL" == "distclean" ]]; then
    echo ""
    echo "  Level --nas (adds to the full cluster reset):"
  ...
  if [[ "$LEVEL" == "level5" ]]; then
    echo ""
    echo "  Level --level5 (adds to the full cluster reset):"
  ...
  case "$LEVEL" in
    ...
    test|recover) echo "    NAS, workstation (operator.age.key, build artifacts), config.yaml" ;;
  ...
  local restore_flag=""
  [[ "$LEVEL" == "recover" ]] && restore_flag=" --restore-pin-file ${RESTORE_PIN_FILE}"
  echo "To proceed: $(basename "$0") --${LEVEL}${node_flag}${restore_flag} --confirm"
}
...
if [[ "$LEVEL" == "recover" ]]; then
  set +e
  run_recover_preflight
  RECOVER_PREFLIGHT_RC=$?
  set -e

  if [[ "$CONFIRM" -eq 0 ]]; then
    if [[ "$RECOVER_PREFLIGHT_RC" -eq 0 ]]; then
      print_dry_run
      exit 0
    fi
    exit 1
  ...
  if [[ "$RECOVER_PREFLIGHT_RC" -ne 0 ]]; then
    exit 1
  fi
...
case "$LEVEL" in
  ...
  test)
    ...
    do_reset_cluster
    ;;
  recover)   do_reset_cluster ;;
  # TODO: `--nas`, `--level5`, and `--distclean` are higher-blast-radius
  # levels that still bypass the `--test`/`--recover` intent split because
  # they dispatch straight into the legacy reset core. Keep that behavior
  # for Sprint 016; tighten their safety envelope in a follow-up issue.
...
if [[ "$LEVEL" == "test" || "$LEVEL" == "recover" ]]; then
  echo "Run next: framework/scripts/rebuild-cluster.sh --restore-pin-file ${RESET_RESTORE_PIN}"
fi
