...
# (issue #630, #698). Unguarded, a missing nodes[0] entry makes V2.2 probe
# `root@null` and report FAIL — a definite verdict derived from an
# indeterminate input. Per .claude/rules/destruction-safety.md a check that
# cannot determine its answer FAILs loudly rather than reporting one.
NODE1_IP=$(yq -r '.nodes[0].mgmt_ip' "$CONFIG")
if [[ -z "$NODE1_IP" || "$NODE1_IP" == "null" ]]; then
  echo "ERROR: nodes[0].mgmt_ip not set in ${CONFIG}" >&2
  exit 1
fi
NODE1_NAME=$(yq -r '.nodes[0].name' "$CONFIG")
if [[ -z "$NODE1_NAME" || "$NODE1_NAME" == "null" ]]; then
  echo "ERROR: nodes[0].name not set in ${CONFIG}" >&2
  exit 1
fi

if ssh -o ConnectTimeout=5 -o BatchMode=yes "root@${NODE1_IP}" "ls ${ISO_PATH}/base-v*.img" &>/dev/null; then
  # Same bounds as the probe above: without BatchMode this ssh can block on
  # an interactive password/passphrase prompt, and without ConnectTimeout it
  # inherits the OS default (~2 min) — either one hangs the validation gate
  # after the probe has already succeeded (issue #698).
  REMOTE_FILE=$(ssh -o ConnectTimeout=5 -o BatchMode=yes "root@${NODE1_IP}" "ls -1 ${ISO_PATH}/base-v*.img | head -1")
...
# Guarded for the same reason as V2.2 above (#698): unguarded, a missing
# nas.ip/nas.postgres_port sends the state-backend probe to `psql -h null
# -p null`, whose connection error is swallowed by the `2>/dev/null ||
# echo "0"` on the psql line below and reported as
# "V2.4 FAIL (prod schema not found in tofu_state)". That is a specific,
# misleading verdict about the state backend produced by a broken config —
# the check could not determine the answer, so it must FAIL loudly here.
NAS_IP=$(yq -r '.nas.ip' "$CONFIG")
if [[ -z "$NAS_IP" || "$NAS_IP" == "null" ]]; then
  echo "ERROR: nas.ip not set in ${CONFIG}" >&2
  exit 1
fi
NAS_PG_PORT=$(yq -r '.nas.postgres_port' "$CONFIG")
if [[ -z "$NAS_PG_PORT" || "$NAS_PG_PORT" == "null" ]]; then
  echo "ERROR: nas.postgres_port not set in ${CONFIG}" >&2
  exit 1
fi
...
if [[ "$MANUAL" -gt 0 ]]; then
  ...
  if [[ -z "$PROD_VLAN" || "$PROD_VLAN" == "null" ]]; then
    echo "ERROR: environments.prod.vlan_id not set in ${CONFIG}" >&2
    exit 1
  fi
  DOMAIN=$(yq -r '.domain' "$CONFIG")
  if [[ -z "$DOMAIN" || "$DOMAIN" == "null" ]]; then
    echo "ERROR: domain not set in ${CONFIG}" >&2
    exit 1
  fi
  ...
  cat <<EOF
V2.5 Instructions:
  1. Add a temporary test_vm module block to framework/tofu/root/main.tf.
     Copy the "hil_boot" invocation in the same file as a template (it is
     the direct framework/tofu/modules/proxmox-vm consumer — testapp_dev
     is a wrapper and has different inputs). Change vm_name/hostname/
     instance_id to "test-step2", use an unused prod VMID/MAC/IP, and set
     vlan_id = local.config.environments.prod.vlan_id (currently ${PROD_VLAN})
     and gateway = local.config.environments.prod.gateway so the VM lands
     on the prod VLAN. Required inputs are enumerated in
     framework/tofu/modules/proxmox-vm/variables.tf.
  2. Run: framework/scripts/tofu-wrapper.sh plan
     Verify: plan shows 1 VM to create
  3. Run: framework/scripts/tofu-wrapper.sh apply -target=module.test_vm
  4. SSH to the test VM at the IP you assigned in step 1.
  5. Verify: hostname -d  →  should show "${PROD_DOMAIN}"
  6. Verify: cloud-init status  →  should show "done"
  7. Cleanup: framework/scripts/tofu-wrapper.sh destroy -target=module.test_vm
  8. Delete the test_vm block from framework/tofu/root/main.tf
EOF
fi
