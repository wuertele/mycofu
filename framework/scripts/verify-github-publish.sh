...
check_runner() {
  ...
  set +e
  ls_output="$(runner_ssh "GIT_SSH_COMMAND='ssh -i /run/secrets/vault-agent/github-deploy-key -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new -o LogLevel=ERROR' git ls-remote '${REMOTE_URL}' refs/heads/main" 2>&1)"
  local ls_exit=$?
  set -e
...
export GIT_SSH_COMMAND='ssh -i /run/secrets/vault-agent/github-deploy-key -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new -o LogLevel=ERROR'
