...
#   - Cert storage backfill and dashboard token provisioning (Vault-gated)
#
# Backup-job reconciliation is NOT invoked here. It lives in safe-apply.sh's
# run_post_success (and on the no-change retry path) so it runs even when
# post-deploy.sh exits early on Vault-uninit / sealed / no-root-token paths.
# Gating the managed vzdump job behind Vault convergence would leave it
# drifted after destructive VM recreation whenever those paths fire.
# See #620 and RCA docs/reports/2026-07-17-dev-precious-backup-jobs-missing-rca.md.
#
# Usage:
...
# Resolve the SOPS age key before any child script needs it (#862).
#
# post-deploy.sh does not call sops itself, but every child it invokes does
# (configure-vault.sh, cert-storage-backfill.sh, configure-dashboard-tokens.sh).
# Resolving here exports the variable into all of them, so the chain behaves
# the same on the operator workstation as it does on the cicd runner — where
# the variable is supplied by the runner service environment and this branch
# never executes.
#
# It is deliberately BEFORE the Vault reachability wait below: a missing key
# is a workstation configuration error, and failing here costs zero seconds
# instead of surfacing as an opaque sops "no identities" error five minutes
# into the deploy, after the wait loop and inside a child script.
#
# The final `else exit 1` is load-bearing: without it a missing key leaves
# SOPS_AGE_KEY_FILE unset and the failure resurfaces downstream as a
# confusing decrypt error rather than a named cause.
...
# PBS backup-job reconciliation is invoked by safe-apply.sh's
# run_post_success (or the no-change retry path) rather than here — see the
# header comment for the #620 rationale.
