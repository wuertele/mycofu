...
" 2>/dev/null)
  ...
  if [[ -n "$VDB_ZVOL" ]]; then
    # Two fixes vs the previous `... | grep -c TYPE" || echo "0"` form
    # which produced "0\n0" when grep matched zero lines (ssh propagated
    # grep's "0\n" AND its non-zero exit, firing the outer `|| echo "0"`
    # to append another "0"):
    #   1. Move the "0" fallback INSIDE the remote command so ssh's
    #      exit code reflects the inner OR (always 0 when echo fires),
    #      avoiding the OUTER `|| echo "0"` that appended a second line.
    #   2. Pipe through `head -1` as the actual guarantee that the
    #      captured value is a single token. The inner fallback can
    #      still produce "0\n0" when grep prints "0\n" then exits 1
    #      and `echo 0` appends another "0\n"; only `head -1` collapses
    #      that to "0". `head -1` is the load-bearing guard — do not
    #      remove it.
    # The subsequent `[[ "$VDB_HAS_DATA" -gt 0 ... ]]` previously choked
    # on the multi-line value.
    #
    # set +e/-e wraps the capture because under `set -euo pipefail` a
    # non-zero exit from ssh (most commonly: ssh succeeded but the inner
    # grep matched zero lines and returned 1) would abort the entire
    # script. The inner `|| echo 0` provides the semantic default.
    # See #451.
    set +e
    VDB_HAS_DATA=$(ssh_node "$HOSTING_IP" \
      "blkid /dev/zvol/${STORAGE_POOL}/data/${VDB_ZVOL} 2>/dev/null | grep -c TYPE 2>/dev/null || echo 0" \
      | head -1)
    set -e
    [[ -z "$VDB_HAS_DATA" ]] && VDB_HAS_DATA=0
