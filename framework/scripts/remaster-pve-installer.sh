...
Stock ISO integrity:
  --input-iso-sha256 HEX  Expected SHA256 of the stock Proxmox ISO. The stock
                          ISO is fetched over plain HTTP, so this checksum is
                          the only integrity anchor those bytes ever get.
                          REQUIRED whenever the ISO is downloaded or read from
                          the managed cache. Optional ONLY in standalone mode
                          when --input-iso names an existing local file you are
                          vouching for yourself.
                          In config mode proxmox.installer.iso_sha256 is always
                          required, including when proxmox.installer.iso is a
                          local path: that key names the upstream artifact, and
                          an unpinned name resolves against the current working
                          directory, so a same-named file beside the invocation
                          would be remastered in place of the real installer.
...
validate_common() {
  ...
  # In config mode proxmox.installer.iso is the CDN artifact's BASENAME (e.g.
  # "proxmox-ve_9.1-1.iso"), not an operator-supplied path — resolve_input_iso
  # basenames it to build the download URL. That means the `-s "$INPUT_ISO"`
  # local short-circuit resolves against the CURRENT WORKING DIRECTORY, so a
  # same-named file next to the invocation would be remastered in place of the
  # real installer with no integrity check at all. The "operator vouches for
  # their own bytes" exception (#721) applies only to an explicit standalone
  # --input-iso PATH; nobody vouches for a CWD collision. Requiring the pin in
  # config mode closes that route without adding path-classification logic,
  # and it matches the documented per-site pin-update procedure, which already
  # updates the iso and iso_sha256 keys together.
  if [[ "$CONFIG_MODE" -eq 1 ]] && [[ -z "$INPUT_ISO_SHA256" || "$INPUT_ISO_SHA256" == "null" ]]; then
    missing+=("proxmox.installer.iso_sha256")
  fi
...
resolve_input_iso() {
  ...
  # Past this point every byte we can return reached this host over plain HTTP
  # (see the download comment below) — either a fetch we are about to perform,
  # or this cache's record of an earlier one. INPUT_ISO_SHA256 is the ONLY
  # integrity anchor those bytes ever get. With no hash configured the question
  # "are these the genuine Proxmox installer bytes?" is not answerable, and an
  # undeterminable safety check FAILs rather than passing
  # (.claude/rules/destruction-safety.md, "When a Safety Check Cannot Determine
  # State"). Refusing here — before the fetch — also avoids pulling ~1.5 GB
  # only to reject it. There is deliberately no override flag: an operator who
  # wants to supply their own bytes passes --input-iso, which returns above.
  if [[ -z "$INPUT_ISO_SHA256" || "$INPUT_ISO_SHA256" == "null" ]]; then
    echo "ERROR: refusing to use the stock ISO cache without a configured SHA256" >&2
    echo "  The stock ISO is fetched over plain HTTP (download.proxmox.com serves" >&2
    echo "  a certificate for enterprise.proxmox.com only), so the checksum is the" >&2
    echo "  only thing standing between a corrupt or tampered CDN response and a" >&2
    echo "  remastered installer that will be booted on a bare-metal node." >&2
    echo "  Set proxmox.installer.iso_sha256 in the config, or pass" >&2
    echo "  --input-iso-sha256 <sha256>. To use a locally supplied ISO instead," >&2
    echo "  pass --input-iso pointing at an existing file." >&2
    exit 1
  fi
  ...
  # HTTP without redirect for this CDN. Integrity is enforced by the SHA256 check
  ...
  # Verify SHA256 BEFORE promoting to the canonical cache name (#317). A download
  # that succeeds at the HTTP layer but delivers corrupt bytes (CDN bug, transient
  # MITM, truncated upstream cache) must never occupy the canonical path: a
  # poisoned canonical file makes every subsequent run skip the download (the
  # `-s "$cached"` short-circuit above) and re-fail the checksum until an operator
  # deletes it by hand. Rejecting on the unpromoted `.partial` and removing it lets
  # the next run re-download cleanly (self-healing cache). iso_sha256_matches
  # writes only to stderr, so the captured resolved path on stdout stays clean.
  if ! iso_sha256_matches "${cached}.partial"; then
    rm -f "${cached}.partial"
    exit 1
  fi
...
# Compare an ISO's SHA256 against the configured INPUT_ISO_SHA256. All output
# (the matching checksum on success, the expected/actual pair on mismatch) goes
# to stderr so this is safe to call from resolve_input_iso, whose stdout the
# caller captures as the resolved ISO path — no caller needs to remember a
# redirect. Returns 0 when the hash matches OR when no expected hash is
# configured; returns 1 on mismatch. This does NOT exit — callers decide the
# failure action: verify_input_iso exits, resolve_input_iso removes the
# unpromoted .partial first (#317). Factored out of verify_input_iso so the
# identical comparison runs at the post-download/pre-promote point too.
#
# The no-hash-configured return-0 branch is reachable ONLY for an ISO the
# operator supplied locally via --input-iso. resolve_input_iso refuses the
# HTTP-fetched cache path outright when no hash is configured (#721), so
# network-sourced bytes never reach this function without an anchor to
# compare against.
iso_sha256_matches() {
  local iso="$1"
  [[ -n "$INPUT_ISO_SHA256" && "$INPUT_ISO_SHA256" != "null" ]] || return 0
  local actual
