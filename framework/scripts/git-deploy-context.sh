...
if [[ -z "${GIT_DEPLOY_SCRIPT_DIR:-}" ]]; then
  if [[ -n "${SCRIPT_DIR:-}" ]]; then
    GIT_DEPLOY_SCRIPT_DIR="${SCRIPT_DIR}"
  else
    GIT_DEPLOY_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  fi
fi
...
classify_scope_impact() {
  ...
  helper_stderr_file="$(mktemp "${TMPDIR:-/tmp}/git-deploy-scope.XXXXXX")" || {
    SCOPE_IMPACT_REASON="failed to create temp file for scope classification stderr"
    return 1
  }
  jq_stderr_file="$(mktemp "${TMPDIR:-/tmp}/git-deploy-jq.XXXXXX")" || {
    rm -f "$helper_stderr_file"
    SCOPE_IMPACT_REASON="failed to create temp file for scope classification parser stderr"
    return 1
  }

  set +e
  result_json="$("${GIT_DEPLOY_SCRIPT_DIR}/vm-scope.sh" scope-impact --scope "${SCOPE:-}" 2>"$helper_stderr_file")"
  scope_exit=$?
  helper_stderr="$(awk 'NF { print; exit }' "$helper_stderr_file" 2>/dev/null)"

  if [[ -n "${result_json}" ]]; then
    SCOPE_IMPACT="$(printf '%s\n' "$result_json" | "${GIT_DEPLOY_JQ_BIN}" -r '.impact // ""' 2>"$jq_stderr_file")"
    impact_exit=$?
    SCOPE_IMPACT_REASON="$(printf '%s\n' "$result_json" | "${GIT_DEPLOY_JQ_BIN}" -r '.reason // ""' 2>>"$jq_stderr_file")"
    reason_exit=$?
    SCOPE_UNKNOWN_TARGETS="$(printf '%s\n' "$result_json" | "${GIT_DEPLOY_JQ_BIN}" -r '.unknown_targets // ""' 2>>"$jq_stderr_file")"
    unknown_exit=$?
    jq_stderr="$(awk 'NF { print; exit }' "$jq_stderr_file" 2>/dev/null)"
  fi

  if [[ -z "${result_json}" ]]; then
    final_exit=1
    SCOPE_UNKNOWN_TARGETS="${SCOPE:-unknown}"
    SCOPE_IMPACT_REASON="scope classification helper failed"
  elif [[ "$impact_exit" -ne 0 || "$reason_exit" -ne 0 || "$unknown_exit" -ne 0 ]]; then
    final_exit=1
    SCOPE_IMPACT=""
    SCOPE_IMPACT_REASON="scope classification JSON parse failed"
  elif [[ "$scope_exit" -ne 0 ]]; then
    final_exit="$scope_exit"
    if [[ -z "$SCOPE_IMPACT_REASON" ]]; then
      SCOPE_IMPACT_REASON="scope classification helper failed"
    fi
  else
    case "$SCOPE_IMPACT" in
      dev_only|prod_affecting|shared_control_plane)
        final_exit=0
        ;;
      "")
        final_exit=1
        SCOPE_IMPACT_REASON="scope classification helper returned missing impact"
        ;;
      *)
        final_exit=1
        SCOPE_IMPACT_REASON="scope classification helper returned invalid impact: ${SCOPE_IMPACT}"
        ;;
    esac
  fi

  if [[ "$final_exit" -ne 0 ]]; then
    if [[ -n "$helper_stderr" ]]; then
      SCOPE_IMPACT_REASON="${SCOPE_IMPACT_REASON}; helper stderr: ${helper_stderr}"
    fi
    if [[ -n "$jq_stderr" ]]; then
      SCOPE_IMPACT_REASON="${SCOPE_IMPACT_REASON}; jq stderr: ${jq_stderr}"
    fi
  fi

  rm -f "$helper_stderr_file" "$jq_stderr_file"
