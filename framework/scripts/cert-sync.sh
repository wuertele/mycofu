...
main() {
  ...
  # Fingerprint-skip: if Vault already has this exact cert (matched by
  # SHA-256 fingerprint of the local fullchain), do not POST. Each POST
  # to mycofu/data/certs/<fqdn> creates a new KV v2 version, and the
  # version count is the LE rate-limit oracle (check-cert-budget.sh).
  # Spurious writes from the retry timer or boot path would over-count
  # and trip the budget gate without any actual LE issuance. cert-sync
  # only writes when the local cert genuinely differs from Vault.
  existing_fingerprint="$(
    curl -sk \
      ...
      2>/dev/null \
      | jq -r '.data.custom_metadata.fingerprint // empty' 2>/dev/null
  )" || true
  if [[ -n "${existing_fingerprint}" && "${existing_fingerprint}" == "${fingerprint}" ]]; then
    log "Vault already has fingerprint ${fingerprint} for ${fqdn}; skipping write"
    disable_retry_timer
    return 0
  fi
