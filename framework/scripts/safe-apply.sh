...
cleanup() {
  ...
  # #706: sweep a half-written gatus temp if we're interrupted between mktemp
  # and mv (the temp lives in site/gatus/ for atomic same-fs rename, so it is
  # not under SAFE_APPLY_TMPDIR). GATUS_CONFIG_TMP is unset until the generation
  # block runs; after a successful mv it names a path that no longer exists.
  if [[ -n "${GATUS_CONFIG_TMP:-}" ]]; then
...
# run_configure_backups PLUS a projection-only
# `configure-replication.sh --refresh-artifact-only` (node-artifact
# redelivery, #693/#501) — but deliberately NOT the FULL post-success
# workflow: running post-deploy.sh (Vault reachability probes → hangs)
# or the full configure-replication run (create/prune/seed, which tears
# down transient first-syncs) on every steady-state no-change deploy
# would introduce operational risk. See the decision-record comment on
# the "No changes" branch below for the full contract.
#
...
run_post_success() {
  ...
  echo ""
  echo "--- configure-replication.sh \"*\" --env \"$ENV\" ---"
  local repl_rc=0
...
# --- Regenerate the Gatus monitoring config in-path (fail closed) — #690 ---
# site/gatus/config.yaml is a generated build artifact (NOT in git). The two
# other data-plane deploy authorities generate it in-path before tofu reads it:
# the pipeline (.gitlab-ci.yml build:merge) and rebuild-cluster.sh (step 7).
# safe-apply.sh is the third — the documented workstation data-plane deploy path
# — and previously did NOT, so a fresh checkout deployed gatus with an EMPTY
# monitoring config (gatus crash-looped "configuration should contain at least 1
# endpoint"; prod monitoring was down — DRT-005, 2026-07-21). Generate it here.
#
# #718 (correcting a claim this comment used to make): this invocation does NOT
# scrub the resolver's test seams, and neither does build:merge — the only
# call-site scrub is rebuild-cluster.sh's, added by #701. The seam lockout now
# lives inside generate-gatus-config.sh, which clears
# GATUS_GITHUB_EXPECTED_SOURCE_COMMIT / GATUS_SOURCE_REPO_DIR /
# GATUS_SOURCE_COMMIT_REMOTE unless invoked with --allow-test-seams. So every
# caller — this one included — converges on the internal
# resolve_gatus_expected_source_commit() (#528) without needing to remember a
# call-site scrub.
#
# Fail closed: a generation failure aborts the deploy loudly (the module.gatus
# precondition is the second, defence-in-depth guard at plan time).
if [[ $DRY_RUN -eq 1 ]]; then
  # #706: a dry run must be read-only. The pre-#706 code ran the generation
  # block unconditionally, so `safe-apply.sh <env> --dry-run` OVERWROTE the
  # operator's real site/gatus/config.yaml (a pipeline-fresh config could be
  # silently replaced with a stale-SHA one) and made the generator's network
  # ref-resolution call. Skip regeneration entirely: the tofu plan below
  # reflects the site/gatus/config.yaml currently on disk (generated by the
  # last real deploy or the pipeline). A real deploy regenerates it atomically
  # (else branch).
  echo "Skipping Gatus config regeneration (dry-run is read-only; using existing site/gatus/config.yaml)."
else
  # --- Regenerate the Gatus monitoring config atomically (fail closed) — #690/#704/#706 ---
  echo "Generating Gatus config (site/gatus/config.yaml)..."
  mkdir -p "${REPO_DIR}/site/gatus"
  GATUS_CONFIG_DEST="${REPO_DIR}/site/gatus/config.yaml"
  # #706 (non-atomic write): generate to a temp file in the same directory and
  # `mv` it into place only on success. The old `>` redirect truncated the real
  # target to 0 bytes BEFORE the generator ran, so a SIGINT mid-generation left
  # a 0-byte file and a transient generator failure destroyed a previously-good
  # config. With temp+mv the real config is never truncated and is replaced
  # atomically; on any failure the prior good file is left untouched.
  GATUS_CONFIG_TMP="$(mktemp "${REPO_DIR}/site/gatus/.config.yaml.XXXXXX")"
  if ! "${SCRIPT_DIR}/generate-gatus-config.sh" > "${GATUS_CONFIG_TMP}"; then
    rm -f "${GATUS_CONFIG_TMP}"
    echo "ERROR: generate-gatus-config.sh failed; refusing to deploy with a missing/empty Gatus monitoring config (#690)." >&2
    echo "  This is fail-closed by design (.claude/rules/destruction-safety.md: FAIL, never SKIP)." >&2
    exit 1
  fi
  # #704: the generator can exit 0 with no output; key the fail-closed check on
  # OUTPUT (non-empty, `-s`), not on the exit code alone. (`-s` catches a 0-byte
  # file; a non-empty-but-invalid config is still caught by the module's
  # config_guard precondition at plan time — defence in depth.)
  if [[ ! -s "${GATUS_CONFIG_TMP}" ]]; then
    rm -f "${GATUS_CONFIG_TMP}"
    echo "ERROR: generate-gatus-config.sh produced an empty Gatus config; refusing to deploy (#690/#704)." >&2
    echo "  This is fail-closed by design (.claude/rules/destruction-safety.md: FAIL, never SKIP)." >&2
    exit 1
  fi
  # mktemp creates the temp at 0600; the prior `>` redirect produced a
  # umask-derived 0644. Restore 0644 so the on-disk config matches the previous
  # behavior and stays readable to same-host consumers.
  chmod 0644 "${GATUS_CONFIG_TMP}"
  mv -f "${GATUS_CONFIG_TMP}" "${GATUS_CONFIG_DEST}"
fi
...
if [[ -z "$TARGETS" ]]; then
  ...
  # DECISION RECORD — what convergence runs on a no-change deploy, and why the
  # rest is deliberately excluded (design-taste principle 9: this comment is a
  # dependent of the contract below; keep it in sync when the contract moves).
  #
  # Two convergence steps run here even though tofu applied nothing:
  #
  #   1. Backup-job reconciliation (run_configure_backups). A prior deploy may
  #      have destructively recreated VMs and then failed somewhere between
  #      Phase 2 and post-success convergence. Proxmox purges destroyed VMIDs
  #      from the managed vzdump job during purge, so the next no-change
  #      safe-apply must still reconcile that drift — otherwise scheduled
  #      backups stay silently missing for restored precious-state VMs. See
  #      #617 and RCA docs/reports/2026-07-17-dev-precious-backup-jobs-missing-rca.md.
  #
  #   2. Replication policy node-artifact refresh
  #      (configure-replication.sh --refresh-artifact-only). The
  #      /etc/repl-policy.vmids artifact is a derived projection of the
  #      git-determined policy (POLICY_ON/OFF, CADENCE_MAP, POLICY_GEN), and
  #      its ONLY writer is configure-replication.sh. Before #693/#501 it was
  #      refreshed solely on the change path, so a ref that changed the
  #      artifact's SHAPE (e.g. Sprint 048 adding CADENCE_MAP) but touched no
  #      VM inputs left every node carrying the stale artifact — a GUARANTEED
  #      false-red on validate's `replication policy conformance` check on the
  #      no-change deploy (pipeline #1779, 2026-07-22). The refresh makes the
  #      artifact a synchronous projection of the deployed ref. It is
  #      idempotent and PBS-independent.
  #
  # STILL EXCLUDED on the no-change path, on purpose:
  #   - The FULL configure-replication run (create/prune/seed/schedule-reconcile).
  #     Its failed-job detection treats an in-progress first sync
  #     (last_sync=="-") as stale and TEARS IT DOWN + re-seeds — acceptable
  #     after a completed apply, but running it on every steady-state no-change
  #     deploy would race and tear down transient first-syncs during transition
  #     windows (e.g. a 24h dev seed). --refresh-artifact-only reads and mutates
  #     NO live replication job, so it carries none of that risk. A genuine job
  #     or schedule divergence is NOT masked within the active env's validation
  #     scope: validate checks live pvesr schedules and env-scoped orphan/
  #     residual jobs independently of the artifact, so it stays red until a real
  #     reconcile runs — the refresh cannot manufacture a false-green for this
  #     env. (Residual: a cadence-VALUE change in git is not a VM input, so it
  #     lands on the no-change path too; the refresh converges the ARTIFACT, but
  #     the live pvesr schedule stays stale and validate stays red until a real
  #     `configure-replication.sh "*"` reconcile runs. That is strictly better
  #     than pre-#693, which was red on BOTH artifact and schedule — but the
  #     no-change path fixes only the artifact half.)
  #   - post-deploy.sh (Vault probes) — up to ~5-minute Vault-unreachable hangs
  #     on legitimate no-change invocations.
  # This resolves #693 (stale-artifact false-red) and closes #501 (no-change
  # deploys must honor the "reconcile replication after each deploy stage"
  # contract) along the artifact-projection dimension that is safe to make
  # unconditional; live-job reconciliation stays on the change/full-reconcile
  # path per the guard rationale above.
  if [[ $DRY_RUN -eq 1 ]]; then
    echo "(dry-run — not applying)"
    exit 0
  fi
  if [[ $NO_RECOVERY -eq 1 ]]; then
    echo "=== Skipping no-change convergence (--no-recovery) ==="
    exit 0
  fi
  echo ""
  ...
  set +e
  run_configure_backups
  NO_CHANGE_BACKUPS_RC=$?
  set -e

  echo ""
  echo "=== Replication policy artifact refresh (no-change path) ==="
  echo "--- configure-replication.sh \"*\" --env \"$ENV\" --refresh-artifact-only ---"
  set +e
  "${SCRIPT_DIR}/configure-replication.sh" "*" --env "$ENV" --refresh-artifact-only
  NO_CHANGE_REPL_RC=$?
  set -e
  if [[ $NO_CHANGE_REPL_RC -ne 0 ]]; then
    echo "ERROR: configure-replication.sh --refresh-artifact-only failed (rc=$NO_CHANGE_REPL_RC)"
  fi

  # First non-zero rc wins in execution order (backups → artifact refresh),
  # matching run_post_success. Each rc is a real deploy failure the caller
  # must see; neither step is gated behind the other's success.
  if [[ $NO_CHANGE_BACKUPS_RC -ne 0 ]]; then
    exit "$NO_CHANGE_BACKUPS_RC"
  fi
  exit "$NO_CHANGE_REPL_RC"
fi
