...
REPLICATED_VMID_HELPER="${VALIDATE_SITE_CONFIG_REPLICATED_HELPER:-${SCRIPT_DIR}/list-replicated-vmids.sh}"
...
# Reject any boolean flag whose rendered text is not canonical `true`/`false`.
#
# Every boolean flag in this config is consumed by shell that compares the
# `yq -r` rendering against the literal string "true" (see
# framework/scripts/install-pve-node.sh:363,
# framework/scripts/regreen-cluster.sh:151,
# framework/scripts/vdb-park-lib.sh:113). A non-canonical value therefore
# reads as not-true and fails closed with no error — the confusing class #295
# fixed for publish.github.enabled and #731 extends to the rest.
#
# The check is on the RENDERED TEXT, not on the YAML tag, because the tag is
# not sufficient: yq parses `True` as !!bool yet renders it as "True", which
# no consumer's string compare matches. Absent and null keys reach here as
# "false" via the `// false` defaults at each call site, so omitting a flag
# stays legal.
#
# Returns non-zero (after recording the error) so callers can skip the
# dependent checks they can no longer evaluate.
check_boolean_text() {
  local source_file="$1"
  local key_path="$2"
  local value="$3"

  valid_boolean_text "$value" && return 0
  err "${source_file}: ${key_path} must be a boolean (got '${value}', expected true or false)"
  return 1
}

valid_replicate_cadence() {
  local value="${1:-}" number unit
  [[ "$value" =~ ^([1-9][0-9]*)([mh])$ ]] || return 1
...
check_replicate_value() {
  local source_file="$1"
  local object_path="$2"
  local value_tag="$3"
  local value_text="$4"
  local backup_true="$5"
  local key_path="${object_path}.replicate"

  if [[ "$value_tag" == "!!bool" && "$value_text" == "false" ]]; then
    if [[ "$backup_true" == "true" ]]; then
      err "${source_file}: ${key_path} must not be false when ${object_path}.backup is true"
    fi
    return
  fi

  # Boolean `true` is not a valid form for `replicate:`. Named explicitly
  # so the error message tells the operator what to do instead.
  if [[ "$value_tag" == "!!bool" && "$value_text" == "true" ]]; then
    err "${source_file}: ${key_path} has invalid cadence 'true' (boolean not accepted; use \"1m\" for the precious floor, or omit to inherit the env default)"
    return
  fi

  if [[ "$value_tag" != "!!str" ]] || ! valid_replicate_cadence "$value_text"; then
    err "${source_file}: ${key_path} has invalid cadence '${value_text}' (expected \"<N>m\" with 1<=N<=59, \"<N>h\" with 1<=N<=24, or false)"
    return
  fi

  if [[ "$backup_true" == "true" && "$value_text" != "1m" ]]; then
    err "${source_file}: ${key_path} must be \"1m\" when ${object_path}.backup is true"
  elif [[ "$backup_true" == "true" && "$value_text" == "1m" ]]; then
    warn "${source_file}: replicate: \"1m\" on backup:true VMs is redundant; the precious-floor default already forces 1m. key=${key_path}"
  fi
}
...
# --- Sprint 044 vdb park bridge scope guard ---
while IFS=$'\t' read -r env_name enabled; do
  [[ -n "$env_name" ]] || continue
  check_boolean_text "$CONFIG" "environments.${env_name}.vdb_park_bridge" "$enabled" || continue
  if [[ "$enabled" == "true" && "$env_name" != "dev" ]]; then
    err "${CONFIG}: environments.${env_name}.vdb_park_bridge may only be true for dev; promotion to non-dev requires the documented promotion process and a follow-up sprint"
  fi
done < <(yq -r '.environments // {} | to_entries[] | [.key, (.value.vdb_park_bridge // false)] | @tsv' "$CONFIG" 2>/dev/null || true)

# --- GitHub remote URL (only required when publish.github.enabled is true) ---
# publish.github.enabled gates prod pipelines and the publish:github script,
# both of which compare it to the literal string "true". A non-canonical
# value (True, yes, 1, or a typo like "tru") silently reads as not-true and
# fails closed with no error. Reject anything that isn't a canonical boolean
# so a downstream adopter who mistypes the flag gets a clear error (#295).
PUBLISH_GITHUB_ENABLED="$(yq -r '.publish.github.enabled // false' "$CONFIG" 2>/dev/null || true)"
check_boolean_text "$CONFIG" "publish.github.enabled" "$PUBLISH_GITHUB_ENABLED" || true
...
# --- Nix builder schema ---
# nixpkgs_ref pins the nixpkgs used to resolve the builder VM itself. Validated
# if present: it must be non-empty and must not be a bare indirect `nixpkgs`
# ref, which floats on unstable. Unstable dropped x86_64-darwin in 26.11 and
# silently disabled DR on the Intel Mac workstation (#564).
BUILDER_NIXPKGS_REF="$(yq -r '.nix_builder.nixpkgs_ref // ""' "$CONFIG" 2>/dev/null || true)"
if yq -e '.nix_builder | has("nixpkgs_ref")' "$CONFIG" >/dev/null 2>&1; then
  if [[ -z "$BUILDER_NIXPKGS_REF" || "$BUILDER_NIXPKGS_REF" == "null" ]]; then
    err "${CONFIG}: nix_builder.nixpkgs_ref must be a non-empty string when present"
  elif [[ ! "$BUILDER_NIXPKGS_REF" =~ ^[A-Za-z0-9:/._+=?\&%-]+$ ]]; then
    # setup-nix-builder.sh interpolates this into a generated shell launcher.
    # A '#attr' suffix, quotes, or shell metacharacters do not belong here.
    err "${CONFIG}: nix_builder.nixpkgs_ref must be a bare flake ref with no '#attribute' suffix and no shell metacharacters: '${BUILDER_NIXPKGS_REF}'"
  elif [[ "$BUILDER_NIXPKGS_REF" =~ ^(flake:)?nixpkgs(/|$) ]]; then
    # An indirect ref resolves through the global flake registry, which floats
    # on nixpkgs-unstable — the exact #564 failure mode.
    err "${CONFIG}: nix_builder.nixpkgs_ref '${BUILDER_NIXPKGS_REF}' is an indirect registry ref that floats on unstable; use an explicit ref such as github:NixOS/nixpkgs/nixpkgs-26.05-darwin"
  elif [[ "$BUILDER_NIXPKGS_REF" =~ (unstable|/master|/main)$ ]]; then
    err "${CONFIG}: nix_builder.nixpkgs_ref '${BUILDER_NIXPKGS_REF}' tracks a rolling branch; use a release branch such as github:NixOS/nixpkgs/nixpkgs-26.05-darwin"
  fi
fi
...
check_boolean_text "$CONFIG" "benchmarks.scheduled.enabled" "$BENCH_SCHEDULED_ENABLED" || true
...
if [[ "$HIL_BOOT_PRESENT" == "true" ]]; then
  ...
  if check_boolean_text "$CONFIG" "vms.hil_boot.backup" "$HIL_BOOT_BACKUP" &&
     [[ "$HIL_BOOT_BACKUP" == "true" ]]; then
    err "${CONFIG}: vms.hil_boot must not set backup: true"
  fi

  if [[ ! -x "$VM_SCOPE_SCRIPT" ]]; then
    err "framework/scripts/vm-scope.sh is required to validate vms.hil_boot taxonomy"
  else
    set +e
    HIL_BOOT_TAXONOMY="$(
      VM_SCOPE_APPS_CONFIG="$APPS_CONFIG" "$VM_SCOPE_SCRIPT" classes --format json 2>/dev/null | \
        python3 -c 'import json, sys
data = json.load(sys.stdin)
rec = data.get("hil_boot")
if rec is None:
    sys.exit(2)
print("%s\t%s" % (rec.get("scope"), str(rec.get("control_plane")).lower()))' 2>/dev/null
    )"
    HIL_BOOT_TAXONOMY_RC=$?
    set -e

    if [[ "$HIL_BOOT_TAXONOMY_RC" -ne 0 ]]; then
      err "framework/scripts/vm-scope.sh must declare hil-boot when ${CONFIG} has vms.hil_boot"
    elif [[ "$HIL_BOOT_TAXONOMY" != $'shared\tfalse' ]]; then
      err "framework/scripts/vm-scope.sh must classify hil-boot as scope=shared control_plane=false"
    fi
  fi
fi

# --- Replication policy schema ---
while IFS=$'\t' read -r vm_key has_replicate replicate_tag replicate_value backup_true; do
  [[ -z "$vm_key" ]] && continue
  [[ "$has_replicate" == "true" ]] || continue

  check_replicate_value "$CONFIG" "vms.${vm_key}" "$replicate_tag" "$replicate_value" "$backup_true"
done < <(yq -r '.vms // {} | to_entries[] |
  ...
  [
    ...
    ($vm | has("replicate")),
    ($vm.replicate | tag),
    ($vm.replicate | tostring),
    ($vm.backup == true)
  ] | @tsv' "$CONFIG" 2>/dev/null || true)

if [[ -f "$APPS_CONFIG" ]]; then
  while IFS=$'\t' read -r app_key has_replicate replicate_tag replicate_value backup_true; do
    [[ -z "$app_key" ]] && continue
    [[ "$has_replicate" == "true" ]] || continue

    check_replicate_value "$APPS_CONFIG" "applications.${app_key}" "$replicate_tag" "$replicate_value" "$backup_true"
  done < <(yq -r '.applications // {} | to_entries[] |
    select((.value | tag) == "!!map") |
    .key as $app_key |
    .value as $app |
    [
      ...
      ($app | has("replicate")),
      ($app.replicate | tag),
      ($app.replicate | tostring),
      ($app.backup == true)
    ] | @tsv' "$APPS_CONFIG" 2>/dev/null || true)
fi

# --- Replication policy helper drift guard ---
if [[ ! -x "$REPLICATED_VMID_HELPER" ]]; then
  err "framework/scripts/list-replicated-vmids.sh is required to validate enabled VM replication policy coverage"
else
  set +e
  REPLICATED_POLICY_ROWS="$(
    LIST_REPLICATED_VMIDS_CONFIG="$CONFIG" \
    LIST_REPLICATED_VMIDS_APPS_CONFIG="$APPS_CONFIG" \
      "$REPLICATED_VMID_HELPER" --format tsv --mode all all 2>&1
  )"
  REPLICATED_POLICY_RC=$?
  set -e

  if [[ "$REPLICATED_POLICY_RC" -ne 0 ]]; then
    err "framework/scripts/list-replicated-vmids.sh --format tsv --mode all all failed"
    while IFS= read -r helper_line; do
      [[ -z "$helper_line" ]] && continue
      echo "  ${helper_line}" >&2
    done <<< "$REPLICATED_POLICY_ROWS"
  else
    REPLICATED_POLICY_VMIDS="$(printf '%s\n' "$REPLICATED_POLICY_ROWS" | awk -F '\t' 'NF {print $1}' | sort -u)"

    while IFS=$'\t' read -r source_file key_path vmid; do
      [[ -z "$key_path" ]] && continue
      if [[ -z "$vmid" || "$vmid" == "null" ]]; then
        continue
      fi
      if ! grep -Fxq "$vmid" <<< "$REPLICATED_POLICY_VMIDS"; then
        err "${source_file}: ${key_path} vmid ${vmid} is missing from framework/scripts/list-replicated-vmids.sh --format tsv --mode all all output"
      fi
    done < <(
      yq -r '.vms // {} | to_entries[] |
        select((.value | tag) == "!!map" and .value.enabled != false) |
        ["site/config.yaml", ("vms." + .key), (.value.vmid // "")] | @tsv' "$CONFIG" 2>/dev/null || true
      if [[ -f "$APPS_CONFIG" ]]; then
        yq -r '.applications // {} | to_entries[] |
          select((.value | tag) == "!!map" and .value.enabled == true) |
          .key as $app |
          (.value.environments // {}) | to_entries[] |
          select((.value | tag) == "!!map") |
          ["site/applications.yaml", ("applications." + $app + ".environments." + .key), (.value.vmid // "")] | @tsv' "$APPS_CONFIG" 2>/dev/null || true
      fi
    )
  fi
fi
...
while IFS=$'\t' read -r node_idx node_name enabled amt_ip amt_user amt_ref nic_driver disk_path node_fs; do
  ...
  # regreen_enabled gates every required-field check below AND the runtime
  # gates in install-pve-node.sh:363 and regreen-cluster.sh:151, all of which
  # compare against the literal string "true". A non-canonical value would
  # silently skip five required-field checks for a node the operator believes
  # is regreen-enabled, then refuse to install it with a confusing message
  # (#731). This is the only canonicality check for regreen_enabled; the PDU
  # loop below reads the same field and relies on this guard having run.
  check_boolean_text "$CONFIG" "${node_label} regreen_enabled" "$enabled" || continue
...
if [[ "$PDU_PRESENT" == "true" ]]; then
  ...
  while IFS=$'\t' read -r node_idx node_name enabled outlet; do
    ...
    # regreen_enabled canonicality is enforced once, in the nodes loop above,
    # which always runs and records an error before this loop is reached.
