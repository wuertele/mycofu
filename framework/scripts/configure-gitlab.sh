  ...
  if [[ -n "$TEST_TOKEN" ]]; then
    ...
    if [[ -n "$INIT_PASSWORD" ]]; then
      ...
      if [[ -n "$INIT_TOKEN" ]]; then
        ...
        echo "ERROR: SOPS gitlab_root_password does not match GitLab, and /var/lib/gitlab/initial_root_password did not authenticate." >&2
        echo "ERROR: Refusing to overwrite SOPS; restore GitLab vdb from a PBS pin whose state matches the SOPS value (.claude/rules/pbs-restore.md), or converge from an existing escrow entry under ~/.mycofu-escrow/." >&2
        exit 1
      fi
    else
...
if [[ -n "$CURRENT_BRANCH" ]]; then
  ...
  # --no-verify bypasses the pre-push hook which blocks direct pushes
  # to dev/prod/main. This push is intentional — it seeds the GitLab
  # repo after a fresh install or rebuild.
  if git -C "$REPO_DIR" push --no-verify gitlab "$CURRENT_BRANCH" --force 2>&1; then
...
# Step 6f: Set merge method to fast-forward
echo ""
echo "=== Step 6f: Set merge method ==="
MERGE_RESPONSE=$(curl -sk -X PUT "${GITLAB_URL}/api/v4/projects/${PROJECT_ID}" \
  -H "$AUTH_HEADER" \
  -d "merge_method=ff" 2>/dev/null)
MERGE_METHOD=$(echo "$MERGE_RESPONSE" | jq -r '.merge_method // empty')
if [[ "$MERGE_METHOD" == "ff" ]]; then
  echo "  Merge method set to fast-forward"
else
  echo "  WARNING: Could not set merge method to fast-forward (got: ${MERGE_METHOD})" >&2
fi
...
echo ""
echo "GitLab URL:     $GITLAB_URL"
echo "Root password:  (in SOPS as gitlab_root_password — decrypt in an attended terminal)"
echo ""
echo "Next steps:"
echo "  1. For root password rotation, run: framework/scripts/rotate-gitlab-root-password.sh --i-mean-it"
echo "  2. Keep root password handling path-only; do not print decrypted SOPS values into logs."
echo "  3. Build and deploy the runner if needed: build-image.sh site/nix/hosts/cicd.nix cicd"
echo "  4. Register or refresh the runner: framework/scripts/register-runner.sh"
