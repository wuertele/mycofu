...
Subcommands:
  ...
  scope-impact --scope "<scope string>"
    Scopes: control-plane, data-plane, env=dev, env=prod, vm=name1,name2
EOF
}
...
def cmd_scope_impact(args, classes):
    ...
    if scope == "data-plane":
        ...
        return 0
    if scope in {"env=dev", "env=prod"}:
        env = scope[len("env=") :]
        impact = "dev_only" if env == "dev" else "prod_affecting"
        print(json.dumps(scope_json(impact, f"environment scope targets {env} modules, excluding control-plane/shared")))
        return 0
