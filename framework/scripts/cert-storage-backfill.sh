...
while IFS=$'\t' read -r vm_label module_name ip_addr vmid fqdn kind; do
  ...
  # Fingerprint is the dedup key — see cert-sync.sh:191-206 for the canonical
  # contract. Each POST to mycofu/data/certs/<fqdn> creates a new KV v2
  # version, and the version count is the LE rate-limit oracle
  # (check-cert-budget.sh). When the live cert and the Vault cert have the
  # same SHA-256 fingerprint, Vault already holds this cert; a duplicate
  # write would inflate the LE budget oracle without any actual issuance.
  # "Should we issue a fresh cert because the current one is close to
  # expiry?" is certbot's job on the VM, not backfill's.
