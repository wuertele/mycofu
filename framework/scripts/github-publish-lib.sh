...
# Resolve the SHA the Gatus github-mirror-main endpoint asserts against.
#
# gatus's github-mirror-main check verifies that the published mirror
# metadata's source_commit equals this SHA. The value is always the tip
# of the prod branch: prod is the only ref published to GitHub (see
# publish-to-github.sh), and gatus asserts what is *published*, not what
# is being built. Consequently, dev pipelines, MR pipelines, and prod
# pipelines must all resolve to the same value from the same repo state.
#
# When resolvers disagree, generated gatus CIDATA differs by that one
# line; the bpg provider treats inline `source_raw.data` content as
# significant and plans a replace on every apply. Issue #528: CI used
# `${CI_COMMIT_SHA}` (whatever branch triggered the pipeline) while
# rebuild-cluster.sh resolved `gitlab/prod`, and dev pipelines churned
# gatus CIDATA every run.
#
# Fast path (local): prefer already-fetched refs pointing at prod.
#   - refs/remotes/gitlab/prod — workstation with a `gitlab` remote
#     (rebuild-cluster.sh case).
#   - refs/remotes/origin/prod — workstation or CI clone where prod has
#     been fetched into origin/.
#   - refs/heads/prod — local prod branch (e.g. a workstation on prod).
#
# Fallback (remote): if none of the local refs point at prod, query the
# remote directly with `git ls-remote`. This is the CI path: GitLab CI's
# default GIT_STRATEGY=fetch + GIT_DEPTH=20 fetches only the pipeline's
# branch, so refs/remotes/origin/prod is absent even though origin is
# set. ls-remote returns the tip of prod without any local fetch.
#
# Deliberately absent: no HEAD fallback. HEAD is what is being deployed,
# not what has been published. Falling back to HEAD is exactly the
# defect #528 documents.
#
# Returns 0 with the 40-hex SHA on stdout on success; returns non-zero
# with no stdout when no path can resolve prod's tip (e.g. a downstream
# adopter that has not yet pushed prod). Callers must decide the
# consequence (fail loud, or skip the endpoint entirely when publishing
# is not opted in).
resolve_gatus_expected_source_commit() {
  ...
  # Remote fallback. Prefer `gitlab` remote if configured (workstation),
  # otherwise `origin` (CI). Configurable via
  # GATUS_SOURCE_COMMIT_REMOTE for tests.
  local remote="${GATUS_SOURCE_COMMIT_REMOTE:-}"
  if [[ -z "${remote}" ]]; then
    if git -C "${repo_dir}" config --get "remote.gitlab.url" >/dev/null 2>&1; then
      remote="gitlab"
    elif git -C "${repo_dir}" config --get "remote.origin.url" >/dev/null 2>&1; then
      remote="origin"
    else
      return 1
    fi
  ...
  local ls_output=""
  set +e
  ls_output="$(git -C "${repo_dir}" ls-remote --refs "${remote}" refs/heads/prod 2>/dev/null)"
  rc=$?
  set -e
  if [[ "${rc}" -ne 0 || -z "${ls_output}" ]]; then
    return 1
  fi

  # ls-remote output shape: "<40-hex>\trefs/heads/prod". Guard against
  # SSH banner / MOTD noise by requiring the ref column too, mirroring
  # github_publish_extract_remote_oid.
  sha="$(printf '%s\n' "${ls_output}" \
    | awk '$1 ~ /^[0-9a-f]{40}$/ && $2 ~ /^refs\/heads\/prod\r?$/ { print $1; exit }')"
