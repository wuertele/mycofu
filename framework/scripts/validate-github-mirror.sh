...
resolve_expected_sha() {
  ...
  # (prod-tip) SHA, not the dev pipeline's HEAD. A dev-pipeline
  # CI_COMMIT_SHA fallback here made dev pipelines emit a spurious
  # "expected X but public Y" WARN because the public mirror tracks
  # prod, not dev.
  #
  # Resolution order:
  #   1. Explicit --expected-sha override (tests, manual callers who
  #      already know the answer).
  #   2. Shared resolver in github-publish-lib.sh, which walks
  #      refs/remotes/gitlab/prod, refs/remotes/origin/prod,
  #      refs/heads/prod, and finally ls-remote — the same resolver
  #      generate-gatus-config.sh uses, so the LHS (deployed Gatus
  #      config) and the RHS (this probe) always agree.
  #
  # Deliberately absent: no CI_COMMIT_SHA fallback (defect #619) and
  # no HEAD fallback (defect #528). HEAD is what is being deployed;
  # gatus asserts what has been published.
  if [[ -n "${EXPECTED_SHA}" ]]; then
    printf '%s\n' "${EXPECTED_SHA}"
  else
    resolve_gatus_expected_source_commit "${REPO_DIR}"
  fi
}
...
# The shared resolver returns non-zero (with empty stdout) when no
# path can resolve prod's tip. Under `set -e`, that would exit the
# script before the validation/error block below runs, hiding the
# diagnostic behind a bare exit 1. Capture the exit and let the
# validation block surface a useful message.
set +e
EXPECTED_SHA="$(resolve_expected_sha)"
RESOLVE_EXIT=$?
set -e
if [[ "${RESOLVE_EXIT}" -ne 0 || -z "${EXPECTED_SHA}" || ! "${EXPECTED_SHA}" =~ ^[0-9a-fA-F]{40}$ ]]; then
  echo "ERROR: could not resolve expected prod-tip SHA. Fetch refs/heads/prod (e.g. 'git fetch gitlab prod') or pass --expected-sha explicitly. Resolver exit=${RESOLVE_EXIT}, value=${EXPECTED_SHA:-<empty>}" >&2
  exit 1
fi
...
if [[ "${CHECK_GATUS_CONFIG}" -eq 1 ]]; then
  ...
  if [[ "${PARSE_EXIT}" -eq 11 ]]; then
    # #718: the old text blamed a set GATUS_GITHUB_EXPECTED_SOURCE_COMMIT. That
    # can no longer be the cause on a production generation — the generator
    # clears the seam unless invoked with --allow-test-seams — so pointing an
    # operator at it during a prod-monitoring outage would send them after a
    # provably-wrong hypothesis. The real causes are publish.github.enabled
    # being false, or a config generated before the endpoint existed.
    echo "ERROR: Gatus config has no github-mirror-main endpoint; generate-gatus-config.sh did not emit one (is .publish.github.enabled true in site/config.yaml, and was this config generated by the current generator?)." >&2
    exit 11
