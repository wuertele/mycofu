...
# Grub-path fixup backstop for the retry / already-converged skip path (#339).
#
# On an overlay-root VM, install-grub.pl during switch-to-configuration emits
# ($drive*)/store/... instead of ($drive*)/nix/store/..., which bricks the VM
# on its next cold boot. converge-lib.sh's converge_fix_grub_paths repairs
# this immediately AFTER the switch — but that repair runs in the deploy
# script, and for a cicd self-deploy the switch stops the gitlab-runner and
# kills this job (converge-lib.sh converge_step_closure header, #206) before
# the post-switch repair runs. The switch itself survives (systemd-run), so
# GitLab retries the job (retry: runner_system_failure); the retry lands on
# the `live == built` skip path below and would skip convergence — and its
# grub fixup — entirely, leaving grub.cfg with )/store/ paths (the
# 2026-07-06 #496 / 2026-07-09 #339 signature; caught only later by the R7.3
# boot-integrity gate). Running the idempotent fixup here means a skipped or
# retried control-plane deploy can never leave a broken grub.cfg.
#
# Keep this behavior-identical to converge_fix_grub_paths in converge-lib.sh:
# the sed only matches )/store/ (after a GRUB drive prefix), never /nix/store/,
# so it is safe to run on an already-correct grub.cfg. Verify after the sed
# and fail loudly (die) if any )/store/ path survives.
fix_grub_paths() {
  local host="$1"
  local ip="$2"
  ssh "${SSH_OPTS[@]}" "root@${ip}" \
    "if [ -f /boot/grub/grub.cfg ]; then sed -i 's|)/store/|)/nix/store/|g' /boot/grub/grub.cfg; if grep -qF ')/store/' /boot/grub/grub.cfg; then echo 'fix_grub_paths: verify-after-write FAILED — grub.cfg still references )/store/ paths after sed:' >&2; grep -nF ')/store/' /boot/grub/grub.cfg >&2; exit 1; fi; fi" \
    || die "${host}: grub-path fixup failed on ${ip} (see ssh stderr above). See converge_fix_grub_paths (#339); framework/scripts/check-boot-integrity.sh --host ${host} pinpoints broken paths."
}
...
deploy_host() {
  ...
  if [[ "${live}" == "${built}" && "${ROOT_DISK_RESIZED}" -eq 0 ]]; then
    # #339 backstop: a prior (possibly killed and retried) attempt may have
    # switched this closure without its post-switch grub fixup running. Repair
    # grub.cfg here so a skipped/retried deploy never leaves a broken bootchain.
    fix_grub_paths "${host}" "${ip}"
    log "${host}: live closure matches built, skipping (grub paths verified)"
    return 0
  fi

  if [[ "${live}" == "${built}" && "${ROOT_DISK_RESIZED}" -eq 1 ]]; then
    # Repair grub BEFORE the resize reboot: rebooting a VM whose grub.cfg
    # still carries )/store/ paths is exactly the cold-boot brick #339 guards.
    fix_grub_paths "${host}" "${ip}"
...
main() {
  ...
  if [[ $# -eq 0 ]]; then
    while IFS= read -r host; do
      [[ -n "$host" ]] && selected_hosts+=("$host")
    done < <("${VM_SCOPE_SCRIPT}" control-plane-built-roles)
  else
    local valid_hosts
    valid_hosts="$("${VM_SCOPE_SCRIPT}" control-plane-built-roles)"
    for host in "$@"; do
      if printf '%s\n' "$valid_hosts" | grep -Fxq "$host"; then
        selected_hosts+=("${host}")
      else
