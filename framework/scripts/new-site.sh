...
if [[ "$DOMAIN" != "example.com" ]]; then
  sed -i.bak "s/example\.com/${DOMAIN}/g" "${SITE_DIR}/config.yaml"
  rm -f "${SITE_DIR}/config.yaml.bak"
fi
...
# The VM-class taxonomy is declared with scope/control_plane and read by
