...
# --- Consolidated probe-bundle helpers (#34/#67, #35/#68/#139) ------------
# Both probes are self-contained: they read every value they need from CONFIG
# and SOPS so they can run either from their early ONLY-gate (hermetic tests)
# or from their section in the main body. They FAIL closed on any inability to
# determine their answer, per .claude/rules/destruction-safety.md — a safety
# check that silently SKIPs an undetectable problem is a false pass.

# Resolve the SOPS age key the same way build-image.sh:55 and validate.sh:1868
# do: honour an already-exported SOPS_AGE_KEY_FILE (CI sets it to
# /var/lib/mycofu-secrets/age-key), else fall back to the operator key at repo root.
probe_ensure_sops_age_key() {
...
# Vault root-token validity probe (#34/#67). The sibling
# "vault-<env> initialized and unsealed" check uses the UNAUTHENTICATED
# /v1/sys/health endpoint, which returns health regardless of any token and so
# cannot detect SOPS/Vault root-token divergence (.claude/rules/vault.md). This
# probe reads the env-scoped root token from SOPS and calls
# /v1/auth/token/lookup-self, which returns 200 for a valid token and 403 for
# an invalid one. A 403 means the SOPS-stored token no longer matches the live
# Vault — the exact divergence the write-once SOPS invariant exists to prevent
# and that a PBS restore would surface. Env-scoped like its Vault siblings.
vault_token_lookup_check() {
  local env="$1"
  local vault_ip secrets_file token resp http_code body attempt

  vault_ip="$(yq -r ".vms.vault_${env}.ip" "$CONFIG" 2>/dev/null || true)"
  if [[ -z "$vault_ip" || "$vault_ip" == "null" ]]; then
    echo "no vault_${env} IP in config.yaml — cannot probe token validity"
    return 1
  fi

  probe_ensure_sops_age_key
  secrets_file="${REPO_DIR}/site/sops/secrets.yaml"
  token="$(sops -d --extract "[\"vault_${env}_root_token\"]" "$secrets_file" 2>/dev/null || true)"
  if [[ -z "$token" || "$token" == "null" ]]; then
    echo "could not decrypt vault_${env}_root_token from SOPS (${secrets_file}) — cannot verify token validity; FAIL-closed per destruction-safety doctrine"
    return 1
  fi

  # Bounded retry tolerates transient network/5xx/503-sealed blips without
  # masking a definitive verdict: HTTP 200 (valid) and 403 (divergence) both
  # short-circuit immediately; only ambiguous/transient codes are retried.
  for attempt in 1 2 3; do
    resp="$(curl -sk -w $'\n%{http_code}' --connect-timeout 5 --max-time 10 \
      -H "X-Vault-Token: ${token}" \
      "https://${vault_ip}:8200/v1/auth/token/lookup-self" 2>/dev/null || true)"
    http_code="$(printf '%s' "$resp" | tail -n1)"
    body="$(printf '%s' "$resp" | sed '$d')"
    case "$http_code" in
      200)
        if printf '%s' "$body" | jq -e '.data.id' >/dev/null 2>&1; then
          echo "vault-${env} root token in SOPS is valid (lookup-self HTTP 200)"
          return 0
        fi
        echo "vault-${env} lookup-self returned HTTP 200 but body carried no .data.id — treating as invalid"
        return 1
        ;;
      403)
        echo "vault-${env} root token in SOPS REJECTED (lookup-self HTTP 403): SOPS/Vault token divergence — the SOPS token no longer matches live Vault (.claude/rules/vault.md write-once invariant)"
        return 1
        ;;
      *)
  ...
  echo "vault-${env} lookup-self returned no definitive result after 3 attempts (last HTTP ${http_code:-000}) — FAIL-closed"
  return 1
}

# Hash a token without a trailing newline so SOPS/vdb fingerprints are directly
# comparable on both NixOS (sha256sum) and the macOS operator workstation
# (shasum). Only the 12-hex prefix is emitted; token material never is.
vault_token_hash_prefix() {
  local token="$1"
...
# Locale-pinned: ERE bracket ranges are collation-dependent, so [A-Za-z] is
# only guaranteed to be exactly the ASCII letters under LC_ALL=C. Pinning it
# here also makes ${#...} a byte count, so the 512 bound means 512 bytes.
vault_token_charset_ok() {
  local LC_ALL=C
  [[ "$1" =~ ^[A-Za-z0-9._-]+$ && ${#1} -le 512 ]]
}

# Vault vdb root-token validity probe (#983). post-deploy.sh reads this exact
# file and configure-vault.sh prefers that exported value over SOPS, so file
# existence alone cannot detect a stale, revoked credential. Read it through
# the real consumer path and authenticate it against lookup-self. Like the SOPS
# sibling above, every indeterminate state FAILs closed.
vault_vdb_token_lookup_check() {
  local env="$1"
  local vault_ip secrets_file vdb_token sops_token vdb_hash sops_hash hash_match vdb_length
  local resp http_code body attempt

  vault_ip="$(yq -r ".vms.vault_${env}.ip" "$CONFIG" 2>/dev/null || true)"
  if [[ -z "$vault_ip" || "$vault_ip" == "null" ]]; then
    echo "no vault_${env} IP in config.yaml — cannot probe /var/lib/vault/root-token on the Vault VM"
    return 1
  fi

  if ! vdb_token="$(ssh_node "root@${vault_ip}" "cat /var/lib/vault/root-token 2>/dev/null")"; then
    echo "could not read /var/lib/vault/root-token from vault-${env} VM (${vault_ip}) — FAIL-closed per destruction-safety doctrine"
    return 1
  fi
  if [[ -z "$vdb_token" ]]; then
    echo "/var/lib/vault/root-token is absent or empty on vault-${env} VM (${vault_ip}) — FAIL-closed per destruction-safety doctrine"
    return 1
  fi

  # Vault tokens use service (hvs.), batch (hvb.), and recovery (hvr.) prefixes
  # with base62 or base64url bodies; legacy tokens are UUIDs. All fit this
  # allowlist. The existing non-empty check is the only lower bound because
  # Vault documents no minimum; a guessed floor would create the false failure
  # barred by design-taste.md Principle 11(c).
  # Bash anchors match the whole string, not each line, so an embedded newline
  # fails the match instead of injecting a second curl-config directive.
  # The 512-byte ceiling cannot false-FAIL a framework-written token: the only
  # writers of /var/lib/vault/root-token are framework/scripts/init-vault.sh
  # (a vault operator init root token) and
  # framework/scripts/rotate-vault-credentials.sh (a sys/generate-root token),
  # both standard hvs. tokens of roughly 95 bytes.
  if ! vault_token_charset_ok "$vdb_token"; then
    if ! vdb_hash="$(vault_token_hash_prefix "$vdb_token")"; then
      echo "could not compute SHA-256 token fingerprints — FAIL-closed"
      return 1
    fi
    vdb_length="$(LC_ALL=C printf '%s' "$vdb_token" | wc -c | tr -d '[:space:]')"
    echo "/var/lib/vault/root-token on vault-${env} VM (${vault_ip}) failed the token charset/length allowlist (possible corrupt file or curl-config injection); this is the file post-deploy.sh reads (SHA-256 prefix=${vdb_hash}, bytes=${vdb_length}) — FAIL-closed per destruction-safety doctrine"
    return 1
  fi

  probe_ensure_sops_age_key
  secrets_file="${REPO_DIR}/site/sops/secrets.yaml"
  sops_token="$(sops -d --extract "[\"vault_${env}_root_token\"]" "$secrets_file" 2>/dev/null || true)"
  if [[ -z "$sops_token" || "$sops_token" == "null" ]]; then
    echo "could not decrypt vault_${env}_root_token from SOPS (${secrets_file}) — cannot compare vdb/SOPS token fingerprints; FAIL-closed per destruction-safety doctrine"
    return 1
  fi
  if ! vdb_hash="$(vault_token_hash_prefix "$vdb_token")" ||
     ! sops_hash="$(vault_token_hash_prefix "$sops_token")"; then
    echo "could not compute SHA-256 token fingerprints — FAIL-closed"
    return 1
  fi
  hash_match="no"
  [[ "$vdb_hash" == "$sops_hash" ]] && hash_match="yes"
  echo "vault-${env} root token SHA-256 prefixes: vdb=${vdb_hash} sops=${sops_hash} match=${hash_match}"

  # Match the bounded retry doctrine of the SOPS sibling: definitive 200/403
  # responses stop immediately, while only transient or ambiguous results retry.
  for attempt in 1 2 3; do
    # Feed the live root-token header through stdin so it never appears in
    # curl's argv (and never needs a credential-bearing file on disk).
    resp="$(printf 'header = "X-Vault-Token: %s"\n' "$vdb_token" \
      | curl -sk --config - -w $'\n%{http_code}' --connect-timeout 5 --max-time 10 \
        "https://${vault_ip}:8200/v1/auth/token/lookup-self" 2>/dev/null || true)"
    http_code="$(printf '%s' "$resp" | tail -n1)"
    body="$(printf '%s' "$resp" | sed '$d')"
    case "$http_code" in
      200)
        if printf '%s' "$body" | jq -e '.data.id' >/dev/null 2>&1; then
          if [[ "$hash_match" == "yes" ]]; then
            echo "vault-${env} root token on vdb is valid (lookup-self HTTP 200; fingerprint match=yes)"
            return 0
          fi
          echo "vault-${env} root token on vdb is valid but its SOPS fingerprint differs: likely a rotation interrupted between vdb delivery and old-token revoke, so an escrowed old root token may still be live"
          return 3
        fi
        echo "vault-${env} vdb token lookup-self returned HTTP 200 but body carried no .data.id — treating as invalid"
        return 1
        ;;
      403)
        echo "vault-${env} token file /var/lib/vault/root-token on vdb is REJECTED by live Vault (lookup-self HTTP 403); this is the file post-deploy.sh reads. Determine whether to rotate with the corrected driver because the vdb copy is simply stale relative to a completed rotation, or to perform a PBS restore because vdb/Raft provenance has diverged."
        return 1
        ;;
      *)
        [[ "$attempt" -lt 3 ]] && sleep 3
  ...
  echo "vault-${env} vdb token lookup-self returned no definitive result after 3 attempts (last HTTP ${http_code:-000}) — FAIL-closed"
  return 1
}

# GitLab runner functional-health probe (#35/#68/#139). The previous check —
# `systemctl is-active gitlab-runner.service` — tested process liveness, not
# functional health: during the 2026-03-30 incident the service was "active"
# while every job request returned 403 (stale auth token), and validate.sh
# reported 52/52 PASS while no job could run. This probe answers the real
# question against the GitLab API in two steps:
#   1. Read the runner's auth token from cicd's config.toml and POST it to
#      /api/v4/runners/verify. A valid token returns the runner id; a stale
#      token returns 403 — reproducing exactly what the runner itself sees.
#   2. Read /api/v4/runners/<id> and require status=="online" and not paused —
#      the API's own truth about whether the runner can pick up jobs. `online`
#      is true for an idle heartbeating runner, so a normally idle runner does
#      not false-FAIL (#139 AC3). Not env-scoped: cicd is shared infra, matching
#      the single (non-per-env) sibling runner check it replaces.
runner_online_check() {
  local cicd_ip gitlab_ip secrets_file root_pw oauth_token
  local runner_token verify_json runner_id detail status paused active attempt

  cicd_ip="$(yq -r '.vms.cicd.ip' "$CONFIG" 2>/dev/null || true)"
  gitlab_ip="$(yq -r '.vms.gitlab.ip' "$CONFIG" 2>/dev/null || true)"
  if [[ -z "$cicd_ip" || "$cicd_ip" == "null" || -z "$gitlab_ip" || "$gitlab_ip" == "null" ]]; then
    echo "missing cicd/gitlab IP in config.yaml — cannot probe runner health"
    return 1
  fi

  # 1. Runner auth token from cicd config.toml (mirrors register-runner.sh:120,
  #    hardened per review: anchored `token =` match tolerates flexible spacing
  #    and single- or double-quoted values and skips the `token_obtained_at`
  #    key. head -1 takes the first [[runners]] block — cicd runs a single
  #    registration; a multi-runner cicd would need per-block iteration.
  runner_token="$(ssh_node "root@${cicd_ip}" \
    "grep -E '^[[:space:]]*token[[:space:]]*=' /etc/gitlab-runner/config.toml 2>/dev/null | head -1 | sed -E 's/^[^=]*=[[:space:]]*.//; s/.\$//'" 2>/dev/null || true)"
  if [[ -z "$runner_token" ]]; then
    echo "could not read runner auth token from ${cicd_ip}:/etc/gitlab-runner/config.toml — runner not registered or cicd unreachable; FAIL-closed"
    return 1
  fi

  # 2. Admin OAuth token from SOPS (mirrors configure-gitlab.sh:185).
  probe_ensure_sops_age_key
  secrets_file="${REPO_DIR}/site/sops/secrets.yaml"
  root_pw="$(sops -d --extract '["gitlab_root_password"]' "$secrets_file" 2>/dev/null || true)"
  if [[ -z "$root_pw" || "$root_pw" == "null" ]]; then
    echo "could not decrypt gitlab_root_password from SOPS — cannot query runners API; FAIL-closed"
    return 1
  fi
  # --data-urlencode each field: the root password is GitLab's random
  # initial_root_password and can contain '&', '+', '%', or '=' which would
  # corrupt a raw -d body and false-red a healthy GitLab (review P1).
  for attempt in 1 2 3; do
    oauth_token="$(curl -sk --connect-timeout 5 --max-time 10 -X POST \
      "https://${gitlab_ip}/oauth/token" \
      --data-urlencode "grant_type=password" \
      --data-urlencode "username=root" \
      --data-urlencode "password=${root_pw}" 2>/dev/null \
      | jq -r '.access_token // empty' 2>/dev/null || true)"
    [[ -n "$oauth_token" ]] && break
    [[ "$attempt" -lt 3 ]] && sleep 3
  done
  if [[ -z "$oauth_token" ]]; then
    echo "could not obtain GitLab OAuth token after 3 attempts — GitLab unreachable; FAIL-closed"
    return 1
  fi

  # 3. Verify the runner's own token — catches the stale-token 403 (#139 AC1).
  #    --tls-max 1.2 mirrors register-runner.sh:121 for the verify endpoint
  #    (an established, verify-endpoint-specific TLS pin, live-verified working).
  #    Capture body + HTTP status and retry like the sibling calls so a
  #    transient 5xx/timeout does not masquerade as a stale-token rejection
  #    (review P1): a real 403 short-circuits immediately; only ambiguous or
  #    transient responses are retried.
  runner_id=""
  local verify_code=""
  for attempt in 1 2 3; do
    verify_json="$(curl -sk --tls-max 1.2 -w $'\n%{http_code}' \
      --connect-timeout 5 --max-time 10 -X POST \
      "https://${gitlab_ip}/api/v4/runners/verify" -F "token=${runner_token}" 2>/dev/null || true)"
    verify_code="$(printf '%s' "$verify_json" | tail -n1)"
    verify_json="$(printf '%s' "$verify_json" | sed '$d')"
    case "$verify_code" in
      200)
        runner_id="$(printf '%s' "$verify_json" | jq -r '.id // empty' 2>/dev/null || true)"
        break
        ;;
      403)
        echo "runner auth token REJECTED by GitLab (/runners/verify HTTP 403) — stale registration; runner cannot pick up jobs (#139)"
        return 1
        ;;
      *)
        [[ "$attempt" -lt 3 ]] && sleep 3
  ...
  if [[ -z "$runner_id" ]]; then
    echo "/runners/verify returned no definitive runner id (last HTTP ${verify_code:-000}) after 3 attempts — cannot confirm runner token validity; FAIL-closed"
    return 1
  fi

  # 4. Online + not paused — the API's truth about job-pickup ability (#68).
  #    Retry the online check: contacted_at can briefly lag after a restart.
  status=""
  for attempt in 1 2 3; do
    detail="$(curl -sk --connect-timeout 5 --max-time 10 \
      -H "Authorization: Bearer ${oauth_token}" \
      "https://${gitlab_ip}/api/v4/runners/${runner_id}" 2>/dev/null || true)"
    status="$(printf '%s' "$detail" | jq -r '.status // empty' 2>/dev/null || true)"
    # NOT `// empty`: jq's // treats boolean false as absent, which would
    # silently drop `active:false` and let a deactivated runner PASS (review
    # P1). Read the raw value ("true"/"false"/"null") and compare explicitly.
    paused="$(printf '%s' "$detail" | jq -r '.paused' 2>/dev/null || true)"
    active="$(printf '%s' "$detail" | jq -r '.active' 2>/dev/null || true)"
    [[ "$status" == "online" ]] && break
    [[ "$attempt" -lt 3 ]] && sleep 3
  done
  if [[ "$status" != "online" ]]; then
    echo "runner id ${runner_id} token is valid but status='${status:-unknown}' (not online) — cannot pick up jobs"
    return 1
  fi
  # paused==true (or active==false on older GitLab) means jobs are refused.
  if [[ "$paused" == "true" || "$active" == "false" ]]; then
    echo "runner id ${runner_id} is online but PAUSED (paused=${paused}, active=${active}) — will not pick up jobs"
    return 1
  fi
  echo "runner id ${runner_id} verified: token accepted, status=online, paused=${paused:-false}"
  return 0
}
...
if [[ "${MYCOFU_VALIDATE_ONLY_VAULT_TOKEN:-0}" == "1" ]]; then
  for ENV in "${ENVS[@]}"; do
    check_capture "vault-${ENV} root token valid (lookup-self)" vault_token_lookup_check "${ENV}"
    # check_warn prints captured output exactly like check_capture, preserving
    # every fingerprint and recovery diagnostic while mapping rc 3 to WARN.
    check_warn "vault-${ENV} vdb root token valid (lookup-self)" vault_vdb_token_lookup_check "${ENV}"
  done
  if [[ "$FAIL" -gt 0 ]]; then
    exit 1
  fi
...
if [[ "${MYCOFU_VALIDATE_ONLY_RUNNER_ONLINE:-0}" == "1" ]]; then
  check_capture "GitLab runner registered and online (API)" runner_online_check
  if [[ "$FAIL" -gt 0 ]]; then
    exit 1
  fi
  exit 0
...
# Sprint 049 MR-2 (A1/A2): the rotation manifest is a fail-closed admission
# ratchet for every declared credential surface. This is CI-only and must not
# become a SKIP, because an unreadable manifest or SOPS file would make the
# destructive rotation plan untrustworthy.
check_capture "rotation manifest conforms" \
...
for ENV in "${ENVS[@]}"; do
  ...
  # #34/#67: /v1/sys/health is unauthenticated and cannot detect SOPS/Vault
  # root-token divergence. Authenticate the SOPS-stored root token against
  # lookup-self (403 => divergence). FAIL-closed if SOPS/Vault unreachable.
  check_capture "vault-${ENV} root token valid (lookup-self)" \
    vault_token_lookup_check "${ENV}"
  # check_warn prints captured output exactly like check_capture, preserving
  # every fingerprint and recovery diagnostic while mapping rc 3 to WARN.
  check_warn "vault-${ENV} vdb root token valid (lookup-self)" \
    vault_vdb_token_lookup_check "${ENV}"
done
...
# Runner functional health (#35/#68/#139). `systemctl is-active` only proves
# the process is running — during the 2026-03-30 incident it reported active
# while the runner got 403 from GitLab and could pick up no jobs. Verify the
# runner's own token against the GitLab API and require it online + not paused.
CICD_IP=$(yq -r '.vms.cicd.ip' "$CONFIG")
check_capture "GitLab runner registered and online (API)" \
  runner_online_check
...
if [[ "$REGRESSION" -eq 1 || "$REGRESSION_SAFE" -eq 1 ]]; then
  ...
  check "R3.3: tofu-wrapper.sh only exports pre-deploy secrets" \
    bash -c "
      ...
      # Should only see: ssh_pubkey, pdns_api_key, app pre-deploy secrets
  ...
  # CI/CD runner (both have /var/lib/mycofu-secrets/age-key or operator.age.key).
  #
