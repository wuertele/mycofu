...
# Uniform LogLevel=ERROR across the github-publish family (#299): suppresses
# SSH's "Warning: Permanently added ..." banner on first connect, whose token
# used to poison the OID parser (#297). The OID helper below is defense-in-depth
# against any residual stderr noise.
#
# Set-if-unset: if the operator has already exported GIT_SSH_COMMAND in their
# shell (custom wrapper, alternate identity), respect it — the OID helper still
# handles the SSH banner case even without LogLevel=ERROR.
if [[ -z "${GIT_SSH_COMMAND:-}" ]]; then
  export GIT_SSH_COMMAND="ssh -o LogLevel=ERROR"
fi

# Capture 2>&1 so the shared OID helper (#298) can drop any stray SSH banner
# from stderr — defense-in-depth on top of LogLevel=ERROR. Fail loudly on
# non-zero exit rather than depending on set -e alone.
set +e
REMOTE_MAIN="$("${SYNC_TO_MAIN_GIT_BIN}" -C "${TMP_DIR}" ls-remote --heads github refs/heads/main 2>&1)"
REMOTE_MAIN_EXIT=$?
set -e
if [[ "${REMOTE_MAIN_EXIT}" -ne 0 ]]; then
  echo "ERROR: GitHub ls-remote failed:" >&2
  printf '%s\n' "${REMOTE_MAIN}" >&2
  exit 1
fi
...
if [[ -n "${REMOTE_MAIN_OID}" ]]; then
  ...
  # Explicit guard call (#292): make the abort visible in the script flow
  # rather than relying on set -e alone to react to the helper's non-zero exit.
  if ! github_publish_initial_rewrite_guard; then
    exit 1
  fi
fi
...
# Quote REMOTE_MAIN_OID (#292): defense against future changes that could
# word-split the value. An empty value at this point is intentional — the
# initial-rewrite guard above only allowed us past when
# GITHUB_PUBLISH_ALLOW_INITIAL_REWRITE=1, and git's `main:` (empty OID) means
# "expected main to not exist yet", the correct first-publish lease.
