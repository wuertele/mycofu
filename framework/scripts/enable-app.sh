...
find_next_octet() {
  ...
  # Also check management network IPs for shared/control-plane VMs.
  local mgmt_ips
...
#
# Enabling an app here implies scope: env-bound in the VM-class taxonomy.
# Run framework/scripts/vm-scope.sh classes to see the resolved view,
# including derived app scopes.
...
yq -i ".applications.${APP}.environments.prod.pool line_comment = \"required for OpenTofu tags; valid: prod\"" "$APPS_CONFIG"
...
yq -i ".applications.${APP}.environments.dev.pool line_comment = \"required for OpenTofu tags; valid: dev\"" "$APPS_CONFIG"
...
echo "  3. Generate SSH host keys: framework/scripts/new-site.sh --fill-ssh-keys"
echo "  4. Add any required secrets to SOPS"
echo "  5. Add the flake output to flake.nix:"
echo "       ${APP}-image = mkImage [ ./site/nix/hosts/${APP}.nix ];"
echo "  6. Add the OpenTofu module instantiation to framework/tofu/root/main.tf."
...
echo '       tags   = ["pool-${local.app_<name>.environments.<env>.pool}"]'
if [[ "$DEFAULT_BACKUP" == "true" ]]; then
  echo "  7. Verify the app's backup=true metadata and VMID are present in"
  echo "     site/applications.yaml so restore-before-start.sh includes it in"
  echo "     preboot restore manifests."
fi
echo "  8. Build the image and deploy via the pipeline"
echo ""
