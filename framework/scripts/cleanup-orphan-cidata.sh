...
#
# Usage:
#   cleanup-orphan-cidata.sh [--dry-run] [--verbose]
#
...
Sweep every Proxmox node listed in config.yaml for orphan cidata zvols
...
# Read node mgmt IPs. macOS ships bash 3.2 (no `mapfile`), so use the
# portable `while read` accumulator.
NODE_IPS=()
...
if [[ "${#NODE_IPS[@]}" -eq 0 || -z "${NODE_IPS[0]}" ]]; then
  echo "ERROR: No nodes found in ${CONFIG} (.nodes[].mgmt_ip)" >&2
  exit 1
fi
...
# one node. A zvol referenced by ANY VM config in the cluster is NOT an
# orphan, even when it lives on a different node (e.g., ZFS replication
# targets vm-<vmid>-disk-N to non-owner nodes for HA-restart speed).
#
# A per-node `qm config <vmid>` check would fail on non-owner nodes (Proxmox
# only stores each VM's config under nodes/<owner>/qemu-server/) and
# misclassify the replicated zvol on those nodes as an orphan. The size
# safety filter alone would prevent destruction of normal-sized data disks,
# but the cluster-wide check is the principled fix and protects against
# future cases where a small data zvol might legitimately exist (and matches
# the cidata size threshold).
# Try each node until one read succeeds. Distinguishes:
#   - grep rc=0 (matched something)        : success, use the output
#   - grep rc=1 (no matches in empty cfg)  : success, attached set is empty
#   - any other rc (SSH 255, grep 2, ...)  : node unreachable or broken,
#                                            try the next one
# If every node fails to read /etc/pve, fail closed (do not proceed with
# an empty attached set, which would cause everything to look orphan).
ATTACHED_RAW=""
ATTACHED_READ_OK=0
for try_node in "${NODE_IPS[@]}"; do
  set +e
  ATTACHED_RAW=$(ssh_node "${try_node}" \
    "grep -h '${STORAGE_POOL}:vm-' /etc/pve/nodes/*/qemu-server/*.conf 2>/dev/null")
  try_rc=$?
  set -e
...
# Extract `vmstore:vm-<vmid>-(cloudinit|disk-<N>)` references and dedupe.
# `grep -o` returns rc=1 when there are no matches (empty fresh cluster).
# pipefail would treat this as a failure, but it isn't — so swallow.
set +e
ATTACHED_SET=$(printf '%s\n' "${ATTACHED_RAW}" \
  | grep -oE "${STORAGE_POOL}:vm-[0-9]+-(cloudinit|disk-[0-9]+)" \
  | sort -u)
set -e

if [[ "${VERBOSE}" -eq 1 ]]; then
  echo "==> cluster-wide attached zvols:"
  printf '  %s\n' ${ATTACHED_SET}
fi
...
for node_ip in "${NODE_IPS[@]}"; do
  ...
  while IFS=$'\t' read -r zvol refer; do
    ...
    # Cluster-wide attached check — protects ZFS replication targets and
    # any future case where the same zvol name appears on multiple nodes.
    zvol_full="${STORAGE_POOL}:${short_name}"
    if grep -qxF "${zvol_full}" <<< "${ATTACHED_SET}"; then
      [[ "${VERBOSE}" -eq 1 ]] && echo "  attached cluster-wide: ${short_name}"
      continue
    fi
