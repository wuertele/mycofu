...
# Issue #528: shared resolver for the Gatus github-mirror source commit.
# The workstation and CI paths must agree on this SHA (prod tip); the
# resolver was inlined here previously and disagreed with CI's use of
# $CI_COMMIT_SHA. Both call sites now share github-publish-lib.sh.
...
if ! GATUS_EXPECTED_SOURCE_COMMIT="$(resolve_gatus_expected_source_commit "${REPO_DIR}")"; then
...
for mod in $CONTROL_PLANE_MODULES; do
  if [[ "$TOFU_TARGETS" == *"$mod"* || -z "$TOFU_TARGETS" ]]; then
    ...
    build_rebuild_default_plan_json "atomic-precheck-${mod_name}" "-target=${mod}" "$ATOMIC_PRECHECK_PLAN_JSON"
