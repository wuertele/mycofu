...
#   framework/scripts/rebuild-cluster.sh --scope env=dev           # dev-only and dev environment modules
#   framework/scripts/rebuild-cluster.sh --scope env=prod          # prod modules, excluding control-plane/shared
#   framework/scripts/rebuild-cluster.sh --recreate                # force new vda + CIDATA (PBS excluded)
#   framework/scripts/rebuild-cluster.sh --recreate --scope env=dev # preserve precious state via park/adopt + preboot restore
...
source "${SCRIPT_DIR}/known-hosts-scope-lib.sh"
source "${SCRIPT_DIR}/lib/converge-incomplete-vm.sh"
VDB_PARK_CONFIG="$CONFIG"
VDB_PARK_APPS_CONFIG="$APPS_CONFIG"
...
if [[ -n "$SCOPE" ]]; then
  case "$SCOPE" in
    ...
    *)
      ...
      echo "Valid: control-plane, data-plane, env=dev, env=prod, vm=name1,name2, onboard-app=name" >&2
      exit 1
      ;;
  esac
fi
...
step_in_scope() {
  ...
  case "$step" in
    networking|storage|kernel|cluster|snippets|sentinel|metrics|pipeline)
...
restore_pin_validate() {
  ...
  if ! jq -e '
    ...
    and all(.pins | to_entries[]?;
      (.key | test("^[0-9]+$")) and
      (
        ((.value | type) == "string" and (.value | length > 0)) or
        ((.value | type) == "object" and (.value.volid | type) == "string" and (.value.volid | length > 0))
      )
    )
...
if [[ -f "$CONFIG" ]]; then
  ...
  # nix_builder.nixpkgs_ref is the pin for the workstation preflight probes
  # below. A bare `nixpkgs#` ref resolves through the global flake registry
  # (floating nixpkgs-unstable) instead of flake.lock — the #564 failure
  # class. Two probes (hello and stdenv path-info) were still floating on
  # the DR path before #566; both now use this pin.
  BUILDER_NIXPKGS=$(yq -r '.nix_builder.nixpkgs_ref // "github:NixOS/nixpkgs/nixpkgs-26.05-darwin"' "$CONFIG")
  # Defensive validation mirroring setup-nix-builder.sh: reject empty/null
  # (mikefarah yq's `//` only fills for null, not "") and reject the bare
  # indirect registry form. If either check fails, mark the ref invalid so
  # the probes below skip — otherwise they would still evaluate against the
  # floating registry, which is exactly what #566 sought to eliminate.
  BUILDER_NIXPKGS_VALID=1
  if [[ -z "$BUILDER_NIXPKGS" || "$BUILDER_NIXPKGS" == "null" ]]; then
    prereq_fail "nix_builder.nixpkgs_ref is empty or null in ${CONFIG} — set to an explicit ref, e.g. github:NixOS/nixpkgs/nixpkgs-26.05-darwin (#566)"
    BUILDER_NIXPKGS_VALID=0
  elif [[ "$BUILDER_NIXPKGS" =~ ^(flake:)?nixpkgs(/|$) ]]; then
    prereq_fail "nix_builder.nixpkgs_ref is an indirect registry ref: ${BUILDER_NIXPKGS} (floats on unstable; #566)"
    BUILDER_NIXPKGS_VALID=0
  fi
  if [[ "$BUILDER_TYPE" == "linux-builder" ]] && [[ "$(uname)" == "Darwin" ]] \
     && [[ "$BUILDER_NIXPKGS_VALID" == "1" ]]; then
    if pgrep -f "qemu.*nixos.qcow2" &>/dev/null; then
      ...
      # Use the pinned nix_builder.nixpkgs_ref, not the impure `<nixpkgs>`
      # channel — the impure channel floats on the global registry
      # (nixpkgs-unstable) and was the failure class behind #564. See #566.
      if nix build --log-format raw "${BUILDER_NIXPKGS}#hello" --system x86_64-linux \
      ...
      # Pin the stdenv path-info probe as well; a bare `nixpkgs#stdenv` still
      # resolves through the global flake registry even with --system set,
      # so an unstable registry hiccup or schema change would fire this
      # branch on a DR preflight. Both probes now use the same pin (#566).
      if ! nix path-info --log-format raw "${BUILDER_NIXPKGS}#stdenv" --system x86_64-linux 2>/dev/null | grep -q '/nix/store'; then
...
# =====================================================================
# Step 1.5: Apply NVMe firmware-quirk kernel parameters
# =====================================================================
# Adds the kernel cmdline workaround for the NVMe D3cold-latch firmware
# bug. Idempotent. Runs BEFORE Step 2 (configure-node-storage) so the
# first ZFS data-pool operation happens on a kernel that already has
# the workaround applied -- the very ZFS operations Step 2 performs
# are the kind of sustained NVMe I/O that triggered the original bug.
#
# When configure-node-kernel.sh returns exit 10 (persistent config
# written, reboot required), this step automates the rolling reboot in
# place IF it is safe to do so:
#
#   - Safe to auto-reboot: no node has any non-stopped VM (running,
#     paused, suspended, prelaunch, or io-error). This is the normal
#     rebuild-cluster.sh case (fresh bringup, post-reset, full-scope
#     rebuild). Sequential `ssh root@<n> reboot`, wait for SSH to
#     return, repeat. Then re-verify and proceed to Step 2.
#
#   - NOT safe to auto-reboot: at least one node has a non-stopped
#     VM. This is the unusual case (operator running rebuild-cluster.sh
#     on a populated healthy cluster, or recovering from a storage-
#     stuck VM). Auto-rebooting would crash live or zombie workloads.
#     Bail out with a banner pointing the operator at the HA-aware
#     procedure in OPERATIONS.md, and exit 10.
#
# See docs/reports/2026-05-03-pve02-storage-failure-and-ha-recovery-retrospective.md
# for the incident retrospective. The fence runbook at
# .claude/rules/storage-failure-fence.md is the INCIDENT response path
# (storage failed but node alive); it is NOT the planned reboot path.
if step_in_scope kernel; then
  step_start 1.5 "Apply NVMe-quirk kernel parameters (drop-in + bootloader refresh)"
  set +e
  "${SCRIPT_DIR}/configure-node-kernel.sh" --all
  KERNEL_RC=$?
  set -e
  if [[ $KERNEL_RC -eq 0 ]]; then
    step_done 1.5 "Kernel parameters already effective on all nodes"
  elif [[ $KERNEL_RC -eq 10 ]]; then
    # Determine if it's safe to auto-reboot. Walk every node from
    # config.yaml; for each, run `qm list` as the remote command and
    # filter the captured output locally for `$3 != "stopped"`.
    #
    # Filter rationale: catches `running` plus `paused`, `suspended`,
    # `prelaunch`, `io-error`, and any future single-token state
    # Proxmox introduces. `io-error` is the exact zombie state from
    # the 2026-05-03 incident this MR mitigates.
    #
    # Failure-mode rationale: both the SSH transport AND `qm list`
    # itself must succeed before we trust the "no non-stopped VMs"
    # conclusion. We do NOT use a remote pipe (`qm list | awk | head`)
    # because remote pipefail is disabled by default and the pipeline
    # would mask a `qm list` failure with `head`'s zero exit code,
    # tricking the safety check into auto-rebooting a node whose VM
    # state we never actually read. Per .claude/rules/destruction-
    # safety.md "When a Safety Check Cannot Determine State", we fail
    # closed instead.
    KERNEL_NODES_WITH_VMS=()
    KERNEL_ALL_NODE_IPS=()
    while IFS= read -r entry; do
      [[ -z "$entry" ]] && continue
      KERNEL_ALL_NODE_IPS+=("${entry#*=}")
      ip="${entry#*=}"
      name="${entry%%=*}"
      KERNEL_QM_STDERR="$(mktemp)"
      set +e
      KERNEL_QM_OUT="$(ssh -n -o StrictHostKeyChecking=no \
      ...
      KERNEL_QM_RC=$?
      set -e
      if [[ $KERNEL_QM_RC -ne 0 ]]; then
        KERNEL_QM_ERR="$(cat "${KERNEL_QM_STDERR}" 2>/dev/null || true)"
        rm -f "${KERNEL_QM_STDERR}"
        die "Step 1.5: cannot determine VM state on ${name} (${ip}) -- ssh+qm rc=${KERNEL_QM_RC}; stderr: ${KERNEL_QM_ERR:-<empty>}. Refusing to auto-reboot. Investigate connectivity / qm health and re-run rebuild-cluster.sh."
      fi
      rm -f "${KERNEL_QM_STDERR}"
      # Filter the captured qm output locally with awk, capturing the
      # awk pipeline's exit code. An awk failure (binary missing, parse
      # error, etc.) under set -uo without pipefail would silently
      # produce empty output -- which our `[[ -n "$KERNEL_QM_FILTERED"
      # ]]` check would interpret as "no non-stopped VMs", a fail-open
      # variant of the same class as the round-5/6 P1s. Capture and
      # check rc explicitly.
      # No `exit` in the awk: with a sufficiently large qm-list output
      # (~64 KiB pipe buffer = ~870 VMs/node, far beyond mycofu scale),
      # `{print; exit}` SIGPIPEs the upstream printf and pipefail
      # captures rc=141 even on a successful first-match -- a spurious
      # die. Letting awk drain the whole input is fine here; mycofu
      # clusters have <100 VMs total. Matches the manual procedure's
      # awk pattern.
      set +e
      KERNEL_QM_FILTERED="$(printf '%s\n' "$KERNEL_QM_OUT" \
        | awk 'NR>1 && $3 != "stopped" {print}')"
      KERNEL_AWK_RC=$?
      set -e
      if [[ $KERNEL_AWK_RC -ne 0 ]]; then
        die "Step 1.5: VM-state filter (awk) failed on ${name} (rc=${KERNEL_AWK_RC}). Refusing to auto-reboot. Investigate awk on the local host."
      fi
      if [[ -n "$KERNEL_QM_FILTERED" ]]; then
        KERNEL_NODES_WITH_VMS+=("${name}")
      fi
    done < <(yq -r '.nodes[] | .name + "=" + .mgmt_ip' "$CONFIG")

    if [[ ${#KERNEL_NODES_WITH_VMS[@]} -gt 0 ]]; then
      # Unsafe. Hand control back to the operator with a clear banner.
      step_done 1.5 "Kernel parameter drop-ins written; rolling reboot required (operator)"
      echo
      echo "================================================================"
      echo "REBUILD INCOMPLETE: rolling reboot required before continuing"
      echo "================================================================"
      echo
      echo "Auto-reboot was DECLINED because the following node(s) have"
      echo "non-stopped VMs (running, paused, suspended, prelaunch, or"
      echo "io-error -- any state other than fully stopped):"
      for n in "${KERNEL_NODES_WITH_VMS[@]}"; do
        echo "  - ${n}"
      done
      echo
      echo "Auto-rebooting would crash running workloads. Coordinate the"
      echo "rolling reboot manually using the framework wrapper:"
      echo "    framework/scripts/reboot-node-rolling.sh <target-node>"
      echo "Run from THIS workstation, once per node listed above. The"
      echo "wrapper picks a reachable+quorate surviving node to"
      echo "orchestrate from, SCPs the inner script there, and runs the"
      echo "HA-aware drain/reboot/quorum-poll/disable dance. On most"
      echo "non-zero exits, the inner script's EXIT trap auto-disables"
      echo "HA maintenance on the target to avoid a stuck-migration loop."
      echo "Step z is the deliberate exception: if the failback cidata"
      echo "guard cannot prove the target safe, the wrapper exits with"
      echo "the distinct refusal code and leaves HA maintenance enabled"
      echo "until you complete the printed remediation."
      echo
      echo "Precondition: all running VMs on the target node must be HA-"
      echo "managed; manually migrate any non-HA running VM first."
      echo
      echo "After reboot-node-rolling.sh returns rc=0 for ALL nodes, run"
      echo "from this workstation:"
      echo "    framework/scripts/rebalance-cluster.sh"
      echo "to migrate VMs back to their intended placements. (Single"
      echo "rebalance at the end is cheaper than rebalancing between"
      echo "every reboot.)"
      echo
      echo "Then re-run rebuild-cluster.sh. Step 1.5 is idempotent and"
      echo "will see params effective on the second pass."
      echo
      echo "Do NOT use 'systemctl stop corosync' for this planned reboot"
      echo "-- that procedure is reserved for the storage-failure incident"
      echo "path documented in .claude/rules/storage-failure-fence.md and"
      echo "triggers a watchdog fence rather than a graceful shutdown."
      echo
      echo "If you intentionally want to skip the kernel-param workaround,"
      echo "comment out the entire Step 1.5 block in this script. Editing"
      echo "step_in_scope's infrastructure-step list is the wrong fix and"
      echo "would cause Step 1.5 to run during every scoped rebuild."
      exit 10
    fi

    # Safe path: no non-stopped VMs anywhere. Sequential reboot in
    # place. The 30s settle before polling matches the precedent in
    # configure-node-network.sh and is needed because systemd
    # shutdown can keep sshd accepting connections for 10-15s on
    # servers with ZFS pools to export, so a shorter delay races
    # against the still-up sshd and reports "back up after 0s".
    echo
    echo "Step 1.5: rebooting all nodes to load NVMe-quirk kernel params"
    echo "          (no non-stopped VMs detected on any node, safe to proceed)"
    for ip in ${KERNEL_ALL_NODE_IPS[@]+"${KERNEL_ALL_NODE_IPS[@]}"}; do
      echo "  rebooting root@${ip} ..."
      set +e
      ssh -n -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
          -o ConnectTimeout=10 -o BatchMode=yes -o LogLevel=ERROR \
          "root@${ip}" "reboot" 2>/dev/null
      KERNEL_REBOOT_RC=$?
      set -e
      if [[ $KERNEL_REBOOT_RC -ne 0 ]]; then
        log "WARNING: Step 1.5 reboot ssh to root@${ip} returned rc=${KERNEL_REBOOT_RC} (may be normal if sshd tore down before reply); will still wait for the SSH down/up cycle below"
      fi
      sleep 30
      # Poll for SSH to return.
      KERNEL_REBOOT_WAIT=0
      until ssh -n -o ConnectTimeout=5 -o StrictHostKeyChecking=no \
                -o UserKnownHostsFile=/dev/null -o BatchMode=yes \
                -o LogLevel=ERROR \
                "root@${ip}" "true" 2>/dev/null; do
        sleep 10
        KERNEL_REBOOT_WAIT=$((KERNEL_REBOOT_WAIT + 10))
        if [[ $KERNEL_REBOOT_WAIT -ge 600 ]]; then
          die "Step 1.5: node root@${ip} did not return from reboot in 10 minutes"
        fi
      done
      echo "  root@${ip} back up after ${KERNEL_REBOOT_WAIT}s"
    done

    # Re-verify that params are now effective. Capture stdout so the
    # die message can name the failing node(s); --verify --all prints
    # per-node lines like "[pveNN] REBOOT REQUIRED..." that get buried
    # in CI logs otherwise.
    echo "Step 1.5: verifying kernel params are now in /proc/cmdline ..."
    set +e
    KERNEL_VERIFY_OUT="$("${SCRIPT_DIR}/configure-node-kernel.sh" --verify --all 2>&1)"
    KERNEL_VERIFY_RC=$?
    set -e
    printf '%s\n' "${KERNEL_VERIFY_OUT}"
    if [[ $KERNEL_VERIFY_RC -ne 0 ]]; then
      KERNEL_FAILING_NODES="$(printf '%s\n' "${KERNEL_VERIFY_OUT}" \
        | sed -En 's/^\[([^]]*)\] (REBOOT REQUIRED|MISSING|ERROR).*/\1/p' \
        | sort -u | tr '\n' ' ' | sed 's/ $//')"
      if [[ -n "${KERNEL_FAILING_NODES}" ]]; then
        die "Step 1.5: kernel params still not effective after reboot on: ${KERNEL_FAILING_NODES} (rc=${KERNEL_VERIFY_RC})"
      else
        die "Step 1.5: kernel params still not effective after reboot (rc=${KERNEL_VERIFY_RC})"
      fi
    fi
    step_done 1.5 "Kernel parameters applied and verified after auto-reboot"
    unset KERNEL_NODES_WITH_VMS KERNEL_ALL_NODE_IPS \
          KERNEL_QM_OUT KERNEL_QM_RC KERNEL_QM_STDERR KERNEL_QM_ERR \
          KERNEL_QM_FILTERED KERNEL_AWK_RC \
          KERNEL_REBOOT_RC KERNEL_REBOOT_WAIT \
          KERNEL_VERIFY_OUT KERNEL_VERIFY_RC KERNEL_FAILING_NODES
  else
    die "configure-node-kernel.sh failed (rc=${KERNEL_RC})"
  fi
else
  step_skip 1.5 "Kernel (not in scope)"
fi
...
#
# #701: do NOT pre-resolve the github-mirror source commit here. The old
# unconditional resolve died on a downstream cluster with
# publish.github.enabled=false even though publishing is off and no
# github-mirror endpoint is emitted. Defer resolution to
# generate-gatus-config.sh's internal opt-in resolver (#528/#294) — the
# single authority the CI build:merge job and safe-apply.sh also use. CI
# build:merge uses a bare redirect; safe-apply.sh converges on the same
# internal resolver but writes atomically. rebuild-cluster.sh is also a
# workstation path, so it follows safe-apply.sh's write safety here.
#
# Fail-closed is preserved AND the DR path is not weakened relative to the
# prior explicit-set form:
#   - env-scrub: the resolver's test-only seams
#     (GATUS_GITHUB_EXPECTED_SOURCE_COMMIT / GATUS_SOURCE_REPO_DIR /
#     GATUS_SOURCE_COMMIT_REMOTE) are cleared so a stale operator-shell
#     export cannot steer production resolution. The old code set
#     GATUS_GITHUB_EXPECTED_SOURCE_COMMIT explicitly, which shadowed any
#     ambient value; the bare form would have re-opened that leak.
#     #718 NOTE: this call-site scrub is no longer what carries the property.
#     generate-gatus-config.sh now clears all three seams itself unless invoked
#     with --allow-test-seams, so every caller (safe-apply.sh, CI build:merge,
#     and this one) is covered by one authority. The `env -u` below is retained
#     as redundant defence-in-depth on the DR path only; it is NOT the
#     mechanism, and a new caller does not need to copy it.
#   - atomic write (#706): generate to a same-dir temp and mv into place
#     only on success, so a failed / interrupted generation never truncates
#     a previously-good config.
#   - empty-output guard (#704): a generator that exits 0 with no output
#     fails closed via -s, not just on a non-zero exit.
# publish.github.enabled=true + unresolvable prod still aborts (the internal
# resolver exits non-zero -> die); the gatus module's plan-time config_guard
# precondition remains the second, defence-in-depth guard (#690/#704).
log "    Generating Gatus config (site/gatus/config.yaml)..."
mkdir -p "${REPO_DIR}/site/gatus"
GATUS_CONFIG_TMP="$(mktemp "${REPO_DIR}/site/gatus/.config.yaml.XXXXXX")"
if ! env -u GATUS_GITHUB_EXPECTED_SOURCE_COMMIT -u GATUS_SOURCE_REPO_DIR \
        -u GATUS_SOURCE_COMMIT_REMOTE \
        "${SCRIPT_DIR}/generate-gatus-config.sh" > "${GATUS_CONFIG_TMP}"; then
  rm -f "${GATUS_CONFIG_TMP}"
  die "generate-gatus-config.sh failed; refusing to deploy with a missing/empty Gatus monitoring config (#690)."
fi
if [[ ! -s "${GATUS_CONFIG_TMP}" ]]; then
  rm -f "${GATUS_CONFIG_TMP}"
  die "generate-gatus-config.sh produced an empty Gatus config; refusing to deploy (#704)."
fi
chmod 0644 "${GATUS_CONFIG_TMP}"
mv -f "${GATUS_CONFIG_TMP}" "${REPO_DIR}/site/gatus/config.yaml"
cd "$TOFU_DIR"
...
if [[ "$TOFU_TARGETS" == "ENV_DEFERRED" ]]; then
  CONFIGURED_MODULES="$(collect_configured_root_modules)" \
    || die "--scope ${SCOPE} could not discover root modules from ${TOFU_DIR}."
  VM_CLASSES_JSON="$("${VM_SCOPE_SCRIPT}" classes --format json)" \
    || die "Cannot read VM classes for --scope ${SCOPE}."
  # Same label rules as vm-scope.sh classify_target: exact classes first,
  # then env suffixes (including numbered instances such as dns1_dev).
  TOFU_TARGETS="$(CONFIGURED_MODULES="$CONFIGURED_MODULES" VM_CLASSES_JSON="$VM_CLASSES_JSON" \
    TARGET_ENV="${SCOPE#env=}" python3 - <<'PY'
import json
import os
import re
...
targets = []
for module in os.environ["CONFIGURED_MODULES"].split():
    label = module[len("module."):].replace("-", "_")
    key, suffix = label, None
    if key not in classes:
        match = re.fullmatch(r"(.+)_(dev|prod)", label)
        if not match:
            continue
    ...
    rec = classes.get(key, {})
    if rec.get("control_plane") or rec.get("scope") == "shared":
        continue
    if (suffix == env and rec.get("scope") == "env-bound") or (
        suffix is None and rec.get("scope") == f"{env}-only"
    ):
        targets.append(f"-target={module}")
print(" ".join(targets))
PY
  )" || die "Could not resolve --scope ${SCOPE}."
  [[ -n "$TOFU_TARGETS" ]] \
    || die "--scope ${SCOPE} produced empty target list. Refusing to apply the whole configuration."
  log "    Environment targets: $TOFU_TARGETS"
fi
...
#
# PBS state consistency: this backup is what makes a pre-destroy known-good
# snapshot exist in PBS before any destructive step. Downstream,
# restore-before-start.sh passes --backup-id <volid> to restore-from-pbs.sh
# using the pinned volid for each precious-state VMID; if no pin is available
# it falls back to "latest" (which can silently return an empty-vdb snapshot
# if a scheduled backup ran mid-rebuild). The pin-linkage is NOT enforced
# in-script: `backup-now.sh` is invoked here without `--pin-out`, so it uses
# its default `build/restore-pin.json`, while $RESTORE_PIN_FILE (the pin
# passed to restore-before-start.sh below) is set only from
# `--restore-pin-file` on the rebuild-cluster.sh CLI. `rebuild-cluster.sh`
# is a workstation-invoked script (not called by CI); the standard entry
# path is `reset-cluster.sh --test`, which writes
# `build/restore-pin-reset.json` and tells the operator to pass it to
# `rebuild-cluster.sh --restore-pin-file`. If the operator omits
# `--restore-pin-file`, the backup this block writes is orphaned and
# restore-before-start.sh falls back to "latest". The invariant this block
# enforces is: take the backup BEFORE any destructive step; the operator
# is responsible for wiring pin-out ↔ restore-pin-file at the entry point.
# See #63, .claude/rules/pbs-restore.md, and
# .claude/rules/destruction-safety.md.
...
build_filtered_targets() {
  ...
  while IFS= read -r candidate; do
    ...
    skip=0
    for excluded in ${ATOMIC_RECREATED_MODULES[@]+"${ATOMIC_RECREATED_MODULES[@]}"}; do
...
write_rebuild_plan_json_from_plan_out() {
  local plan_out="$1"
  local show_raw="$2"
...
build_rebuild_default_plan_json() {
  ...
  "${SCRIPT_DIR}/tofu-wrapper.sh" plan $target_flags \
    -out="$plan_out" \
    -no-color
  write_rebuild_plan_json_from_plan_out "$plan_out" "$show_raw" "$plan_json_out"
}

# Only VM instances already in state need -replace; an empty cold state is
# valid. Keep these flags separate from APPLY_ARGS so the start apply cannot
# destroy the VMs whose state was just restored.
build_recreate_flags() {
  local state_output address root target selected cp
  state_output="$("${SCRIPT_DIR}/tofu-wrapper.sh" state list)" \
    || { echo "Cannot list VM state for --recreate" >&2; return 1; }
  while IFS= read -r address; do
    [[ "$address" == module.*.proxmox_virtual_environment_vm.vm ]] || continue
    root="${address#module.}"
    root="${root%%.*}"
    root="module.${root%%\[*}"
    [[ "$root" == module.pbs ]] && continue
    selected=1
    if [[ -n "$BULK_TARGETS" ]]; then
      selected=0
      for target in $BULK_TARGETS; do
        [[ "$root" == "${target#-target=}" ]] && selected=1
      done
    fi
    for cp in $CONTROL_PLANE_MODULES; do
      [[ "$root" == "$cp" ]] && selected=0
    done
    [[ "$selected" -eq 1 ]] && printf ' -replace=%s' "$address"
  done <<< "$state_output"
  return 0
}

assert_recreate_bulk_plan() {
  PLAN_JSON="$1" TARGET_FLAGS="$BULK_TARGETS" RECREATE_FLAGS="$RECREATE_FLAGS" \
  CONTROL_PLANE_MODULES="$CONTROL_PLANE_MODULES" \
  ASSERT_OUT="${LOG_DIR}/recreate-plan-assert-bulk.json" python3 - <<'PY'
import json
import os
import re
import sys
...
with open(os.environ["PLAN_JSON"]) as f:
    plan = json.load(f)
targets = {s[len("-target="):] for s in os.environ["TARGET_FLAGS"].split()}
excluded = set(os.environ["CONTROL_PLANE_MODULES"].split()) | {"module.pbs"}
addresses, offenders = [], []
for rc in plan["resource_changes"]:
    address = rc.get("address", "")
    if not address.endswith(".proxmox_virtual_environment_vm.vm"):
        continue
    root = re.sub(r"\[.*\]$", "", ".".join(address.split(".")[:2]))
    if root in excluded or (targets and root not in targets):
        continue
    actions = (rc.get("change") or {}).get("actions")
    if actions == ["create"] or (isinstance(actions, list) and "create" in actions and "delete" in actions):
        addresses.append(address)
    else:
        offenders.append(f"{address}: {actions!r}")
requested = {s[len("-replace="):] for s in os.environ["RECREATE_FLAGS"].split()}
seen = {rc.get("address") for rc in plan["resource_changes"]}
offenders.extend(f"{address}: missing from plan" for address in sorted(requested - seen))
if offenders:
    print("Recreate plan assertion failed:\n  " + "\n  ".join(offenders), file=sys.stderr)
    sys.exit(1)
with open(os.environ["ASSERT_OUT"], "w") as f:
    json.dump({"expected": len(addresses), "replacing": len(addresses), "addresses": sorted(addresses)}, f)
    f.write("\n")
PY
}

write_rebuild_prevent_destroy_intent_plan_json() {
  local target_flags="${1:-}"
  local plan_json_out="$2"
  local vm_classes_json

  vm_classes_json="$("${VM_SCOPE_SCRIPT}" classes --format json)"

  TARGET_FLAGS="$target_flags" \
  PLAN_JSON_OUT="$plan_json_out" \
  CONFIG_FILE="$CONFIG" \
  VM_CLASSES_JSON="$vm_classes_json" \
  IMAGE_VERSIONS_FILE="${REPO_DIR}/site/tofu/image-versions.auto.tfvars" \
  python3 - <<'PY'
import json
import os
import re
...
def strip_module_label(module):
    label = str(module)
    if label.startswith("module."):
        label = label[len("module.") :]
    return re.sub(r"\[.*\]$", "", normalize_label(label))
    ...
    with open(path) as f:
        for line in f:
            ...
            if not stripped or stripped.startswith(("#", "//")):
                continue
            if not in_block:
                if re.match(r"^image_versions\s*=\s*\{\s*$", stripped):
                    in_block = True
                continue
            if stripped == "}":
                break
...
def target_modules(target_flags):
    return [
        match.group(1)
        for match in re.finditer(r"-target=(module\.[^\s]+)", target_flags or "")
    ]


def disk_size_text(value):
    if value in (None, "", "null"):
        return None
    return str(value)


targets = target_modules(os.environ.get("TARGET_FLAGS", ""))
if len(targets) != 1:
    print(
        "ERROR: prevent_destroy precheck fallback requires exactly one "
        f"atomic -target=module.NAME flag, got: {targets!r}",
        file=sys.stderr,
    )
    sys.exit(1)

module = targets[0]
label = strip_module_label(module)
config = load_yaml_json(os.environ["CONFIG_FILE"], {}, required=True)
classes = json.loads(os.environ["VM_CLASSES_JSON"])
class_key = resolve_class(label, classes)
if class_key is None:
    print(f"ERROR: VM class '{label}' is not declared in vm-scope manifests", file=sys.stderr)
    sys.exit(1)

class_record = classes[class_key]
role = class_record.get("role") or class_key
category = class_record.get("category")
after = {
    "started": True,
    "disk": [
        {
...
if category != "vendor":
    image_versions = load_image_versions(os.environ["IMAGE_VERSIONS_FILE"])
    image = image_versions.get(role)
    if not image or image.startswith("PLACEHOLDER_") or not image.endswith(".img"):
        print(
            "ERROR: cannot build prevent_destroy precheck image gate for "
            f"{module}: image_versions[{role!r}] is missing or not a real .img",
            file=sys.stderr,
        )
        sys.exit(1)
    after["disk"][0]["file_id"] = f"local:iso/{image}"
    after["initialization"] = [{"synthetic": True}]

if label == "gitlab":
    after["disk"].append({"interface": "scsi1", "size": "50"})
elif label == "cicd":
    cicd_cfg = config.get("cicd") or {}
    after["disk"][0]["size"] = disk_size_text(cicd_cfg.get("runner_disk_gb") or 256)
elif label == "pbs":
    after["disk"][0]["size"] = "32"

plan = {
    "format_version": "1.2",
    "resource_changes": [
        {
            "address": f"{module}.module.{label}.proxmox_virtual_environment_vm.vm"
            if category != "vendor"
            else f"{module}.proxmox_virtual_environment_vm.vm",
            "module_address": f"{module}.module.{label}" if category != "vendor" else module,
            "mode": "managed",
...
build_rebuild_prevent_destroy_precheck_plan_json() {
  local label="$1"
  local target_flags="${2:-}"
  local plan_json_out="$3"
  local plan_out="${LOG_DIR}/preboot-restore-plan-${label}.out"
  local show_raw="${LOG_DIR}/preboot-restore-plan-${label}.raw"
  local plan_stderr="${LOG_DIR}/preboot-restore-plan-${label}.stderr"
  local plan_rc=0
  ...
  if grep -qE "Instance cannot be destroyed|Resource cannot be destroyed|prevent_destroy" "$plan_stderr" 2>/dev/null && [[ "$plan_rc" -eq 1 ]]; then
    log "    ${label}: precheck plan blocked by prevent_destroy — using recreate-intent JSON"
    rm -f "$plan_stderr"
    write_rebuild_prevent_destroy_intent_plan_json "$target_flags" "$plan_json_out"
    return 0
  fi

  log "    ERROR: tofu precheck plan failed for ${label}:"
  tail -10 "$plan_stderr" >&2 || true
  rm -f "$plan_stderr"
  return "$plan_rc"
}

check_plan_images_present() {
  local label="$1"
  local plan_json="$2"

  log "    Checking plan image availability (${label})..."
...
run_preboot_restore_for_modules() {
  ...
  local target_flags="${2:-}"
  local plan_json="$3"
  local manifest="${4:-${LOG_DIR}/preboot-restore-${label}.json}"
  local park_status_file="${5:-${LOG_DIR}/vdb-park-status-${label}.json}"
  local args=(all --manifest "$manifest")
  # Per-phase status file (#518, A6). Each atomic module and the bulk phase get
  # their OWN status file keyed by label. A single shared status-all.json used to
  # be clobbered by every phase (restore-before-start.sh re-inits {entries:[]} and
  # cp-overwrites whole), so a legitimately empty bulk pass erased the atomic
  # entries. The canonical all.json is produced later by aggregate-preboot-status.sh.
  local status_file="${LOG_DIR}/preboot-restore-status-${label}.json"
  local restore_rc=0
  ...
  args+=(--park-status "$park_status_file")
  if [[ -n "$RESTORE_PIN_FILE" ]]; then
  ...
  if [[ "$restore_rc" -eq 2 ]]; then
    handle_rebuild_incomplete_restore_rc2 "$status_file"
    return $?
  fi
  return "$restore_rc"
}

is_rebuild_backup_backed_vmid() {
  local vmid="$1"
  "${SCRIPT_DIR}/list-backup-backed-vmids.sh" --format tsv all \
    | awk -F '\t' '{print $1}' \
    | grep -Fxq "$vmid"
}

handle_rebuild_incomplete_restore_rc2() {
  local status_file="$1"
  local rows vmid entry_env conv_env pin rc
  local failures=0

  if [[ ! -f "$status_file" ]]; then
    log "    ERROR: restore-before-start returned rc=2 but status file is missing: ${status_file}"
    return 2
  fi
  if [[ -z "$RESTORE_PIN_FILE" || ! -f "$RESTORE_PIN_FILE" ]]; then
    log "    ERROR: incomplete VM convergence requires --restore-pin-file for rebuild-cluster.sh"
    return 2
  fi

  rows="$(jq -r '.entries[]? | select(.status == "incomplete") | [.vmid, .env] | @tsv' "$status_file" | sort -n -u)"
  if [[ -z "$rows" ]]; then
    log "    ERROR: restore-before-start returned rc=2 but no incomplete VMIDs were recorded"
    return 2
  fi

  while IFS=$'\t' read -r vmid entry_env; do
    [[ -z "$vmid" ]] && continue
    if ! is_rebuild_backup_backed_vmid "$vmid"; then
      log "    VM ${vmid} is incomplete; non-precious VM needs full recreate because vdb-only restore cannot repair missing boot topology. Recover with qmrestore <exact-pin> ${vmid} --force (whole-VM; requires operator approval per .claude/rules/destructive-operations.md) and re-run."
      failures=$((failures + 1))
      continue
    fi

    pin="$(jq -r --arg vmid "$vmid" '(.pins[$vmid] // "") | if type == "object" then (.volid // "") else . end' "$RESTORE_PIN_FILE")"
    if [[ -z "$pin" ]]; then
      log "    VM ${vmid} is incomplete; no exact pin available in ${RESTORE_PIN_FILE}. This needs full recreate because vdb-only restore cannot repair missing boot topology. Recover with qmrestore <exact-pin> ${vmid} --force (whole-VM; requires operator approval per .claude/rules/destructive-operations.md) and re-run."
      failures=$((failures + 1))
      continue
    fi

    conv_env="$entry_env"
    if [[ "$conv_env" == "shared" ]]; then
      conv_env="all"
    fi

    set +e
    converge_incomplete_vm "$conv_env" "$vmid" "$pin"
    rc=$?
    set -e
    if [[ "$rc" -ne 0 ]]; then
      log "    VM ${vmid} is incomplete; convergence failed: rc=${rc}. This needs full recreate because vdb-only restore cannot repair missing boot topology. Recover with qmrestore ${pin} ${vmid} --force (whole-VM; requires operator approval per .claude/rules/destructive-operations.md) and re-run."
      failures=$((failures + 1))
      continue
    fi
    log "    VM ${vmid}: incomplete topology converged from exact pin ${pin}"
  done <<< "$rows"
...
# Control-plane VMs (gitlab, cicd) use proxmox-vm-field-updatable with prevent_destroy.
...
# then apply creates the VM fresh. Used when prevent_destroy blocks a plan,
# or when --recreate explicitly requests the atomic ladder (except PBS).
STATE_OUTPUT=$("${SCRIPT_DIR}/tofu-wrapper.sh" state list 2>&1) || {
  [[ "$RECREATE" -eq 0 ]] || die "Cannot read state before forced recreation: ${STATE_OUTPUT}"
}
for mod in $CONTROL_PLANE_MODULES; do
  if [[ "$TOFU_TARGETS" == *"$mod"* || -z "$TOFU_TARGETS" ]]; then
    mod_name=$(echo "$mod" | sed 's/module\.//')

    if [[ "$RECREATE" -eq 1 && "$mod" != module.pbs ]]; then
      log "    ${mod_name}: recreate requested — entering atomic ladder"
    else
    if [[ "$RECREATE" -eq 1 && "$mod" == module.pbs ]]; then
      log "    pbs: excluded from forced recreation by design — using normal plan path"
    fi
    ...
    if [[ "$PLAN_EXIT" -eq 0 ]]; then
      ...
      # All control-plane VMs (gitlab, pbs, cicd) set prevent_destroy = true —
      # gitlab/pbs via proxmox-vm-precious, cicd via proxmox-vm-field-updatable
      # (see framework/tofu/modules/proxmox-vm-field-updatable/vm.tf).
      # exit 2 therefore means the change does NOT require VM destruction
      # (if it did, prevent_destroy would have fired → exit 1). For cicd this
      # is typically an in-place field update (e.g. memory). Safe for tofu apply.
      log "    ${mod_name}: pending changes detected (deployable by tofu apply)"
      rm -f "$plan_stderr"
      continue
    elif [[ "$PLAN_EXIT" -eq 1 ]]; then
      # Check for prevent_destroy signal (gitlab, pbs, cicd image/CIDATA
      # changes). OpenTofu has emitted several distinct wordings for this
      # error across versions (#741 G2-attended-window recreation
      # attempt died at Step 7 because the detection matched only the
      # first wording):
      #
      #   older tofu: "Error: Instance cannot be destroyed"
      #   current tofu: leader "Error: Resource cannot be destroyed" +
      #                 body "has lifecycle.prevent_destroy set, but
      #                 the plan calls for this resource to be destroyed"
      #                 (the body may wrap across visual lines in the
      #                 stderr capture, so we cannot rely on the full
      #                 sentence appearing on a single line for `grep`).
      #
      # Match on three alternatives:
      #   (a) "Instance cannot be destroyed" — the OLD tofu leader
      #   (b) "Resource cannot be destroyed" — the CURRENT tofu leader
      #   (c) "prevent_destroy" — the stable body-token that always
      #       appears in a prevent_destroy-blocked plan error, and is
      #       safe to match here because we already know tofu exited 1
      #       (this branch is only reached under PLAN_EXIT==1).
      #
      # The fixture test test_prevent_destroy_detection.sh pins all
      # three alternatives + a fail-closed negative case (unrelated
      # infra error must NOT match). Adding a new wording variant
      # means one edit here + a fixture case that exercises it; the
      # ratchet then fires regression-if-someone-drops-a-variant.
      if grep -qE "Instance cannot be destroyed|Resource cannot be destroyed|prevent_destroy" "$plan_stderr" 2>/dev/null; then
    ...
    fi  # normal plan decision (forced recreation enters the ladder directly)

    # Reached when prevent_destroy blocked a change or recreation was requested.
    ...
    if [[ -z "$VMID" || "$VMID" == "null" ]]; then
      [[ "$RECREATE" -eq 0 ]] || die "No VMID found for forced recreation of ${mod_name}."
      log "    WARNING: No VMID found for ${mod_name} in config.yaml — skipping"
      continue
    fi

    ATOMIC_PRECHECK_PLAN_JSON="${LOG_DIR}/preboot-restore-plan-atomic-precheck-${mod_name}.json"
    ATOMIC_MANIFEST="${LOG_DIR}/preboot-restore-atomic-${mod_name}.json"
    ATOMIC_PARK_STATUS="${LOG_DIR}/vdb-park-status-atomic-${mod_name}.json"
    if [[ "$RECREATE" -eq 1 && "$mod" != module.pbs ]]; then
      write_rebuild_prevent_destroy_intent_plan_json "-target=${mod}" "$ATOMIC_PRECHECK_PLAN_JSON"
    else
      build_rebuild_prevent_destroy_precheck_plan_json "atomic-precheck-${mod_name}" "-target=${mod}" "$ATOMIC_PRECHECK_PLAN_JSON"
    fi
    check_plan_images_present "atomic-${mod_name}" "$ATOMIC_PRECHECK_PLAN_JSON"
    write_preboot_manifest_for_modules "$ATOMIC_MANIFEST" "-target=${mod}" "$ATOMIC_PRECHECK_PLAN_JSON"
    log "    Parking eligible vdb volumes for ${mod_name} before atomic destroy..."
    VDB_PARK_PIN_FILE="$RESTORE_PIN_FILE" vdb_park_batch "$ATOMIC_MANIFEST" "$ATOMIC_PARK_STATUS" all
    ...
    log "    Adopting parked vdb volumes for ${mod_name} before restore..."
    ADOPT_RC=0
    set +e
    vdb_adopt_batch "$ATOMIC_PARK_STATUS"
    ADOPT_RC=$?
    set -e
    if [[ "$ADOPT_RC" -ne 0 ]]; then
      log "    WARNING: vdb_adopt_batch returned rc=${ADOPT_RC}; continuing to preboot restore so failed entries can use the pinned PBS fallback."
    fi

    log "    Restoring ${mod_name} before start..."
    run_preboot_restore_for_modules "atomic-${mod_name}" "-target=${mod}" "$ATOMIC_PLAN_JSON" "$ATOMIC_MANIFEST" "$ATOMIC_PARK_STATUS" \
...
if [[ -z "$BULK_TARGETS" && -n "$TOFU_TARGETS" ]]; then
  ...
  BULK_MANIFEST="${LOG_DIR}/preboot-restore-bulk.json"
  BULK_PARK_STATUS="${LOG_DIR}/vdb-park-status-bulk.json"
  RECREATE_FLAGS=""
  if [[ "$RECREATE" -eq 1 ]]; then
    rm -f "${LOG_DIR}/recreate-plan-assert-bulk.json"
    RECREATE_FLAGS="$(build_recreate_flags)" || die "Cannot determine replacements for --recreate."
  fi
  build_rebuild_default_plan_json "bulk" "$BULK_TARGETS$RECREATE_FLAGS" "$BULK_PLAN_JSON"
  if [[ "$RECREATE" -eq 1 ]]; then
    assert_recreate_bulk_plan "$BULK_PLAN_JSON" || die "Refusing bulk recreation: plan did not recreate every selected VM."
  fi
  check_plan_images_present "bulk" "$BULK_PLAN_JSON"
  write_preboot_manifest_for_modules "$BULK_MANIFEST" "$BULK_TARGETS" "$BULK_PLAN_JSON"

  echo ""
  echo "=== vdb park bridge (bulk pre-destroy) ==="
  VDB_PARK_PIN_FILE="$RESTORE_PIN_FILE" vdb_park_batch "$BULK_MANIFEST" "$BULK_PARK_STATUS" all

  echo "=== OpenTofu stopped apply ==="
  # Both strings intentionally contain separate CLI flags, as in the plan builder.
  # shellcheck disable=SC2086
  "${SCRIPT_DIR}/tofu-wrapper.sh" apply $APPLY_ARGS $RECREATE_FLAGS \
    -var=start_vms=false \
    -var=register_ha=false
  ...
  echo ""
  echo "=== vdb park bridge adopt (bulk) ==="
  ADOPT_RC=0
  set +e
  vdb_adopt_batch "$BULK_PARK_STATUS"
  ADOPT_RC=$?
  set -e
  if [[ "$ADOPT_RC" -ne 0 ]]; then
    log "    WARNING: vdb_adopt_batch returned rc=${ADOPT_RC}; continuing to preboot restore so failed entries can use the pinned PBS fallback."
  ...
  run_preboot_restore_for_modules "bulk" "$BULK_TARGETS" "$BULK_PLAN_JSON" "$BULK_MANIFEST" "$BULK_PARK_STATUS"
...
# Aggregate the per-phase preboot-restore status files into the canonical
# preboot-restore-status-all.json (#518, A6). Each atomic module and the bulk
# phase wrote its own status file; union them so the empty bulk phase can no
# longer clobber the atomic entries. Non-fatal: the rebuild has already restored
# and started VMs, so a reporting-artifact glitch must not abort it — the CI
# check_bulk_artifacts_populated.sh step is the gate that FAILs on a bad artifact.
log "    Aggregating preboot-restore status artifacts (#518)..."
if ! "${SCRIPT_DIR}/aggregate-preboot-status.sh" \
      --out "${LOG_DIR}/preboot-restore-status-all.json" \
      --glob-dir "${LOG_DIR}"; then
  log "    WARNING: preboot-restore status aggregation failed (non-fatal reporting step)"
fi
...
cd "$REPO_DIR"
# Refresh SSH host keys for VMs whose root module is in the current scope.
# A scoped rebuild (--scope vm=X) must NOT invalidate known_hosts for VMs
# outside scope — that silently breaks workstation tooling that depends on
# host-key continuity for unrelated VMs (#349). The helper preserves the
# old broad-sweep behavior when TOFU_TARGETS is empty (full rebuild).
log "    Refreshing SSH host keys for VMs in scope..."
