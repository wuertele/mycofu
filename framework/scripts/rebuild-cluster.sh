...
if step_in_scope kernel; then
  ...
  if [[ $KERNEL_RC -eq 0 ]]; then
    ...
    if [[ ${#KERNEL_NODES_WITH_VMS[@]} -gt 0 ]]; then
      ...
      echo "HA-aware drain/reboot/quorum-poll/disable dance. If any step"
      echo "fails, the inner script's EXIT trap auto-disables HA"
      echo "maintenance on the target so the cluster is never left in a"
      echo "stuck-migration state."
      echo
