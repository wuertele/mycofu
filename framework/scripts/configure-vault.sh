...
# Resolve the SOPS age key before the first decrypt (#862).
#
# This script decrypts SECRETS_FILE for the root token, for the AppRole
# write-once check, and for the KV seed values. On the cicd runner the
# variable arrives from the runner service environment, so this branch is
# never taken there; on the operator workstation it was unset, and sops
# failed with its "no identities" error even though the correct key was
# sitting at the repo root.
#
# Resolving here matters beyond convenience: the AppRole write-once check
# below reads the existing credentials with `2>/dev/null || true`, so a
# failed decrypt is indistinguishable from "no credentials in SOPS". A
# resolvable key is what keeps that check able to see the credentials it is
# meant to preserve.
#
# Guarded by DRY_RUN because --dry-run deliberately requires no sops at all
# (see REQUIRED_TOOLS above); demanding a key would regress that contract.
# The final `else exit 1` is load-bearing: it names the missing key instead
# of letting the run die later inside an opaque decrypt failure.
if [[ "$DRY_RUN" -eq 0 ]]; then
...
role_policies_for_vm_key() {
  ...
  case "$base_role" in
    ...
    cicd)      role_policies="default-policy,github-publish-policy,cicd-tailscale-policy" ;;
...
CICD_TAILSCALE_POLICY_CONTENT=$(cat <<EOF
path "secret/data/tailscale/nodes/${DOMAIN}/cicd" {
  capabilities = ["create", "update", "read"]
}
EOF
)
echo "Writing cicd-tailscale-policy..."
vault_api PUT "sys/policies/acl/cicd-tailscale-policy" \
  -d "$(jq -n --arg policy "$CICD_TAILSCALE_POLICY_CONTENT" '{policy: $policy}')"
