...
# which always returns prod's tip. The env var stays as an override for
# tests. Only resolve when publishing is opted in — downstream adopters
# with publish disabled must not fail on a missing prod branch (issue
# #294 opt-in semantics preserved).
if [[ -z "${GATUS_GITHUB_EXPECTED_SOURCE_COMMIT}" && "${PUBLISH_GITHUB_ENABLED}" == "true" ]]; then
  ...
  if ! GATUS_GITHUB_EXPECTED_SOURCE_COMMIT="$(resolve_gatus_expected_source_commit "${RESOLVER_REPO_DIR}")"; then
    ...
    echo "  Fetch prod (git fetch origin prod), or set GATUS_GITHUB_EXPECTED_SOURCE_COMMIT explicitly." >&2
    exit 1
  fi
fi
