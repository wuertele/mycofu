...
KEY_FILE="${MYCOFU_BOOTSTRAP_SOPS_KEY_FILE:-${REPO_DIR}/operator.age.key}"
FORCE=0
  ...
  case "$1" in
    ...
    *)
      if [[ "$CONFIG_SET" -eq 1 ]]; then
        echo "ERROR: only one config path may be supplied" >&2
        exit 2
      fi
      CONFIG="$1"
      CONFIG_SET=1
...
SOPS_DIR="${MYCOFU_BOOTSTRAP_SOPS_DIR:-${CONFIG_DIR}/sops}"
