...
while [[ $WAIT -lt $TIMEOUT ]]; do
  ...
  if [[ "$OBSERVED_CONCURRENT" =~ ^[0-9]+$ ]] && [[ "$OBSERVED_CONCURRENT" -ge 1 ]] && [[ "$OBSERVED_CONCURRENT" -le 8 ]]; then
...
if [[ $WAIT -ge $TIMEOUT ]]; then
  echo "ERROR: /etc/gitlab-runner/config.toml did not converge to a top-level concurrent value in 1..8 within ${TIMEOUT}s" >&2
