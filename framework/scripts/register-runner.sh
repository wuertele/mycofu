...
EXPECTED_CONCURRENT=8
WAIT=0
TIMEOUT=30
while [[ $WAIT -lt $TIMEOUT ]]; do
  if runner_ssh "head -1 /etc/gitlab-runner/config.toml | grep -q \"^concurrent = ${EXPECTED_CONCURRENT}$\"" 2>/dev/null; then
    echo "  Concurrent normalized: concurrent = ${EXPECTED_CONCURRENT}"
    break
  fi
  sleep 1
  WAIT=$((WAIT + 1))
...
if [[ $WAIT -ge $TIMEOUT ]]; then
  echo "ERROR: /etc/gitlab-runner/config.toml did not converge to top-level concurrent = ${EXPECTED_CONCURRENT} within ${TIMEOUT}s" >&2
