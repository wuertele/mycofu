...
Next steps (operator, from workstation):
  ...
  0. PRECONDITION — check the intended node. This deploy re-creates the VM on
     its site/config.yaml vms.<name>.node intended node. If that node is the
     one that just failed, first set vms.<name>.node to a surviving node and
     commit, otherwise the deploy tries to place the VM back on the dead node
     and fails.
...
Next steps (operator, from workstation):
  ...
  0. PRECONDITION — check the intended node. This deploy re-creates the VM on
     its intended node: vms.<name>.node in site/config.yaml for framework VMs,
     or applications.<app>.environments.<env>.node in site/applications.yaml
     for catalog apps. If that node is the one that just failed, first set the
     VM's node field to a surviving node and commit, otherwise the deploy tries
     to place the VM back on the dead node and fails.
