...
if [[ $EXTERNAL_MODE -eq 1 ]]; then
  ...
  case "$BUILDER_TYPE" in
    local|linux-builder)
      ...
      BUILD_LOG=$(mktemp)
      if ! nix build "${NIX_BUILD_FLAGS[@]}" --log-format raw ".#packages.x86_64-linux.${FLAKE_ATTR}" --print-build-logs 2>&1 | tee "$BUILD_LOG"; then
    ...
    remote)
      ...
      nix build "${NIX_BUILD_FLAGS[@]}" --log-format raw ".#packages.x86_64-linux.${FLAKE_ATTR}" --print-build-logs \
  ...
  mkdir -p "$GC_ROOT_DIR"
  OUT_PATH=$(nix build "${NIX_BUILD_FLAGS[@]}" --log-format raw ".#packages.x86_64-linux.${FLAKE_ATTR}" \
