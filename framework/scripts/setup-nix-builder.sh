...
resolve_qemu_img() {
  ...
  # We avoid `nix shell nixpkgs#qemu-utils` here because that downloads on
  ...
  if path=$(nix shell nixpkgs#qemu-utils --command sh -c 'command -v qemu-img' 2>/dev/null); then
...
verify_linux_builder() {
  ...
  # Check start-builder.sh exports STORE_GB. Without the export, the
  # patching block at boot defaults DISK_MB to 40960 regardless of
  # config (see commit 8b89600).
  case "$(start_script_status)" in
    ok) echo "  ✓ ${START_SCRIPT} exports STORE_GB=${STORE_GB}" ;;
    missing|wrong-export)
      echo "  ✗ ${START_SCRIPT} does not export STORE_GB=${STORE_GB}"
  ...
  case "$(qcow2_size_status)" in
    ...
    missing)
      ...
      ;;
    unknown)
      echo "  ! qemu-img not on PATH — skipping qcow2 size check"
      ;;
    wrong-size:*)
