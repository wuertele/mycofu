...
Usage:
  ...
  certbot-persisted-state.sh --mode probe --cert-name NAME

--probe-account-liveness (repair mode only): after confirming the persisted
  account exists on disk, verify it against the live ACME server. A stateless
  ACME server (Pebble) can be recreated, leaving a persisted account dangling
  server-side; renewal then fails accountDoesNotExist. On a definitive
  server-side rejection the dangling account is removed and re-registered,
  and the renewal config's account = field is rewritten. When the probe
  cannot reach a verdict (network/server outage) the repair FAILS CLOSED and
  does NOT re-register — a dead account and a dead network are otherwise
  indistinguishable. A live account is a strict no-op.
EOF
}
...
# Probe the LIVE ACME server for the persisted account. Read-only:
# `certbot show_account` performs a POST-as-GET to the stored account URL,
# which the server answers with the account object when it exists and
# rejects with accountDoesNotExist when it does not. Echoes exactly one of:
#   live        — server confirms the account (strict no-op for the caller)
#   dead        — server definitively rejected it (accountDoesNotExist)
#   unreachable — no verdict; the caller MUST fail closed and MUST NOT
#                 re-register blind, because a dead account and a dead
#                 network are otherwise indistinguishable (#525 / G4).
# The classification token is the ONLY thing written to stdout; the raw
# certbot output is kept on stderr so a caller capturing $(...) gets a clean
# token (#506 stderr discipline).
probe_account_liveness() {
  ...
  mkdir -p "${WORK_DIR}" "${LOGS_DIR}"
  set +e
  probe_output="$("${CERTBOT_BIN}" show_account \
    --non-interactive \
    --server "${server_url}" \
    --config-dir "${LETSENCRYPT_DIR}" \
    --work-dir "${WORK_DIR}" \
    --logs-dir "${LOGS_DIR}" 2>&1)"
  probe_rc=$?
  set -e
  ...
  # A definitive account death is rendered by certbot two ways: the ACME URN
  # token (urn:ietf:params:acme:error:accountDoesNotExist) embedded in
  # acme.messages.Error, and the plain-text CLI form `Account does not exist`
  # that `certbot show_account` printed on vault-dev. Match both
  # (case/whitespace-robust); matching only the URN missed the plain-text form
  # and left the dead account un-repaired (#610). Both stay narrow enough not
  # to fire on generic connection failures.
  if grep -qiE 'accountDoesNotExist|Account[[:space:]]+does[[:space:]]+not[[:space:]]+exist' <<<"${probe_output}"; then
    printf 'dead\n'
    return 0
  fi

  warn "account-liveness probe against ${server_url} returned no verdict (rc=${probe_rc}); failing closed"
  printf '%s\n' "${probe_output}" | sed 's/^/    probe: /' >&2
  printf 'unreachable\n'
  return 0
}
...
repair_renewal_state() {
  local renewal_file="$1"
  local cert_name="$2"
  local current_server current_account desired_account issuer_line liveness
  ...
  elif [[ "${PROBE_LIVENESS}" -eq 1 ]]; then
    # The account exists on disk, but a stateless ACME server (Pebble) may
    # have been recreated, leaving the persisted account dangling
    # server-side. Verify liveness against the live server before trusting
    # the on-disk account (#525).
    liveness="$(probe_account_liveness "${EXPECTED_ACME_URL}")"
    case "${liveness}" in
      live)
        : # server confirms the account — strict no-op
        ;;
      dead)
        warn "${cert_name}: ACME server ${EXPECTED_ACME_URL} rejected persisted account ${current_account} (accountDoesNotExist); re-registering"
        # Remove the dangling on-disk account so ensure_expected_account
        # registers a fresh one instead of re-selecting the dead directory.
        rm -rf "$(account_storage_root "${EXPECTED_ACME_URL}")/${current_account:?refusing to remove account dir for empty account id}"
        EXPECTED_ACCOUNT_CACHE=""
        desired_account="$(ensure_expected_account)"
        ;;
      unreachable)
        error "${cert_name}: could not verify account ${current_account} against ${EXPECTED_ACME_URL}; failing closed (not re-registering — G4)"
        return 1
        ;;
    esac
  fi
...
run_operator_probe() {
  local cert_name="$1"
  local acme_url="${EXPECTED_ACME_URL}"

  if [[ -z "${cert_name}" ]]; then
    echo "ERROR: --mode probe requires --cert-name" >&2
    return 2
  fi

  if [[ -z "${acme_url}" && -s /run/secrets/certbot/acme-server-url ]]; then
    acme_url="$(tr -d '[:space:]' < /run/secrets/certbot/acme-server-url)"
  fi

  if [[ "${EXPECTED_MODE:-}" == "staging" || "${acme_url}" != "https://acme-v02.api.letsencrypt.org/directory" ]]; then
    echo "ERROR: --mode probe not supported on Pebble/staging; use --mode check" >&2
    return 3
  fi
...
if [[ "${MODE}" != "check" && "${MODE}" != "repair" && "${MODE}" != "probe" ]]; then
  echo "ERROR: --mode must be check, repair, or probe" >&2
  exit 2
fi

if [[ "${MODE}" == "probe" ]]; then
  run_operator_probe "${FQDN}"
  exit $?
fi
