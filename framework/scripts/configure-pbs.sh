...
# retention policy, API token, and Proxmox storage. Backup jobs are owned by
# configure-backups.sh after restore verification.
#
# Usage:
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --skip-backup-jobs)
      echo "Note: --skip-backup-jobs is retained for compatibility; configure-pbs.sh no longer manages backup jobs."
      shift
      ;;
    --help|-h)
      ...
      echo "  --skip-backup-jobs  Compatibility no-op; backup jobs are managed by configure-backups.sh"
...
if [[ "$MODE" == "verify" ]]; then
  ...
  # V7: Backup job contract is delegated to the single writer. On fresh
  # bringup this is expected to fail until configure-backups.sh runs after
  # preboot restore/application verification.
  echo -n "  Backup job contract after restore (configure-backups.sh --verify): "
  if "${REPO_DIR}/framework/scripts/configure-backups.sh" --verify >/dev/null 2>&1; then
    echo "PASS"
    PASS=$((PASS + 1))
  else
    echo "FAIL (run configure-backups.sh after restore verification)"
    FAIL=$((FAIL + 1))
  fi
...
if [[ "$MODE" == "dry-run" ]]; then
  ...
  echo "  H. Leave backup jobs to configure-backups.sh after restore verification"
  exit 0
fi
...
# Two-step check: `mount` proves the kernel has a mount registered at this
# path, `df` proves the mount is actually FUNCTIONAL. Both are required
# because a stale NFS handle appears in `mount` output normally (the kernel
# still has the mount registered) but any I/O against the mount point —
# including `df` — either blocks forever (hard mount) or times out with an
# error (soft mount). Using `df` (an unconditional stat on the mount) as the
# liveness probe is cheaper and less side-effecting than `ls`/`stat`/read-
# back; using `mount` alone would silently pass a stale NFS mount, and then
# every downstream proxmox-backup-* operation would hang. See #63.
...
# --- H. Backup jobs ---
echo ""
echo "--- H. Backup jobs managed separately ---"
echo "  configure-pbs.sh does not create or update /cluster/backup jobs."
echo "  configure-backups.sh owns the managed backup job after restore verification."
