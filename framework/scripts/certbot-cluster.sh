...
if [[ -z "${CERTBOT_CLUSTER_RENEWABILITY_SCRIPT:-}" ]]; then
  CERTBOT_CLUSTER_RENEWABILITY_SCRIPT="${_certbot_cluster_script_dir}/certbot-renewability.sh"
fi
...
certbot_cluster_env_for_vm_label() {
  local vm_label="$1"
  case "${vm_label}" in
    *_prod)
      printf 'prod\n'
...
certbot_cluster_env_matches() {
  local vm_label="$1"
  local env_filter="${2:-}"
  local record_env=""

  [[ -z "${env_filter}" ]] && return 0
  record_env="$(certbot_cluster_env_for_vm_label "${vm_label}")"
  [[ "${record_env}" == "${env_filter}" ]]
}
...
certbot_cluster_run_remote_renewability_probe() {
  ...
  if output="$("${CERTBOT_CLUSTER_SSH_BIN}" \
    -o ConnectTimeout=10 \
    -o StrictHostKeyChecking=no \
    -o UserKnownHostsFile=/dev/null \
    -o LogLevel=ERROR \
    "root@${vm_ip}" \
    bash -s -- --cert-name "${cert_name}" < "${CERTBOT_CLUSTER_RENEWABILITY_SCRIPT}" 2>&1)"; then
    [[ -n "${output}" ]] && printf '%s\n' "${output}"
    return 0
  else
    rc=$?
...
certbot_cluster_cert_storage_records() {
  ...
  emit_infra_record() {
    ...
    certbot_cluster_env_matches "${infra_vm_key}" "${env_filter}" || return 0
  ...
  # like vault that terminate LE certs locally. As of #303 the budget check
  # is sourced from Vault KV metadata version counts; missing entries fail
  # closed (no SSH fallback). cert-storage-backfill.sh seeds Vault for any
  # FQDN that was issued before Vault-backed storage existed.
  case "${env_filter}" in
    ""|prod)
