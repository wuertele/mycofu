...
# Do NOT use jq's // (alternative operator) for booleans. // treats
# false as null and returns the default, so '.sealed // true' returns
# true when sealed IS false. Use 'if' or 'tostring' instead.
INITIALIZED=$(echo "$HEALTH" | jq -r 'if .initialized then "true" else "false" end')
SEALED=$(echo "$HEALTH" | jq -r 'if .sealed then "true" else "false" end')
