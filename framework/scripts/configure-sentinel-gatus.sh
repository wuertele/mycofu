...
# `.domain` is read ONCE and guarded once, then both consumers derive from
# it — the same shape #704 gave the sibling generator
# (generate-gatus-config.sh:174-175,191). Two independent `$(yq ...)`
# interpolations were not just duplication: mikefarah `yq -r` prints the
# string "null" and exits 0 for a missing key, so a config.yaml with no
# `.domain` silently rendered `prod.null` and `gatus@null` into the Gatus
# config this script deploys to the NAS. Unlike #630's here-doc, that
# reaches a deployed monitoring artifact — the sentinel would then watch a
# nonexistent hostname and report green while prod DNS was unmonitored.
