...
#
# Usage:
#   framework/scripts/configure-replication.sh <vm-name-pattern> [--park-status <file>] [--env <dev|prod>] [--refresh-artifact-only]
#
# Examples:
...
usage() {
  cat <<EOF
Usage: $(basename "$0") <vm-name-pattern> [--park-status <file>] [--env <dev|prod>] [--refresh-artifact-only]
...
Options:
  ...
  --refresh-artifact-only
                      Re-derive the git-determined replication policy and
                      redeliver ONLY the /etc/repl-policy.vmids node artifact
                      (POLICY_ON/OFF, CADENCE_MAP, POLICY_GEN). Skips VM
                      discovery, orphan cleanup, and all job create/prune/
                      schedule-reconcile/seed work — no live replication job
                      is read or mutated. Used by safe-apply.sh's no-change
                      path to keep the node artifact a synchronous projection
                      of the deployed ref without tearing down transient
                      first-syncs (#693/#501). --park-status and --env (a
                      prune-only scope) have no effect in this mode.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --refresh-artifact-only) REFRESH_ARTIFACT_ONLY=1; shift ;;
    --env)
...
# deliver_policy_artifact — build and deliver /etc/repl-policy.vmids to every
# node (Sprint 047 A3.6). The artifact is a PURE projection of the
# git-determined policy in HELPER_TSV_CACHE (GLOBAL_POLICY_ON/OFF + per-VM
# cadence): it reads NO live replication-job state and mutates NO job, so it
# is safe to run without the create/prune/seed machinery. Called both at the
# end of a full run and on the --refresh-artifact-only fast path. Reads
# HELPER_TSV_CACHE, GLOBAL_POLICY_ON, GLOBAL_POLICY_OFF, NODE_NAMES, CONFIG.
deliver_policy_artifact() {
  # Build CADENCE_MAP=vmid1:secs1,vmid2:secs2,... sorted by vmid.
  local cadence_map policy_gen artifact_content node node_ip
...
"
  for node in $NODE_NAMES; do
    ...
    if [[ -z "$node_ip" || "$node_ip" == "null" ]]; then
      echo "  ERROR: could not resolve mgmt_ip for node ${node} — artifact NOT delivered" >&2
      failed=$((failed + 1))
      continue
    fi
    if echo "$artifact_content" | ssh -o ConnectTimeout=5 "root@${node_ip}" \
        "cat > /etc/repl-policy.vmids.tmp && mv /etc/repl-policy.vmids.tmp /etc/repl-policy.vmids"; then
      delivered=$((delivered + 1))
    else
      echo "  ERROR: failed to deliver artifact to ${node} (${node_ip})" >&2
      failed=$((failed + 1))
    fi
  done
  # Fail closed (#693 review, codex P1 / agy P2 / claude P3): a partial or empty
  # delivery leaves nodes carrying a stale artifact and would re-create exactly
  # the stale-artifact validate false-red this fix exists to kill — just shifted
  # from "refresh never ran" to "refresh silently half-failed". A green deploy
  # must mean every node has the current projection. Aligns with
  # .claude/rules/destruction-safety.md "unknown state → FAIL, not SKIP".
  if [[ $failed -gt 0 || $delivered -eq 0 ]]; then
    echo "==> ERROR: /etc/repl-policy.vmids delivery incomplete (delivered=${delivered}, failed=${failed}); refusing to report success" >&2
    return 1
  fi
  echo "==> Delivered /etc/repl-policy.vmids to all ${delivered} nodes (POLICY_GEN=${policy_gen:0:8}...)"
  return 0
}
...
# --- Refresh-artifact-only fast path (#693/#501) ---
# Deliver the node artifact as a pure projection of the git-determined policy
# and exit, WITHOUT touching any live replication job. This is what
# safe-apply.sh's no-change deploy path invokes so the node artifact stays a
# synchronous projection of the deployed ref (killing the stale-artifact
# false-red class) while deliberately NOT running the create/prune/seed
# machinery that tears down transient first-syncs (the reason the full run is
# excluded from the no-change path — see safe-apply.sh).
if [[ $REFRESH_ARTIFACT_ONLY -eq 1 ]]; then
  echo "==> Refresh-artifact-only: delivering /etc/repl-policy.vmids from git-derived policy; skipping VM discovery, orphan cleanup, and all job create/prune/reconcile/seed work (no live replication job is read or mutated)."
  refresh_rc=0
  deliver_policy_artifact || refresh_rc=$?
  exit "$refresh_rc"
fi
...
if [[ "$STRICT_CREATED_COUNT" -gt 0 ]]; then
  ...
  NUDGE_MIN_TICK_GAP=3
  NUDGE_MAX_PER_JOB=3
  NUDGE_LOG_FILE=$(mktemp); CLEANUP_FILES="$CLEANUP_FILES $NUDGE_LOG_FILE"

  # #984/#985: after one failed run, PVE defers the next retry by
  # 60*(5*fail_count) seconds. The first five-minute backoff consumes this
  # gate's entire 30x10s budget, so ask the source node to retry immediately
  # while retaining the same bounded, fail-closed polling gate. Cap forced
  # retries at three per job: this bounds the induced fail_count increment at
  # +3, keeping it below OPERATIONS.md's sustained fail_count > 5 real-failure
  # threshold, and bounds induced natural backoff at 60*5*4 = 1200s instead of
  # about 3300s. Three retries span the first ~100s, where the measured
  # 2026-08-14 fleet seed cleared 16 jobs in ~110s; if all three fail, the
  # condition is not the transient this nudge targets.
  nudge_failed_job() {
    local job_id="$1"
    local source_node="$2"
    local attempt="$3"
    local nudge_count last_nudge source_ip nudge_output nudge_rc

    IFS=$'\t' read -r nudge_count last_nudge <<< "$(
      awk -F'\t' -v id="$job_id" '
        $1 == id { count++; last = $2 }
        END { printf "%d\t%d\n", count + 0, last + 0 }
      ' "$NUDGE_LOG_FILE"
    )"
    if [[ "$nudge_count" -ge "$NUDGE_MAX_PER_JOB" ]]; then
      return 0
    fi
    if [[ "$nudge_count" -gt 0 ]] && [[ $((attempt - last_nudge)) -lt "$NUDGE_MIN_TICK_GAP" ]]; then
      return 0
    fi

    source_ip=$(yq -r ".nodes[] | select(.name == \"${source_node}\") | .mgmt_ip" "$CONFIG")
    if [[ -z "$source_ip" || "$source_ip" == "null" ]]; then
      printf '%s\t%s\t%s\n' "$job_id" "$attempt" "-1" >> "$NUDGE_LOG_FILE"
      echo "    NUDGE: skipped ${job_id} on ${source_node} — source node mgmt_ip unavailable"
      if [[ $((nudge_count + 1)) -eq "$NUDGE_MAX_PER_JOB" ]]; then
        echo "    NUDGE: ${job_id} reached the ${NUDGE_MAX_PER_JOB}-nudge cap — no further forced retries (each failed retry inflates PVE's backoff)"
      fi
      return 0
    fi

    set +e
    nudge_output=$(ssh -n -o ConnectTimeout=5 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
      "root@${source_ip}" "pvesr schedule-now ${job_id}" 2>&1)
    nudge_rc=$?
    set -e
    printf '%s\t%s\t%s\n' "$job_id" "$attempt" "$nudge_rc" >> "$NUDGE_LOG_FILE"
    if [[ "$nudge_rc" -eq 0 ]]; then
      echo "    NUDGE: pvesr schedule-now ${job_id} on ${source_node} — retrying now instead of waiting out PVE's failure backoff"
    else
      echo "    NUDGE: pvesr schedule-now ${job_id} on ${source_node} FAILED (rc=${nudge_rc}), continuing to poll: ${nudge_output}"
    fi
    if [[ $((nudge_count + 1)) -eq "$NUDGE_MAX_PER_JOB" ]]; then
      echo "    NUDGE: ${job_id} reached the ${NUDGE_MAX_PER_JOB}-nudge cap — no further forced retries (each failed retry inflates PVE's backoff)"
    fi
  ...
  for attempt in $(seq 1 30); do
    ...
    while IFS= read -r job_id; do
      ...
      if [[ "$job_unsynced" -eq 1 ]]; then
        if [[ -n "$job_nudge_node" && "$attempt" -lt 30 ]]; then
          nudge_failed_job "$job_id" "$job_nudge_node" "$attempt"
        fi
  ...
  if [[ "$UNSYNCED_COUNT" -gt 0 ]]; then
    ...
    while IFS=$'\t' read -r job_id detail; do
      [[ -z "$job_id" ]] && continue
      nudge_count=$(awk -F'\t' -v id="$job_id" '$1 == id { count++ } END { print count + 0 }' "$NUDGE_LOG_FILE" || echo 0)
      accepted_count=$(awk -F'\t' -v id="$job_id" '$1 == id && $3 == 0 { count++ } END { print count + 0 }' "$NUDGE_LOG_FILE" || echo 0)
      if [[ "$nudge_count" -gt 0 ]]; then
        echo "  FAILED: ${job_id} — ${detail}(schedule-now requested ${nudge_count} time(s), ${accepted_count} accepted; still failing)" >&2
      else
...
# Pure projection of the git-determined policy; see deliver_policy_artifact.
deliver_policy_artifact
