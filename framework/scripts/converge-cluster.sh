...
# against Tier 2 modules (gitlab, cicd, pbs, workstation-dev) because
...
#   - proxmox_virtual_environment_file — snippet uploads
#
...
#   - Anything not in SAFE_RESOURCE_TYPES (Phase A) or anything outside
...
#
# Usage:
#   converge-cluster.sh [--dry-run] [--allow-empty-expected]
#
# Exit codes:
#   0  no drift, or drift fully reconciled
#   1  tofu state list / plan / apply failed (Phase A) or
#      orphan enumeration / deletion failed (Phase B) or
...
# Resource types considered SAFE to apply on a running cluster.
# Matching is anchored TWO ways (see read_safe_targets below):
#   1. The component before the type name must be `module.<name>.` (zero
#      or more times) or the start-of-line — so a substring like
#      "_file_extra" cannot match "_file".
#   2. The component AFTER the type name must be a resource name — not
#      another `module.` — so a state address like
#      `module.proxmox_virtual_environment_file.proxmox_virtual_environment_vm.vm`
#      (a VM living under a module *named* like our safe type) does NOT
#      get targeted. This was a real safety vulnerability flagged in
#      adversarial review (codex + gemini, round 1).
SAFE_RESOURCE_TYPES=(
...
usage() {
  cat <<EOF
Usage: $(basename "$0") [--dry-run] [--allow-empty-expected]

Non-destructive cluster-state reconciliation. Targets only safe-to-apply
resource classes (currently snippet uploads). Idempotent — safe to run
repeatedly. Never touches VMs or HA state.
...
echo "=== converge-cluster Phase A: enumerating safe-to-apply resources ==="

# Capture state list with explicit rc handling. set -e would kill the
# script on a failed `$(...)` assignment before we could report the rc.
set +e
ALL_RESOURCES="$("${SCRIPT_DIR}/tofu-wrapper.sh" state list 2>&1)"
SL_RC=$?
set -e

if [[ $SL_RC -ne 0 ]]; then
  echo "ERROR: tofu state list failed (rc=$SL_RC):" >&2
  printf '%s\n' "$ALL_RESOURCES" >&2
  exit 1
fi

TARGETS=()
for resource_type in "${SAFE_RESOURCE_TYPES[@]}"; do
  # Match the resource type at the resource-type POSITION of a tofu state
  # address. Address shapes:
  #   <type>.<name>                        -- root-level resource
  #   <type>.<name>["<key>"]               -- root-level for_each
  #   module.<x>.<type>.<name>             -- single-level module
  #   module.<x>.module.<y>.<type>.<name>  -- nested modules
  # The component AFTER the type must be a non-`module.` component (a
  # resource name). This blocks the trap address
  #   module.<resource_type as module-name>.proxmox_virtual_environment_vm.vm
  # which the old regex would have matched (and pulled in the VM).
  while IFS= read -r addr; do
    [[ -z "$addr" ]] && continue
    TARGETS+=(-target="$addr")
  done < <(printf '%s\n' "$ALL_RESOURCES" \
    | grep -E "^(module\\.[^.]+\\.)*${resource_type}\\.[^.]" \
    || true)
done

if [[ ${#TARGETS[@]} -eq 0 ]]; then
  echo "No safe-to-apply resources in state. Phase A is a no-op."
else
  echo "Found ${#TARGETS[@]} target resource specs (types: ${SAFE_RESOURCE_TYPES[*]})."
...
#
# Hardcoded to snippet path; if SAFE_RESOURCE_TYPES grows to other
# resource classes, Phase B will need parallel logic for them (or
...
set +e
SHOW_JSON_RAW="$("${SCRIPT_DIR}/tofu-wrapper.sh" show -json 2>/dev/null)"
SJ_RC=$?
set -e

if [[ $SJ_RC -ne 0 ]]; then
  echo "ERROR: tofu show -json failed (rc=$SJ_RC); cannot determine expected snippets." >&2
  exit 1
fi

# Strip wrapper preamble before the first '{' at start-of-line. The wrapper
# prints "Decrypting secrets..." etc. on stdout. The sed expression is
# anchored to ^{ so a stray `{` mid-preamble does not start the extraction.
# The shape validation below confirms we extracted a real tofu state-show
# output (gemini review P1 — defense against brittle preamble stripping).
...
# Shape validation: confirm SHOW_JSON parses as JSON and has the structure
# tofu state-show always produces (.values.root_module). Without this, any
# valid-but-irrelevant JSON object would be silently accepted and the
# downstream jq filter would emit zero expected pairs — which the
# empty-expected safety gate must then catch. This validation is the first
# line of defense; the gate is the second.
