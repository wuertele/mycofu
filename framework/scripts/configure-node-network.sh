...
iface veth-mgmt inet manual
    # Explicit existence check on BOTH ends of the veth pair — either end
    # existing (partial cleanup, orphaned peer) makes `ip link add ... peer`
    # fail with EEXIST on that end. Only recreate when NEITHER end exists.
    # Idempotent on re-run without masking real failures the way a blanket
    # `|| true` would (#697 P2).
    pre-up ip link show veth-mgmt >/dev/null 2>&1 || ip link show veth-mgmt-br1 >/dev/null 2>&1 || ip link add veth-mgmt type veth peer name veth-mgmt-br1
...
verify_node() {
  ...
  if [[ "$peer_count" != "0" && "$peer_count" != "null" ]]; then
    ...
    if [[ -n "$repl_ip" && "$repl_ip" != "null" ]]; then
      ...
      # ifquery --check confirms the persistent stanza matches live state — a
      # narrow but load-bearing gate on the family-(e) migration (#697): if
      # dummy0's address is present at runtime but the stanza is malformed or
      # missing link-type, this catches the drift.
      local ifquery_rc
      ifquery_rc=$(ssh_node "$mgmt_ip" "ifquery --check dummy0 >/dev/null 2>&1; echo \$?" || echo "unreadable")
      if [[ "$ifquery_rc" == "0" ]]; then
        echo "  ✓ ifquery --check dummy0 passes"
      else
        echo "  ✗ ifquery --check dummy0 failed (rc=${ifquery_rc})"
        fail=1
      fi

      # Module preload: /etc/modules-load.d/dummy.conf ensures the dummy kernel
      # module is loaded before networking.service runs — closes the second
      # boot-race source called out in the #697 RCA.
      local dummy_preload
      dummy_preload=$(ssh_node "$mgmt_ip" "test -s /etc/modules-load.d/dummy.conf && grep -qxF dummy /etc/modules-load.d/dummy.conf && echo present || echo missing" || echo "unreadable")
      if [[ "$dummy_preload" == "present" ]]; then
        echo "  ✓ /etc/modules-load.d/dummy.conf preloads dummy module"
      else
        echo "  ✗ /etc/modules-load.d/dummy.conf ${dummy_preload} — re-run configure-node-network.sh"
        fail=1
      fi
...
deploy_node() {
  ...
  if [[ $DRY_RUN -eq 1 ]]; then
    ...
    echo "  [DRY RUN] Would deploy /etc/modules-load.d/dummy.conf (preload dummy for dummy0)"
...
IRDMA"
  ...
  # Deploy dummy module preload so systemd-modules-load.service loads the
  # dummy kernel module before networking.service runs (see #697 RCA:
  # ifupdown2's link addon needs `dummy` present at boot to hand a fresh
  # RTM_NEWLINK to netlink; module autoload via the shell pre-up form
  # added an extra timing window on cold boot).
  echo "  Deploying /etc/modules-load.d/dummy.conf"
  # /etc/modules-load.d exists on stock Proxmox (systemd-modules-load ships it),
  # but mkdir -p is cheap defense against a minimal chroot / rescue environment
  # that might drop it.
  ssh_node "$mgmt_ip" "mkdir -p /etc/modules-load.d && cat > /etc/modules-load.d/dummy.conf << 'DUMMYMOD'
# Deployed by configure-node-network.sh
# Preload the dummy kernel module so dummy0 (corosync-routable /32) can be
# created deterministically by ifupdown2's native link addon at boot.
dummy
DUMMYMOD"
  # Load into the running kernel now so ifreload -a can compose the current
  # stanza. Emit a warning instead of a silent `|| true` — a missing dummy
  # module is unusual and worth surfacing before ifreload -a fails opaquely
  # (sub-claude P3-1).
  ssh_node "$mgmt_ip" "modprobe dummy 2>/dev/null || echo 'WARNING: modprobe dummy failed — dummy0 stanza will be composed at next boot via /etc/modules-load.d/dummy.conf'"
