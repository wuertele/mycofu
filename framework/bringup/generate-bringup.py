...
def main():
    ...
    tailscale_enabled = any(
        vm_cfg.get("tailscale") is True for vm_cfg in config.get("vms", {}).values()
    )
    ...
    sections = [
        ...
        ("common/tailscale.md.j2", "Tailscale", False),
    ...
    for template_path, section_name, is_required in sections:
        if template_path == "common/tailscale.md.j2" and not tailscale_enabled:
            continue
        try:
