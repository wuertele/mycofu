...
in
{
  ...
  # Closes #392. base.nix defaults overlayTmpfsSize to 256M; on a 256 MB
  # testapp VM that consumes 100% of system RAM as a tmpfs cap.
  #
  # The fix works via journald, not directly: base.nix:579-580 sets
  # services.journald.SystemMaxUse = overlayTmpfsSize / 4. So lowering
  # the overlay cap from 256M to 64M automatically lowers the journal
  # cap from 64M to 16M. The journal in /var/log/journal is the bulk
  # of overlay growth (~50M out of ~57M total Shmem at 9d uptime on
  # the pre-fix testapp). With a 16M journald cap, steady-state
  # overlay usage drops to ~17-20M, reclaiming ~40M of Shmem and
  # giving vault-agent's ~22M startup allocation room. See
  # docs/reports/2026-05-25-testapp-prod-vault-agent-oom-investigation.md
  # (Layers 1-3 and the post-MR addendum on the journald mechanism).
  # Mirrors framework/nix/modules/dns.nix:25 and
  # site/nix/hosts/roon.nix:17 — both run with 13-15M /var (journal-
  # dominated) at multi-day uptime, well under their 16M journald cap.
  mycofu.overlayTmpfsSize = lib.mkDefault "64M";
