...
let
  ...
  gitlabNginxPath = "/var/lib/gitlab/nginx";
  gitlabNginxLogPath = "${gitlabNginxPath}/logs";
  gitlabNginxClientBodyTempPath = "${gitlabNginxPath}/client-body";
  gitlabNginxProxyTempPath = "${gitlabNginxPath}/proxy";
  gitlabNginxFastcgiTempPath = "${gitlabNginxPath}/fastcgi";
  gitlabNginxUwsgiTempPath = "${gitlabNginxPath}/uwsgi";
  gitlabNginxScgiTempPath = "${gitlabNginxPath}/scgi";
  gitlabNginxTempPaths = [
    gitlabNginxClientBodyTempPath
    gitlabNginxProxyTempPath
    gitlabNginxFastcgiTempPath
    gitlabNginxUwsgiTempPath
    gitlabNginxScgiTempPath
  ];
...
in
{
  ...
  fileSystems."/var/log/nginx" = {
    device = gitlabNginxLogPath;
    fsType = "none";
    depends = [ "/var/lib/gitlab" ];
    # nofail is load-bearing: without it, this mount would be ordered before
    # local-fs.target and cycle with the storage oneshot ordered before it.
    options = [ "bind" "nofail" ];
  };
  ...
  # Keep nginx's stock log path while moving the backing storage to the
  # GitLab data disk. The setup service must run after /var/lib/gitlab is
  # mounted; otherwise it would create the bind source on the overlay root.
  systemd.services.gitlab-nginx-storage = {
    description = "Prepare data-disk-backed nginx storage for GitLab";
    after = [ "var-lib-gitlab.mount" ];
    before = [ "var-log-nginx.mount" "nginx.service" ];
    requires = [ "var-lib-gitlab.mount" ];
    requiredBy = [ "var-log-nginx.mount" ];
    ...
    serviceConfig = {
      ...
      Restart = "on-failure";
      RestartSec = "5s";
      ExecStart = pkgs.writeShellScript "gitlab-nginx-storage" ''
        set -euo pipefail
        export PATH=${lib.makeBinPath [ pkgs.coreutils pkgs.util-linux ]}:$PATH

        if [ "$(findmnt -rn -o TARGET --mountpoint /var/lib/gitlab)" != "/var/lib/gitlab" ]; then
          echo "ERROR: /var/lib/gitlab is not mounted; refusing to create nginx storage on the overlay" >&2
          exit 1
        fi

        install -d -m 0750 -o nginx -g nginx \
          "${gitlabNginxPath}" \
          "${gitlabNginxLogPath}" \
          "${gitlabNginxClientBodyTempPath}" \
          "${gitlabNginxProxyTempPath}" \
  ...
  systemd.services.gitlab-gen-secrets = {
    ...
    serviceConfig = {
      ...
      ExecStart = pkgs.writeShellScript "gitlab-gen-secrets" ''
        set -euo pipefail
        export PATH=${lib.makeBinPath [ pkgs.coreutils pkgs.openssl pkgs.findutils pkgs.gnugrep ]}:$PATH

        SECRETS_DIR="/var/lib/gitlab/secrets"
        mkdir -p "$SECRETS_DIR"

        # Recursive chown that tolerates files disappearing mid-traversal.
        # On closure-switch re-runs, this service runs while postgres is live;
        # postgres dynamically creates and deletes files like pg_internal.init
        # under /var/lib/gitlab/postgresql. The recursive chown's list-then-act
        # window can race with those deletes → "No such file or directory" →
        # exit 1 under `set -e`. Race-victim files are naturally re-owned by
        # postgres (which always creates files as itself), so filtering ENOENT
        # is safe. Any other chown error (permission denied, invalid uid, I/O,
        # etc.) still surfaces. See issue #430.
        #
        # Hardening details:
        #   - LC_ALL=C forces the C locale so the ENOENT diagnostic is the
        #     literal English string regardless of system locale.
        #   - The ENOENT match is anchored to `: No such file or directory$`
        #     (chown's exact suffix), so filenames containing the phrase
        #     don't accidentally hide real errors.
        #   - The `[[ -d "$dir" ]]` precondition ensures we don't silently
        #     treat "top-level missing" as success.
        #   - Filtered races are counted and logged at warn-level so the
        #     operator has observability into how often the race fires.
        chown_recursive_tolerant() {
          local owner_group="$1" dir="$2" err real race_count
          if [[ ! -d "$dir" ]]; then
            printf 'chown_recursive_tolerant: %s does not exist or is not a directory\n' "$dir" >&2
            return 1
          fi
          if ! err=$(LC_ALL=C chown -R "$owner_group" "$dir" 2>&1 >/dev/null); then
            # chown exited non-zero with no diagnostic — fail loudly. (chown
            # should always emit a diagnostic on error; absence means
            # something is wrong outside the helper's tolerance contract.)
            if [[ -z "$err" ]]; then
              printf 'chown -R %s %s failed with no diagnostic\n' "$owner_group" "$dir" >&2
              return 1
            fi
            real=$(printf '%s\n' "$err" | grep -vE ': No such file or directory$' || true)
            if [[ -n "$real" ]]; then
              printf 'chown -R %s %s failed: %s\n' "$owner_group" "$dir" "$real" >&2
              return 1
            fi
            race_count=$(printf '%s\n' "$err" | grep -cE ': No such file or directory$' || true)
            if (( race_count > 0 )); then
              printf 'chown_recursive_tolerant: filtered %d ENOENT race(s) on %s\n' "$race_count" "$dir" >&2
            fi
          fi
          return 0
        }
        ...
        chown_recursive_tolerant "postgres:postgres" "/var/lib/gitlab/postgresql"
        chmod 700 "/var/lib/gitlab/postgresql"

        chown_recursive_tolerant "gitlab:gitlab" "${gitlabStatePath}"
        ...
        chown_recursive_tolerant "gitlab:gitlab" "$SECRETS_DIR"
  ...
  services.nginx = {
    ...
    appendHttpConfig = ''
      client_body_temp_path ${gitlabNginxClientBodyTempPath};
      proxy_temp_path ${gitlabNginxProxyTempPath};
      fastcgi_temp_path ${gitlabNginxFastcgiTempPath};
      uwsgi_temp_path ${gitlabNginxUwsgiTempPath};
      scgi_temp_path ${gitlabNginxScgiTempPath};
    '';
  ...
  systemd.services.nginx.after = [
    "gitlab-cert-link.service"
    "gitlab-nginx-storage.service"
    "var-log-nginx.mount"
  ];
  ...
  systemd.services.nginx.serviceConfig.ReadWritePaths = gitlabNginxTempPaths;

  # Logrotate is checked hourly. Retention is bounded by whichever comes first:
  # 14 daily rotations or 14 size-triggered rotations at 32 MiB per log.
