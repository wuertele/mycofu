...
in
{
  ...
  # Documented exception to the certbot.nix vault-backed-cert-recovery
  # requirement (see .claude/rules/certificates.md). Vault must not depend
  # on Vault to restore its own TLS cert. /etc/letsencrypt is symlinked to
  # /var/lib/vault/letsencrypt on vdb (see vault-cert-persist.service
  # below), and PBS backs up vdb — recovery is disk-restore, not
  # Vault-restore. This flag exempts vault.nix from the certbot assertion.
  certbot.vdbPersistedCerts = true;
  ...
  systemd.services.vault = {
    ...
    serviceConfig = {
      ...
      # #642: signal the main process directly. systemd runs ExecReload
      # control processes as the unit's User (vault), and the previous
      # `systemctl kill -s HUP vault.service` form asked PID 1 for a D-Bus
      # KillUnit() -- which requires root or polkit manage-units (polkit is
      # not installed on the Vault VMs). Every reload therefore failed with
      # "Access denied" and renewed certs were never served (RCA:
      # docs/reports/rca-2026-07-18-vault-execreload-access-denied.md).
      # Same-UID kill(2) needs no privilege; this is the canonical
      # systemd.service(5) ExecReload example. SIGHUP makes Vault re-read
      # tls_cert_file/tls_key_file from the stable live/ paths.
      # Guarded by tests/test_vault_execreload_642.sh.
