...
# Default kept in lockstep with mycofu.runnerBudget.maxConcurrent in
# gitlab-runner.nix (#681: 8 -> 16). In production the module always exports
# RUNNER_BUDGET_MAX_CONCURRENT, so this fallback is only reached by the hermetic
# tests (which do not set it) and ad-hoc runs — but a reader must not see a stale
# 8 next to a module that defaults 16.
MAX_CONCURRENT="${RUNNER_BUDGET_MAX_CONCURRENT:-16}"
...
make_tmp_in_dir() {
  local _dir="$1" _prefix="$2" _out_var="$3" _tmp
  mkdir -p "$_dir"
  _tmp="$(mktemp "${_dir}/${_prefix}.XXXXXX")"
  TMP_FILES+=("$_tmp")
  # Return via an out-variable: stdout capture would register the temp in a
  # subshell, leaving the parent's EXIT trap unaware of it; underscored locals
  # avoid dynamic-scope out-variable collisions.
...
if [ "$defer_reload" = true ]; then
  ...
  tmp_config=""
  make_tmp_in_dir "$config_dir" ".runner-budget-config" tmp_config
  ...
  if ! cmp -s "$tmp_config" "$CONFIG_FILE"; then
    ...
    mkdir -p "$persist_dir"
    persist_tmp=""
    make_tmp_in_dir "$persist_dir" ".runner-budget-persist" persist_tmp
...
tmp_nix=""
make_tmp_in_dir "$nix_dir" ".nix-budget" tmp_nix
{
  printf 'max-jobs = %s\n' "$nix_max_jobs"
  printf 'cores = %s\n' "$nix_cores"
} > "$tmp_nix"
...
tmp_state=""
make_tmp_in_dir "$state_dir" ".runner-budget-state" tmp_state
{
  printf 'samples=%s\n' "$samples_out"
...
tmp_status=""
make_tmp_in_dir "$status_dir" ".runner-budget-json" tmp_status
