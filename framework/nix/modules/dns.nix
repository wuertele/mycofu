...

{
  ...
  # STOPGAP (issue #561 — remove when the DNS-role memory trim lands): a small
  # compressed-swap valve so a transient memory spike degrades to zram instead of
  # triggering a global OOM that kills pdns_server. The DNS VMs run at 212 MiB with
  # ~25 MiB free and no swap; a CI certbot-renewability probe (SSH'd from cicd,
  # ~45-55 MiB) tips them into OOM and the kernel kills pdns as collateral — see
  # docs/reports/rca-2026-07-11-dns-dev-oom.md. The durable fix is #561 (trim the
  # resident footprint + bound the probe), which supersedes and removes this.
