...
# Sprint-047 M3 mass-recreation trigger (2026-07-19): this comment edit
# deliberately rehashes every image (base.nix is a shared image input) so the
# dev deploy recreates the full dev Tier-1 fleet — the acceptance test that
# post-policy replication absorbs a mass recreation with no initial-sync
# timeout (seed criterion 1). Content-addressed naming makes a comment the
# minimal safe trigger; see docs/sprints/SPRINT-047.md M3.
#
...
{ config, pkgs, lib, systemdPackage ? pkgs.systemd, ... }:

let
  cfg = config.mycofu;
  getRealRootDevice = import ../lib/get-real-root-device.nix { inherit pkgs; };
  systemdBpfFdLeakPatchName = "systemd-256-bpf-firewall-close-cgroup-runtime.patch";
  systemdHasMycofuBpfFdLeakMarker =
    config.systemd.package.mycofuBpfFdLeakFix or false;
  systemdHasMycofuBpfFdLeakBackport =
    systemdHasMycofuBpfFdLeakMarker
    && lib.any
      (patch: builtins.baseNameOf (toString patch) == systemdBpfFdLeakPatchName)
      (config.systemd.package.patches or []);
  systemdHasBpfFdLeakFix =
    if config.system.nixos.release == "24.11"
    # NixOS 24.11 stage 1 crashed with the unstable systemd swap even though
    # that package was new enough to contain the FD-leak fix. For 24.11, only
    # accept the Mycofu backport marker plus patch; revisit this when the base
    # channel moves to a release whose native systemd is boot-proven here.
    then systemdHasMycofuBpfFdLeakBackport
    else
      lib.versionAtLeast config.systemd.package.version "257"
      || systemdHasMycofuBpfFdLeakBackport;
in

{
  options.mycofu.overlayTmpfsSize = lib.mkOption {
    type = lib.types.str;
    default = "256M";
    description = ''
      Size of the initrd tmpfs used as the writable upperdir for the overlay root.
      Low-memory roles can override this to reduce RAM pressure.
    '';
  };

  options.mycofu.fieldUpdatable = lib.mkEnableOption "field-updatable VM (enables generation cleanup timer)";

  config = lib.mkMerge [
  {
  # systemd 256 leaks PID 1 BPF map/program FDs when DefaultIPAccounting=yes
  # and short-lived service units churn. NixOS enables DefaultIPAccounting by
  # default, and Mycofu relies on that standard behavior being safe. Require
  # either an upstream-fixed systemd or the Mycofu backport of upstream
  # 5a8c2c95598f17fb79cca07365e70e54a7abcc88, which closes IP accounting BPF
  # FDs during CGroupRuntime teardown. See root/mycofu#422.
  systemd.package = systemdPackage;

  assertions = [
    {
      assertion = systemdHasBpfFdLeakFix;
      message = ''
        Mycofu VM images require a systemd with the BPF FD leak fix.
        systemd 256 leaks PID 1 BPF FDs under DefaultIPAccounting=yes during
        short-lived service churn unless the upstream fix is backported; see
        root/mycofu#422.
      '';
    }
  ];
  ...
  boot.initrd.postMountCommands = lib.mkAfter ''
    # Safety net: create a read-only bind-mount of the root for the
    # overlay lowerdir. The ext4 superblock stays rw so that /nix and
    # /boot bind-mounts from the real partition are writable. The ro
    # bind-mount is used only as the overlay lower layer — any write
    # that bypasses the overlay hits the ro bind and fails with EROFS.
    #
    # Why not remount,ro on /mnt-root directly? That transitions the
    # ext4 superblock to ro. Bind-mounts from the same filesystem
    # cannot override the superblock flag, so /nix and /boot would
    # stay read-only regardless of remount,rw attempts. This was the
    # bug that broke certbot on 2026-04-11.
    mkdir -p /mnt-root-ro
    mount --bind /mnt-root /mnt-root-ro
    mount -o remount,ro,bind /mnt-root-ro

    # Create overlay tmpfs
    mkdir -p /mnt-overlay-tmpfs
    mount -t tmpfs -o size=${cfg.overlayTmpfsSize},mode=755 tmpfs /mnt-overlay-tmpfs
    mkdir -p /mnt-overlay-tmpfs/upper /mnt-overlay-tmpfs/work

    # Mount overlayfs: ro bind-mount lower + rw tmpfs upper
    mkdir -p /mnt-overlay-combined
    mount -t overlay overlay \
      -o lowerdir=/mnt-root-ro,upperdir=/mnt-overlay-tmpfs/upper,workdir=/mnt-overlay-tmpfs/work \
      /mnt-overlay-combined

    # Bind-mount /nix and /boot from the real rw ext4 (persistent, writable).
    #
    # Trade-off: OverlayFS kernel docs say modifying the lower filesystem
    # while the overlay is mounted is unsupported. Our /nix and /boot
    # bind-mounts write to the same ext4 that is the overlay lowerdir.
    # This works in practice because:
    # - /nix and /boot are accessed via bind-mount, not through the overlay
    # - The overlay only serves paths NOT covered by bind-mounts
    # - This is the same pattern used by nix-community/impermanence and
    #   "erase your darlings" NixOS setups in production
    # If the kernel ever enforces the restriction, the fix is a separate
    # partition for /nix (Phase 0 research noted this as an option).
    mkdir -p /mnt-overlay-combined/nix
    mount --bind /mnt-root/nix /mnt-overlay-combined/nix

    # Bind-mount /boot from the real rw ext4 (persistent, writable)
    mkdir -p /mnt-overlay-combined/boot
    mount --bind /mnt-root/boot /mnt-overlay-combined/boot

    # Create /nix/persist directories (runner config, certbot state).
    # /nix is rw (ext4 superblock is rw), so mkdir succeeds even on
    # first boot when these directories don't exist yet.
    mkdir -p /mnt-overlay-combined/nix/persist
    mkdir -p /mnt-overlay-combined/nix/persist/gitlab-runner
    mkdir -p /mnt-overlay-combined/nix/persist/letsencrypt

    # Bind-mount certbot state from /nix/persist (rw, under /nix).
    # This MUST be active before any stage-2 certbot service runs.
    # Sprint 005 outage was caused by certbot seeing "no cert" when
    # this mount wasn't ready — do not weaken this ordering.
    mkdir -p /mnt-overlay-combined/etc/letsencrypt
    mount --bind /mnt-overlay-combined/nix/persist/letsencrypt /mnt-overlay-combined/etc/letsencrypt

    # Preserve mounts for debugging. /mnt-real-root is the ro bind
    # (safe to inspect without accidentally writing to ext4).
    # /mnt-real-root-rw is the rw ext4 (kept mounted so the kernel
    # retains the superblock — bind-mounts depend on it).
    mkdir -p /mnt-overlay-combined/mnt-real-root
    mount --move /mnt-root-ro /mnt-overlay-combined/mnt-real-root
    mkdir -p /mnt-overlay-combined/mnt-real-root-rw
    mount --move /mnt-root /mnt-overlay-combined/mnt-real-root-rw
    mkdir -p /mnt-overlay-combined/mnt-overlay-tmpfs
    mount --move /mnt-overlay-tmpfs /mnt-overlay-combined/mnt-overlay-tmpfs

    # Replace initrd root with overlay
    mount --move /mnt-overlay-combined /mnt-root
  '';
  ...
  nix.enable = true;                       # required for nix copy --to ssh://
  ...
  services.resolved.extraConfig = "CacheMaxNegativeTTL=30\n";
    ...
    serviceConfig = {
      ...
      ExecStart = pkgs.writeShellScript "configure-mgmt-nic" ''
        ...
        if [ -n "$PRIMARY_ADDR" ] && [ -n "$PRIMARY_GW" ]; then
          # Derive the subnet from the CIDR address.
    ...
    serviceConfig = {
      ...
      ExecStart = pkgs.writeShellScript "configure-mounts" ''
        ...
        ${pkgs.jq}/bin/jq -c '.[]' "$MOUNTS_FILE" | while IFS= read -r mount; do
          ...
          UNIT_NAME=$(${systemdPackage}/bin/systemd-escape --path "$MOUNT_PATH").mount
          AUTOMOUNT_NAME=$(${systemdPackage}/bin/systemd-escape --path "$MOUNT_PATH").automount
          ...
          ${systemdPackage}/bin/systemctl daemon-reload
          # Runtime units (/run/systemd/system/) can't be "enabled" —
          # enable creates symlinks in /etc which isn't supported for
          # runtime units. Just start the automount. The unit is
          # recreated on every boot by this service. (#184)
          ${systemdPackage}/bin/systemctl start "$AUTOMOUNT_NAME" 2>/dev/null || true
  ...
  # When CIDATA delivers deterministic SSH host keys (from SOPS via
  # write_files), this service copies them to /etc/ssh/ before sshd starts.
  # This ensures VMs keep the same host key fingerprint across reboots,
  # field updates, and factory rebuilds — eliminating known_hosts warnings.
  #
  # The ConditionPathExists guard makes this a no-op when keys are not
  # provisioned (before the operator runs new-site.sh --fill-ssh-keys).
  systemd.services.ssh-host-key-restore = {
    description = "Install deterministic SSH host keys from CIDATA";
    wantedBy = [ "multi-user.target" ];
    before = [ "sshd.service" ];
    after = [ "local-fs.target" "nocloud-init.service" ];
    unitConfig.ConditionPathExists = "/run/secrets/ssh/ssh_host_ed25519_key";
    serviceConfig = {
      Type = "oneshot";
      RemainAfterExit = true;
    };
    script = ''
      ...
      install -m 600 /run/secrets/ssh/ssh_host_ed25519_key     /etc/ssh/ssh_host_ed25519_key
      install -m 644 /run/secrets/ssh/ssh_host_ed25519_key.pub /etc/ssh/ssh_host_ed25519_key.pub
    '';
  };
  ...
  services.openssh = {
    enable = true;
    # Only generate ed25519 host keys. RSA is not persisted via CIDATA,
    # so it would regenerate on every overlay reboot, causing known_hosts
    # warnings for clients that cached the RSA fingerprint.
    hostKeys = [
  ...
  systemd.tmpfiles.rules = [
    ...
    "d /nix/persist/gitlab-runner 0700 root root -"
    "d /nix/persist/letsencrypt 0700 root root -"
  ];

  # ── Journal ─────────────────────────────────────────────────────────
  # The journal writes to the overlay tmpfs. Its size limit must be a
  # fraction of the overlay size, leaving headroom for certs, SSH keys,
  # redis, nginx logs, and other overlay writes. 25% of the overlay size
  # is the budget. The previous 500M hardcoded limit exceeded the 256M
  # default overlay, allowing the journal to fill it and crash GitLab
  # (#208). VMs with smaller overlays (dns: 64M, roon: 64M) need
  # proportionally smaller journal limits.
  services.journald.extraConfig =
    let
      # Parse the numeric portion of overlayTmpfsSize (e.g., "256M" → 256)
      overlayMB = lib.toInt (lib.removeSuffix "M" cfg.overlayTmpfsSize);
      journalMB = builtins.toString (overlayMB / 4);
    in ''
      SystemMaxUse=${journalMB}M
      Compress=yes
    '';
  # GRUB path fixup is NOT an activation script. install-grub.pl generates
  # wrong paths (/store/... instead of /nix/store/...) only when it runs on
  # a live overlay-root VM (during switch-to-configuration switch). On
  # factory-deployed VMs, install-grub.pl runs during image build without
  # the overlay, so paths are correct. An activation script would corrupt
  # correct paths on every boot by matching /store/ inside /nix/store/.
  # The fixup lives in converge-lib.sh's closure step, which runs only
  # after switch-to-configuration switch — the one time the bug occurs.
  ...
  # ── Generation cleanup timer (field-updatable VMs only) ────────────
  # Field-updatable VMs (gitlab, cicd) accumulate NixOS generations from
  # repeated closure pushes. This weekly timer removes generations older
  # than 14 days and collects unreferenced store paths.
  # Data-plane VMs are recreated on every deploy and never accumulate
  # generations, so the timer is not enabled for them.
  (lib.mkIf cfg.fieldUpdatable {
    systemd.services.mycofu-generation-cleanup = {
      description = "Trim old NixOS generations on field-updatable VMs";
      serviceConfig = {
        Type = "oneshot";
        # NixOS system profiles are legacy profiles (managed by nix-env/
        # switch-to-configuration), NOT nix-profile-managed. "nix profile
        # wipe-history" fails with "incompatible" on legacy profiles.
        # Use nix-env --delete-generations instead.
        ExecStart = pkgs.writeShellScript "generation-cleanup" ''
          ${pkgs.nix}/bin/nix-env --delete-generations --profile /nix/var/nix/profiles/system 14d
          ${pkgs.nix}/bin/nix-collect-garbage
        '';
      };
    };
    systemd.timers.mycofu-generation-cleanup = {
