...
pkgs.runCommand "source-filter-check" {} ''
  ...
  # Test 3: Host-tool package sources are intentionally outside nixSrc.
  ...
  if [ -e "${nixSrc}/framework/nix/pkgs/meshcmd/default.nix" ]; then
    echo "FAIL: MeshCmd host-tool package leaked into nixSrc"
    echo "  Path: ${nixSrc}/framework/nix/pkgs/meshcmd/default.nix"
    exit 1
  else
    echo "PASS: MeshCmd host-tool package correctly excluded from nixSrc"
  fi
