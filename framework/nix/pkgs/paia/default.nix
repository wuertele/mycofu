{ lib, stdenvNoCC, makeWrapper, bash, coreutils, findutils, gnused, xorriso }:
...
stdenvNoCC.mkDerivation rec {
  ...
  installPhase = ''
    ...
    # Pin every filesystem entry's mtime to epoch 0 — not just the two new
    # .toml files. xorriso embeds per-entry times in the repacked ISO
    # whether they were preserved from the source ISO or synthesized
    # during extraction (Rock Ridge symlink entries in particular are
    # given extraction-time mtimes). Without a recursive pin, those
    # times drift across runs and the output ISO becomes
    # non-deterministic for the same input. `-h` is required so the
    # touch normalizes the symlink object's own mtime (lstat) rather
    # than dereferencing it — that also keeps the command from chasing
    # a symlink target outside the extracted tree. Closes #402.
    find "$extract" -exec touch -h --date='@0' {} +
    ...
    wrapProgram "$out/bin/proxmox-auto-install-assistant" \
      --prefix PATH : ${lib.makeBinPath [ coreutils findutils gnused xorriso ]}
