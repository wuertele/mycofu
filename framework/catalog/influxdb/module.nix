...
let
  ...
  bucketsJsonPath = cfg.configDir + "/buckets.json";
  ...
  hasBucketsJson = builtins.pathExists bucketsJsonPath;
  ...
  influxdbConfigScript = pkgs.writeShellScript "influxdb-write-tls-config" ''
    ...
    TIMEOUT=900
    ELAPSED=0
    # Use test -s (non-empty), not test -f (exists). Certbot can leave
    # 0-byte PEM files after a failed ACME challenge. A 0-byte fullchain
    # or privkey is as fatal as a missing one — InfluxDB crashes trying
    # to parse an empty PEM. See .claude/rules/platform.md.
    ...
    # traversal. InfluxDB runs as root so no private key group fix needed.
    chmod 755 /etc/letsencrypt /etc/letsencrypt/live /etc/letsencrypt/archive
    mkdir -p /etc/letsencrypt/live
    ln -sfn "$CERT_DIR" /etc/letsencrypt/live/influxdb
  ...
  # Idempotent declarative bucket reconciliation. Runs after influxdb-setup
  # on every boot. Reads /etc/influxdb2/buckets.json and ensures each bucket
  # exists with the requested retention. NEVER deletes buckets — removing
  # an entry from buckets.json has no effect on InfluxDB.
  # See framework/catalog/influxdb/bucket-reconcile.sh for the contract.
  influxdbReconcileBucketsScript = pkgs.writeShellApplication {
    name = "influxdb-reconcile-buckets";
    runtimeInputs = [ pkgs.curl pkgs.jq pkgs.coreutils ];
    text = builtins.readFile ./bucket-reconcile.sh;
  };
  ...
  config = {
    vaultAgent = {
      ...
      extraConfig = ''
        template {
          contents = <<EOF
{{- \$env := file "/run/secrets/dashboard/environment" | trimSpace -}}
{{- with secret (printf "secret/data/dashboard/%s/proxmox-api-token" \$env) -}}
{{ .Data.data.value }}
{{- end -}}
        EOF
          destination = "/run/secrets/proxmox-api-token"
          perms = "0400"
        }
        ...
        template {
          contents = <<EOF
{{- \$env := file "/run/secrets/dashboard/environment" | trimSpace -}}
{{- with secret (printf "secret/data/dashboard/%s/influxdb-token" \$env) -}}
{{ .Data.data.value }}
{{- end -}}
        EOF
          destination = "/run/secrets/dashboard-influxdb-token"
          perms = "0400"
        }
      '';
    };

    services.clusterDashboard.enable = true;
    ...
    # Optional: buckets.json — declarative additional bucket management.
    # Consumed by influxdb-reconcile-buckets.service on every boot.
    environment.etc."influxdb2/buckets.json" = lib.mkIf hasBucketsJson {
      source = bucketsJsonPath;
      mode = "0640";
    };
    ...
    # This service waits for the TLS cert to exist, sets up cert
    # permissions, and writes tls.env. It runs the same script as
    # InfluxDB's ExecStartPre — the duplication is intentional:
    # - This oneshot provides cert-readiness ordering for nginx (dashboard)
    # - The ExecStartPre re-creates tls.env after RuntimeDirectory cleanup
    #   on InfluxDB failure, preventing the crash loop.
    systemd.services.influxdb-tls-config = {
      description = "Wait for TLS cert and set up cert permissions";
    ...
    # TLS cert wait and tls.env creation are in ExecStartPre, not a
    # separate service. This avoids the RuntimeDirectory race: when
    # InfluxDB fails, systemd cleans up RuntimeDirectory (removing
    # tls.env). If tls.env were written by a separate oneshot service,
    # the file would be gone on the next restart attempt, creating a
    # self-reinforcing crash loop. With ExecStartPre, tls.env is
    # recreated on every start attempt under the same lifecycle.
    systemd.services.influxdb = {
      ...
      serviceConfig = {
        ExecStartPre = "+" + influxdbConfigScript;
        ExecStart = "${pkgs.influxdb2-server}/bin/influxd";
        # The - prefix on tls.env makes it optional at unit load time.
        # RuntimeDirectory cleanup removes it on failure, but ExecStartPre
        # recreates it before InfluxDB starts. Without the prefix, systemd
        # would refuse to start the unit before ExecStartPre can run.
        EnvironmentFile = [
    ...
    # --- Idempotent bucket reconciliation (runs every boot, after setup) ---
    # Only wired when the operator has provided buckets.json. Reads from
    # /etc/influxdb2/buckets.json (placed by environment.etc above) and
    # ensures each desired bucket exists with the requested retention.
    # Create + update only — never deletes. To apply buckets.json changes
    # mid-life without a reboot:
    #   systemctl start influxdb-reconcile-buckets
    systemd.services.influxdb-reconcile-buckets = lib.mkIf hasBucketsJson {
      description = "Reconcile InfluxDB buckets from buckets.json (idempotent)";
      after = [ "influxdb.service" "influxdb-setup.service" ];
      requires = [ "influxdb.service" "influxdb-setup.service" ];
      wantedBy = [ "multi-user.target" ];
      serviceConfig = {
        Type = "oneshot";
        RemainAfterExit = true;
        ExecStart = "${influxdbReconcileBucketsScript}/bin/influxdb-reconcile-buckets";
      };
    };
