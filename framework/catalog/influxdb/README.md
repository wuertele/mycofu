...
Do NOT put runtime-generated tokens that the framework or VM bootstrap
consume back into SOPS or CIDATA — that creates the VM recreation cycle.

### Client tokens (operator delivered)

Client InfluxDB authorizations are declared in
`site/apps/influxdb/tokens.json` and reconciled after the target bucket
exists. Each entry names the bucket, allowed actions, per-environment
InfluxDB authorization description, and per-environment SOPS key:
...
Reconcile all configured tokens:

```bash
framework/scripts/configure-influxdb-tokens.sh prod
```

Or reconcile one token:

```bash
framework/scripts/configure-influxdb-tokens.sh prod --token homeassistant_read
```

The reconciler:

- Resolves the InfluxDB org from `site/apps/influxdb/setup.json`
- Resolves each configured bucket ID through the InfluxDB API
- Creates one InfluxDB authorization per configured token description
- Grants only the configured bucket actions (`read` and/or `write`)
- Stores the returned token in the configured SOPS key

For Home Assistant monitoring, deliver the
`homeassistant_influxdb_read_token` value to Home Assistant as
`influxdb_homeassistant_read_token` in `/config/secrets.yaml`.

These SOPS values are operator-delivery secrets for consuming systems,
not Mycofu deployment inputs. They are not exported by `tofu-wrapper.sh`,
not written to CIDATA, and not read by the InfluxDB VM on boot.

Rotation is always explicit. If the SOPS token exists but the InfluxDB
authorization is missing, or the authorization exists but the configured
scope is wrong or inactive, the script refuses to replace credentials
silently; rerun with `--rotate` and update the consuming client with the
new secret. If the authorization exists, is active, has the expected
scope, and InfluxDB exposes its token but SOPS is missing the value, the
script restores that token into SOPS without rotating it. If InfluxDB is
configured to store hashed tokens and no longer exposes the plaintext
token, an existing active auth with the expected scope and an existing
SOPS value is treated as idempotent, but a missing SOPS value still
requires `--rotate`.

The `homeassistant_1h` bucket is provisioned for future aggregation. This
script does not populate it and does not configure Home Assistant's
`influxdb:` integration; that belongs in `homeassistant-config`.
