...
#
# Provides:
#   - Grafana server on 0.0.0.0:3000 (HTTPS via certbot)
...
let
  ...
  grafanaConfigScript = pkgs.writeShellScript "grafana-write-tls-config" ''
    ...
    GF_SERVER_ROOT_URL=https://$FQDN:3000/
    EOF
...
in
{
  ...
  config = {
    ...
    systemd.services.grafana-place-dashboards = lib.mkIf hasDashboardsDir {
      ...
      serviceConfig = {
        ...
        ExecStart = pkgs.writeShellScript "grafana-place-dashboards" ''
          ...
          cp -r ${dashboardsDir}/*.json /var/lib/grafana/dashboards/ 2>/dev/null || true
