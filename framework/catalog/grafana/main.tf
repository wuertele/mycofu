# grafana — Deploy a Grafana VM for one environment.
#
# Wraps the proxmox-vm base module with Grafana-specific defaults.
...
module "grafana" {
  ...
  vm_id         = var.vm_id
  vm_name       = "grafana-${var.environment}"
  hostname      = "grafana"
  instance_id   = "grafana-${var.environment}"
  ...
  write_files = concat(
    [
      ...
      },
      {
        ...
        content     = var.grafana_admin_password
        permissions = "0400"
      },
      {
        path        = "/run/secrets/grafana/influxdb-token"
        content     = var.grafana_influxdb_token
        permissions = "0400"
      }
    ],
    var.acme_ca_cert != "" ? [
      {
        path        = "/run/secrets/certbot/acme-ca-cert"
        content     = var.acme_ca_cert
        permissions = "0400"
      }
    ] : []
  )
