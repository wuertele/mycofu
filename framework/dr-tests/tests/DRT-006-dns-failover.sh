...
#
# DRT-006 is a SERVICE-LEVEL test — it exercises the peer-serves-and-
# certbot-renews path, not the underlying HA failover mechanism. Both
# DNS tiers are replicated (universal replication): prod at 1m
# (default-prod cadence), dev at 24h (default-dev cadence). A DNS node
# failure fence-recovers the survivor from replica at its declared
# cadence, and DRT-006's service-level assertion (peer serves) holds
# regardless of which tier fails.
