...
# DRT-TIME: ~50 min
# DRT-DESTRUCTIVE: yes
# DRT-DESC: Validate the rebuild path from a fresh-clone (cold) checkout.
...
drt_record_plan_replacements "${TEMP_CLONE}/home-infrastructure/build/preboot-restore-plan-bulk.json"
printf "  Rebuild took: %dm %ds\n" $((REBUILD_ELAPSED / 60)) $((REBUILD_ELAPSED % 60))

drt_step "Ensuring runner registration and secret delivery"
# The MR-3 runner age key and framework deploy keypair delivery lives in the
# canonical register-runner.sh convergence step. Re-running it here is
# idempotent and makes the cold-path dependency explicit.
drt_assert "register-runner.sh completes successfully" \
  bash -c "cd '${TEMP_CLONE}/home-infrastructure' && framework/scripts/register-runner.sh"
