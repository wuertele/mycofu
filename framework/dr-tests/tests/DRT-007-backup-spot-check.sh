...
SITE_ACME_MODE=$(drt_config '.acme // "production"')
  ...
  gitlab)
    ...
    if [[ -z "$TEMP_VM_IP" ]]; then
      ...
      if [[ "$SSH_READY" != "true" ]]; then
        ...
        echo ""
        echo "    Layer 3.5: Certbot lineage integrity"

        if [[ "${SITE_ACME_MODE}" == "production" ]]; then
          GITLAB_RENEWAL_SERVER=$(ssh ${SSH_OPTS} "root@${TEMP_VM_IP}" \
            "awk -F= '/^server[[:space:]]*=/{gsub(/^[[:space:]]+|[[:space:]]+$/, \"\", \$2); print \$2; exit}' /etc/letsencrypt/renewal/gitlab.prod.${DOMAIN}.conf 2>/dev/null" 2>/dev/null || echo "")
          GITLAB_ISSUER=$(ssh ${SSH_OPTS} "root@${TEMP_VM_IP}" \
            "openssl x509 -in /etc/letsencrypt/live/gitlab.prod.${DOMAIN}/fullchain.pem -noout -issuer 2>/dev/null" 2>/dev/null || echo "")

          echo "      Renewal server: ${GITLAB_RENEWAL_SERVER:-missing}"
          echo "      Leaf issuer: ${GITLAB_ISSUER:-missing}"

          drt_assert "GitLab renewal lineage uses the production ACME URL" \
            bash -c "[[ '${GITLAB_RENEWAL_SERVER}' == 'https://acme-v02.api.letsencrypt.org/directory' ]]"
          drt_assert "GitLab live leaf issuer is not staging" \
            bash -c '[[ -n "$1" && "$1" != *"Fake LE"* && "$1" != *"STAGING"* ]]' _ "${GITLAB_ISSUER}"
        else
          echo "      Site ACME mode: ${SITE_ACME_MODE} — production issuer assertion skipped"
        fi
