  ...
  gitlab)
    ...
    if [[ -z "$TEMP_VM_IP" ]]; then
      ...
      if [[ "$SSH_READY" != "true" ]]; then
        ...
        # cert-restore.service rewrites /etc/letsencrypt/live/ on every boot
        # from Vault KV, so a backup-content probe of the temp VM's on-disk
        # cert cannot verify backup content. The live-cert-issuer assertion
        # lives in validate.sh (gitlab_live_issuer_check) instead.
