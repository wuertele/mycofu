...
| ID | Name | Tier | Last Result | Last Run | Commit | Est. Time |
|----|------|------|-------------|----------|--------|-----------|
| DRT-001 | Warm Rebuild | destructive-judgment | PASS (coverage-limited) | 2026-09-13T18:42:50Z | 0044c55 — harness criteria met (validate green, fingerprint 5/5) but ZERO VMs recreated: retained default plan all no-op (178 changes incl. all 20 VM resources) → stop/start only (#1020). Establishes the #1002 fail-closed join across the missing-token transient of the serialized Phase-2 start; does NOT establish recreation/PBS-restore, the #1002 cold-recreation acceptance, or #1011 R-I-2. Prior FAILs 2026-09-12 d6a0f7c and 2026-08-31 964b625 (the #1002 catch — also a restart; control plane not recreated in either). Last row with ground-truth recreation evidence for named VMs: 2026-07-08 53f37d7 | ~16 min (warm) / ~88 min (full image rebuild) |
| DRT-002 | Cold Rebuild | destructive-judgment | PASS | 2026-07-08 | d401d3f4 (#505 fix verified — fresh-clone preflight clean; apply was 173/173 no-op) | ~50 min |
| DRT-003 | PBS Restore | destructive-judgment | PASS | 2026-07-08 | 53f37d7 | ~9 min |
| DRT-004 | Vault HA Failover | destructive-judgment | PASS | 2026-08-31T00:17:52Z | 6bb6297 | ~3 min |
| DRT-005 | Node Failure Recovery | destructive-judgment | PASS | 2026-07-23T09:06:02Z | b9fc097 M4 attempt 3 — CLEAN PASS, closes M4, Sprint 048 failover acceptance, and Sprint 047 (open-blocked-on-successor ruling 2026-07-21). Recovery 136s (series 146/146/136 vs 600s analytic ceiling). First fully unassisted full cycle: family-(e) dummy0 stanza (#697) held from cold boot, failback autonomous, zero HA errors, zero qmstart hangs, app state preserved. Harness #696 + tooling #697 exercised. Bands ratcheted: WARN 166s / FAIL 252s (MR-7). | 20m 8s |
| DRT-006 | DNS Failover | cheap-routine | PASS | 2026-09-21T01:40:06Z | eb8452e8 (R-I-2 rerun for !627/#1025; certbot-cluster.sh invalidation) | ~5 min |
| DRT-007 | Backup Spot-Check | cheap-routine | PASS | 2026-09-21T01:44:23Z | eb8452e8 (R-I-2 rerun for !627/#1025; certbot-cluster.sh invalidation) | ~1 min |
| DRT-008 | Reset Contract Ratchet | cheap-routine | PASS | 2026-09-21T02:16:08Z | 912ba096 | ~2 min |
| DRT-009 | Key Rotation | cheap-routine at `--depth rekey` | PASS | 2026-08-04T12:11:45Z | 04b00e5c — G3 two-acts window `20260804T121127Z`. Act 1 rekey PASS 12m 4s (age-key rotation 5; !567 quiescence gate waited 2s — second consecutive PASS). Act 2 GitLab M4: rotation SUCCEEDED — converge via escrowed-old OAuth + PUT (!570), old password dead (live-verified HTTP 400 `invalid_grant`, escrow hash match), new password verified granting. Step 6 initially failed closed on a wrong 401 expectation (GitLab/Doorkeeper returns 400 `invalid_grant`): #871, fixed in !573; Act 2 result adjudicated PASS by operator on driver evidence + live probes (ratified by the merge of the G3-close MR). Both #680 leaked halves rotated. Follow-ups: #872, #873, #874, #875. **G5 window record (2026-08-30, appended to this row per the SPRINT-049 G5 gate row — "registry Last-Run block extended with this act's line" — and that plan's DRT-009 invalidation disposition, "G5 extends the row with the prod-Vault line if S3 gate ran"):** this is a G-window record of the M4-Vault class exercise, NOT a `run-dr-test.sh DRT-009 --depth` invocation, so the Result / Last Run / Est. Time columns above are unchanged and still describe the 2026-08-04 first invocation. Two legs, driver `f4b714c`. Dev leg 2026-08-30T00:54Z (!599, `0f8b08d`): native rekey + generate-root PASS, old root token revoked with live prove-negative, post-rotation pin `vm/303/2026-08-30T00:54:43Z` restore-verified (#978 probe race closed out read-only). Prod leg 2026-08-30T01:27Z (!600, `0f74d37`): preceded by an operator-approved SOPS recoupling that retired the 2026-03-28 divergence minted by pipeline 174's since-removed `post-deploy.sh` auto-init (RCA: `docs/reports/2026-08-30-rca-vault-prod-sops-divergence.md`); rekey + generate-root PASS, post-rotation pin `vm/403/2026-08-30T01:27:47Z` restore-verified, SOPS==vdb coupled. First-run defects found and fixed en route: #947 (!596), #961 (!597), #970 (!598) — the #961 stale-share RCA is `docs/reports/2026-08-29-rca-g5-generate-root-stale-share.md`. Open residuals: #978 (cosmetic probe race); #969 — the #961 driver fix invalidated the 2026-08-04 DRT-009 PASS, so R-I-2 owes a rerun. That rerun rides Sprint 050 post-promotion and is **NOT** discharged by this window: G5 exercised the driver live, but a driver run is not a `run-dr-test.sh DRT-009` invocation and does not refresh this row's Result / Last Run. **Correction to the G5 record above (appended 2026-08-30, preserving the #998 observation; the original G5 text stands as the historical record and is not deleted):** the G5 prod leg's "SOPS==vdb coupled" claim proved the UNSEAL-KEY half only. The G5-era driver (`f4b714c`) had no root-token delivery path — the sole writer of `/var/lib/vault/root-token` was `init-vault.sh`'s fresh-init branch — so after G5 both Vaults' vdb token files were still pre-rotation vintage (prod 2026-03-28, dev 2026-04-05) and held root tokens G5 had just revoked. That surfaced as the pipeline-2172 `deploy:prod` failure, where `post-deploy.sh` read the stale vdb token and Vault answered 403. RCA: `docs/reports/2026-08-30-rca-pipeline-2172-deploy-prod.md`. Fixed by !605 (root-token delivery + post-restore read-back + `validate.sh` coverage of the vdb token) and !607 (the post-restore boot wait that lets the Step-8 verification actually run); proven by the re-rotation window recorded next. **2026-08-30 re-rotation window record (appended to this row on the same G-window convention as the G5 record above):** this is a G-window record of the M4-Vault class exercise, NOT a `run-dr-test.sh DRT-009 --depth` invocation, so the Result / Last Run / Est. Time columns remain those of the 2026-08-04 first invocation. Three operator-attended legs. Leg 1 — dev, 2026-08-30T20:54Z, driver `03914b5` (!605 merged): Steps 1–7 green, including the first live execution of the Step-4 root-token delivery (`/var/lib/vault/root-token` mtime 20:54:03Z); Step-8 restore completed; red exit at the Step-8 probe race — #978's second occurrence, cause established. The read-only closeout then verified full coupling (vdb == SOPS, SHA-256 prefix `029d4fe54804`; `lookup-self` HTTP 200). This occurrence supersedes the "cosmetic" characterization in the G5 residual list above: the race exits the driver red after a completed restore. Log: `logs/rotate-vault-credentials-02.log` (workstation). Leg 2 — dev acceptance, 22:40Z, driver `e97ca73` (!607 boot wait): **GREEN end-to-end, exit 0** — the boot wait rode the restart (API answered in 22s / 2 probes), after which health, the new token's `lookup-self`, and the !605 vdb read-back were all verified BY THE DRIVER (first live execution of the read-back). Pre-pin `vm/303/2026-08-30T22:39:59Z`; post-pin `vm/303/2026-08-30T22:40:27Z` restore-verified. Escrow `20260830T223948Z`. Log: `logs/rotate-vault-credentials-03.log`. Leg 3 — prod acceptance, 22:45Z, same driver: **GREEN end-to-end, exit 0**, identical profile (22s / 2 probes). Pre-pin `vm/403/2026-08-30T22:43:55Z`; post-pin `vm/403/2026-08-30T22:46:59Z` restore-verified. Escrow `20260830T224345Z`. Log: `logs/rotate-vault-credentials-04.log`. Window close-out: the SOPS transport for both envs merged as !609 (`c4ebb82`); dev deploy pipeline 2194 fully green, including `validate.sh`'s vault coupling checks for BOTH envs; and `post-deploy.sh prod` completed green 2026-08-30 ~23:45Z through the vdb-token consumer path — the exact read that failed pipeline 2172. #978 is closed by this window: the fix merged in !607, and legs 2 and 3 are its acceptance. **R-I-2 disposition for this window:** !605 and !607 modified `framework/scripts/rotate-vault-credentials.sh`, a DRT-009 invalidation input per the Invalidation Quick Reference row `framework/scripts/rotate-*.sh` → DRT-009. That invalidation is **discharged** by the operator-attended legs above — the changed driver was exercised live, end-to-end, green, on both environments, which is the R-I-1 live exercise R-I-2 requires before the work is done. It does **NOT** discharge #969: the `--depth resecret-all` rerun owed because the #961 driver fix invalidated the 2026-08-04 DRT-009 PASS is a separate, unchanged obligation riding Sprint 050 post-promotion. As with G5, this window was not a `run-dr-test.sh DRT-009` invocation and does not refresh this row's Result / Last Run. | ~12 min + ~3 min |
| DRT-010 | Full-Fleet Recreate | destructive-judgment | PASS (`--scope dev`; `--scope all` not yet run) | 2026-09-21T02:16:52Z | 912ba096 — first invocation; recreation observed 8/8 on hosting nodes, 3/3 precious dev vdb GUIDs preserved (park/adopt), app fingerprints unchanged; 97m19s of which 62m was Step 5 (image build; rebuild.log) | ~30 min (dev, estimate assuming images already built; the first run's Step 5 took 62 min) / ~90 min (all) |

---

## Rotation DRT Tier Doctrine (Sprint 049 S4)

Rotation pressure has two sources: change-induced drift, which ordinary
touch-triggered tests cover, and time/environment-induced drift, proven by the
dummy0/#697 class where nothing changed in Git but the live recovery contract
rotted. The registry therefore carries two DRT tiers. Cheap-routine tests are
expected to be runnable often with low per-run risk: DRT-006, DRT-007,
DRT-008, and DRT-009 at `--depth rekey` are in this tier. Destructive-judgment
tests require an attended window or meaningful blast-radius judgment:
DRT-001, DRT-002, DRT-003, DRT-004, and DRT-005. Standing pressure is to
demote tests downward by driving per-run cost and risk toward zero, as the
Sprint 048 DRT-005 work did for node-failure recovery.

## Sprint 049 Invalidation Note

**Status:** Sprint 049 adds the rotation manifest, runner secret-delivery
changes, M4 rotation drivers, and DRT-009. Live destructive reruns remain
operator-owned; G3 closes DRT-009's first run.

**Recomputed invalidation:**

- **DRT-001 (Warm Rebuild)** - INVALIDATED by MR-3. `tofu-wrapper.sh`,
  `framework/tofu/root/`, and register-runner delivery are DRT-001 inputs.
  Rerun operator-scheduled after G2; G2 itself exercises the control-plane
  slice live.
- **DRT-002 (Cold Rebuild)** - INVALIDATED by MR-3 and MR-4. The cold path
  now requires `register-runner.sh` secret delivery for the runner to decrypt,
  and an M1 rotation changes which workstation key file the fresh-clone
  operator must carry. Rerun operator-scheduled after G3, including one
  pipeline push that proves delivered-key decrypt from cold.
- **DRT-003 (PBS Restore)** - NOT mechanically invalidated by MR code changes.
  The S2 dev restore-verify beat is gate-mandatory at G3 when the dev Vault M4
  leg runs; later `resecret-all` or PBS credential rotation should schedule a
  hygiene rerun.
- **DRT-004 (Vault HA Failover)** - INVALIDATED by the first M4 Vault
  credential event, not by MR code alone. G3 dev and G5 prod exit checks close
  the relevant post-rotation proof.
- **DRT-005 (Node Failure Recovery)** - NOT invalidated. Replication/HA
  recovery contracts are unchanged.
- **DRT-006 (DNS Failover)** - NOT invalidated.
- **DRT-007 (Backup Spot-Check)** - NOT invalidated; rotation drivers consume
  pins but do not change backup capture semantics.
- **DRT-008 (Reset Contract Ratchet)** - NOT invalidated.
- **DRT-009 (Key Rotation)** - G3 CLOSED 2026-08-04: first invocation PASS,
  window `20260804T121127Z`, both S6 two-acts-one-window acts (DRT-009
  `--depth rekey` and `rotate-gitlab-root-password.sh --i-mean-it`). See the
  Test Index row for the combined result and follow-up issues.
  **G5 CLOSED 2026-08-30**: both M4-Vault legs (dev `0f8b08d`, prod
  `0f74d37`) rotated and restore-verified — the G-window record is appended
  to the same Test Index row. G5 was not a `--depth` invocation, so it does
  not discharge #969: the #961 driver fix invalidated the 2026-08-04 DRT-009
  PASS, and R-I-2 owes a rerun for that.

## Pending First Exercise Rows

| Class | Cheapest representative row | Retirement rule |
|-------|-----------------------------|-----------------|
| M2 | `pdns_api_key` | PENDING-FIRST-EXERCISE until an operator-chosen day runs the M2 ordinary deploy/recreate path and records the result here. Known first-exercise defects: #751 (DRT-009 does not currently dispatch M2 `driver:` rows), #752 (prove-negative aggregation permits false-positive at `resecret-all`). |
| M3 | `gitlab_runner_registration_token` | PENDING-FIRST-EXERCISE until an operator-chosen day runs the M3 owning-script/probe path and records the result here. Known first-exercise defects: #750 (manifest `driver:` string omits required argv for M3 configure scripts), #752 (prove-negative aggregation). |
| M5-automated | `github_deploy_key` | PENDING-FIRST-EXERCISE until an operator-chosen day runs the GitHub deploy-key rotation path and records the result here. Known first-exercise defects: #750 (manifest `driver:` string omits required argv for M5-automated scripts), #752 (prove-negative aggregation). |

## !627 (#1025) Invalidation Note

**Status:** !627 (merged to dev 2026-09-20, `eb8452e8`; deletes the runtime
LE-staging override) changed `framework/scripts/converge-lib.sh`,
`framework/scripts/converge-vm.sh`, `framework/scripts/certbot-cluster.sh`
and `.gitlab-ci.yml`. Per the Invalidation Quick Reference rows for the three
scripts (and the per-section `.gitlab-ci.yml` triggers of DRT-001/DRT-003)
that invalidates DRT-001, DRT-002, DRT-003, DRT-006 and DRT-007.

**Dispositions (R-I-2):**
- DRT-006, DRT-007: **rerun 2026-09-21 on `eb8452e8` — PASS** (rows above/below).
- DRT-001: pending operator ruling — literal rerun at the promoted prod
  tip, or credit the queued DRT-010 `--scope all` run with a recorded
  coverage argument (forced recreation is a different path from
  drift-driven warm reuse; not a superset).
- DRT-002, DRT-003: pending operator ruling — rerun in this window, or
  record the deferral here. Both are also on the #937 backlog for earlier
  invalidations (last run 2026-07-08).

The #1025 dev-acceptance evidence (attended converge exercise on the live
dev network with a before/after per-VM record; the pipeline never executes
the changed step) is `docs/reports/2026-09-21-1025-dev-acceptance.md`.
The prod-side proof is the DRT-010 `--scope all` evidence window after
promotion; #1025 closes on that.
...
**Status:** PASS on 2026-09-13 commit `0044c55` by the harness criteria
(validate.sh green, fingerprint 5/5, elapsed WARN) — **coverage-limited:
zero VMs were recreated.** The retained default plan
(`build/preboot-restore-plan-bulk.json`, 2026-09-13 12:56 local) has 178
resource changes, all `no-op`, including all 20 VM resources; rebuild.log
records `gitlab/cicd/pbs: no pending changes — skipping`;
`build/preboot-restore-bulk.json` = `entries: []`; and a post-run ZFS
listing (operator-observed, not a harness before/after comparison) shows
no `vm-*` zvol on any node with a 2026-09-13 creation date. The run was a
full-fleet stop → serialized Phase-2 start → validate. Likely explanation
(inference, not a retained image comparison): the fleet had been deployed
at `0044c55` by the 2026-09-12 pipelines (prod pipeline #2264 succeeded on
`0044c55`), so the fresh full-fleet build reproduced the deployed
content-addressed images. Recreation, CIDATA regeneration, and preboot
PBS restore were NOT exercised — filed as #1020 (the harness must assert
or force recreation instead of passing on a restart). What the run did
establish: the #1002 fail-closed tailscale join across the missing-token
transient of the serialized Phase-2 start (a reboot also discards
`/var/lib/tailscale` via the tmpfs root) — on gitlab and cicd the join
refused to fresh-enroll, was retried once by systemd, and restored the
Vault identity (operator-observed journals). The #1002 cold-recreation
acceptance, the Sprint-049 MR-3 invalidation, and #1011's R-I-2 rerun
therefore remain OWED. gitlab and cicd have not been recreated by any
DRT-001 run since their zvols were created (May 14 / Jul 26 2026); the
most recent row with ground-truth recreation evidence for named VMs is
2026-07-08 (`53f37d7`).

**What it validates:** The rebuild path from the current (warm) checkout:
build → preflight → phased apply → converge → validate. VM replacements
are drift-driven, with precious state carried through park/adopt or PBS
preboot restore when replacement occurs. The registry block records
replacements observed in the retained bulk plan; replacement is not a pass
criterion. Forced recreation coverage belongs to DRT-010.
...
```
DRT-001 Warm Rebuild
Date:    2026-09-13T18:42:50Z
Commit:  0044c55
Result:  PASS (harness criteria) — COVERAGE-LIMITED: zero VMs recreated
Time:    88m 13s harness total; rebuild-cluster.sh 83m 48s, of which
         Step 5 image build 3202s (53m 22s) and Step 7 (OpenTofu
         init/apply and associated pre-deploy backup, HA checks,
         planning) 573s (9m 33s). WARN > 45 min, build-dominated.
Notes:   Run from a prod checkout at 0044c55 (== gitlab/dev tip at the
         time; rebuild.log records the commit). Harness checks all
         PASS: fingerprint 5/5 (GitLab projects 8/8, Vault initialized
         + 14 mounts, InfluxDB homelab, Roon 1816M vs 1859M pre-test,
         threshold 929M); validate.sh green after the rebuild; Step 14
         completed and the runner is registered and online (validate's
         API check). Fingerprints measure final application state
         only — they do not certify restoration provenance.
         WHAT WAS NOT EXERCISED: no VM was recreated. Retained default
         plan build/preboot-restore-plan-bulk.json (12:56 local): 178
         resource changes, all no-op, including all 20 VM resources;
         rebuild.log: "gitlab/cicd/pbs: no pending changes —
         skipping"; build/preboot-restore-bulk.json = {"entries":[]};
         post-run ZFS listing, operator-observed (zfs list -H -r -t
         volume -o name,creation,guid on pve01/02/03; not a harness
         before/after comparison): every vm-*-disk-*/cloudinit zvol
         predates the run (dev VMs 2026-09-12 19:1x local, prod VMs
         2026-09-12 20:3x-20:4x local, gitlab May 14, cicd Jul 26, pbs
         Apr/May); guest uptimes ~21-24 min at 20:24Z. Inference for
         the no-op plan (not a retained image comparison): the fleet
         had been deployed at 0044c55 by the 2026-09-12 pipelines
         (prod pipeline #2264 succeeded on 0044c55), so the fresh
         build reproduced the deployed content-addressed images. So
         this run was a full-fleet stop → -parallelism=1 Phase-2 start
         → validate. CIDATA regeneration, preboot PBS restore, and
         vdb park/adopt were not run. The harness's "This will destroy
         all VMs" line is an announcement, not an observation. Filed as
         #1020: DRT-001 must assert or force recreation and FAIL
         closed on a zero-replacement plan. NOT discharged by this
         row: the #1002 cold-recreation acceptance, the Sprint-049
         MR-3 invalidation, and #1011's R-I-2 rerun (no allocation
         storm occurred; -parallelism=1 remains unverified against
         one).
         WHAT WAS ESTABLISHED (#1002 fix under the real ordering): a
         reboot also discards /var/lib/tailscale (tmpfs root), so the
         serialized Phase-2 start reproduced the missing-token window.
         Operator-observed journals on gitlab and cicd (journalctl -b
         -u tailscale-join, quoted, not harness-captured): 20:01:06Z
         "NeedsLogin but no Vault token yet (Vault/DNS not ready);
         refusing to fresh-enroll on a transient. Failing so systemd
         retries"; 20:01:21Z systemd restart counter 1; 20:02:13Z
         "restored identity into /var/lib/tailscale"; 20:02:15Z
         "restored Vault identity and connected" (gitlab
         100.83.168.51, cicd 100.88.181.113 — identity restored from
         Vault, not re-enrolled; IP continuity is the operator's
         comparison). Same failure CLASS as the 2026-08-31 and
         2026-09-12 FAILs (missing-token transient during Phase-2
         start), not an identical cross-VM timeline.
         Environment (unverified operator report — du/df observed
         during the run, no retained record): /nix on a dedicated SSD
         RAID volume and the repo + build/ on /Volumes/Media after the
         #1001 escalation; build/ roughly 40G at peak with several
         hundred GiB free on /nix; the earlier same-day attempt had
         ENOSPC'd (row below).
         Status artifacts: build/preboot-restore-status-{all,bulk}.json
         and build/vdb-park-status-bulk.json (mtimes within this run's
         Step 7 window) all contain entries: [] — consistent with zero
         replacements this run, so not evidence for or against the
         2026-07-08 reporting caveat.
         Test log: logs/DRT-001-19.log.
```
...
|------|--------|--------|------|-------|
| 2026-09-13 | 0044c55 per #1001; no retained harness confirmation | ABORTED (env) | not recorded | Unverified operator report — earlier same-day attempt, before the PASS above. The only record is the #1001 note of 2026-09-13T15:51Z (the harness log was lost with a removed worktree; neither rebuild.log nor any retained log covers this attempt). Per that note: workstation ENOSPC in the `build-all-images.sh` image-build phase before any destructive step, with zero cluster impact reported afterward (20/20 VMs running, prod DNS up, vault-prod lookup-self 200, InfluxDB org present, gitlab/cicd on the tailnet). The note escalated #1001 to P1 (workstation auto-GC gap) and references one further same-day ENOSPC that is not otherwise recorded. Remediation reported before the PASS run: `/nix` moved to a dedicated SSD RAID volume, repo + `build/` to /Volumes/Media (unverified operator report). |
| 2026-09-12 | d6a0f7c | FAIL | ~23m (rebuild-cluster.sh script interval 20:05:01Z→20:28:14Z = 23m 13s; harness total not recorded) | Step 16 final validation failed after 3 attempts (validate 88/4: gitlab + cicd off the tailnet) — same failure class as 2026-08-31. Independent codex RCA (docs/reports/2026-09-12-rca-tailscale-drt001-vault-dns-ordering.md) established the cause and corrected the first-line read: NO VM recreation occurred this run (default plan had zero replacements; the two applies stopped and restarted the 19 existing VMs; preboot restore manifest empty), so the fingerprint proved data PRESERVATION, not PBS restoration. `-parallelism=1` (#1011) serialized the Phase-2 starts: the prod DNS VMs were launched 136-149s after the consumer VMs (launch spread 15s on 2026-08-31 → 241s here); gitlab/cicd's tailscale units abandoned their fixed 60s Vault-token waits at 20:17:27Z/20:17:41Z, ~15s and ~1s before the first DNS VM start (20:17:42Z, DNS readiness later still), and began fallback enrollment with the expired CIDATA key. Confirmed aggravator, not origin. Also establishes that #1011 was NOT storm-proofed by this run (no creates). Fix shape (iii), fail-closed enrollment in tailscale.nix: !620 → 0044c55. Harness log not retained (worktree removed); rebuild.log carries the run. |
| 2026-08-31 | 964b625 | FAIL | 53m 51s (rebuild 49m 28s, warm store) | Rerun of the same-night ENOSPC abort after cleanup. Recorded at the time as "recovery mechanics passed end-to-end: full destroy/recreate with preboot restores"; CORRECTED 2026-09-13 (!622): the run's own log shows the control plane was NOT recreated — rebuild.log `gitlab/cicd/pbs: no pending changes — skipping` (2026-08-30 21:13:37–44 local) — and gitlab's/cicd's zvols still date from May 14 / Jul 26 2026, so the gitlab/cicd tailnet failure was reboot-induced state loss (the tmpfs root discards /var/lib/tailscale), not a recreation. How many data-plane VMs were replaced in this run is not recoverable from the retained logs (unverified). Fingerprint 5/5 AND the #1000 content-grade supplement verified byte-identical precious state (six GitLab repo head SHAs, Influx org/buckets/21 measurements, Vault mount set, Roon DB above threshold) — final application state, not restoration provenance. All 4 validate failures were ONE real caught regression: tailscale onboarding did not survive the restart (state on ephemeral disk + expired CIDATA auth key → gitlab/cicd off the tailnet) — filed as #1002, the battery's first live catch of the dummy0/#697 time-rot class. Sprint-049 MR-3 invalidation: the 2026-09-13 run passed the harness without recreating anything, so MR-3's qualifying rerun remains owed (#1020). (Previously the Last Run entry.) |
| 2026-08-31 | 964b625 | ABORTED (env) | not recorded (01:57Z) | Workstation ENOSPC during the image build, before any destructive phase; zero cluster impact. Filed as #1001. Rerun the same night is the FAIL row above. |
...
**What it validates:** The rebuild path from a fresh-clone (cold) checkout:
build → preflight → phased apply → converge → validate, exercising the
new-operator checkout path. VM replacements are drift-driven; the registry
block records replacements observed in that clone's retained bulk plan.
Replacement is not a pass criterion. Forced recreation coverage belongs
to DRT-010.
...
```
DRT-004 Vault HA Failover
Date:    2026-08-31T00:17:52Z
Commit:  6bb6297
Result:  PASS
Time:    3m 55s (recovery: 16s)
Notes:   DR battery Phase 1 (post pipeline-2172 incident closure). First
         failover proof of the fully re-rotated vault-prod on its
         promotion-recreated VM: QEMU killed, HA restart, auto-unseal
         with the NEW (2026-08-30T22:45Z) unseal key from vdb, recovery
         16s vs 30s RTO (baseline ~15s). Rotated SOPS root token valid
         post-failover; validate.sh vault checks green.
```
...
```
DRT-006 DNS Failover
Date:    2026-09-21T01:40:06Z
Commit:  eb8452e8
Result:  PASS
Time:    4m 17s
Notes:   R-I-2 rerun after !627 (#1025) touched certbot-cluster.sh, an
         invalidation input for this test. Run from the operator-
         attended #1025 dev-acceptance window, ~2h after dev pipeline
         #2457 recreated the dev fleet (the prod DNS pair was not
         recreated by that pipeline). Phase A dns1-prod (401) HA-stopped:
         dns2 resolved vault.prod. Phase B dns2-prod (402) HA-stopped:
         dns1 resolved. validate.sh green with both up.
         COVERAGE-LIMITED: both `certbot renew` invocations exited 0
         because no renewal was due ("No renewals were attempted",
         DRT-006.log) — resolution through each surviving server is
         proven; an ACME renewal/challenge through it is not.
         Log: DRT-006.log in ~/mycofu-archives/2026-09-20-mr627-issue-1025/build/live/.
```
...
| Date | Commit | Result | Time | Notes |
|------|--------|--------|------|-------|
| 2026-08-31T00:13:12Z | 6bb6297 | PASS | 4m 34s | DR battery Phase 1, first run on the promotion-recreated prod DNS pair (both VMs recreated 2026-08-30 by pipeline 2172's deploy). dns2-prod HA-stopped; resolution held via dns1-prod; certbot renew succeeded with dns2 down; dns2 answering 5s after HA start; validate.sh green with both up. |
...
```
DRT-007 Backup Spot-Check
Date:    2026-09-21T01:44:23Z
Commit:  eb8452e8
Result:  PASS
Time:    0m 58s
Notes:   R-I-2 rerun after !627 (#1025) touched certbot-cluster.sh, an
         invalidation input for this test. GitLab (VMID 150) backup
         restored to temp VMID 9999 on pve03: PostgreSQL data dir 1003M,
         PG_VERSION present, superuser connect OK, 8 real projects via
         psql. API layer informational-only as documented. Harness
         cleanup ran (qm stop + destroy --purge in the log); a separate
         architect read of `qm list` on pve03 right after the run showed
         no VMID >= 9000 (pve03 only; not in the log). Log: DRT-007.log
         in ~/mycofu-archives/2026-09-20-mr627-issue-1025/build/live/.
```
...
| Date | Commit | Result | Time | Notes |
|------|--------|--------|------|-------|
| 2026-08-31T00:11:55Z | 6bb6297 | PASS | 1m 3s | DR battery Phase 1. GitLab backup restored to temp VMID 9999; integrity verified at filesystem, PostgreSQL (superuser connect), and application layers (6 real projects). API layer informational-only as documented (peer-auth limitation on the temp VM). Temp VM destroyed clean. |
...
```
DRT-008 Reset Contract Ratchet
Date:    2026-09-21T02:16:08Z
Commit:  912ba096
Result:  PASS
Time:    0m 1s
Notes:   Dev-only DRT rerun window after !627/#1025 (operator ruling
         2026-09-21, architect session: rerun the DRTs whose recreation
         touches only dev; prod-recreating DRTs wait for promotion). All contract sections pass: incomplete-
         pin naming, nonexistent-volid rejection, no destructive remote
         commands in dry-run. Log: DRT-008.log in
         ~/mycofu-archives/2026-09-21-drt-dev-rerun/build/drt/.
```
...
| Date | Commit | Result | Time | Notes |
|------|--------|--------|------|-------|
| 2026-08-31T00:13:07Z | 6bb6297 | PASS | 0m 1s | DR battery Phase 1. All contract sections pass, including incomplete-pin naming, nonexistent-volid rejection, and the no-destructive-path assertion in dry-run. |
...
## DRT-009: Key Rotation

**Status:** PENDING first invocation at G3. Born cheap-routine at
`--depth rekey`; deeper depths are attended destructive-judgment operations.

**What it validates:** That `site/rotation-manifest.yaml` can route rotation
by class and depth, that M1 rekey is plan-zero after recipient rotation, that
M4 legs stop before mutation when unattended, and that prove-negative evidence
is captured under `build/drt/DRT-009/<UTC>/` without leaking values.

**Invalidated by changes to:**
- `framework/dr-tests/tests/DRT-009-key-rotation.sh`
- `framework/dr-tests/run-dr-test.sh`
- `site/rotation-manifest.yaml`
- `.sops.yaml`
- `framework/scripts/check-rotation-manifest.sh`
- `framework/scripts/rotate-sops-recipient.sh`
- `framework/scripts/rotate-vault-credentials.sh`
- `framework/scripts/rotate-gitlab-root-password.sh`
- `framework/scripts/register-runner.sh` delivery step

**Pass criteria:**
- `--depth` is explicit and one of `rekey`, `resecret-stateless`, or
  `resecret-all`
- `--only` is accepted only at `--depth resecret-all` and narrows M4 dispatch
  only
- Inventory and fresh backup pin preconditions pass
- M1 completes, holder probes pass, and `tofu-wrapper.sh plan
  -detailed-exitcode` exits 0
- M4 attended gates block under `DRT_HEADLESS` before mutation
- Prove-negative evidence precedes the final registry paste block
- Final output names depth, `--only` scope, classes touched, escrow path,
  pre/post pin volids, and evidence sink path

**Estimated time:** depth-dependent.
...
Time:    depth-dependent
Notes:   First invocation occurs at G3. The paste block covers both S6
         two-acts-one-window acts: DRT-009 --depth <ruled> and
         rotate-gitlab-root-password.sh --i-mean-it from the same attended
         terminal, sharing build/drt/DRT-009/<UTC>/. The runbook exports
         DRT009_UTC_STAMP and ROTATE_UTC_STAMP to a common value and runs
         a post-process step (see OPERATIONS.md G3 subsection) that
         concatenates act 1's drt009-summary.txt with act 2's evidence
         pointers into one g3-registry-block.txt. That file is the paste
         block.
```
...
| Date | Commit | Result | Time | Notes |
|------|--------|--------|------|-------|
| pending G3 | pending | PENDING-FIRST-RUN | depth-dependent | Row created by Sprint 049 MR-5. |

---

## DRT-010: Full-Fleet Recreate

**Status:** `--scope dev` PASS 2026-09-21 (first invocation, `912ba096`); `--scope all` not yet run — it is the prod-side #1025 acceptance and the #1002 cold-recreation / #1011 R-I-2 vehicle, scheduled after the next promotion from a prod checkout.

**What it validates:** Forced VM recreation with precious-state continuity,
even at a converged commit: new vda and CIDATA, dev vdb park/adopt,
prod/shared PBS-pin restore before first boot, and unconditional gitlab/cicd
recreation through the control-plane atomic ladder — all anchored to a
run-scoped restore pin (`backup-now.sh --pin-out build/restore-pin-drt010.json`
handed to `rebuild-cluster.sh --restore-pin-file`), never to an unpinned
"latest" backup. `--scope dev` selects dev environment modules; `--scope all`
includes the control plane and shared framework VMs. PBS is excluded from
forced recreation and retains its normal rebuild path. DRT-003's in-place PBS
restore exercises a different mechanism.

**Invalidated by changes to:**
- `framework/dr-tests/tests/DRT-010-full-fleet-recreate.sh`
- `framework/dr-tests/lib/common.sh`
- `framework/dr-tests/run-dr-test.sh`
- `framework/scripts/rebuild-cluster.sh`
- `framework/scripts/git-deploy-context.sh`
- `framework/scripts/vm-scope.sh`
- `framework/images.yaml` / `site/images.yaml` VM taxonomy fields
- `framework/scripts/list-backup-backed-vmids.sh`
- `framework/scripts/restore-before-start.sh`
- `framework/scripts/restore-from-pbs.sh`
- `framework/scripts/aggregate-preboot-status.sh`
- `framework/scripts/backup-now.sh`
- `framework/scripts/vdb-park-lib.sh`
- `framework/scripts/tofu-wrapper.sh`
- `framework/tofu/modules/proxmox-vm*/vm.tf`
- `framework/nix/modules/tailscale.nix`
- `framework/nix/modules/vault.nix`
- `flake.nix`
- `framework/nix/modules/base.nix`

**Pass criteria:**
- Explicit `--scope dev|all`; `backup-now.sh --pin-out
  build/restore-pin-drt010.json` succeeds and the pin covers every
  backup-backed in-scope VMID (checked before any destruction); the rebuild
  runs with `--restore-pin-file` on that pin and succeeds; manifest
  `recreate: true` with the requested scope. The paste block records
  `Coverage: restore pin: build/restore-pin-drt010.json`.
- Bulk plan assertion succeeds: every selected VM is created or replaced,
  recorded as `expected == replacing` in `recreate-plan-assert-bulk.json`.
- `validate.sh` passes and the state fingerprint is verifiable and preserved.
- Per-VM non-cloudinit zvol creation evidence on the VM's current hosting
  node (resolved from `qm list` at each read; replica-node rows are
  informational) is at or after the rebuild start, including gitlab and cicd
  for `all`. The registry paste block must report `Coverage: recreation
  observed N/N`; an unchanged VM, an unresolvable hosting node, or an
  unavailable node fails the run.
- `dev`: every backup-backed VM retains at least one baseline disk GUID on
  its hosting node (a replica still carrying the old GUID is not evidence).
- `all`: current preboot status artifacts cover every backup-backed VM with
  `status` in `{spared, restored}`; missing/empty evidence fails (#518).
- Elapsed time warns above 45 min (`dev`) / 120 min (`all`).

**Estimated time:** ~30 min (`--scope dev`; the first run took 97 min, of which Step 5 image build was 62 min per rebuild.log — the estimate assumes images are already built) / ~90 min (`--scope all`).

**Last Run:**
```
DRT-010 Full-Fleet Recreate
Date:    2026-09-21T02:16:52Z
Commit:  912ba096
Result:  PASS
Time:    97m 19s
Coverage: scope: dev (PBS excluded)
Coverage: restore pin: build/restore-pin-drt010.json
Coverage: recreation observed 8/8
Warnings:
  - Recreation exceeded 45 min for --scope dev
Notes:   FIRST INVOCATION, dev scope, from a worktree at the dev tip.
         Window: operator ruling 2026-09-21 (architect session) — rerun
         the DRTs whose RECREATION touches only dev; prod-recreating DRTs
         wait for promotion. Note the boundary is recreation, not
         activity: the harness's `backup-now.sh --pin-out` is unfiltered,
         so the run pin (restore-pin-drt010.json) and rebuild.log:95-105
         show PBS backups of prod/shared precious VMs 403, 150, 601, 603
         as well as dev 303, 501, 503 — non-destructive, but a dev
         invocation does back up prod. Preflight: validate.sh green,
         clean tree, PBS reachable; pin covered every backup-backed
         in-scope VMID. Rebuild 91m28s: Step 0 953s, Step 5 image build
         3709s, Step 7 547s (rebuild.log). Step 5 dominated; that every
         image was rebuilt from a cold workstation cache after the !627
         shared-source rehash is the architect's inference, not logged.
         recreate-plan-assert-bulk.json: expected 8 == replacing 8
         (acme_dev, dns1/dns2 dev, grafana_dev, influxdb_dev, roon_dev,
         testapp_dev, vault_dev). hil_boot was not selected: its label is
         unsuffixed (site/config.yaml) and the DRT lib classifies
         unsuffixed labels as shared (framework/dr-tests/lib/common.sh
         ~:370-374). vdb park/adopt: 303/501/503 adopted, preboot status
         `spared` with fallback pins (vdb-park-status-bulk.json,
         preboot-restore-status-bulk.json). Harness ground truth
         (DRT-010-dev.log): for all 8 VMs a non-cloudinit disk created on
         the hosting node since rebuild start; baseline vdb GUID
         preserved on the hosting node for 303 (75877312634578624), 501
         (12909741953900063688), 503 (16832345101317218025). Separately,
         an architect `zfs list -Hp -o name,creation,guid` read on all
         three nodes (session observation, not in the archive) put the
         boot zvol creation times at 03:43:12-03:44:17Z. App state:
         GitLab 8 projects, Vault initialized/14 mounts, InfluxDB org
         homelab, Roon 1797M — all equal to pre-test. validate.sh green
         after recreation. The 45-min WARN is elapsed rebuild time; the
         estimate now names Step 5. Evidence: ~/mycofu-archives/
         2026-09-21-drt-dev-rerun/build/ (drt/DRT-010-dev.log,
         rebuild.log, recreate-plan-assert-bulk.json,
         vdb-park-status-bulk.json, preboot-restore-status-bulk.json,
         restore-pin-drt010.json).
```
...
|-------------|-----------------|
| `rebuild-cluster.sh` | DRT-001, DRT-002, DRT-010 |
| `new-site.sh` | DRT-002 |
| `reset-cluster.sh` | DRT-008 |
| `framework/dr-tests/run-dr-test.sh` | DRT-009, DRT-010; scan DRT-001..DRT-008 invocation compatibility |
| `framework/dr-tests/tests/DRT-010-full-fleet-recreate.sh` | DRT-010 |
| `framework/dr-tests/lib/common.sh` | DRT-010; scan DRT-001..DRT-009 shared-library compatibility |
| `framework/scripts/git-deploy-context.sh` | DRT-010 |
| `framework/scripts/aggregate-preboot-status.sh` | DRT-010 |
| `framework/scripts/tofu-wrapper.sh` | DRT-010 |
| `framework/tofu/modules/proxmox-vm*/vm.tf` | DRT-010 |
| `framework/nix/modules/tailscale.nix` | DRT-010 |
| `framework/dr-tests/tests/DRT-009-key-rotation.sh` | DRT-009 |
| `framework/scripts/rotate-*.sh` | DRT-009 |
| `framework/scripts/check-rotation-manifest.sh` | DRT-009 |
| `site/rotation-manifest.yaml` | DRT-009 |
| `.sops.yaml` | DRT-009 |
| `framework/scripts/register-runner.sh` delivery step | DRT-001, DRT-002, DRT-009 |
| `backup-now.sh` | DRT-001, DRT-003, DRT-007, DRT-010 |
| `framework/scripts/configure-backups.sh` | DRT-007 |
| `framework/scripts/list-backup-backed-vmids.sh` | DRT-007, DRT-010 |
| `framework/scripts/converge-lib.sh` | DRT-001, DRT-002, DRT-003 |
| `framework/scripts/converge-vm.sh` | DRT-001, DRT-002, DRT-003 |
| `framework/scripts/vm-scope.sh` | DRT-001, DRT-002, DRT-003, DRT-007, DRT-010 |
| `framework/images.yaml` / `site/images.yaml` VM taxonomy fields | DRT-001, DRT-002, DRT-003, DRT-007, DRT-010 |
| `restore-from-pbs.sh` | DRT-001, DRT-003, DRT-007, DRT-010 |
| `restore-before-start.sh` | DRT-001, DRT-003, DRT-010 |
| `framework/scripts/vdb-park-lib.sh` | DRT-001, DRT-002, DRT-003, DRT-005, DRT-010 |
...
| `flake.nix` (image derivations) | DRT-002, DRT-010 |
...
| `framework/nix/modules/base.nix` | DRT-001, DRT-002, DRT-003, DRT-007, DRT-010 |
| `framework/nix/modules/certbot.nix` | DRT-001, DRT-002, DRT-003, DRT-007 |
| `framework/nix/modules/vault.nix` | DRT-001, DRT-004, DRT-007, DRT-010 |
