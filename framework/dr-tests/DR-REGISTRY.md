...
|----|------|------|-------------|----------|--------|-----------|
| DRT-001 | Warm Rebuild | destructive-judgment | FAIL | 2026-08-31T03:50:47Z | 964b625 — recovery mechanics PASS (fingerprint 5/5 + content supplement); FAIL is the real #1002 tailscale-rot catch; same-night ENOSPC first attempt is #1001; prior clean PASS 2026-07-08 53f37d7 | ~16 min |
...
| DRT-006 | DNS Failover | cheap-routine | PASS | 2026-08-31T00:13:12Z | 6bb6297 | ~5 min |
| DRT-007 | Backup Spot-Check | cheap-routine | PASS | 2026-08-31T00:11:55Z | 6bb6297 | ~1 min |
| DRT-008 | Reset Contract Ratchet | cheap-routine | PASS | 2026-08-31T00:13:07Z | 6bb6297 | ~2 min |
...
```
DRT-001 Warm Rebuild
Date:    2026-08-31T03:50:47Z
Commit:  964b625
Result:  FAIL
Time:    53m 51s (rebuild 49m 28s, warm store)
Failures:
  - rebuild-cluster.sh completes successfully (its final validate: 88/4)
  - validate.sh passes after rebuild (same 4)
Notes:   Rerun of the same-night ENOSPC FAIL (01:57Z, workstation disk,
         zero cluster impact, #1001) after cleanup, on a warm store.
         THE RECOVERY MECHANICS PASSED END-TO-END: full destroy/recreate
         with preboot restores; fingerprint 5/5 AND the #1000
         content-grade supplement verified byte-identical precious state
         (six GitLab repo head SHAs, Influx org/buckets/21 measurements,
         Vault mount set, Roon DB above threshold). All 4 validate
         failures are ONE real caught regression: tailscale onboarding
         does not survive recreation (state on ephemeral disk + expired
         CIDATA auth key -> gitlab/cicd off the tailnet) — filed as
         #1002, the battery's first live catch of the dummy0/#697
         time-rot class. Sprint-049 MR-3 invalidation: the rebuild path
         itself is exercised and sound; a clean PASS row still awaits a
         run after #1002's remediation.
```
...
```
DRT-006 DNS Failover
Date:    2026-08-31T00:13:12Z
Commit:  6bb6297
Result:  PASS
Time:    4m 34s
Notes:   DR battery Phase 1, first run on the promotion-recreated prod
         DNS pair (both VMs recreated 2026-08-30 by pipeline 2172's
         deploy). dns2-prod HA-stopped; resolution held via dns1-prod;
         certbot renew succeeded with dns2 down; dns2 answering 5s
         after HA start; validate.sh green with both up.
```
...
```
DRT-007 Backup Spot-Check
Date:    2026-08-31T00:11:55Z
Commit:  6bb6297
Result:  PASS
Time:    1m 3s
Notes:   DR battery Phase 1. GitLab backup restored to temp VMID 9999;
         integrity verified at filesystem, PostgreSQL (superuser
         connect), and application layers (6 real projects). API layer
         informational-only as documented (peer-auth limitation on the
         temp VM). Temp VM destroyed clean.
```
...
```
DRT-008 Reset Contract Ratchet
Date:    2026-08-31T00:13:07Z
Commit:  6bb6297
Result:  PASS
Time:    0m 1s
Notes:   DR battery Phase 1. All contract sections pass, including
         incomplete-pin naming, nonexistent-volid rejection, and the
         no-destructive-path assertion in dry-run.
```
...
| `framework/dr-tests/run-dr-test.sh` | DRT-009; scan DRT-001..DRT-008 invocation compatibility |
