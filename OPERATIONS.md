...
- **Phase A (additive, #358):** apply safe-to-apply tofu resources
  (currently CIDATA snippet uploads) across **all** modules including
  Tier 2 (gitlab, cicd, pbs, workstation-dev). Heals "state says X
  should exist, reality doesn't."
- **Phase B (orphan cleanup, #359):** delete files on nodes that no
  tofu state tracks. Heals "reality says X exists, state doesn't."
...
```bash
framework/scripts/converge-cluster.sh             # both phases: plan + apply, then orphan delete
framework/scripts/converge-cluster.sh --dry-run   # both phases: report only, no writes
framework/scripts/converge-cluster.sh --allow-empty-expected   # override the empty-expected safety gate
```
...
2. **Existing cluster with VMs running**, for each node `<N>` in turn:
   ...
   and runs the HA-aware dance: maintenance-enable → drain wait →
   reboot → quorum poll → maintenance-disable. On any non-zero
   exit (die from any step, SIGTERM interrupt), the inner script's
   EXIT trap auto-disables HA maintenance so the cluster is never
   left in a stuck-migration state. See `#338`.
