...
cicd's ~318 GB zvol and all 24h-cadence jobs cluster-wide. For this
...
**When invoked through `rebuild-cluster.sh` Step 1.5:** the rebuild
...
than `stopped`), Step 1.5 reboots each node sequentially, waits for
SSH to return, re-verifies, and proceeds to Step 2 -- no operator
...
stopped VM, or if Step 1.5 cannot read VM state on any node (ssh
failure, qm failure), the script declines to auto-reboot and prints
...
1. **Pre-cluster** (fresh bringup, no VMs yet): just reboot each node in
   ...
   is normally handled automatically by `rebuild-cluster.sh` Step 1.5.
...
accepting the first-pass exposure), comment out the entire Step 1.5
block in `framework/scripts/rebuild-cluster.sh`. Editing
`step_in_scope` to remove `kernel` from its infrastructure-step list
is the *wrong* fix: it would cause Step 1.5 to run during every
