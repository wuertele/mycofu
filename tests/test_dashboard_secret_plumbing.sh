...
if grep -Fq 'variable "vault_approle_influxdb_dev_role_id"' "${ROOT_TF}" && \
   ...
   grep -Eq 'vault_approle_role_id[[:space:]]*=[[:space:]]*var\.vault_approle_role_id' "${INFLUXDB_TF}" && \
   grep -Eq 'vault_approle_secret_id[[:space:]]*=[[:space:]]*var\.vault_approle_secret_id' "${INFLUXDB_TF}"; then
