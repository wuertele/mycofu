...
PIPELINE_ONLY_SCRIPTS=(
  ...
  pre-deploy-vm-completeness-gate.sh
  check-plan-images-present.sh
  converge-incomplete-vm.sh
  list-backup-backed-vmids.sh
  tofu-bin.sh
  vm-scope.sh
  vdb-park-lib.sh
  check_bulk_artifacts_populated.sh
)
...
pipeline_only_reason_for() {
  case "$1" in
    ...
    # In-path generation of the Gatus monitoring config (site/gatus/config.yaml)
    # before the apply plan (#690). Not a post-deploy convergence step: the
    # workstation DR path (rebuild-cluster.sh) and CI build:merge already run
    # the same generator in-path, so parity is inherent and needs no mapping.
    generate-gatus-config.sh) printf '%s\n' "in-path gatus config generation before apply (#690)" ;;
    ...
    # CI pre-gate before backup-now.sh. Workstation rebuilds use the preboot
    # manifest/restore flow directly rather than this pipeline gate wrapper.
    pre-deploy-vm-completeness-gate.sh) printf '%s\n' "pipeline-only -- Move 3 CI pre-gate runs before backup-now.sh" ;;

    # Deploy/rebuild precondition invoked before destructive apply; not an
    # end-state convergence hook and therefore has no converge_run_all step.
    check-plan-images-present.sh) printf '%s\n' "deploy precondition -- image-present guard before destructive apply" ;;

    # Shared preboot recovery routine for incomplete precious VMs; not an
    # end-state convergence hook and therefore has no converge_run_all step.
    converge-incomplete-vm.sh) printf '%s\n' "preboot recovery -- Move 3a convergence routine for incomplete precious VMs" ;;

    # Helper used by preboot gates to identify backup-backed VMs; not an
    # end-state convergence hook and therefore has no converge_run_all step.
    list-backup-backed-vmids.sh) printf '%s\n' "preboot helper -- enumerates backup-backed VMs for restore gates" ;;

    # Shared OpenTofu binary resolver sourced by deploy wrappers; it does not
    # converge cluster state and has no standalone converge_run_all step.
    tofu-bin.sh) printf '%s\n' "OpenTofu binary resolver helper" ;;

    # Manifest reader: derives VM-class scope/control_plane/category from
    # framework/images.yaml + site/images.yaml + site/applications.yaml.
    # Consumed by safe-apply.sh, rebuild-cluster.sh, and other classifier
    # sites; not a converge step (Sprint 039).
    vm-scope.sh) printf '%s\n' "pipeline VM-class taxonomy reader" ;;

    # Helper sourced by safe-apply.sh/rebuild-cluster.sh to park/adopt vdb
    # around a destructive recreate. It is deploy-window choreography, not an
    # end-state converge_run_all step.
    vdb-park-lib.sh) printf '%s\n' "deploy preboot helper -- vdb park/adopt bridge" ;;

    # Post-deploy read-only artifact gate (#518): verifies the aggregated
    # preboot-restore status covers every manifest VMID and fails on the H1
    # clobber symptom. It inspects build artifacts and converges no state.
    check_bulk_artifacts_populated.sh) printf '%s\n' "post-deploy bulk-artifact population gate -- read-only, converges no state" ;;
    *) return 1 ;;
  esac
}
