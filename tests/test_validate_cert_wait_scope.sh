...
CERTBOT_SCOPE_DIR="${TMP_DIR}/certbot-scope"
mkdir -p "${CERTBOT_SCOPE_DIR}/site"
cat > "${CERTBOT_SCOPE_DIR}/site/config.yaml" <<'EOF'
domain: example.test
vms:
  dns1_prod:
    ip: 10.0.0.50
  ...
  testapp_dev:
    ...
    vmid: 500
EOF
cat > "${CERTBOT_SCOPE_DIR}/site/applications.yaml" <<'EOF'
applications: {}
EOF

test_start "6" "certbot validate inventory filter excludes prod/shared VMs from dev scope"
OUT="$(CERTBOT_CLUSTER_CONFIG="${CERTBOT_SCOPE_DIR}/site/config.yaml" \
  CERTBOT_CLUSTER_APPS_CONFIG="${CERTBOT_SCOPE_DIR}/site/applications.yaml" \
  bash -c 'source framework/scripts/certbot-cluster.sh; certbot_cluster_cert_storage_records "$CERTBOT_CLUSTER_CONFIG" "$CERTBOT_CLUSTER_APPS_CONFIG" dev')"
if grep -Fq 'vault_dev' <<< "$OUT" &&
   ! grep -Fq 'vault_prod' <<< "$OUT" &&
   ! grep -Fq 'gitlab' <<< "$OUT"; then
  test_pass "dev certbot inventory stays dev-scoped"
else
  test_fail "dev certbot inventory leaked prod/shared records"
  printf '    output:\n%s\n' "$OUT" >&2
fi

test_start "7" "certbot validate inventory filter includes prod/shared VMs in prod scope"
OUT="$(CERTBOT_CLUSTER_CONFIG="${CERTBOT_SCOPE_DIR}/site/config.yaml" \
  CERTBOT_CLUSTER_APPS_CONFIG="${CERTBOT_SCOPE_DIR}/site/applications.yaml" \
  bash -c 'source framework/scripts/certbot-cluster.sh; certbot_cluster_cert_storage_records "$CERTBOT_CLUSTER_CONFIG" "$CERTBOT_CLUSTER_APPS_CONFIG" prod')"
if grep -Fq 'vault_prod' <<< "$OUT" &&
   grep -Fq 'gitlab' <<< "$OUT" &&
   ! grep -Fq 'vault_dev' <<< "$OUT"; then
  test_pass "prod certbot inventory stays prod-scoped"
else
  test_fail "prod certbot inventory leaked dev records or missed shared cert VM"
  printf '    output:\n%s\n' "$OUT" >&2
fi
