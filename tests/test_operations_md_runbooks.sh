...
test_start "${idx}" "OPERATIONS.md nightly-benchmark runbook covers Sprint 035 scoped-down flow"
if grep -q 'benchmarks.scheduled.enabled' "$OPS_MD" && \
   grep -q 'BENCH_SCHEDULE_KEY' "$OPS_MD" && \
   grep -q 'in-script gate' "$OPS_MD" && \
   grep -q 'publish:github' "$OPS_MD" && \
   grep -q 'allow_failure: true' "$OPS_MD" && \
   grep -q 'does not page' "$OPS_MD" && \
   grep -q 'Gatus recent-run sentinel' "$OPS_MD" && \
   grep -q 'Grafana #268' "$OPS_MD"; then
  test_pass "nightly-benchmark runbook documents the opt-in field, the GitLab UI setup, the in-script gate matching publish:github, non-paging failures, and follow-ups"
else
  test_fail "nightly-benchmark runbook is missing scoped-down Sprint 035 guidance"
fi
