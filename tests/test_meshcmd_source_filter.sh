...
test_start "s034.0.6" "flake.nix has explicit host-tool source exclusions"
if grep -q 'isHostToolPackage' flake.nix && grep -q 'framework/nix/pkgs' flake.nix; then
  test_pass "source filters name framework/nix/pkgs as excluded host tooling"
else
  test_fail "flake.nix does not visibly exclude framework/nix/pkgs from image sources"
fi
