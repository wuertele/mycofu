...
test_start "5" "cicd AppRole is attached to the cicd-tailscale-policy"
if grep -Fq 'cicd: default-policy,github-publish-policy,cicd-tailscale-policy' <<< "${PROD_OUTPUT}"; then
  test_pass "cicd-tailscale-policy is attached to cicd's AppRole role policies"
else
  test_fail "cicd-tailscale-policy is missing from cicd's AppRole role policies"
fi
