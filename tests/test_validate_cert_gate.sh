...
make_fixture_repo() {
  ...
  # validate.sh sources vdb-park-lib.sh (Sprint 044 ratchet single-source)
  cp "${REPO_ROOT}/framework/scripts/vdb-park-lib.sh" "${fixture_root}/framework/scripts/vdb-park-lib.sh"
  cp "${REPO_ROOT}/framework/scripts/certbot-cluster.sh" "${fixture_root}/framework/scripts/certbot-cluster.sh"
  cp "${REPO_ROOT}/framework/scripts/certbot-renewability.sh" "${fixture_root}/framework/scripts/certbot-renewability.sh"
  cp "${REPO_ROOT}/framework/scripts/certbot-persisted-state.sh" "${fixture_root}/framework/scripts/certbot-persisted-state.sh"
  cp "${REPO_ROOT}/framework/scripts/generate-gatus-config.sh" "${fixture_root}/framework/scripts/generate-gatus-config.sh"
  cp "${REPO_ROOT}/framework/scripts/github-publish-lib.sh" "${fixture_root}/framework/scripts/github-publish-lib.sh"
  chmod +x \
    "${fixture_root}/framework/scripts/validate.sh" \
    "${fixture_root}/framework/scripts/certbot-cluster.sh" \
    "${fixture_root}/framework/scripts/certbot-renewability.sh" \
    "${fixture_root}/framework/scripts/certbot-persisted-state.sh" \
    "${fixture_root}/framework/scripts/generate-gatus-config.sh" \
    "${fixture_root}/framework/scripts/github-publish-lib.sh"

  cat > "${fixture_root}/framework/scripts/cleanup-orphan-cidata.sh" <<'EOF'
#!/usr/bin/env bash
printf '%s\n' 'Summary (dry-run): 0 orphan(s) found, 0 destroyed'
EOF

  chmod +x "${fixture_root}/framework/scripts/cleanup-orphan-cidata.sh"
...
case "${remote_cmd}" in
  ...
  "ha-manager status")
    printf '%s\n' 'node pve01 (online)'
    printf '%s\n' 'service vm:301 (pve01, started)'
    printf '%s\n' 'service vm:302 (pve01, started)'
    exit 0
    ;;
  "pvesh get /cluster/options --output-format json")
    ...
    exit 0
    ;;
  "pvesh get /cluster/ha/rules --output-format json")
    printf '%s\n' '[{"type":"resource-affinity","affinity":"negative","resources":"vm:301,vm:302"}]'
    exit 0
    ;;
  "pvesh get /cluster/resources --output-format json")
    printf '%s\n' '[{"type":"node","node":"pve01","status":"online"},{"type":"qemu","vmid":301,"node":"pve01"},{"type":"qemu","vmid":302,"node":"pve01"}]'
    exit 0
    ;;
  ...
  *"/etc/gitlab-runner/config.toml"*)
    # Runner functional-health probe (#35/#68/#139) reads the auth token
    # from config.toml; return a healthy fixture token.
    printf '%s\n' 'glrt-fixturetoken'
    exit 0
    ;;
  "cat /var/lib/vault/root-token 2>/dev/null")
    # vdb root-token probe (#983) reads the file post-deploy.sh consumes.
    # Return the same value the sops shim serves so the healthy fixture also
    # models a coupled vdb/SOPS pair (fingerprint match=yes). The generic
    # `*) exit 0` arm below would return an EMPTY token, which the probe
    # correctly treats as an absent credential and FAILs closed on.
    printf '%s\n' 'hvs.fixture'
    exit 0
    ;;
  n=0\;\ for\ f\ in\ /etc/pve/nodes/*)
    printf '%s\n' '__SCAN_FILES__ 1'
    printf '%s\n' '__SCAN_DONE__'
    exit 0
    ;;
...
# Probe-bundle endpoints (#34/#67 Vault lookup-self, #35/#68/#139 runner API).
# Handled before the generic -w handler because lookup-self needs a real body.
case "${url}" in
  https://10.0.0.13:8200/v1/auth/token/lookup-self)
    # -w $'\n%{http_code}' => body then code on the last line.
    printf '%s\n%s' '{"data":{"id":"hvs.fixture"}}' '200'
    exit 0
    ;;
  https://10.0.0.15/oauth/token)
    printf '%s\n' '{"access_token":"fixture-oauth"}'
    exit 0
    ;;
  https://10.0.0.15/api/v4/runners/verify)
    # Probe uses -w $'\n%{http_code}': emit body then code on the last line.
    printf '%s\n%s' '{"id":2}' '200'
    exit 0
    ;;
  https://10.0.0.15/api/v4/runners/2)
    printf '%s\n' '{"id":2,"status":"online","paused":false,"active":true}'
    exit 0
    ;;
esac
...
EOF
  ...
  # sops shim: the probe bundle decrypts the Vault root token and GitLab root
  # password from SOPS (#34/#67, #35/#68/#139). Return healthy fixture values.
  cat > "${shim_dir}/sops" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
for a in "$@"; do
  case "$a" in
    '["vault_prod_root_token"]') printf '%s\n' 'hvs.fixture'; exit 0 ;;
    '["gitlab_root_password"]')  printf '%s\n' 'fixture-pw'; exit 0 ;;
  esac
done
exit 1
EOF
