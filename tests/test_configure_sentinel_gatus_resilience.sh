...
# ---------------------------------------------------------------------------
# Issue #698 — `.domain` must be guarded against yq's null-with-exit-0.
#
# mikefarah `yq -r` prints the string "null" and exits 0 for a missing key.
# `.domain` feeds DNS_DOMAIN ("prod.${DOMAIN}") and EMAIL_FROM
# ("gatus@${DOMAIN}"), both of which are rendered into the Gatus config this
# script DEPLOYS to the NAS. Unlike the #630 here-doc class, a null here
# reaches a live monitoring artifact: the sentinel would watch
# "vault.prod.null" — a name that never resolves — and alert from an address
# nobody receives, while prod DNS went unwatched.
#
# Per .claude/rules/destruction-safety.md the script must FAIL loudly rather
# than deploy a config it cannot determine to be correct. These cases run the
# REAL script against a fixture repo root (find_repo_root walks up to
# flake.nix), with a PATH-shimmed ssh, so they assert on behavior rather than
# on source text.
# ---------------------------------------------------------------------------

# make_sentinel_fixture <config_body> — build a fake repo root that
# find_repo_root() will resolve to, containing the real script. Echoes the
# root path (runs in a subshell, so it cannot register its own cleanup).
make_sentinel_fixture() {
  local config_body="$1" root
  root="$(mktemp -d "${TMP_DIR}/sentinel-fixture.XXXXXX")"
  mkdir -p "${root}/framework/scripts/lib" "${root}/site"
  : > "${root}/flake.nix"
  cp "${SCRIPT_PATH}" "${root}/framework/scripts/configure-sentinel-gatus.sh"
  chmod +x "${root}/framework/scripts/configure-sentinel-gatus.sh"
  # The script only reads CIDATA_GUARD_LIB_VERSION out of the guard lib
  # before the config reads under test; a stub with that one line is enough
  # and keeps the fixture independent of the real lib's contents.
  printf 'CIDATA_GUARD_LIB_VERSION="test"\n' \
    > "${root}/framework/scripts/lib/cidata-guard.sh"
  printf '%s' "$config_body" > "${root}/site/config.yaml"
  printf '%s' "$root"
}

# ssh shim that records every invocation and always fails, so if the script
# gets past the .domain guard it stops at the NAS prereq check instead of
# touching docker.
SENTINEL_SHIM_DIR="${TMP_DIR}/sentinel-shims"
mkdir -p "${SENTINEL_SHIM_DIR}"
cat > "${SENTINEL_SHIM_DIR}/ssh" <<'SENTSHIM'
#!/usr/bin/env bash
echo "$*" >> "${SENTINEL_SSH_LOG}"
exit 1
SENTSHIM
chmod +x "${SENTINEL_SHIM_DIR}/ssh"

SENTINEL_NAS_CONFIG=$'nas:\n  ip: 192.0.2.2\n  ssh_user: root\n'

run_sentinel() {
  local root="$1" out="$2" err="$3" rc
  set +e
  PATH="${SENTINEL_SHIM_DIR}:${PATH}" \
    "${root}/framework/scripts/configure-sentinel-gatus.sh" >"$out" 2>"$err"
  rc=$?
  set -e
  printf '%s' "$rc"
}

test_start "29" "#698: missing .domain aborts before any NAS contact, no prod.null deployed"
if ! command -v yq >/dev/null 2>&1; then
  test_skip "yq not available in this environment"
else
  sent_root="$(make_sentinel_fixture "${SENTINEL_NAS_CONFIG}")"
  SENTINEL_SSH_LOG="${sent_root}/ssh.log"; : > "${SENTINEL_SSH_LOG}"
  export SENTINEL_SSH_LOG
  sent_out="${sent_root}/out"; sent_err="${sent_root}/err"
  sent_rc="$(run_sentinel "$sent_root" "$sent_out" "$sent_err")"
  sent_stderr="$(cat "$sent_err")"
  # Assert the WHOLE ssh log, not just the absence of two verbs: the only
  # invocation permitted on the abort path is the EXIT trap's ControlMaster
  # teardown (`ssh -O exit`), which closes a mux socket and is not NAS
  # contact. Enumerating the log this way means the case cannot pass while
  # some other pre-guard ssh ran — an "absence of two known-bad strings"
  # assertion could.
  sent_ssh_lines=$(wc -l < "${SENTINEL_SSH_LOG}" | tr -d ' ')
  sent_non_teardown=$(grep -vc -- '-O exit' "${SENTINEL_SSH_LOG}" || true)
  if [[ "$sent_rc" -ne 0 \
        && "$sent_stderr" == *"domain not set"* \
        && "$sent_stderr" != *"prod.null"* \
        && "$sent_ssh_lines" -eq 1 \
        && "$sent_non_teardown" -eq 0 ]]; then
    test_pass "aborts naming .domain; only ssh is the EXIT-trap mux teardown"
  else
    test_fail "missing .domain not guarded (rc=$sent_rc)"
    { echo "STDERR:"; echo "$sent_stderr"; echo "SSH LOG:"; cat "${SENTINEL_SSH_LOG}"; } \
      | sed 's/^/    /' >&2
  fi
  unset SENTINEL_SSH_LOG
fi

test_start "30" "#698: present .domain passes the guard and reaches the NAS prereq probe"
if ! command -v yq >/dev/null 2>&1; then
  test_skip "yq not available in this environment"
else
  # Positive control: proves the guard is not a blanket abort and that it sits
  # BEFORE the prereq probe (so case 29's silence on that probe is meaningful).
  sent_root="$(make_sentinel_fixture "${SENTINEL_NAS_CONFIG}"$'domain: wuertele.com\n')"
  SENTINEL_SSH_LOG="${sent_root}/ssh.log"; : > "${SENTINEL_SSH_LOG}"
  export SENTINEL_SSH_LOG
  sent_out="${sent_root}/out"; sent_err="${sent_root}/err"
  sent_rc="$(run_sentinel "$sent_root" "$sent_out" "$sent_err")"
  sent_stderr="$(cat "$sent_err")"
  if [[ "$sent_stderr" != *"domain not set"* ]] \
     && grep -q 'command -v timeout' "${SENTINEL_SSH_LOG}"; then
    test_pass "valid .domain clears the guard; execution reaches the prereq probe"
  else
    test_fail "valid .domain did not clear the guard (rc=$sent_rc)"
    { echo "STDERR:"; echo "$sent_stderr"; echo "SSH LOG:"; cat "${SENTINEL_SSH_LOG}"; } \
      | sed 's/^/    /' >&2
  fi
  unset SENTINEL_SSH_LOG
