...
create_sync_fixture() {
  ...
  mkdir -p \
    ...
    "${work_dir}/docs/research" \
    "${work_dir}/docs/forensics" \
  ...
  printf 'research\n' > "${work_dir}/docs/research/private.md"
  printf 'forensics\n' > "${work_dir}/docs/forensics/private.md"
...
if [[ ! -e "${ARCHIVE_DIR}/site" ]] && \
   ...
   [[ ! -e "${ARCHIVE_DIR}/docs/research" ]] && \
   [[ ! -e "${ARCHIVE_DIR}/docs/forensics" ]] && \
...
# Issue #364: framework/step-ca/root-ca.key is a private CA signing key.
# Publishing it lets anyone mint TLS certificates trusted by Mycofu dev
# VMs. The exclusion must remove the key from the filtered archive while
# preserving the public certificate at framework/step-ca/root-ca.crt so
# dev VMs can still consume the trust anchor via CIDATA.
test_start "2b" "publish_filter_archive excludes step-ca root CA private key"
if [[ ! -e "${ARCHIVE_DIR}/framework/step-ca/root-ca.key" ]] && \
   [[ -e "${ARCHIVE_DIR}/framework/step-ca/root-ca.crt" ]]; then
  test_pass "root-ca.key removed, root-ca.crt retained"
else
  if [[ -e "${ARCHIVE_DIR}/framework/step-ca/root-ca.key" ]]; then
    test_fail "root-ca.key was NOT removed from the filtered archive"
  else
    test_fail "root-ca.crt was accidentally removed alongside root-ca.key"
  fi
fi

test_start "2c" "publish_filter_path_allowed rejects step-ca root CA private key"
set +e
publish_filter_path_allowed "framework/step-ca/root-ca.key"
KEY_ALLOWED_EXIT=$?
publish_filter_path_allowed "framework/step-ca/root-ca.crt"
CRT_ALLOWED_EXIT=$?
set -e
if [[ "${KEY_ALLOWED_EXIT}" -ne 0 ]] && [[ "${CRT_ALLOWED_EXIT}" -eq 0 ]]; then
  test_pass "path_allowed rejects the key and still accepts the cert"
else
  test_fail "path_allowed did not reject root-ca.key or wrongly rejected root-ca.crt (key=${KEY_ALLOWED_EXIT} crt=${CRT_ALLOWED_EXIT})"
fi

test_start "2d" "publish_filter_verify fails when step-ca root CA key is present"
make_temp_dir ARCHIVE_LEAK_DIR
publish_filter_archive HEAD "${ARCHIVE_LEAK_DIR}"
mkdir -p "${ARCHIVE_LEAK_DIR}/framework/step-ca"
printf '%s\n' '-----BEGIN PRIVATE KEY-----' 'fixture-key-material' > "${ARCHIVE_LEAK_DIR}/framework/step-ca/root-ca.key"
set +e
VERIFY_LEAK_OUTPUT="$(publish_filter_verify "${ARCHIVE_LEAK_DIR}" 2>&1)"
VERIFY_LEAK_EXIT=$?
set -e
if [[ "${VERIFY_LEAK_EXIT}" -ne 0 ]] && grep -q 'framework/step-ca/root-ca.key' <<< "${VERIFY_LEAK_OUTPUT}"; then
  test_pass "verify caught the reintroduced private key"
else
  test_fail "publish_filter_verify did not catch the planted root-ca.key"
fi
...
if [[ "${SYNC_FORCE_EXIT}" -eq 0 ]] && \
   ...
   ! grep -q '^docs/research/' <<< "${SYNC_TREE}" && \
   ! grep -q '^docs/forensics/' <<< "${SYNC_TREE}" && \
...
if [[ "${PUBLISH_SUCCESS_EXIT}" -eq 0 ]] && \
   ...
   ! grep -q '^docs/research/' "${PUBLISH_CAPTURE_DIR}/tree.txt" && \
   ! grep -q '^docs/forensics/' "${PUBLISH_CAPTURE_DIR}/tree.txt" && \
