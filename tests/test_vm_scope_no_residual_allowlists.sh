...
while IFS= read -r match; do
  ...
  case "$match" in
    ...
    # Match sanctioned entries by file + content only, never line number (#797).
    # Principle 11 count (c): a check that fires on movement is worse than none
    # (2026-07-28 ratchet-economics ruling).
    "${REPO_ROOT}/framework/scripts/converge-lib.sh:"*"for VM_KEY in \$(yq -r \".vms | to_entries[] | select(.key | test(\\\"_\${ENV}\$\\\")) | .key\" \"\$CONFIG\""*) continue ;;
    "${REPO_ROOT}/framework/scripts/converge-lib.sh:"*"for APP_KEY in \$(yq -r '.applications // {} | to_entries[] | select(.value.enabled == true) | .key' \"\$APPS_CONFIG\""*) continue ;;
    "${REPO_ROOT}/framework/scripts/converge-lib.sh:"*"for VM_KEY in gitlab gatus"*) continue ;;
    "${REPO_ROOT}/framework/scripts/converge-lib.sh:"*"for VM_KEY in \$(yq -r \".vms | to_entries[] | select(.key | test(\\\"_\${ENV}\$\\\")) | .key\" \"\$CONFIG\""*) continue ;;
    "${REPO_ROOT}/framework/scripts/converge-lib.sh:"*"for APP_KEY in \$(yq -r '.applications // {} | to_entries[] | select(.value.enabled == true) | .key' \"\$APPS_CONFIG\""*) continue ;;
    "${REPO_ROOT}/framework/scripts/converge-lib.sh:"*"for VM_KEY in gitlab gatus"*) continue ;;
    # GE1: converge-vm step routing, not VM scope taxonomy.
    "${REPO_ROOT}/framework/scripts/converge-vm.sh:"*"converge_vm_target_selected \"module.dns_dev\" || converge_vm_target_selected \"module.dns_prod\""*) continue ;;
    "${REPO_ROOT}/framework/scripts/converge-vm.sh:"*"converge_vm_target_selected \"module.vault_dev\" || converge_vm_target_selected \"module.vault_prod\""*) continue ;;
    "${REPO_ROOT}/framework/scripts/converge-vm.sh:"*"converge_vm_target_selected \"module.gitlab\""*) continue ;;
    "${REPO_ROOT}/framework/scripts/converge-vm.sh:"*"converge_vm_target_selected \"module.gitlab\" || converge_vm_target_selected \"module.cicd\""*) continue ;;
    "${REPO_ROOT}/framework/scripts/converge-vm.sh:"*"converge_vm_target_selected \"module.gitlab\""*) continue ;;
    "${REPO_ROOT}/framework/scripts/converge-vm.sh:"*"vm_keys_output=\"\$(yq -r '.vms | keys | .[]' \"\$CONFIG\" | sort)\""*) continue ;;
    "${REPO_ROOT}/framework/scripts/converge-vm.sh:"*"yq -r '.applications // {} | to_entries[] | select(.value.enabled == true) | .key' \"\$APPS_CONFIG\""*) continue ;;
    "${REPO_ROOT}/framework/scripts/converge-vm.sh:"*"yq -r \".applications.\${app_key}.environments // {} | keys | .[]\" \"\$APPS_CONFIG\""*) continue ;;
    # GE1: build manifest role enumeration; scope/control-plane taxonomy is not reinterpreted here.
    "${REPO_ROOT}/framework/scripts/build-all-images.sh:"*"FRAMEWORK_ROLES=\$(yq -r '.roles | keys | .[]' \"\$FRAMEWORK_MANIFEST\""*) continue ;;
    "${REPO_ROOT}/framework/scripts/build-all-images.sh:"*"SITE_ROLES=\$(yq -r '.roles | keys | .[]' \"\$SITE_MANIFEST\""*) continue ;;
    "${REPO_ROOT}/framework/scripts/build-all-images.sh:"*"APP_ROLES=\$(yq -r '.applications // {} | to_entries[] | select(.value.enabled == true) | .key' \"\$APPS_CONFIG\""*) continue ;;
    "${REPO_ROOT}/framework/scripts/merge-image-versions.sh:"*"< <(yq -r '.roles | keys | .[]' \"\$FRAMEWORK_MANIFEST\""*) continue ;;
    "${REPO_ROOT}/framework/scripts/merge-image-versions.sh:"*"< <(yq -r '.roles | keys | .[]' \"\$SITE_MANIFEST\""*) continue ;;
    "${REPO_ROOT}/framework/scripts/merge-image-versions.sh:"*"< <(yq -r '.applications // {} | to_entries[] | select(.value.enabled == true) | .key'"*) continue ;;
  esac
