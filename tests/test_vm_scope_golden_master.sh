...
setup_fixture_repo() {
  ...
  # golden fixture is self-sufficient (real generator exercised by
  # tests/test_safe_apply_gatus_gen.sh).
  cat > "${repo_dir}/framework/scripts/generate-gatus-config.sh" <<'GATUS_STUB'
#!/usr/bin/env bash
set -euo pipefail
printf 'endpoints:\n  - name: stub\n    url: https://stub\n'
GATUS_STUB
...
test_start "GM4" "env scope impact composes with branch safety and manifest metadata"
for scope_env in dev prod; do
  scope_output="$(
    SCRIPT_DIR="${fixture}/framework/scripts" REPO_DIR="$fixture" SCOPE="env=${scope_env}" \
      bash -c 'source "${SCRIPT_DIR}/git-deploy-context.sh"; classify_scope_impact; printf "%s" "$SCOPE_IMPACT"'
  )"
  expected_impact=prod_affecting
  [[ "$scope_env" != dev ]] || expected_impact=dev_only
  if [[ "$scope_output" == "$expected_impact" ]]; then
    test_pass "env=${scope_env} classifies as ${expected_impact}"
  else
    test_fail "env=${scope_env} classification: ${scope_output}"
  fi
done

for recreate in 0 1; do
  SCRIPT_DIR="${fixture}/framework/scripts" REPO_DIR="$fixture" LOG_DIR="${fixture}/build" \
    RECREATE="$recreate" SCOPE=env=dev OVERRIDE_BRANCH_CHECK=1 \
    bash -c 'source "${SCRIPT_DIR}/git-deploy-context.sh"
      GIT_COMMIT=fixture; GIT_COMMIT_SHORT=fixture; GIT_SUBJECT=test; GIT_DATE=test
      GIT_TREE_STATE=clean; INITIAL_DEPLOY_REASON=test; SCOPE_IMPACT=dev_only
      write_deploy_manifest' > "${TMP_DIR}/manifest.out"
  if jq -e --argjson recreate "$recreate" \
    '.recreate == ($recreate == 1) and .recreate_scope == "env=dev" and .override_branch_check == true' \
    "${fixture}/build/rebuild-manifest.json" >/dev/null; then
    test_pass "manifest records recreate=${recreate} with scope and branch override"
  else
    test_fail "manifest missing recreate/scope/override contract"
  fi
done
