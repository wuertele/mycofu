...
#
# Historical motivation: adversarial review (codex, gemini) found that an
# earlier build:merge in CI set GATUS_GITHUB_EXPECTED_SOURCE_COMMIT to
# ${CI_COMMIT_SHA} unconditionally, and rebuild-cluster.sh pre-resolved the
# SHA unconditionally too. Both call sites have since converged on letting
# generate-gatus-config.sh's own internal opt-in resolver run without an
# injected env var (#528 for CI, #701 for rebuild-cluster.sh, which also
# env-scrubs the test seams). This case keeps generate-gatus-config.sh's own
# defense-in-depth: even if a caller DOES pass the env var, with publishing
# disabled the github-mirror block is skipped so downstream adopters with
# publish disabled never crash under set -e (github_remote_to_raw_metadata_url
...
set +e
# #718: --allow-test-seams is required for the env var to be honoured at all.
# It is passed here deliberately: the point of this case is that the
# publish-disabled gate wins even when the seam IS active, which is a strictly
# harder condition than the seam being scrubbed.
GATUS_OUTPUT="$(
...
# --- Case L: non-boolean publish.github.enabled → validation fails (#295) ---
#
# publish.github.enabled is compared to the literal string "true", so a
# non-canonical value (True, yes, 1, or a typo) reads as not-true and would
# silently disable publishing with no error. The validator must reject any
# value that isn't a canonical boolean so the mistype is caught loudly.

test_start "12" "non-boolean publish.github.enabled is rejected"
make_fixture NONBOOL_ENABLED_FIXTURE
"${REAL_YQ}" -i '.publish.github.enabled = "yes"' "${NONBOOL_ENABLED_FIXTURE}/config.yaml"
run_validator "${NONBOOL_ENABLED_FIXTURE}"
if [[ "$(cat "${NONBOOL_ENABLED_FIXTURE}/exit.txt")" -ne 0 ]] && \
   grep -q 'publish.github.enabled must be a boolean' "${NONBOOL_ENABLED_FIXTURE}/output.txt"; then
  test_pass "non-boolean publish.github.enabled ('yes') is rejected with a boolean error"
else
  test_fail "non-boolean publish.github.enabled was not rejected"
  sed 's/^/    /' "${NONBOOL_ENABLED_FIXTURE}/output.txt" >&2
fi
