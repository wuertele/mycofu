...
# Sprint 039: the fixture's safe-apply.sh/rebuild-cluster.sh defaults
# VM_SCOPE_SCRIPT to ${SCRIPT_DIR}/vm-scope.sh — i.e. inside FIXTURE_REPO. The
# fixture doesn't ship vm-scope.sh (or its manifests), so point at the real
# helper which reads the real images.yaml/applications.yaml.
export VM_SCOPE_SCRIPT="${REPO_ROOT}/framework/scripts/vm-scope.sh"
# Also pin the real yq path: this test prepends a SHIM_DIR with a fake yq to
# PATH, which would otherwise intercept vm-scope.sh's manifest reads.
...
mkdir -p "${FIXTURE_REPO}/framework/scripts/lib" "${FIXTURE_REPO}/framework/tofu/root" "${SHIM_DIR}"
...
for recovery_script in restore-before-start.sh configure-replication.sh post-deploy.sh configure-backups.sh; do
