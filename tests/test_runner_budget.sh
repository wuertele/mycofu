...
[[ "$(target_for "$high_root")" == "16" ]] || fail "55 GiB available should hit maxConcurrent=16"
[[ "$(first_concurrent "$high_root")" == "16" ]] || fail "high fixture config did not converge to 16"
...
# Repeated steady-state cycles must consume every temp file, including the
# config and nix temps whose no-change paths do not rename them into place.
steady_root="$TMP_ROOT/temp-steady-state"
steady_config_dir="$steady_root/config"
steady_persist_dir="$steady_root/persist"
steady_nix_dir="$steady_root/nix"
steady_state_dir="$steady_root/state"
steady_status_dir="$steady_root/status"
mkdir -p "$steady_config_dir" "$steady_persist_dir"
{
  printf '[[runners]]\n'
  printf '  name = "fixture"\n'
} > "$steady_config_dir/config.toml"
write_meminfo "$steady_root/meminfo" 20480
run_steady_budget() {
  local cycle="$1"
  RUNNER_BUDGET_APPLY=0 \
    RUNNER_BUDGET_MEMINFO="$steady_root/meminfo" \
    RUNNER_BUDGET_STATE_FILE="$steady_state_dir/runner-budget.state" \
    RUNNER_BUDGET_STATUS_FILE="$steady_status_dir/runner-budget.json" \
    RUNNER_BUDGET_NIX_CONF="$steady_nix_dir/nix-budget.conf" \
    GITLAB_RUNNER_CONFIG_FILE="$steady_config_dir/config.toml" \
    GITLAB_RUNNER_PERSISTED_CONFIG="$steady_persist_dir/config.toml" \
    bash "$SCRIPT" > "$steady_root/stdout.${cycle}" 2> "$steady_root/stderr.${cycle}"
}
for cycle in warm-1 warm-2 warm-3; do
  run_steady_budget "$cycle"
done
for cycle in unchanged-1 unchanged-2 unchanged-3 unchanged-4 unchanged-5; do
  run_steady_budget "$cycle"
done
config_tmp_count="$(temp_count "$steady_config_dir" '.runner-budget-config.*')"
persist_tmp_count="$(temp_count "$steady_persist_dir" '.runner-budget-persist.*')"
nix_tmp_count="$(temp_count "$steady_nix_dir" '.nix-budget.*')"
state_tmp_count="$(temp_count "$steady_state_dir" '.runner-budget-state.*')"
status_tmp_count="$(temp_count "$steady_status_dir" '.runner-budget-json.*')"
[[ "$config_tmp_count" -eq 0 ]] || fail "steady-state left ${config_tmp_count} .runner-budget-config.* temp files"
[[ "$persist_tmp_count" -eq 0 ]] || fail "steady-state left ${persist_tmp_count} .runner-budget-persist.* temp files"
[[ "$nix_tmp_count" -eq 0 ]] || fail "steady-state left ${nix_tmp_count} .nix-budget.* temp files"
[[ "$state_tmp_count" -eq 0 ]] || fail "steady-state left ${state_tmp_count} .runner-budget-state.* temp files"
[[ "$status_tmp_count" -eq 0 ]] || fail "steady-state left ${status_tmp_count} .runner-budget-json.* temp files"
ok "steady-state no-change cycles strand no temp files"

# A failure after config-temp creation must leave cleanup to the EXIT trap.
abnormal_root="$TMP_ROOT/temp-abnormal-exit"
abnormal_config_dir="$abnormal_root/config"
mkdir -p "$abnormal_config_dir"
{
  printf '[[runners]]\n'
  printf '  name = "fixture"\n'
} > "$abnormal_config_dir/config.toml"
printf 'regular file blocks mkdir beneath it\n' > "$abnormal_root/persist-blocker"
write_meminfo "$abnormal_root/meminfo" 20480
set +e
RUNNER_BUDGET_APPLY=0 \
  RUNNER_BUDGET_MEMINFO="$abnormal_root/meminfo" \
  RUNNER_BUDGET_STATE_FILE="$abnormal_root/state/runner-budget.state" \
  RUNNER_BUDGET_STATUS_FILE="$abnormal_root/status/runner-budget.json" \
  RUNNER_BUDGET_NIX_CONF="$abnormal_root/nix/nix-budget.conf" \
  GITLAB_RUNNER_CONFIG_FILE="$abnormal_config_dir/config.toml" \
  GITLAB_RUNNER_PERSISTED_CONFIG="$abnormal_root/persist-blocker/config.toml" \
  bash "$SCRIPT" > "$abnormal_root/stdout" 2> "$abnormal_root/stderr"
abnormal_rc=$?
set -e
[[ "$abnormal_rc" -ne 0 ]] || fail "blocked persisted-config parent did not make the script exit non-zero"
# Literal same-line test for both tokens. Not `grep -F ... | grep -q`: grep -q
# exits at the first match and the upstream grep takes SIGPIPE (141), which
# `pipefail` turns into a false FAIL on a *matching* assertion. Not a regex
# either: $abnormal_root carries mktemp's `.` (over-matches) and any TMPDIR
# metacharacter would break the match. ENVIRON, not -v, since -v would process
# backslash escapes in the path.
blocker_path="$abnormal_root/persist-blocker" awk '
  index($0, "mkdir:") && index($0, ENVIRON["blocker_path"]) { found = 1 }
  END { exit !found }
' "$abnormal_root/stderr" \
  || fail "abnormal exit did not reach the blocked persisted-config mkdir"
abnormal_tmp_count="$(temp_count "$abnormal_config_dir" '.runner-budget-config.*')"
[[ "$abnormal_tmp_count" -eq 0 ]] || fail "abnormal exit left ${abnormal_tmp_count} .runner-budget-config.* temp files"
ok "abnormal exit trap removes a registered config temp"
...
printf 'samples=56320 56320 56320\nlast_target=99\nup_confirm=0\nmemory_high_mb=0\nmemory_max_mb=0\n' > "$corrupt_root/state"
run_budget "$corrupt_root" "$corrupt_root/meminfo"
[[ "$(target_for "$corrupt_root")" -le 16 ]] || fail "corrupted last_target=99 was not clamped to the cap"
...
[[ "$(first_concurrent "$reload_root")" == "16" ]] || fail "idle apply did not write concurrent=16"
...
run_budget_apply "$bd_root" "$bd_root/meminfo"
[[ "$(first_concurrent "$bd_root")" == "16" ]] || fail "busy runner rewrote concurrent (must defer to next idle cycle)"
