...
# #286 hermeticity (codex P3): scrub the new opt-in stub knobs from the ambient
# environment so an operator who exported them cannot perturb scenarios
# 3.3-3.22. Each scenario that needs them exports them inside its own subshell.
unset STUB_MANIFEST_CAPTURE STUB_CLUSTER_RESOURCES

# Sprint 049 MR-3 hermeticity: scrub any inherited SOPS_AGE_KEY_FILE from
# the ambient environment (runner-service, operator shell, or CI env) so
# rebuild-cluster.sh's Step 0 prereq check falls back to the fixture's
# synthetic ${REPO_DIR}/operator.age.key. Mirrors the STUB_* scrub above
# — hermetic tests must not depend on any real-VM secret path being
# present outside the fixture.
...
setup_fixture_repo() {
  ...
  cat > "${repo_dir}/framework/scripts/generate-gatus-config.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
echo 'endpoints: []'
EOF
  chmod +x "${repo_dir}/framework/scripts/generate-gatus-config.sh"
  cp "${REPO_ROOT}/framework/scripts/converge-lib.sh" "${repo_dir}/framework/scripts/converge-lib.sh"
  cp "${REPO_ROOT}/framework/scripts/vm-scope.sh" "${repo_dir}/framework/scripts/vm-scope.sh"
  cp "${REPO_ROOT}/framework/scripts/vdb-park-lib.sh" "${repo_dir}/framework/scripts/vdb-park-lib.sh"
  chmod +x "${repo_dir}/framework/scripts/rebuild-cluster.sh"
  chmod +x "${repo_dir}/framework/scripts/converge-lib.sh"
  chmod +x "${repo_dir}/framework/scripts/vm-scope.sh"

  cat > "${repo_dir}/framework/scripts/lib/converge-incomplete-vm.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
converge_incomplete_vm() { :; }
EOF
  chmod +x "${repo_dir}/framework/scripts/lib/converge-incomplete-vm.sh"
...
EOF
  ...
  cat > "${repo_dir}/framework/scripts/known-hosts-scope-lib.sh" <<'EOF'
#!/usr/bin/env bash
# Test stub for #349's known-hosts scope helper.
vm_key_to_module_name() { printf '%s' "$1"; }
refresh_known_hosts_for_scope() { return 0; }
EOF
  chmod +x "${repo_dir}/framework/scripts/known-hosts-scope-lib.sh"
...
# #286: when STUB_MANIFEST_CAPTURE is set, copy the --manifest argument's file
# VERBATIM to the capture path so the caller can jq-assert what the rebuild's
# plan->manifest pipeline actually produced (not merely that
# restore-before-start.sh was invoked). Copying the raw JSON (rather than a
# decorated log) lets the assertion parse it with jq and check that a SINGLE
# entry carries the correct {label, vmid, module, reason} tuple, instead of
# grepping decoupled substrings. Additive and inert for the existing 3.19-3.22
# ordering fixtures, which do not set the env var. The scenario that sets it
# (--scope vm=vault_dev) triggers exactly one restore-before-start.sh call, so
# a last-writer copy is unambiguous.
if [[ -n "${STUB_MANIFEST_CAPTURE:-}" ]]; then
  prev=""
  for arg in "$@"; do
    if [[ "${prev}" == "--manifest" ]]; then
      cp "${arg}" "${STUB_MANIFEST_CAPTURE}" 2>/dev/null \
        || printf '{"error":"manifest file %s not found"}\n' "${arg}" > "${STUB_MANIFEST_CAPTURE}"
    fi
    prev="${arg}"
  done
fi
...
EOF
  ...
  # DEFAULT_REBUILD_FIXTURE_NOOP_SCRIPTS covers the shared base; the
  # extras below are scripts this fixture noops but others handle
  # specially. See tests/lib/rebuild_fixture_helpers.sh.
  setup_rebuild_fixture_noops "${repo_dir}" \
    install-pbs.sh \
...
if [[ "${remote_cmd}" == *"/cluster/resources"* ]]; then
  # #286: default [] preserves every existing scenario; a scenario that needs
  # a precious VM's hosting node (Step 18 immediate backup) sets
  # STUB_CLUSTER_RESOURCES to a JSON array of {"vmid":N,"node":"..."} rows.
...
EOF
cat >> "${DATA_PLANE_SYMLINK_REPO}/site/images.yaml" <<'EOF'
  site-extra:
    category: nix
    host_config: site/nix/hosts/site-extra.nix
    scope: env-bound
    control_plane: false
EOF
ln -s "../../../site/tofu/overrides.tf" \
  "${DATA_PLANE_SYMLINK_REPO}/framework/tofu/root/overrides.tf"
"${REAL_GIT}" -C "${DATA_PLANE_SYMLINK_REPO}" add site/images.yaml site/tofu/overrides.tf framework/tofu/root/overrides.tf
...
# --- #286: restore manifest CORRECTNESS (not just invocation ordering).
# The 3.19-3.22 fixtures pin ordering (restore before start, backups after
# restore) but every VM in their config.yaml is backup:false and every plan is
# empty, so restore-before-start.sh is always handed an EMPTY manifest. Those
# tests would still pass if the rebuild built an empty manifest, omitted a
# precious VM, or pointed at the wrong VMIDs — they only assert the shim was
# INVOKED. This scenario closes that gap: a precious VM (backup:true) that the
# plan marks as a recreate must appear in the plan->manifest pipeline's output
# with the correct VMID and module path. Existing manifest-content coverage
# lives in tests/test_rebuild_preboot_restore.sh (17.2) and
# tests/test_rebuild_pbs_install_before_preboot_restore.sh (31.B.2); this
# broadens it onto the HA-flow fixture per the issue.
#
# Implementation: a dedicated fixture repo (existing scenarios are left
# byte-identical) whose config.yaml flips vault_dev to backup:true, driven with
# a STUB_REBUILD_PLAN_JSON that marks module.vault_dev's VM resource as a
# create. STUB_MANIFEST_CAPTURE makes the shared restore-before-start.sh shim
# copy the --manifest file's contents so we can assert on them.
MANIFEST_REPO="${TMP_DIR}/manifest-correctness-repo"
MANIFEST_SHIMS="${TMP_DIR}/manifest-correctness-shims"
MANIFEST_OUT="${TMP_DIR}/manifest-correctness.out"
MANIFEST_LOG="${TMP_DIR}/manifest-correctness.log"
MANIFEST_CAPTURE="${TMP_DIR}/manifest-correctness.capture"

setup_fixture_repo "${MANIFEST_REPO}"
setup_shims "${MANIFEST_SHIMS}"

# Flip vault_dev (vmid 302) to backup:true so it is a precious VM the manifest
# writer must enumerate. vault is declared in the fixture framework/images.yaml,
# so vm-scope class resolution for backup_kind() succeeds.
"${REAL_GIT}" -C "${MANIFEST_REPO}" >/dev/null 2>&1 || true
python3 - "${MANIFEST_REPO}/site/config.yaml" <<'PY'
import sys
path = sys.argv[1]
with open(path) as f:
    text = f.read()
# Set vault_dev's backup flag to true (it is false in the shared fixture).
marker = "  vault_dev:\n    vmid: 302\n    ip: 10.0.0.21\n    backup: false\n"
replacement = "  vault_dev:\n    vmid: 302\n    ip: 10.0.0.21\n    backup: true\n"
assert marker in text, "vault_dev backup:false marker not found in fixture config.yaml"
open(path, "w").write(text.replace(marker, replacement))
PY
"${REAL_GIT}" -C "${MANIFEST_REPO}" add site/config.yaml
"${REAL_GIT}" -C "${MANIFEST_REPO}" commit -m "fixture: vault_dev precious" >/dev/null

# Plan JSON marks module.vault_dev's VM resource as a fresh create, with a
# scsi1 data disk so the manifest carries the data-disk metadata too.
MANIFEST_PLAN_JSON='{"resource_changes":[{"address":"module.vault_dev.module.vault.proxmox_virtual_environment_vm.vm","change":{"actions":["create"],"before":null,"after":{"disk":[{"interface":"scsi0"},{"interface":"scsi1","size":"32G"}]}}}]}'

set +e
(
  export PATH="${MANIFEST_SHIMS}:${PATH}"
  export STUB_LOG_FILE="${MANIFEST_LOG}"
  export STUB_GITLAB_PLAN_MODE="none"
  export STUB_TOFU_STATE_OUTPUT=""
  export STUB_DIRECT_TOFU_STATE_OUTPUT=""
  export STUB_REBUILD_PLAN_JSON="${MANIFEST_PLAN_JSON}"
  export STUB_MANIFEST_CAPTURE="${MANIFEST_CAPTURE}"
  # vault_dev is now precious, so Step 18's immediate-backup loop must locate
  # its hosting node via /cluster/resources.
  export STUB_CLUSTER_RESOURCES='[{"vmid":302,"node":"pve01"}]'
  cd "${MANIFEST_REPO}"
  framework/scripts/rebuild-cluster.sh --scope vm=vault_dev --override-branch-check
) > "${MANIFEST_OUT}" 2>&1
MANIFEST_STATUS=$?
set -e

test_start "3.23" "rebuild plan->manifest names the precious recreate VM by VMID and module in ONE entry (#286)"
# jq-assert that a SINGLE manifest entry carries the full correct tuple, so a
# malformed manifest with the right vmid and module scattered across DIFFERENT
# entries cannot pass (codex P2). The manifest is the verbatim JSON the rebuild
# handed restore-before-start.sh.
if [[ "${MANIFEST_STATUS}" == "0" ]] \
   && [[ -s "${MANIFEST_CAPTURE}" ]] \
   && jq -e '.entries[]
             | select(.label == "vault_dev"
                      and .vmid == 302
                      and .module == "module.vault_dev"
                      and .reason == "create")' \
        "${MANIFEST_CAPTURE}" >/dev/null 2>&1; then
  test_pass "one manifest entry has {label:vault_dev, vmid:302, module:module.vault_dev, reason:create}"
else
  test_fail "manifest correctness not met (status=${MANIFEST_STATUS})"
  printf '    captured manifest:\n%s\n    ---- rebuild output (tail) ----\n%s\n' \
    "$(cat "${MANIFEST_CAPTURE}" 2>/dev/null || echo '(no capture)')" \
    "$(tail -25 "${MANIFEST_OUT}")" >&2
fi

test_start "3.24" "the manifest names EXACTLY the precious recreate VM — no empty/omitted/extra entry (#286)"
# Guard against the three regressions #286 calls out: an empty manifest, a
# manifest that omits the precious VM, or a manifest with wrong VMIDs. Assert
# the entries array has exactly one entry and it is vault_dev's VMID.
MANIFEST_ENTRY_VMIDS="$(jq -r '.entries[].vmid' "${MANIFEST_CAPTURE}" 2>/dev/null | sort -n | paste -sd, -)"
if [[ "${MANIFEST_STATUS}" == "0" && "${MANIFEST_ENTRY_VMIDS}" == "302" ]]; then
  test_pass "manifest entries == exactly [302] (vault_dev), no empty/omitted/spurious entry"
else
  test_fail "manifest entries VMIDs = '${MANIFEST_ENTRY_VMIDS}', want '302' (status=${MANIFEST_STATUS})"
  printf '    captured manifest:\n%s\n' \
    "$(cat "${MANIFEST_CAPTURE}" 2>/dev/null || echo '(no capture)')" >&2
fi
