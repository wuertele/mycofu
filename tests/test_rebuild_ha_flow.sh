...
setup_fixture_repo() {
  ...
  # Issue #528: rebuild-cluster.sh sources github-publish-lib.sh at load time.
  cat > "${repo_dir}/framework/scripts/github-publish-lib.sh" <<'EOF'
#!/usr/bin/env bash
resolve_gatus_expected_source_commit() {
  # Fixture stub: fixture repo has no real git remote/prod branch, so the
  # real resolver would fail. This stub keeps rebuild-cluster.sh's step 7
  # traceable while asserting nothing about the gatus config's correctness
  # (a distinct test covers that: tests/test_gatus_source_commit_resolver.sh).
