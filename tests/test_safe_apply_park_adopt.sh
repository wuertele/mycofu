...
setup_fixture() {
  ...
  # fixture repo is self-sufficient (real generator exercised by
...
setup_real_lib_fixture() {
  ...
  # #690: stub the in-path Gatus config generator for the vdb fixture repo too.
  cat > "${VDB_FIXTURE_REPO}/framework/scripts/generate-gatus-config.sh" <<'GATUS_STUB'
#!/usr/bin/env bash
set -euo pipefail
printf 'endpoints:\n  - name: stub\n    url: https://stub\n'
GATUS_STUB
  chmod +x "${VDB_FIXTURE_REPO}/framework/scripts/generate-gatus-config.sh"
