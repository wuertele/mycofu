...
# #692 remediation 3: this failure branch used to print the canonical-verb
# label for ANY non-zero rc, so an SSH/PXE/timing failure presented as a fake
# verb mismatch (the pipeline-1750 job-134727 misdiagnosis). Emit the real
# exit code and the script's own output BEFORE any label, and only claim a
# verb mismatch when the install actually reached green (rc=0) and the HIL
# log still lacks the canonical verbs. The diagnostics are unconditional on
# the failure path so the true cause is recoverable from the job log alone.
if [[ "$INSTALL_RC" -eq 0 ]] && \
   grep -q 'meshcmd amtpower --poweroff ' "$HIL_LOG" && \
   grep -q 'meshcmd amtpower --poweron --bootdevice pxe ' "$HIL_LOG" && \
  ...
  if [[ "$INSTALL_RC" -ne 0 ]]; then
    test_fail "install did not reach green (rc=${INSTALL_RC}); canonical-verb assertion inconclusive — see INSTALL_RC/INSTALL_OUTPUT above"
  else
