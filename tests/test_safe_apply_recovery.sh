...
# #690: safe-apply.sh generates site/gatus/config.yaml in-path before the tofu
# plan and fails closed on generation failure. Stub the generator so this
# fixture repo is self-sufficient (the real generator + fail-closed path is
# exercised by tests/test_safe_apply_gatus_gen.sh).
cat > "${FIXTURE_REPO}/framework/scripts/generate-gatus-config.sh" <<'GATUS_STUB'
#!/usr/bin/env bash
set -euo pipefail
printf 'endpoints:\n  - name: stub\n    url: https://stub\n'
GATUS_STUB
...
# ============================================================
# Test 9: no-change deploy convergence (#617 backup jobs; #693/#501 artifact)
#
# A no-change deploy (tofu plan shows nothing to apply) must still run the
# convergence steps that are safe to make unconditional and are NOT tied to a
# fresh apply:
#
#   1. configure-backups.sh — reconciles managed vzdump-job membership drift
#      left by a prior destructive-recreate-then-fail (#617).
#   2. configure-replication.sh --refresh-artifact-only — redelivers the
#      /etc/repl-policy.vmids node artifact as a synchronous projection of the
#      deployed ref, so a ref that changed the artifact SHAPE but touched no VM
#      inputs cannot leave a stale artifact that false-reds validate's
#      `replication policy conformance` check (#693, pipeline #1779; closes
#      #501's "reconcile after each deploy stage" contract). This is the
#      no-change-deploy staleness case.
#
# Deliberately NOT run here: post-deploy.sh (Vault probes → hangs) and the FULL
# configure-replication run (create/prune/seed/schedule-reconcile → tears down
# transient first-syncs). The refresh invocation MUST carry
# --refresh-artifact-only and must NOT be the bare full-reconcile form.
#
# rc contract: first non-zero wins in order backups → artifact refresh; neither
# step is gated behind the other's success.
#
# End-to-end verification that the reconcilers actually converge state is by
# pipeline observation (validate.sh); the seam-opening tests below prove the
# invocations happen with the right arguments.
# ============================================================
test_start "9.1" "no-change deploy runs configure-backups AND the artifact refresh (not the full reconcile / post-deploy)"
reset_logs
...
RC="${OUT##*__EXITCODE__:}"
COUNTS="$(count_recovery restore-before-start.sh) $(count_recovery configure-replication.sh) $(count_recovery post-deploy.sh) $(count_recovery configure-backups.sh)"
REPL_LINE="$(grep -F 'configure-replication.sh' "${RECOVERY_LOG}" || true)"
export STUB_PLAN_JSON="${SAVED_PLAN_JSON}"
# Staleness-case assertion: configure-replication is invoked exactly once, in
# --refresh-artifact-only mode, scoped to the env — NOT the bare full-reconcile
# form (which would tear down transient first-syncs).
if [[ "${RC}" == "0" && "${COUNTS}" == "0 1 0 1" ]] &&
   grep -Fq 'No changes for dev environment' <<< "${OUT}" &&
   grep -Fq 'Backup-job reconciliation (no-change path)' <<< "${OUT}" &&
   grep -Fq 'Replication policy artifact refresh (no-change path)' <<< "${OUT}" &&
   grep -Fq -- '--refresh-artifact-only' <<< "${REPL_LINE}" &&
   grep -Fq -- '--env dev' <<< "${REPL_LINE}"; then
  test_pass "no-change deploy ran configure-backups + configure-replication --refresh-artifact-only; post-deploy NOT invoked"
else
  test_fail "got rc=${RC}, counts (restore repl post-deploy backups)=${COUNTS}; want rc=0 counts='0 1 0 1'; repl_line='${REPL_LINE}'"
  cat "${RECOVERY_LOG}" >&2
  printf 'out:\n%s\n' "${OUT}" >&2
fi

test_start "9.1b" "no-change deploy refresh is NOT the full reconcile form (guard: no first-sync teardown)"
# The refresh invocation must not be the bare `configure-replication.sh "*"
# --env dev` full-reconcile form. Assert the ONLY configure-replication line
# recorded carries --refresh-artifact-only (mutation guard for the guard).
if [[ -n "${REPL_LINE}" ]] &&
   grep -Fq -- '--refresh-artifact-only' <<< "${REPL_LINE}" &&
   [[ "$(grep -cF 'configure-replication.sh' "${RECOVERY_LOG}")" == "1" ]]; then
  test_pass "the sole configure-replication invocation on the no-change path is --refresh-artifact-only"
else
  test_fail "expected exactly one configure-replication.sh --refresh-artifact-only invocation; got: ${REPL_LINE}"
  cat "${RECOVERY_LOG}" >&2
fi

test_start "9.2" "no-change deploy propagates configure-backups.sh failure; artifact refresh still runs"
reset_logs
SAVED_PLAN_JSON="${STUB_PLAN_JSON}"
export STUB_PLAN_JSON='{"resource_changes":[]}'
...
COUNTS="$(count_recovery restore-before-start.sh) $(count_recovery configure-replication.sh) $(count_recovery post-deploy.sh) $(count_recovery configure-backups.sh)"
export STUB_PLAN_JSON="${SAVED_PLAN_JSON}"
unset STUB_CONFIGURE_BACKUPS_EXIT
# backups rc=9 wins (first non-zero), but the artifact refresh is NOT gated
# behind backups success — it still runs (repl count == 1).
if [[ "${RC}" == "9" && "${COUNTS}" == "0 1 0 1" ]]; then
  test_pass "rc=9 propagated from configure-backups; artifact refresh still ran independently"
else
  test_fail "got rc=${RC}, counts=${COUNTS}; want rc=9 counts='0 1 0 1'"
  cat "${RECOVERY_LOG}" >&2
fi

test_start "9.2b" "no-change deploy propagates artifact-refresh failure when backups succeed"
reset_logs
SAVED_PLAN_JSON="${STUB_PLAN_JSON}"
export STUB_PLAN_JSON='{"resource_changes":[]}'
export STUB_CONFIGURE_REPLICATION_EXIT=7
OUT="$(run_safe_apply dev)"
RC="${OUT##*__EXITCODE__:}"
COUNTS="$(count_recovery restore-before-start.sh) $(count_recovery configure-replication.sh) $(count_recovery post-deploy.sh) $(count_recovery configure-backups.sh)"
export STUB_PLAN_JSON="${SAVED_PLAN_JSON}"
unset STUB_CONFIGURE_REPLICATION_EXIT
if [[ "${RC}" == "7" && "${COUNTS}" == "0 1 0 1" ]] &&
   grep -Fq 'configure-replication.sh --refresh-artifact-only failed (rc=7)' <<< "${OUT}"; then
  test_pass "rc=7 from the artifact refresh propagates (backups green, refresh red)"
else
  test_fail "got rc=${RC}, counts=${COUNTS}; want rc=7 counts='0 1 0 1'"
  cat "${RECOVERY_LOG}" >&2
  printf 'out:\n%s\n' "${OUT}" >&2
fi

test_start "9.3" "no-change deploy with --no-recovery skips ALL convergence"
reset_logs
SAVED_PLAN_JSON="${STUB_PLAN_JSON}"
export STUB_PLAN_JSON='{"resource_changes":[]}'
OUT="$(run_safe_apply dev --no-recovery)"
...
if [[ "${RC}" == "0" && "${COUNTS}" == "0 0 0 0" ]] &&
   grep -Fq 'Skipping no-change convergence (--no-recovery)' <<< "${OUT}"; then
  test_pass "no-change + --no-recovery skips both backups and the artifact refresh"
else
...
test_start "9.4" "no-change + --dry-run is fully read-only (no reconciliation, no artifact write)"
reset_logs
SAVED_PLAN_JSON="${STUB_PLAN_JSON}"
export STUB_PLAN_JSON='{"resource_changes":[]}'
OUT="$(run_safe_apply dev --dry-run)"
...
COUNTS="$(count_recovery restore-before-start.sh) $(count_recovery configure-replication.sh) $(count_recovery post-deploy.sh) $(count_recovery configure-backups.sh)"
export STUB_PLAN_JSON="${SAVED_PLAN_JSON}"
# The artifact refresh writes /etc/repl-policy.vmids on nodes (a mutation), so
# it MUST stay behind the dry-run guard — counts must be all-zero.
if [[ "${RC}" == "0" && "${COUNTS}" == "0 0 0 0" ]] &&
   grep -Fq '(dry-run — not applying)' <<< "${OUT}"; then
  test_pass "dry-run with no changes exits 0 without invoking any convergence step (artifact refresh gated)"
else
...
test_start "9.5" "no-change deploy with PBS unregistered still refreshes the artifact (PBS-independent)"
reset_logs
SAVED_PLAN_JSON="${STUB_PLAN_JSON}"
export STUB_PLAN_JSON='{"resource_changes":[]}'
...
COUNTS="$(count_recovery restore-before-start.sh) $(count_recovery configure-replication.sh) $(count_recovery post-deploy.sh) $(count_recovery configure-backups.sh)"
export STUB_PLAN_JSON="${SAVED_PLAN_JSON}"
unset STUB_PBS_AVAILABLE
# configure-backups is correctly skipped when PBS storage isn't registered, but
# the artifact refresh does not depend on PBS and still runs (repl count == 1).
if [[ "${RC}" == "0" && "${COUNTS}" == "0 1 0 0" ]] &&
   grep -Fq 'PBS storage not available' <<< "${OUT}"; then
  test_pass "PBS-unavailable no-change exits 0, skips configure-backups, still runs the artifact refresh"
else
  test_fail "got rc=${RC}, counts=${COUNTS}; want rc=0 counts='0 1 0 0'"
  cat "${RECOVERY_LOG}" >&2
  printf 'out:\n%s\n' "${OUT}" >&2
fi
