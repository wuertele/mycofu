...
# Target set is built in tests/lib/vdb_gate_targets.sh so that a fixture-based
# test (tests/test_vdb_gate_registry_exclusion.sh) can exercise the same
# exclusion rules against controlled inputs (issue #526).
RULE_DOC_TARGETS=()
while IFS= read -r -d '' path; do
  RULE_DOC_TARGETS+=("$path")
done < <(vdb_gate_collect_rule_doc_targets "${REPO_ROOT}")
