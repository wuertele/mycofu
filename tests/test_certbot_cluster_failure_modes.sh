...
test_start "3.6" "remote renewability helper preserves rc=3 unknowable verdict"
reset_stub_state
export STUB_REMOTE_HELPER_EXIT="3"
run_capture \
  certbot_cluster_run_remote_renewability_probe \
  "10.0.0.31" \
  "gitlab.prod.example.test"
assert_exit 3 "remote renewability helper returns rc=3 from target"
assert_output_contains "remote helper failed on 10.0.0.31" "renewability helper execution reached the VM shim"
