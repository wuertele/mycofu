...
if ! jq -e 'has("regreen-bfnet")' <<< "$rendered" >/dev/null && \
   ! jq -e 'has("build:image:hil-boot") or has("validate:hil-boot-source-filter") or has("validate:install-pve-node") or has("validate:pdu-cycle") or has("validate:regreener-host-module")' <<< "$rendered" >/dev/null && \
...
if ! yq -e '."build:image".parallel.matrix[].ROLE[] | select(. == "hil-boot")' "$CI_FILE" >/dev/null 2>&1 && \
   ...
   yq -e '."build:image:hil-boot".script[] | select(. | contains("tests/test_hil_boot_iso_real_build.sh"))' "$CI_FILE" >/dev/null && \
   ! yq -e 'has("build:hil-boot-iso-real")' "$CI_FILE" >/dev/null 2>&1 && \
   yq -e '."build:merge".needs[] | select(.job == "prepare:cicd-storage" and .artifacts == false and .optional == true)' "$CI_FILE" >/dev/null && \
   yq -e '."build:merge".needs[] | select(.job == "build:image:hil-boot" and .artifacts == true and .optional == true)' "$CI_FILE" >/dev/null; then
