...
test_start "C4" "matching fingerprint skips regardless of stored not_after"
# Even if Vault's stored not_after says the cert has expired, matching
# fingerprint is sufficient evidence that Vault holds the same cert that
# is on the VM. Backfill is not in the business of issuing fresh certs —
# that is certbot's job on the VM. A duplicate-content KV write here
# would inflate the check-cert-budget.sh oracle without any real LE
# issuance. See issue #416.
setup_case "c4"
...
run_backfill
[[ "${CASE_STATUS}" -eq 0 ]] && test_pass "expired stored not_after with matching fingerprint exits 0" || test_fail "expired stored not_after with matching fingerprint exits 0"
[[ "$(count_data_writes)" == "0" ]] && test_pass "expired stored not_after with matching fingerprint performs no data write" || test_fail "expired stored not_after with matching fingerprint performs no data write"
assert_output_has "seeded 0, skipped 1, failures 0" "expired stored not_after with matching fingerprint summary"
...
test_start "C8" "dedup uses fingerprint only — no remaining-days clause"
# The dedup decision is "does Vault hold this exact cert?" Answered by
# fingerprint match alone. Any staleness/remaining-days clause ANDed
# into the dedup conditional would re-introduce the LE rate-limit
# false-positive that issue #416 fixed: an unchanged cert in its last
# 30 days of validity would generate a duplicate-content KV version on
# every prod deploy, inflating check-cert-budget.sh's oracle without
# any real LE issuance. See cert-sync.sh:191-206 for the canonical
# contract. C2 and C4 enforce the behavior; this case prevents the
# symbol from being reintroduced.
backfill_src="${REPO_ROOT}/framework/scripts/cert-storage-backfill.sh"
if ! grep -E 'STALE_THRESHOLD_DAYS|validity_days_remaining|\bremaining_days\b' "${backfill_src}" >/dev/null; then
  test_pass "no staleness/remaining-days symbols in backfill source"
else
  test_fail "no staleness/remaining-days symbols in backfill source"
fi
