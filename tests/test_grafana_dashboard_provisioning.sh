...
GRAFANA_INI="${REPO_ROOT}/site/apps/grafana/grafana.ini"
...
HA_ENVIRONMENTAL_FILE="${REPO_ROOT}/site/apps/grafana/dashboards/home-assistant-environmental-data.json"
...
test_start "6" "Home Assistant environmental dashboard has stable embedding contract"
if [[ "$(jq -r '.uid' "${HA_ENVIRONMENTAL_FILE}")" == "home-assistant-environmental-data" ]] && \
   [[ "$(jq -r '.timezone' "${HA_ENVIRONMENTAL_FILE}")" == "America/Los_Angeles" ]] && \
   [[ "$(jq -r '.panels[0].id' "${HA_ENVIRONMENTAL_FILE}")" == "1" ]]; then
  test_pass "Home Assistant environmental dashboard exposes the expected UID, timezone, and panel ID"
else
  test_fail "Home Assistant environmental dashboard embedding contract changed"
fi

test_start "7" "Home Assistant environmental dashboard queries the explicit raw bucket"
if jq -e '.panels[0].targets[0].query | contains("from(bucket: \"homeassistant_raw\")")' "${HA_ENVIRONMENTAL_FILE}" >/dev/null; then
  test_pass "Home Assistant environmental dashboard queries homeassistant_raw explicitly"
else
  test_fail "Home Assistant environmental dashboard is relying on datasource default bucket selection"
fi

test_start "8" "Grafana permits Home Assistant iframe embedding"
if grep -Eq '^[[:space:]]*allow_embedding[[:space:]]*=[[:space:]]*true[[:space:]]*$' "${GRAFANA_INI}"; then
  test_pass "Grafana allow_embedding is enabled for embedded Home Assistant cards"
else
  test_fail "Grafana allow_embedding is not enabled"
fi
