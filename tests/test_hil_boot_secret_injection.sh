...
if [[ "$eval_rc" -ne 0 && ( "$eval_output" == *"cannot connect to socket"*"Operation not permitted"* || "$eval_output" == *"unable to open database file"* ) ]]; then
  ...
  # #414: this branch conflates two materially different failures —
  # eval_rc=0 ("nix build unexpectedly succeeded") and eval_rc!=0 with output
  # that never mentions the secret ("failed for some non-secret reason"). On
  # pipeline #1091 the branch fired but discarded eval_output, so neither case
  # was distinguishable from the retained CI trace. Surface bounded diagnostics
  # BEFORE test_fail (per issue #414 directive) so a future flip is investigable
  # from the job log alone. The emitter self-guards every command; the extra
  # set +e/set -e is defence in depth so a non-zero diagnostic cannot abort the
  # script under `set -e` before the failure is recorded (.claude/rules/platform.md).
  set +e
  emit_s037_diagnostics "$eval_rc" "$eval_output" "$REPO_ROOT"
  set -e
...
# #728: this subtest used to write git-grep output to a predictable
# /tmp/hil-secret-grep.$$ and remove it with a bare `rm -f`. A predictable
# /tmp path is a symlink-collision risk on a shared runner, and without a
# trap the scratch file leaked whenever the subtest aborted before the `rm`
# (this script runs under `set -euo pipefail`). Reuse the `mktemp -d`
# pattern already used by s037.0.16 below, and register an EXIT trap so
# cleanup happens on every exit path rather than only the happy one.
secret_grep_dir="$(mktemp -d)"
trap 'rm -rf "$secret_grep_dir" || true' EXIT
secret_grep_out="${secret_grep_dir}/hil-secret-grep.txt"
if git -C "$REPO_ROOT" grep -n 'BfnetLab2026!' -- . ':!docs/**' ':!tests/test_hil_boot_secret_injection.sh' >"$secret_grep_out" 2>/dev/null; then
  test_fail "plaintext HIL password is committed outside docs"
  sed 's/^/    /' "$secret_grep_out"
else
