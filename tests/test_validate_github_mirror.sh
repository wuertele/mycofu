...
# in production.
#
# Issue #632: the original single-line ratchet pinned the exact token
# order "VALIDATE_GITHUB_MIRROR_CONFIG_FILE=${CONFIG}" immediately
# followed by "${SCRIPT_DIR}/validate-github-mirror.sh" on one line.
# Three #629 reviewers converged that this is over-strict: it false-fails
# on shape-preserving refactors (line-continuation wraps, a sibling env
# var inserted before the sub-script, the `env VAR=... cmd` prefix form,
# unbraced "$CONFIG", or sourcing MYCOFU_VALIDATE_CONFIG directly). Adopt
# the test-13 two-grep idiom: check independently that (A) the sub-script
# is still invoked and (B) VALIDATE_GITHUB_MIRROR_CONFIG_FILE is set from
# the operator's config override. This drops the "same command context"
# guarantee but keeps teeth: dropping the env-forward removes the sole
# VALIDATE_GITHUB_MIRROR_CONFIG_FILE= assignment (expected to remain 1)
# and check B fails.
test_start "14" "validate.sh forwards MYCOFU_VALIDATE_CONFIG (as CONFIG) to validate-github-mirror.sh (#629)"
VALIDATE_SH="${REPO_ROOT}/framework/scripts/validate.sh"
if ! grep -nE '"\$\{SCRIPT_DIR\}/validate-github-mirror\.sh"' "${VALIDATE_SH}" >/dev/null; then
  test_fail "validate.sh no longer invokes \${SCRIPT_DIR}/validate-github-mirror.sh"
elif ! grep -nE 'VALIDATE_GITHUB_MIRROR_CONFIG_FILE=.*(CONFIG|MYCOFU_VALIDATE_CONFIG)' "${VALIDATE_SH}" >/dev/null; then
  test_fail "validate.sh does not set VALIDATE_GITHUB_MIRROR_CONFIG_FILE from the config override (from MYCOFU_VALIDATE_CONFIG)"
else
  test_pass "validate.sh forwards a config override to validate-github-mirror.sh"
fi
