...
# Infra image manifest — the fabrication path (#505/#520) derives its role
# universe from framework/images.yaml + site/images.yaml + enabled apps.
# The main.tf below references `dns` and `acme-dev`; declaring them here as
# infra roles lets case 17.7 exercise the placeholder-generation path with
# a legitimate universe. Without this the wrapper fails closed on an empty
# universe (correct behavior, but not what 17.7 aims to test).
