...
setup_fixture_repo() {
  ...
  cp "${REPO_ROOT}/framework/scripts/vm-scope.sh" \
    "${repo_dir}/framework/scripts/vm-scope.sh"
  chmod +x \
    "${repo_dir}/framework/scripts/deploy-control-plane.sh" \
    "${repo_dir}/framework/scripts/vm-scope.sh"
...
# #339 grub-path fixup (deploy-control-plane.sh fix_grub_paths). The remote
# command seds grub.cfg then greps for surviving )/store/ paths.
# STUB_GRUB_BROKEN=1 simulates a grub.cfg the sed cannot clean
# (verify-after-write fails -> the remote script exits 1).
if [[ "${remote_cmd}" == *"grub.cfg"* ]]; then
  printf 'grubfix:%s\n' "${target}" >> "${STUB_LOG_FILE}"
  if [[ "${STUB_GRUB_BROKEN:-0}" == "1" ]]; then
    printf 'fix_grub_paths: verify-after-write FAILED\n' >&2
    exit 1
  fi
  exit 0
fi
...
EOF
  ...
  (
    ...
    export STUB_GRUB_BROKEN="${STUB_GRUB_BROKEN:-0}"
...
# #339 regression: the skip path (which the runner-stop retry lands on for a
# cicd self-deploy) must still run the grub-path fixup, or a broken grub.cfg
# written by the killed first attempt survives until the R7.3 cold-boot gate.
test_start "2.3-grub" "skip path runs the grub-path fixup on every control-plane host"
if grep -q 'grubfix:10.0.0.10' "${MATCH_LOG_FILE}" && \
   grep -q 'grubfix:10.0.0.20' "${MATCH_LOG_FILE}"; then
  test_pass "grub fixup invoked for gitlab and cicd on the skip path"
else
  test_fail "grub fixup was skipped on the live==built path"
  cat "${MATCH_LOG_FILE}" >&2
fi

# A grub.cfg the fixup cannot clean (verify-after-write fails) must fail the
# deploy loudly on the skip path — it must not be silently skipped/reported OK.
test_start "2.3-grub-broken" "unrepairable grub.cfg fails the skip-path deploy"
GRUB_BROKEN_ARTIFACTS="$(
  STUB_GRUB_BROKEN=1 \
  run_fixture grub-broken dev
)"
GRUB_BROKEN_STATUS="$(artifact_status "${GRUB_BROKEN_ARTIFACTS}")"
GRUB_BROKEN_OUTPUT_FILE="$(artifact_output_file "${GRUB_BROKEN_ARTIFACTS}")"
if [[ "${GRUB_BROKEN_STATUS}" == "1" ]] && \
   grep -q 'grub-path fixup failed' "${GRUB_BROKEN_OUTPUT_FILE}"; then
  test_pass "broken grub.cfg fails the deploy loudly"
else
  test_fail "broken grub.cfg did not fail the deploy (status=${GRUB_BROKEN_STATUS})"
  cat "${GRUB_BROKEN_OUTPUT_FILE}" >&2
fi
