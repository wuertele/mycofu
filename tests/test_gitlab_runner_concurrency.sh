...
grep -q 'root_disk_gb        = coalesce(try(local.config.cicd.runner_disk_gb, null), 256)' "$ROOT_TOFU" || fail "root module must thread cicd.runner_disk_gb with a null-safe 256GB compatibility default"
grep -q 'vda_size_gb = var.root_disk_gb' "$CICD_TOFU" || fail "cicd module must apply root_disk_gb to vda_size_gb"
