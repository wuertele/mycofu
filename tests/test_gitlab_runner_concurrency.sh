...
# #681 drift-guard: the ceiling is intentionally hard-coded in several coordinated
# sites (that is what #681 is about). Assert the other copies agree with the module
# default so a future bump can't silently update one and miss another.
budget_fallback="$(grep -oE 'RUNNER_BUDGET_MAX_CONCURRENT:-[0-9]+' "${REPO_ROOT}/framework/nix/modules/gitlab-runner-budget.sh" | grep -oE '[0-9]+' | head -1)"
[[ "$budget_fallback" == "$option_default" ]] \
  || fail "gitlab-runner-budget.sh fallback (:-${budget_fallback}) disagrees with module default (${option_default})"
register_cap="$(grep -oE 'OBSERVED_CONCURRENT" -le [0-9]+' "$REGISTER" | grep -oE '[0-9]+' | head -1)"
[[ "$register_cap" == "$option_default" ]] \
  || fail "register-runner.sh convergence cap (-le ${register_cap}) disagrees with module default (${option_default})"
ok "coordinated ceiling sites agree (budget fallback + register cap == module default ${option_default})"
...
grep -q '\-le 16' "$REGISTER" || fail "polling upper bound missing"
