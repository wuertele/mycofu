...
# pre-resolves the gatus source commit, so no resolver stub is needed. The
# generate-gatus-config.sh stub below emits NON-EMPTY output because step 7
# now fails closed on an empty gatus config (-s, #704).
