...
test_start "s037.0.10" "hil-boot is isolated through generalized role source filtering"
if grep -q 'mkRoleSrc = role:' "${REPO_ROOT}/flake.nix" && \
   grep -q 'sourceForRole = role: mkRoleSrc role' "${REPO_ROOT}/flake.nix" && \
   grep -q '"hil-boot" = \[' "${REPO_ROOT}/flake.nix" && \
   grep -q '"tests/hil/"' "${REPO_ROOT}/flake.nix" && \
   ! grep -q 'hilNixSrc\|mkNixSrc' "${REPO_ROOT}/flake.nix"; then
  test_pass "flake models hil-boot as a normal role with HIL-owned extra paths"
else
  test_fail "flake does not expose the generalized hil-boot role source model"
fi

test_start "s037.0.11" "hil-boot config is referenced only through the hil-boot role source"
if grep -q '\${nixSrc}/tests/hil/bfnet/config.yaml' "${REPO_ROOT}/site/nix/lib/hil-boot-artifacts.nix" && \
   grep -q 'nixSrc = mkRoleSrc "hil-boot"' "${REPO_ROOT}/flake.nix" && \
  ...
  test_fail "HIL config reference bypasses the hil-boot role source or leaks into framework Nix"
fi
