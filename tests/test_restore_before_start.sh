...
cp "${REPO_ROOT}/framework/scripts/vm-topology-lib.sh" \
  "${FIXTURE_REPO}/framework/scripts/vm-topology-lib.sh"
...
case "$cmd" in
  ...
  *'qm status 403'*)
    ...
    ;;
  *'qm config 303'*)
    printf '%s\n' "${STUB_QM_CONFIG_303:-scsi0: vmstore:vm-303-disk-0.raw}"
    ;;
  *'qm config 403'*)
    printf '%s\n' "${STUB_QM_CONFIG_403:-scsi0: vmstore:vm-403-disk-0.raw}"
    ;;
  'ha-manager status')
    printf '%s\n' "${STUB_HA_STATUS:-}"
    ;;
...
run_restore_before_start_manifest_only() {
  framework/scripts/restore-before-start.sh dev \
    --manifest "$MANIFEST_FILE" \
    --first-deploy-allow-file "$ALLOW_FILE" \
    --status-file "$STATUS_FILE"
...
assert_status_env() {
  ...
  actual="$(jq -r '.entries[0].env // empty' "$STATUS_FILE" 2>/dev/null || true)"
  if [[ "$actual" == "$expected" ]]; then
    test_pass "$label"
  else
    test_fail "$label"
    printf '    expected env %s, got %s\n' "$expected" "${actual:-<missing>}" >&2
    printf '    status file:\n%s\n' "$(cat "$STATUS_FILE" 2>/dev/null || true)" >&2
  fi
}
...
assert_status_env "dev" "status JSON carries manifest env"
...
test_start "14.5" "object restore pin passes volid to restore-from-pbs"
reset_fixture_state
write_manifest 303 vault_dev
write_object_pin 303 'pbs-nas:backup/vm/303/2026-04-12T18:30:00Z'
run_capture run_restore_before_start
assert_exit 0 "object pinned restore succeeds"
assert_file_contains "$RESTORE_LOG" "--target 303 --force --backup-id pbs-nas:backup/vm/303/2026-04-12T18:30:00Z --leave-stopped" "restore-from-pbs receives object pin volid and leave-stopped"
assert_status_value "restored" "status JSON records restored for object pin"

test_start "14.5a" "manifest-only object pin fallback passes only volid to restore-from-pbs"
reset_fixture_state
write_manifest_object_pin 303 vault_dev 'pbs-nas:backup/vm/303/2026-04-12T18:30:00Z'
run_capture run_restore_before_start_manifest_only
assert_exit 0 "manifest-only object pinned restore succeeds"
assert_file_contains "$RESTORE_LOG" "--target 303 --force --backup-id pbs-nas:backup/vm/303/2026-04-12T18:30:00Z --leave-stopped" "manifest-only fallback passes object pin volid"
assert_file_lacks "$RESTORE_LOG" '"volid"' "manifest-only fallback does not pass JSON object as backup-id"
assert_status_value "restored" "status JSON records restored for manifest-only object pin"

test_start "14.6" "no backup without first-deploy approval fails closed"
reset_fixture_state
...
test_start "14.7" "no backup with first-deploy approval records empty-vdb approval"
reset_fixture_state
write_manifest 403 vault_prod
export STUB_PBS_CONTENT='[]'
...
test_start "14.8" "restore failure exits non-zero and records failed status"
reset_fixture_state
write_manifest 303 vault_dev
write_pin 303 'pbs-nas:backup/vm/303/2026-04-12T18:30:00Z'
...
test_start "14.9" "running VM in manifest aborts before restore"
reset_fixture_state
write_manifest 303 vault_dev
write_pin 303 'pbs-nas:backup/vm/303/2026-04-12T18:30:00Z'
...
test_start "14.10" "pre-existing HA resource aborts before restore"
reset_fixture_state
write_manifest 303 vault_dev
write_pin 303 'pbs-nas:backup/vm/303/2026-04-12T18:30:00Z'
...
test_start "14.11" "PBS query failure aborts before first-deploy approval"
reset_fixture_state
write_manifest 303 vault_dev
...
test_start "14.12" "missing pbs-nas storage honors first-deploy approval"
reset_fixture_state
write_manifest 403 vault_prod
export FIRST_DEPLOY_ALLOW_VMIDS=403
...
test_start "14.13" "pvesm status failure aborts before first-deploy approval"
reset_fixture_state
write_manifest 303 vault_dev
export FIRST_DEPLOY_ALLOW_VMIDS=303
