...
#   - later missed `renew_hook` after certbot renew persisted the deploy hook
...
rewrite_conf_fixture() {
  local file="$1" key="$2" value="$3" tmp
  tmp="$(mktemp "${TMP_DIR}/rewrite.XXXXXX")"
...
repair_renew_hook_fixture() {
  local file="$1" expected="$2" current_renew_hook
  current_renew_hook=$(extract_conf_fixture "$file" "renew_hook")
  if [[ "$current_renew_hook" != "$expected" ]]; then
    rewrite_conf_fixture "$file" "renew_hook" "$expected"
  fi
}
...
test_start "4a" "heredoc contains vault-backed cert-sync renew_hook"
if grep -qE '^renew_hook[[:space:]]*=[[:space:]]*\$\{certSyncTool\}/bin/cert-sync' <<< "${heredoc}"; then
  test_pass "renew_hook = cert-sync present"
else
  test_fail "heredoc missing renew_hook = cert-sync — R1.3 will fail after native renewals"
fi

test_start "4b" "initial certbot issuance includes vault-backed cert-sync deploy hook"
initial_issue_block=$(awk '
  /certbot certonly \\/ { in_block=1 }
  in_block              { print }
  in_block && /--domain "\$FQDN"/ { exit }
' "${CERTBOT_NIX}")
if grep -Fq -- '--deploy-hook' <<< "${initial_issue_block}" &&
   grep -Fq -- '${certSyncTool}/bin/cert-sync' <<< "${initial_issue_block}"; then
  test_pass "certbot certonly persists cert-sync as a deploy hook"
else
  test_fail "certbot certonly is missing the cert-sync deploy hook"
fi

test_start "4c" "certbot repair path no longer rewrites persisted renew_hook"
if ! grep -Eq 'EXPECTED_RENEW_HOOK|repair_renew_hook' "${CERTBOT_NIX}"; then
  test_pass "renew_hook repair function is absent under CLI-override design"
else
  test_fail "renew_hook repair function should be deleted under CLI-override design"
fi

test_start "4d" "certbot repair extract_conf preserves values containing equals signs"
if grep -Fq 'sub(/^[^=]*=/, "", $0)' "${CERTBOT_NIX}"; then
  test_pass "extract_conf strips only the first key=value separator"
else
  test_fail "extract_conf can still truncate values containing '='"
fi
...
#   ${certSyncTool} — nix-store path of the cert-sync script
...
certSyncTool="/nix/store/STUB-cert-sync"
...
test_start "8" "T4: Vault-backed repair ordering still runs after cert-restore"
if grep -Fq '++ lib.optionals vaultBackedCertStorage [ "cert-restore.service" ]' "${CERTBOT_NIX}" &&
   grep -Fq 'before = [ "certbot-repair-persisted-state.service" "certbot-initial.service" ];' "${CERTBOT_NIX}"; then
  test_pass "certbot repair remains ordered after cert-restore on Vault-backed hosts"
else
  test_fail "certbot repair/cert-restore ordering changed"
fi

test_start "9" "T5: Vault-restored stale manual hooks are advisory to D2 predicate"
renewability_stub="${TMP_DIR}/stub-certbot.sh"
cat > "${renewability_stub}" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
cat <<'OUT'
Found the following certs:
  Certificate Name: vault.prod.wuertele.com
    Domains: vault.prod.wuertele.com
    Expiry Date: 2026-10-01 00:00:00+00:00 (VALID: 87 days)
    Certificate Path: /etc/letsencrypt/live/vault.prod.wuertele.com/fullchain.pem
OUT
EOF
chmod +x "${renewability_stub}"
set +e
predicate_output="$(CERTBOT_BIN="${renewability_stub}" bash "${REPO_ROOT}/framework/scripts/certbot-renewability.sh" --cert-name vault.prod.wuertele.com 2>&1)"
predicate_rc=$?
set -e
if [[ "${predicate_rc}" -eq 0 ]] && grep -Fq 'days_remaining=87' <<< "${predicate_output}"; then
  test_pass "D2 predicate reports restored healthy lineage as renewable even if persisted hooks are stale"
else
  test_fail "D2 predicate did not accept restored healthy lineage"
  printf '    rc=%s\n%s\n' "${predicate_rc}" "${predicate_output}" >&2
fi
