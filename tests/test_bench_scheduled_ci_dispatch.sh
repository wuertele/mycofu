...
job_matches() {
  local job="$1" source="$2" branch="$3" bench_key="$4" reclaim_image_store="${5:-}"
  ...
  while true; do
    ...
    case "$rule_if" in
      '$BENCH_SCHEDULE_KEY')
        ...
        ;;
      '$CI_PIPELINE_SOURCE == "schedule" && $RECLAIM_IMAGE_STORE == "1"')
        if [[ "$source" == "schedule" && "$reclaim_image_store" == "1" ]]; then
          [[ "$when" == "never" ]] && return 1
          return 0
        fi
        ;;
      '$CI_PIPELINE_SOURCE == "web" && $RECLAIM_IMAGE_STORE == "1"')
        if [[ "$source" == "web" && "$reclaim_image_store" == "1" ]]; then
          [[ "$when" == "never" ]] && return 1
          return 0
        fi
        ;;
...
matching_jobs() {
  local source="$1" branch="$2" bench_key="$3" reclaim_image_store="${4:-}"
  while IFS= read -r job; do
    [[ -n "$job" ]] || continue
    if job_matches "$job" "$source" "$branch" "$bench_key" "$reclaim_image_store"; then
      printf '%s\n' "$job"
    fi
  done < <(job_names)
}
...
if [[ "$(yq -r '."bench:scheduled".allow_failure' "$CI_YML")" == "true" ]] &&
   ...
   [[ "$(yq -r '."bench:scheduled".rules[0].if' "$CI_YML")" == '$CI_PIPELINE_SOURCE == "schedule" && $RECLAIM_IMAGE_STORE == "1"' ]] &&
   [[ "$(yq -r '."bench:scheduled".rules[0].when' "$CI_YML")" == "never" ]] &&
   [[ "$(yq -r '."bench:scheduled".rules[1].if' "$CI_YML")" == '$CI_PIPELINE_SOURCE == "schedule" && $BENCH_SCHEDULE_KEY == "bench-nightly"' ]]; then
...
while IFS= read -r job; do
  ...
  second_if="$(yq -r ".\"${job}\".rules[1].if // \"\"" "$CI_YML")"
  second_when="$(yq -r ".\"${job}\".rules[1].when // \"\"" "$CI_YML")"
  if [[ "$job" == "reclaim:image-store" ]]; then
    if [[ "$first_if" != '$BENCH_SCHEDULE_KEY' || "$first_when" != "never" ]]; then
      missing+=("$job")
    fi
  elif [[ "$second_if" != '$BENCH_SCHEDULE_KEY' || "$second_when" != "never" ]]; then
    missing+=("$job")
  fi
done < <(job_names)
...
test_start "7a" "every non-reclaim job short-circuits reclaim schedules"
missing=()
while IFS= read -r job; do
  [[ "$job" == "reclaim:image-store" ]] && continue
  first_if="$(yq -r ".\"${job}\".rules[0].if // \"\"" "$CI_YML")"
  first_when="$(yq -r ".\"${job}\".rules[0].when // \"\"" "$CI_YML")"
  if [[ "$first_if" != '$CI_PIPELINE_SOURCE == "schedule" && $RECLAIM_IMAGE_STORE == "1"' || "$first_when" != "never" ]]; then
    missing+=("$job")
  fi
done < <(job_names)
if [[ "${#missing[@]}" -eq 0 ]]; then
  test_pass "all non-reclaim jobs have the universal reclaim short-circuit"
else
  test_fail "jobs missing reclaim schedule short-circuit: ${missing[*]}"
fi

test_start "7b" "reclaim schedule runs only reclaim:image-store"
matches="$(matching_jobs schedule dev "" 1 | sort | tr '\n' ' ')"
if [[ "$matches" == "reclaim:image-store " ]]; then
  test_pass "reclaim schedule excludes every non-reclaim job"
else
  test_fail "reclaim schedule matched unexpected jobs: $matches"
fi
