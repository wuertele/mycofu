...
if [[ "$plan_json" == *preboot-restore-plan-bulk.json ]]; then
  jq -e '.resource_changes == []' "$plan_json" >/dev/null
  exit 0
fi
...
case "${1:-}" in
  control-plane-modules)
    echo "module.cicd"
    [[ "${STUB_INCLUDE_PBS:-0}" -eq 0 ]] || echo "module.pbs"
    ;;
  classes)
...
case "${1:-}" in
  ...
  state)
    ...
    ;;
  plan)
    if [[ "${STUB_PLAN_ERROR_KIND:-current}" != clean && " $* " == *" -target=module.cicd "* ]] && [[ ! -f "${STATE_REMOVED_MARKER}" ]]; then
      prevent_destroy_stderr
      exit 1
    fi
    for arg in "$@"; do
...
test_start "743.4" "--recreate enters full atomic ladder on a clean control-plane plan; PBS stays normal"
set +e
STUB_INCLUDE_PBS=1 run_fixture_case "$FIXTURE_REPO" "$SHIM_DIR" "$TRACE_LOG" "$STATE_REMOVED_MARKER" clean \
  --recreate --scope vm=cicd,pbs
FORCE_RC=$?
set -e
if [[ "$FORCE_RC" -eq 0 ]] && python3 - "$TRACE_LOG" "$FIXTURE_REPO" <<'PY'
import json, sys
from pathlib import Path
events = Path(sys.argv[1]).read_text().splitlines()
def position(prefix):
    return next(i for i, line in enumerate(events) if line.startswith(prefix))
assert any("cicd: recreate requested — entering atomic ladder" in s for s in events)
assert not any("tofu-wrapper plan -target=module.cicd -detailed-exitcode" in s for s in events)
assert not any("tofu-wrapper plan" in s and "atomic-precheck" in s for s in events)
assert any("pbs: excluded from forced recreation by design" in s for s in events)
assert any("tofu-wrapper plan -target=module.pbs -detailed-exitcode" in s for s in events)
assert not any("pbs: recreate requested" in s or "qm destroy 190" in s for s in events)
assert position("vdb_park_batch ") < position("ssh qm destroy 150") < position("tofu-wrapper state rm ")
assert position("tofu-wrapper apply -target=module.cicd -var=start_vms=false") < position("vdb_adopt_batch ")
assert position("vdb_adopt_batch ") < position("restore-before-start.sh all") < position("tofu-wrapper apply -target=module.cicd -var=start_vms=true")
assert position("tofu-wrapper apply -target=module.cicd -var=start_vms=true") < position("converge_run_all")
manifest = json.loads((Path(sys.argv[2]) / "build/preboot-restore-atomic-cicd.json").read_text())
assert len(manifest["entries"]) == 1, manifest
assert manifest["entries"][0]["reason"] == "replace", manifest
plan = json.loads((Path(sys.argv[2]) / "build/preboot-restore-plan-atomic-precheck-cicd.json").read_text())
assert plan["resource_changes"][0]["change"]["actions"] == ["delete", "create"]
assert not any("-replace=" in s for s in events if s.startswith("tofu-wrapper "))
PY
then
  test_pass "clean-plan control-plane force uses intent manifest, park/destroy/adopt/restore/start/converge; PBS excluded"
else
  test_fail "forced atomic ladder did not complete safely"
  cat "$TRACE_LOG" >&2
fi
