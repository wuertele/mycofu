...
chmod +x "${FIXTURE_REPO}/framework/scripts/backup-now.sh" "${FIXTURE_REPO}/framework/scripts/vm-scope.sh"
...
append_backup_record() {
  ...
  local ctime="$2"
  local volid="$3"
  # #591: BACKUP_NOW_MARKER extracted from vzdump's --notes-template arg.
  # Passed in by the vzdump case handlers below so the fabricated PBS
  # record carries the identity marker the post-vzdump gate expects.
  local marker="${4:-}"
  local size=1048576
  ...
  jq \
    ...
    --arg notes "$marker" \
    '. + [{"vmid":$vmid,"ctime":$ctime,"volid":$volid,"size":$size,"verification":{"state":$state},"notes":$notes}]' \
    "$STUB_PBS_JSON_FILE" > "${STUB_PBS_JSON_FILE}.tmp"
  mv "${STUB_PBS_JSON_FILE}.tmp" "$STUB_PBS_JSON_FILE"
}

# #591: pull --notes-template value out of a vzdump command line so
# subsequent PBS records carry the run's identity marker.
extract_notes_template_marker() {
  local cmdline="$1"
  printf '%s\n' "$cmdline" | sed -n "s/.*--notes-template '\([^']*\)'.*/\1/p"
}
...
case "$cmd" in
  *'pvesm status'*)
  ...
  "date -u +%s")
    ...
    # Option 2). Return a value 10 s in the past so the subsequent
    # vzdump ctime (which append_backup_record writes via `date -u +%s`)
    # is strictly greater than pre_vzdump_epoch, satisfying the strict
    # `>` freshness gate in verify_backup_landed_in_pbs /
    # verify_backup_pins.
    printf '%s\n' "$(( $(date -u +%s) - 10 ))"
    exit 0
    ;;
  ...
  *'/storage/pbs-nas/content --output-format json'*)
  ...
  vzdump\ 303\ --storage\ pbs-nas*)
    marker="$(extract_notes_template_marker "$cmd")"
    append_backup_record 303 "$(date -u +%s)" 'pbs-nas:backup/vm/303/2026-04-12T18:30:00Z' "$marker"
    exit 0
    ;;
  vzdump\ 150\ --storage\ pbs-nas*)
    marker="$(extract_notes_template_marker "$cmd")"
    append_backup_record 150 "$(date -u +%s)" 'pbs-nas:backup/vm/150/2026-04-12T18:30:01Z' "$marker"
    exit 0
    ;;
  vzdump\ 403\ --storage\ pbs-nas*)
    marker="$(extract_notes_template_marker "$cmd")"
    append_backup_record 403 "$(date -u +%s)" 'pbs-nas:backup/vm/403/2026-04-12T18:30:02Z' "$marker"
    exit 0
    ;;
  *)
...
if [[ "$(jq -r '.pins["303"].volid' "${FIXTURE_REPO}/build/pin-dev.json")" == 'pbs-nas:backup/vm/303/2026-04-12T18:30:00Z' ]]; then
...
if [[ "$(jq -r '.pins["150"].volid' "${FIXTURE_REPO}/build/pin-all.json")" == 'pbs-nas:backup/vm/150/2026-04-12T18:30:01Z' ]]; then
