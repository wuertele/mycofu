...
# measure network throughput instead. Both substituters AND
# extra-substituters must be cleared: a globally-configured
# extra-substituters would otherwise still contribute paths (#265).
# Strip comment lines before matching so the ratchet asserts the flags on the
# actual exec invocation, not the explanatory comment above it (the comment
# also mentions `substituters ""`).
if grep -vE '^[[:space:]]*#' benchmarks/application/run-target.sh | grep -qE -- '--option substituters ""' \
   && grep -vE '^[[:space:]]*#' benchmarks/application/run-target.sh | grep -qE -- '--option extra-substituters ""'; then
  pass "run-target.sh scratch measure disables both substituters and extra-substituters"
else
  fail "run-target.sh scratch measure must pass --option substituters \"\" AND --option extra-substituters \"\" so deleted outputs are rebuilt locally rather than fetched from a binary cache"
fi
...
# --- measure scratch ---
# The nix shim logs $*, so the two empty "" values collapse to whitespace:
# `... --option substituters  --option extra-substituters ` (extra space where
# each empty value sits). Anchor on that so a NON-empty value — e.g. a leaked
# `--option extra-substituters https://cache` — puts a URL between the flag and
# EOL and fails here, not only at the source-level static ratchet above. This
# is the runtime assertion that both substituter lists are actually cleared.
run_with_nix_shim --internal-measure scratch fake-target
if grep -qF 'build .#fake-target --no-link --no-eval-cache' "${RUN_TARGET_NIX_LOG}" \
   && grep -qE -- '--option substituters[[:space:]]+--option extra-substituters[[:space:]]*$' "${RUN_TARGET_NIX_LOG}" \
   && ! grep -- '--rebuild' "${RUN_TARGET_NIX_LOG}" >/dev/null \
   && ! grep -- '--eval-cache false' "${RUN_TARGET_NIX_LOG}" >/dev/null; then
  pass "run-target.sh measure scratch invokes nix build with --no-eval-cache and disables substituters + extra-substituters"
else
...
# ----------------------------------------------------------------------
# benchmarks/application/run-target.sh: space-path hyperfine wiring (#604)
# ----------------------------------------------------------------------
# The #555 source-level single-quote grep and the #263 argv-wiring assertions
# above confirm run-target.sh single-quotes ${SCRIPT_DIR}/run-target.sh, but
# both run from the repo's own space-free path — the constructed hyperfine
# argv is never asserted with a SCRIPT_DIR that ACTUALLY contains a space, the
# adversarial path the original child-shell re-tokenization bug needed.
#
# Copy run-target.sh and the lib it sources into a "with space/" tree and run
# it through the same hermetic hyperfine + nix shims (no real hyperfine
# required — the shims are provided on PATH here just as in the wiring test
# above, so this runs in CI regardless of hyperfine availability). This is an
# argv-level assertion, NOT an execution-level one: the hyperfine shim only
# logs argv, it does not itself run `sh -c`. But asserting that ARGV[6]/ARGV[9]
# carry the single-quoted space-containing path as ONE argument is exactly the
# property that makes hyperfine's real `sh -c` re-tokenize the path as a single
# token — a refactor that mis-nests the quoting (e.g. escapes so sh -c would
# see literal \' marks) alters ARGV[6] and fails this assertion.
SPACE_WIRING_ROOT="${TMP_DIR}/with space"
mkdir -p "${SPACE_WIRING_ROOT}/application" "${SPACE_WIRING_ROOT}/lib"
cp benchmarks/application/run-target.sh "${SPACE_WIRING_ROOT}/application/run-target.sh"
cp -R benchmarks/lib/. "${SPACE_WIRING_ROOT}/lib/"
chmod +x "${SPACE_WIRING_ROOT}/application/run-target.sh"
SPACE_RUN_TARGET="${SPACE_WIRING_ROOT}/application/run-target.sh"
...
HYPERFINE_LOG="${HYPERFINE_LOG}" \
  ...
  "${SPACE_RUN_TARGET}" \
    ...
    --target bench-fake \
    --runs 5 \
    --output "${SPACE_WIRING_ROOT}/out-scratch.json"

if grep -qxF "ARGV[6]='${SPACE_RUN_TARGET}' --internal-prepare scratch bench-fake" "${HYPERFINE_LOG}" \
   && grep -qxF "ARGV[9]='${SPACE_RUN_TARGET}' --internal-measure scratch bench-fake" "${HYPERFINE_LOG}"; then
  pass "run-target.sh hyperfine wiring: single-quoted SCRIPT_DIR survives a space in the path (#604)"
else
  fail "run-target.sh hyperfine wiring re-tokenizes a space-containing SCRIPT_DIR: $(cat "${HYPERFINE_LOG}")"
fi
