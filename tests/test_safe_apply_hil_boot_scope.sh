...
# #706: a dry run must be READ-ONLY. Before this fix the generation block ran
# unconditionally, so `safe-apply <env> --dry-run` overwrote the operator's real
# site/gatus/config.yaml (and made the generator's network call) as a side
# effect. This test runs the REAL safe-apply.sh against the real repo, so it is
# the regression guard: snapshot the artifact's state (present? content hash?)
# around a dry-run and assert it is untouched.
GATUS_CFG="${REPO_ROOT}/site/gatus/config.yaml"
snapshot_gatus_cfg() {
  if [[ -f "$GATUS_CFG" ]]; then
    shasum "$GATUS_CFG" | awk '{print $1}'
  else
    echo "ABSENT"
  fi
}

test_start "s706.1" "safe-apply --dry-run does not mutate site/gatus/config.yaml"
BEFORE_STATE="$(snapshot_gatus_cfg)"
run_safe_apply dev
AFTER_STATE="$(snapshot_gatus_cfg)"
if [[ "$SAFE_APPLY_RC" -eq 0 && "$BEFORE_STATE" == "$AFTER_STATE" ]] && \
   grep -Fq 'dry-run is read-only' <<< "$SAFE_APPLY_OUTPUT"; then
  test_pass "dry-run left site/gatus/config.yaml unchanged (${BEFORE_STATE})"
else
  test_fail "dry-run mutated site/gatus/config.yaml (before=${BEFORE_STATE} after=${AFTER_STATE})"
  printf '%s\n' "$SAFE_APPLY_OUTPUT" >&2
fi
