...
# Sprint 049 MR-3 hermeticity: unset SOPS_AGE_KEY_FILE so rebuild-cluster.sh
# uses the fixture-created operator.age.key (see rebuild_fixture_helpers.sh).
...
reset_case() {
  export GIT_DEPLOY_SCRIPT_DIR="${REPO_ROOT}/framework/scripts"
  export GIT_DEPLOY_JQ_BIN="${REAL_JQ}"
...
test_start "RBG-8a" "scope classification parser failure fails closed"
reset_case
checkout_ref dev-no-config
SCOPE="control-plane"
export GIT_DEPLOY_JQ_BIN="${TMP_DIR}/missing-jq"
if run_context; then
  test_fail "missing jq should fail scope classification before branch safety"
else
  test_pass "missing jq failed closed"
fi
assert_eq "" "${SCOPE_IMPACT}" "missing jq leaves no derived impact"
assert_contains "${SCOPE_IMPACT_REASON}" "scope classification JSON parse failed" "missing jq reason names parser failure"
assert_contains "${SCOPE_IMPACT_REASON}" "missing-jq" "missing jq stderr is surfaced"

test_start "RBG-8b" "missing impact from helper fails closed and surfaces helper stderr"
reset_case
checkout_ref dev-no-config
SCOPE="control-plane"
STUB_SCOPE_DIR="${TMP_DIR}/stub-scope-helper"
mkdir -p "$STUB_SCOPE_DIR"
cat > "${STUB_SCOPE_DIR}/vm-scope.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
printf '%s\n' '{"reason":"fixture omitted impact","unknown_targets":""}'
printf '%s\n' 'fixture helper stderr' >&2
EOF
chmod +x "${STUB_SCOPE_DIR}/vm-scope.sh"
export GIT_DEPLOY_SCRIPT_DIR="$STUB_SCOPE_DIR"
if run_context; then
  test_fail "helper output without impact should fail closed"
else
  test_pass "helper output without impact failed closed"
fi
assert_eq "" "${SCOPE_IMPACT}" "missing impact leaves no derived impact"
assert_contains "${SCOPE_IMPACT_REASON}" "missing impact" "missing impact reason is explicit"
assert_contains "${SCOPE_IMPACT_REASON}" "fixture helper stderr" "helper stderr is surfaced"
