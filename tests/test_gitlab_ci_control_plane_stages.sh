...
test_start "3.4f" "gitlab jobs declare the retry allow-list (#711)"
# The gitlab control-plane jobs drive an in-place switch of the gitlab VM they
# report through; a reaped/unreportable attempt must re-run onto the idempotent
# live==built skip path rather than fail the stage chain (RCA #707 note 6376).
# They mirror the cicd siblings' typed retry.when allow-list (see 3.4e).
DEV_GITLAB_RETRY="$(job_value 'deploy:control-plane:gitlab:dev' '.retry.when[]')"
PROD_GITLAB_RETRY="$(job_value 'deploy:control-plane:gitlab:prod' '.retry.when[]')"
if grep -qx 'runner_system_failure' <<< "${DEV_GITLAB_RETRY}" && \
   grep -qx 'stuck_or_timeout_failure' <<< "${DEV_GITLAB_RETRY}" && \
   grep -qx 'job_execution_timeout' <<< "${DEV_GITLAB_RETRY}" && \
   grep -qx 'runner_system_failure' <<< "${PROD_GITLAB_RETRY}" && \
   grep -qx 'stuck_or_timeout_failure' <<< "${PROD_GITLAB_RETRY}" && \
   grep -qx 'job_execution_timeout' <<< "${PROD_GITLAB_RETRY}"; then
  test_pass "gitlab jobs include the typed retry.when allow-list"
else
  test_fail "gitlab jobs include the typed retry.when allow-list"
fi
