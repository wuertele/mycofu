...
if [[ "${remote}" == *"/etc/pve/nodes/"* ]]; then
  ...
  for dir in "${FIXTURE_DIR}"/node-*; do
    [[ -d "${dir}" ]] || continue
    # Extract IP from dir name.
    dip="${dir##*/node-}"
    # Resolve IP to name for the simulated path.
    dname=""
    case "${dip}" in
      10.0.0.51) dname="pve01" ;;
      10.0.0.52) dname="pve02" ;;
      10.0.0.53) dname="pve03" ;;
    esac
    for cfg in "${dir}"/qm-*.cfg; do
      [[ -f "${cfg}" ]] || continue
      vmid="${cfg##*/qm-}"
      vmid="${vmid%.cfg}"
      # Simulate grep -r output: /etc/pve/nodes/<node>/qemu-server/<vmid>.conf:<line>
      while read -r line; do
        echo "/etc/pve/nodes/${dname}/qemu-server/${vmid}.conf:${line}"
      done < "${cfg}"
      found=1
    done
  done
  # grep -r's empty-input rc is 1; mirror that.
  [[ "${found}" -eq 1 ]] && exit 0 || exit 1
fi

if [[ "${remote}" == *"cat /etc/pve/replication.cfg"* ]]; then
  if [[ -f "${FIXTURE_DIR}/replication.cfg" ]]; then
    cat "${FIXTURE_DIR}/replication.cfg"
  fi
  exit 0
fi
...
setup_case "c1"
run_cleanup
[[ "$(exit_code)" -eq 0 ]] && test_pass "empty cluster exits 0" || (echo "Output log for C1:"; cat "${FIXTURE_DIR}/output.log") || test_fail "empty cluster exits 0"
...
# replication config and NOT destroy the replica.
...
# Add replication jobs
cat > "${FIXTURE_DIR}/replication.cfg" <<EOF
replication: 150-0
	guest: 150
	target: pve01
replication: 150-1
...
# The size filter alone would destroy this. The node-aware replication
# check prevents destruction because the zvol IS replicated to this node.
...
test_start "C18" "Vaccine: orphan on destination node destroyed even if active on source node"
setup_case "c18"
# VM 160 is owned by pve01 (Node A). It has canonical cidata there.
# pve02 (Node B) has an orphan cidata from a previous run.
add_zvol 10.0.0.51 vmstore/data/vm-160-cloudinit 18.5K
add_qm_config 10.0.0.51 160 "ide2: vmstore:vm-160-cloudinit,media=cdrom"
add_zvol 10.0.0.52 vmstore/data/vm-160-cloudinit 18.5K
run_cleanup
[[ "$(exit_code)" -eq 0 ]] && test_pass "exits 0" || test_fail "exits 0"
# New node-aware logic should destroy the orphan on Node B (pve02).
# The old cluster-wide logic would have skipped it.
destroy_log_has "vmstore/data/vm-160-cloudinit" && test_pass "destroys the orphan on destination node" || test_fail "did NOT destroy orphan on destination"
grep -q "10.0.0.52" "${FIXTURE_DIR}/destroy.log" && test_pass "destroyed on correct node" || test_fail "destroyed on WRONG node"

test_start "C19" "--node restricts zvol scan to one node only"
setup_case "c19"
add_zvol 10.0.0.51 vmstore/data/vm-301-cloudinit 18.5K
add_zvol 10.0.0.52 vmstore/data/vm-302-cloudinit 18.5K
run_cleanup --node pve02
[[ "$(exit_code)" -eq 0 ]] && test_pass "exits 0" || test_fail "exits 0"
[[ "$(destroy_count)" == "1" ]] && test_pass "destroys exactly one scoped orphan" || test_fail "destroys exactly one scoped orphan"
destroy_log_has "10.0.0.52 vmstore/data/vm-302-cloudinit" && test_pass "destroys orphan on requested node" || test_fail "destroys orphan on requested node"
! destroy_log_has "vmstore/data/vm-301-cloudinit" && test_pass "does not scan/destroy other node" || test_fail "does not scan/destroy other node"

test_start "C20" "--node keeps cluster-wide attached-set failover unfiltered"
setup_case "c20"
add_zvol 10.0.0.52 vmstore/data/vm-303-cloudinit 18.5K
# If --node incorrectly narrows the attached-set read to pve02, this fails
# before the zvol scan. Correct behavior reads pmxcfs from pve01, then scans
# only pve02's zvols.
mark_node_unreachable 10.0.0.52
run_cleanup --node pve02
[[ "$(exit_code)" -eq 0 ]] && test_pass "exits 0 using another node for attached-set read" || test_fail "exits 0 using another node for attached-set read"
[[ "$(destroy_count)" == "1" ]] && test_pass "still scans requested node" || test_fail "still scans requested node"
destroy_log_has "10.0.0.52 vmstore/data/vm-303-cloudinit" && test_pass "destroys requested-node orphan" || test_fail "destroys requested-node orphan"

test_start "C21" "--vmid restricts destruction to one VMID"
setup_case "c21"
add_zvol 10.0.0.51 vmstore/data/vm-304-cloudinit 18.5K
add_zvol 10.0.0.52 vmstore/data/vm-305-cloudinit 18.5K
run_cleanup --vmid 305
[[ "$(exit_code)" -eq 0 ]] && test_pass "exits 0" || test_fail "exits 0"
[[ "$(destroy_count)" == "1" ]] && test_pass "destroys exactly one VMID-scoped orphan" || test_fail "destroys exactly one VMID-scoped orphan"
destroy_log_has "vmstore/data/vm-305-cloudinit" && test_pass "destroys requested VMID" || test_fail "destroys requested VMID"
! destroy_log_has "vmstore/data/vm-304-cloudinit" && test_pass "leaves other VMID alone" || test_fail "leaves other VMID alone"

test_start "C22" "default full sweep still scans all nodes"
setup_case "c22"
add_zvol 10.0.0.51 vmstore/data/vm-306-cloudinit 18.5K
add_zvol 10.0.0.52 vmstore/data/vm-307-cloudinit 18.5K
run_cleanup
[[ "$(exit_code)" -eq 0 ]] && test_pass "exits 0" || test_fail "exits 0"
[[ "$(destroy_count)" == "2" ]] && test_pass "default destroys both node orphans" || test_fail "default destroys both node orphans"
output_has "Summary: 2 orphan(s) found, 2 destroyed" && test_pass "default summary unchanged for full sweep" || test_fail "default summary unchanged for full sweep"
