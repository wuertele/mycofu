...
assert_variable_default_true() {
  ...
  if grep -Eq "type[[:space:]]*=[[:space:]]*bool" <<< "$block" &&
     grep -Eq "default[[:space:]]*=[[:space:]]*true" <<< "$block"; then
    return 0
  fi
  return 1
}
...
for file in \
  ...
  if ! grep -Eq "started[[:space:]]*=[[:space:]]*var\.start_vms" "$file"; then
...
if grep -Eq "count[[:space:]]*=[[:space:]]*var\.ha_enabled && var\.register_ha \? 1 : 0" \
...
if [[ "$start_count" -eq 19 && "$ha_count" -eq 19 ]]; then
  test_pass "root passes controls to 19 NixOS module instances"
else
  test_fail "root pass-through count mismatch: start_vms=${start_count}, register_ha=${ha_count}, want 19"
fi
...
if grep -Fq "vendor-appliance exception" framework/tofu/modules/pbs/main.tf &&
   grep -Eq "started[[:space:]]*=[[:space:]]*true" framework/tofu/modules/pbs/main.tf &&
   grep -Eq "count[[:space:]]*=[[:space:]]*var\.ha_enabled \? 1 : 0" framework/tofu/modules/pbs/main.tf; then
