...
# G11 (#42/#280): the root-caller assignment now flows through coalesce()
# instead of a bare try(..., null) to prevent the null-cascade at the
# proxmox provider boundary. Accept both shapes:
#   vdb_size_gb = try(local.app_workstation.data_disk_size_dev, ...)
#   vdb_size_gb = coalesce(try(local.app_workstation.data_disk_size_dev, ...), ...)
if grep -Eq 'vdb_size_gb[[:space:]]*=[[:space:]]*var\.vdb_size_gb' "${WORKSTATION_TF}" && \
   grep -Eq 'vdb_size_gb[[:space:]]*=[[:space:]]*(coalesce\()?try\(local\.app_workstation\.data_disk_size_dev' "${ROOT_TF}" && \
   grep -Eq 'vdb_size_gb[[:space:]]*=[[:space:]]*(coalesce\()?try\(local\.app_workstation\.data_disk_size_prod' "${ROOT_TF}" && \
