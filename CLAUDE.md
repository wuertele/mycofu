...
- `.claude/rules/` — operational rules. Start at `.claude/rules/README.md`,
  which maps every rule by kind and audience and defines per-role reading
  lists; read every rule on your role's list before starting work, and the
  area-specific rules for whatever your diff touches
- `docs/proposals/` — analyses that were never operator-ratified
  (e.g. `docs/proposals/roadmap.md`). Evidence to verify, never authority
...
- Validate: `framework/scripts/validate.sh` (must report 0 failed)
- Decrypt secrets: `sops -d site/sops/secrets.yaml`
- Replication cleanup: `framework/scripts/configure-replication.sh "*" [--env <dev|prod>]`
  (scope the policy-off prune to one env with `--env`; bare `"*"` prunes
  cluster-wide and is the full-cluster / DR form)
...
- `framework/tofu/root/main.tf` — OpenTofu root module (site-level resource config)
...
3. Replication: `configure-replication.sh "*" [--env <dev|prod>]` (scope the
   prune to one env with `--env`; bare `"*"` is cluster-wide / DR)
4. Runner: `register-runner.sh` if GitLab OR cicd was recreated
   (post-SPRINT-049-MR-3, cicd recreation loses persistent secrets under
   `/var/lib/mycofu-secrets/`; use `--deliver-secrets` for delivery-only)
