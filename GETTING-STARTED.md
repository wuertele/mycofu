...
1.5. Apply NVMe firmware-quirk kernel parameters and reboot nodes if
...
7. Deploy all VMs via the stopped OpenTofu apply
7.5. Install and configure PBS (backup server, connects to NAS NFS)
7.6. Restore backup-backed vdb disks before VM start, using the
...
7.7. Run the start/register OpenTofu apply
8. Load DNS zones
...
The rebuild runs validation automatically (step 16). If you
