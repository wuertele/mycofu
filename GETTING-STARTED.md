...
| macOS Local Network privacy disabled or verified for CLI tools | macOS only | No — `tofu` must reach the NAS PostgreSQL backend directly |
...
On macOS, Mycofu's workstation path requires command-line network clients to
connect directly to the management subnet. In particular, `tofu` connects to
the NAS PostgreSQL state backend and `glab` connects to the self-hosted GitLab
API. macOS Local Network privacy can NECP-block these clients with a misleading
`No route to host` error even when the route and firewall are correct.

Before first `tofu init` or `safe-apply.sh` from macOS, check the policy state:

```bash
framework/scripts/bootstrap-macos-local-network-tools.sh status
```

If it reports a blocking Network Privacy PathController/default rule, disable
the blocker locally:
...
The `apply` command creates a timestamped backup of
`/Library/Preferences/com.apple.networkextension.plist` and restarts the
NetworkExtension policy daemons. This is a workstation-wide Local Network
privacy policy change, not a Mycofu-scoped process allow. Rollback and
...
- `site/tofu/` — site-local OpenTofu artifacts (overrides.tf, image-versions.auto.tfvars, tfstate). The root module itself lives at `framework/tofu/root/`
...
The generic default is a manual Proxmox VE install on each node from the ISO:
...
AMT-equipped sites can automate this install instead of using USB. Add the
optional `regreen_enabled`, `amt_ip`, `amt_password_ref`,
`install_nic_driver`, and `install_disk_id_path` fields to the node records,
then generate SOPS before running the regreener because the installed root
password is read from `proxmox_api_password`.

```bash
framework/scripts/bootstrap-sops.sh
framework/scripts/regreen-cluster.sh --config site/config.yaml --dry-run
framework/scripts/regreen-cluster.sh --config site/config.yaml
```

Run the regreener from a Linux host or Linux CI runner that can reach the AMT
interfaces. MeshCmd runs on that executor as a pinned Nix host package; it is
not installed by hand on Proxmox nodes. See `framework/scripts/README.md` for
the three regreener primitives and first-light evidence to capture.
...
2. Prompts for the **Proxmox root password** (the one you set during manual
   install, or the one the regreener should install)
...
One command. No interaction. It runs the full bootstrap sequence, in order:

- Verify prerequisites (workstation tools, node reachability, NAS setup)
- Configure node networking (auto-discovers NICs, pins interfaces, deploys
  bridge/replication config — reboots nodes if naming changes are needed)
- Apply NVMe firmware-quirk kernel parameters and reboot nodes if
  needed (writes a drop-in for GRUB or `/etc/kernel/cmdline` for
  systemd-boot, refreshes the bootloader, and -- if the running
  kernel doesn't yet have the params -- automatically reboots each
  node sequentially. Runs BEFORE storage so the first ZFS data-pool
  operation lands on a patched kernel. See "Applying Kernel
  Parameters" in OPERATIONS.md and the 2026-05-03 retrospective for
  context.)
- Configure node storage (creates ZFS data pool on each node's data NVMe)
- Form the Proxmox cluster
- Configure storage snippets
- Build all VM images (NixOS, content-addressed)
- Deploy all VMs via the stopped OpenTofu apply
- Install and configure PBS (backup server, connects to NAS NFS)
- Restore backup-backed vdb disks before VM start, using the
  plan-derived manifest and pinned PBS backups. On an intentional first
  deploy with no PBS datastore, backup-backed VMIDs require explicit
  `FIRST_DEPLOY_ALLOW_VMIDS` approval for that run.
- Run the start/register OpenTofu apply
- Load DNS zones
- Acquire TLS certificates (Let's Encrypt for prod, Pebble for dev)
- Initialize and unseal Vault
- Configure Vault policies
- Set up ZFS replication
- Configure GitLab (push repo, create branches, protect `prod`)
- Register CI/CD runner
- Deploy sentinel Gatus on NAS, configure backup jobs, set up metrics
- Run validation suite
- Verify CI/CD pipeline
- Take immediate backups of all VMs with precious state

The script logs each step with its own label as it runs; that log, not this
list, is the authority on step identity.
...
The rebuild runs validation automatically in its final-validation step
(`validate.sh`). If you want to re-run it independently:
...
The rebuild's `configure-gitlab.sh` step already set up everything
...
```bash
# 1. Opt in via site/config.yaml
yq -i '.publish.github.enabled = true' site/config.yaml
yq -i '.github.remote_url = "git@github.com:<owner>/<repo>.git"' site/config.yaml

# 2. Add the workstation `github` remote (used by sync-to-main.sh fallback)
git remote add github git@github.com:<owner>/<repo>.git

# 3. Seed the pipeline's deploy key
ssh-keygen -t ed25519 -f /tmp/mycofu-github-deploy-key -N "" \
  -C "mycofu-cicd-publish"
cat /tmp/mycofu-github-deploy-key.pub
# Add the public key to GitHub deploy keys with write access, then:
...
If you do not need a public mirror, leave `publish.github.enabled: false`
(the scaffolded default). The prod pipeline's `publish:github` job will
self-skip and `github.remote_url` is optional.

The workstation `github` remote is only for the fallback
`framework/scripts/sync-to-main.sh` path. The prod pipeline publishes with the
Vault deploy key seeded above.
...
- **Enable Tailscale remote access** — if any VMs in `site/config.yaml`
  have `tailscale: true`, complete the Tailscale setup steps in your
  `site/bringup.md` before deploying:

  1. Create or log into your Tailscale account
  2. Install Tailscale on the operator workstation and join the tailnet
  3. Apply an ACL policy that lets a `tag:mycofu` auth key advertise the
     control-plane tag used by GitLab:

     ```json
     {
       "tagOwners": {
         "tag:mycofu":      ["autogroup:admin", "tag:mycofu"],
         "tag:mycofu-ctl":  ["autogroup:admin", "tag:mycofu"],
         "tag:mycofu-prod": ["autogroup:admin", "tag:mycofu"],
         "tag:mycofu-dev":  ["autogroup:admin", "tag:mycofu"]
       },
       "acls": [
         {"action": "accept", "src": ["autogroup:owner"], "dst": ["tag:mycofu:*"]},
         {"action": "accept", "src": ["tag:mycofu-ctl"], "dst": ["tag:mycofu:*"]},
         {"action": "accept", "src": ["tag:mycofu"], "dst": ["tag:mycofu:*"]}
       ]
     }
     ```

  4. Generate a reusable auth key at
     `https://login.tailscale.com/admin/settings/keys`
     with the tag `tag:mycofu`
  5. Store it in SOPS as `tailscale_auth_key`
  ...
  The auth key is a bootstrap secret delivered via CIDATA at
  `/run/secrets/tailscale/auth-key`. After the first successful join, the VM
  writes its node identity to Vault at
  `secret/tailscale/nodes/<domain>/<role>`, which preserves the same
  Tailscale identity across later VM recreations. See OPERATIONS.md
  "Tailscale Remote Access" for troubleshooting and key rotation.
...
**`rebuild-cluster.sh` fails at NAS prerequisites (the prerequisite-verification step):**
...
**`tofu`, `glab`, or Homebrew Python fails with `No route to host` on macOS:**
- Check for NECP policy drops:
...
- If the drop names `tofu`, `glab`, or Python, run:
...
- If `verify` says `nix` is missing, enter `nix develop` or add
...
**Backups fail at the immediate-backup step:**
