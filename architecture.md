...
13. **Reusable framework, site-specific configuration**
    ...
    - **Framework features with site-side effects default to off.** Features that, when enabled, consume site resources, call external APIs, run scheduled programs, or take destructive action must default to disabled in `framework/templates/config.yaml.example`. The site opts in by setting the feature's enable flag in `site/config.yaml`; framework code gates execution on that flag, not on framework-level presence of the feature. The framework provides capability; the site grants consent.
...
21. **Commitments must be enforced, not assumed**

    The framework's commitments are made true by structural mechanism,
    not by operator discipline or by hope that conditions outside
    testing don't matter.

    **Mechanisms must be deployed where they should apply.** When the
    framework introduces a mechanism that should apply universally, its
    application must be enforced structurally (build-time assertion,
    validation gate, drift check), not by convention. Convention-based
    adoption fails predictably: the gap between "merged" and "deployed
    everywhere" expands without active enforcement.

    **Verification must cover the conditions of use.** A property is
    verified only across the conditions actually exercised. A test
    passing under simplified or design-center conditions does not
    verify the property under the conditions where it must hold in
    operation. Identify the conditions under which a property must
    hold, and the verification that exercises each.

    Both principles share the same operational lesson: the absence of
    enforcement and the absence of verification look identical to the
    operator until something goes wrong. Where Goal 20 forbids escape
    hatches that bypass correctness mechanisms, Goal 21 forbids the
    absence of those mechanisms — both in deployment and in
    verification scope.
...
issues recovery — each affected VM is started on a surviving node
from its most recent local ZFS replica. The VM boots fresh; QEMU
process and guest OS state do not transfer.
...
**Data loss bound:** Up to the replication interval (1 minute) of
post-replication writes on the data disk. OS disks are derivable
from Git (rebuilt fresh on boot via NixOS image), so application
data on vdb is the only at-risk surface.
...
5. Data loss: up to 1 minute of writes on vdb (bounded by replication
   interval)
