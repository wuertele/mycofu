...
12. **Unknowable state must fail, not pass**
...
- Initial deploy (empty tofu state, no existing VMs in Proxmox) is auto-detected
  and the branch check is skipped — the current checkout is definitively correct
...
**The vdb-ready pattern as an example:**

The `wait-for-vdb.service` and `vdb-ready.target` mechanism gates
application services during the atomic rebuild sequence. A VM boots
twice: once with empty vdb (pre-restore), once with populated vdb
(post-restore). Services that write to vdb or depend on vdb contents
must not run on the first boot.

The implementation uses `requires = ["vdb-ready.target"]` on every
gated service. The following was discovered through adversarial review
to be insufficient:

- `certbot-initial` gated → cert-link waits via `after =` → vault waits
  via `wants =` on cert-link. **This does not gate vault.** `after =`
  and `wants =` do not propagate the blocking behavior. vault.service
  starts regardless when certbot-initial is inactive.

The fix: every service that must not run on empty vdb has a direct
`requires = ["vdb-ready.target"]`. There is no safe indirect propagation
through `after =` or `wants =` chains.

**Gated services in the vdb-ready mechanism:**

Services gated in `wait-for-vdb.nix` (framework-level):
- `certbot-initial.service`
- `vault.service`
- `vault-cert-link.service`
- `gitlab-gen-secrets.service`
- `gitlab-cert-link.service`
- `postgresql.service`

Services gated in their own catalog modules:
- `influxdb.service` (influxdb/module.nix)
- `grafana.service` (grafana/module.nix)
- `roon-server.service` (roon/module.nix)
- `testapp-heartbeat.service` (testapp.nix)

**When adding a new service that writes to vdb or depends on vdb
contents:** add `requires = ["vdb-ready.target"]` and
`after = ["vdb-ready.target"]` to that service's definition. Do not
assume that gating an upstream service in the dependency chain will
propagate the block.

---

### Contract 6: vdb Readiness — wait-for-vdb.service

This contract defines how VMs with precious state (Category 3) signal
that their vdb contains real application data before allowing services
to start.

**Problem:** During atomic control-plane rebuilds, a VM boots twice:
1. First boot: fresh vda, empty vdb (before PBS restore)
2. Second boot: fresh vda, restored vdb (after PBS restore)

On the first boot, application services must not start. They would
write stale state to vda or initialize empty vdb, conflicting with the
PBS-restored data on the second boot.

**Mechanism:**

The sentinel is a two-part check:
1. `/run/secrets/vdb-restore-expected` present in CIDATA (set by tofu
   for VMs with `backup: true` in config.yaml or applications.yaml)
2. vdb mount point contains content beyond `lost+found/`

**NixOS option:** `mycofu.vdbMountPoint` (string, default `""`)

Set by each role module that mounts vdb. When non-empty, activates the
entire `wait-for-vdb` mechanism. When empty (DNS, pebble, gatus, cicd),
no service or target exists — those VMs are unaffected.

**Behavior matrix:**

| Flag in CIDATA | vdb content | wait-for-vdb result | Services start? |
|----------------|-------------|--------------------|--------------:|
| Absent | Any | exit 0 (pass-through) | Yes |
| Present | Empty (lost+found only) | exit 1 (block) | No |
| Present | Real state | exit 0 (pass) | Yes |
| Present | vdb not mounted | exit 1 (error) | No |

**CIDATA cascade:** Adding `vdb_restore_expected = true` to CIDATA
changes the content hash for precious VMs and triggers VM recreation
on next deploy. This is a one-time cost — the flag is stable and does
not change between subsequent deploys.

**Adding a new precious-state application:** Two changes required:
1. Set `mycofu.vdbMountPoint` in the NixOS module to the vdb mount path
2. Add `requires = ["vdb-ready.target"]` and
   `after = ["vdb-ready.target"]` to the application's primary service
3. Set `backup: true` in applications.yaml (which flows to tofu and
   sets `vdb_restore_expected = true` in CIDATA)

Missing any of these three creates a silent gap — the application
starts on empty vdb without protection.
