...
These goals are the project's value function: every design decision is
judged by whether it serves them. They are stated tersely here and
interpreted, with examples, in section 2.1. They are labeled G1–G7 and
layered by dependency — violating an earlier goal makes the later goals
unreachable.

> **Note on numbering.** These seven goals distill the original
> twenty-one. Later sections of this document, `.claude/rules/`, sprint
> documents, and issues still cite the original numbering ("Goal 17",
> "Design Goal 18"). Section 2.2 maps every original goal onto this
> set; resolve old references there.

Dependency structure:

```
G1  A Git commit fully determines the system            (root axiom)
 └─ G2  The system converges; the region only grows     (what G1 buys)
     ├─ G3  Failures stop the line                      (defends G2 after a failure)
     ├─ G4  Invariants are enforced by mechanism        (defends G1–G3 before a failure)
     ├─ G5  One authority per boundary                  (makes G2 tractable and safe)
     ├─ G6  Environment-ignorant images                 (makes G2 deterministic across environments)
     └─ G7  Files in, instructions out                  (governs the human in the loop)
```

**G1. A Git commit fully determines the system.**
Software, configuration, and the relationships between configuration
values are all functions of one commit. State flows one way: the
operator writes Git; automation reads Git and converges the cluster to
it, and never writes back. The only state outside the commit is
explicitly classified precious data (runtime state such as database
contents), owned by the backup system. There is no hidden state. The
system's history is the Git history, reviewable and explainable.

**G2. The system converges, and the convergence region only grows.**
From any starting state within the demonstrated region, one execution
of any convergence mechanism reaches the state the commit determines:
a single pass, with no manual steps. All convergence mechanisms produce
equivalent end states from the same commit, and when the desired state
evolves, the mechanism itself carries the transition. Any state the
system's own tooling can produce, the system must converge from
automatically. The derivable system must converge on working hardware
from bare metal: the age key and one script, nothing else. The region
has no fixed outer boundary — it grows monotonically, and no starting
state is permanently out of scope. Precious data's protection level is
an explicit, configured trade-off; its recovery is bounded only by
physical survival. Where recovery requires an irreversible judgment,
the judgment is the operator's, and it enters the mechanism as data —
never as a manual procedure.

**G3. Failures stop the line.**
A convergence failure is diagnostic, not operational: the response is a
structural fix that eliminates the failure mode, plus a DR test seeded
from the revealed starting state — never a manual workaround. A
temporary manual intervention is permitted only during an active
incident when the structural fix cannot ship immediately; a blocking
issue is filed first, and the incident stays open until the structural
fix is deployed. DFMEA selects which starting states must be tested;
the DR registry records the demonstrated frontier.

**G4. Invariants are enforced by mechanism, and mechanisms are trustworthy.**
Every invariant that matters is enforced structurally — a build-time
assertion, a validation gate, a drift check — never by operator
discipline, convention, or documentation. Enforcement is deployed
everywhere the invariant applies and is verified under the conditions
where it must actually hold. Enforcement mechanisms carry no standing
bypass flags; a bypass is acceptable only as a transition tool with an
explicit sunset. A check that cannot determine safety fails loudly; it
may skip only when the condition it checks is genuinely not applicable.

**G5. One authority per boundary; isolation by construction.**
Every layer of the stack has exactly one controlling authority. Dev and
prod are isolated by network, DNS, and policy — by construction, not
convention. The framework is site-agnostic: all site-specific values
live in one clearly identified configuration layer, and framework
features with site-side effects are disabled until the site opts in.

**G6. Images are environment-ignorant; identity binds at deployment.**
One artifact serves every environment. A VM's environment comes from
the network at creation time, its role from boot metadata, and its
credentials are generated in place — never transported, never baked
into the image.

**G7. Files are the operator interface; automation output is operator instruction.**
YAML files are how the operator expresses intent: programs read files
and write files, they do not replace them. Generators are optional
helpers — safe to re-run, never gatekeepers, never required. Everything
automation prints will be acted on: every printed command must be safe
to run exactly as printed, and every remediation must route through the
safe path.

### 2.1 Interpreting the Goals

**Two missions.** Mycofu has two audiences with two distinct missions.
The *operator* runs a cluster: their mission is their own applications,
and the framework should demand as little of their attention as
possible. The *Mycofu developer* builds the framework, and their
mission is captured by G2's frontier clause:

> **The recovery region is not a boundary to define — it is a frontier
> to grow, and growing it is the mission.**

Every state the system cannot yet converge from automatically is not
out of scope; it is unfinished work. Auto-recovery from a single dead
node today; from two dead nodes tomorrow. The operator benefits from
the frontier's growth but is never obligated to push it.

**G1 — the commit is the system.** When you test a commit, you test
everything: there is no separate configuration state that could differ
between test and deployment. Hidden state is the enemy of convergence —
a dependency that is not in the repo is a starting condition the system
cannot converge from, which is why "no manual fixes" and "no click-ops"
are operational rules, not etiquette.

*Deployment granularity.* G1 is a claim about the *whole* system, not
about each part separately. A cluster whose VMs were updated
independently is running a mosaic of commits, and "what is this system
running?" has no single answer — G1 is violated in practice even though
every part came from some commit. The deployable unit is therefore the
entire system at one commit, and every VM is assumed updatable on every
release. Partial application is permitted as a transparent optimization
and never as a contract: it must always be correct, and routinely
exercised, to deploy everything. Eliminating churn that provably
cannot change anything is a legitimate cycle-time optimization, but
it is skip logic, and skip logic must be airtight: a false negative
leaves the system running something the commit does not determine —
a G1 violation that announces itself late, if at all. The soundness
machinery such an optimization requires is part of its price.
Experience with fleet update systems shows the deltas between releases
*grow* as a system stabilizes and its builders' capability increases —
the asymptote is that everything changes at every release, releases
becoming more frequent. A system that absorbs that lets velocity rise
without bound; one that cannot caps velocity at whatever its deployment
granularity can absorb.

*Relationships are configuration too.* When two values have a
constraint relationship — A must fit within B, A must precede B, A must
match B's format — the relationship is expressed as a derivation, not
maintained as two independent values. A system where A=500M and B=256M
are both committed is deterministic but not consistent. The journal
overlay incident (2026-04-17) is the canonical example: `SystemMaxUse=500M`
was set when root filesystems were ext4; Sprint 010 moved to a 256M
tmpfs overlay-root; both values were locally correct, and after 40
hours of log accumulation the journal filled the overlay and GitLab
became unreachable. The fix derived the journal limit from the overlay
size. Wherever a constraint can be enforced at build time (NixOS
`assertions`, cross-layer facts passed through config.yaml → tofu →
NixOS), it must be; runtime monitoring is the complement for
constraints only knowable at runtime, not a substitute. If a
relationship is important enough to document, it is important enough to
derive — the journal limit was documented, and the documentation did
not prevent the incident.

*State flows one way.* Automation may hold credentials to read secrets
and deploy (the CI runner decrypts SOPS), but it never commits to the
repo. Every commit has a human behind it, and a pipeline that writes
its own input is a feedback loop. A consequence: any operation that
must produce new committed state — such as onboarding a new
application's Vault bootstrap credentials into SOPS — is an
operator-side flow followed by a pipeline deploy of the pushed commit.
The "two-path deployment model" (section 10.3) is this consequence, not
an independent commitment.

*The encryption root.* The age private key is the one secret that
cannot live in the repository — it decrypts the repository's secrets,
so by definition it cannot be encrypted with itself. Every other secret
is either in `site/sops/secrets.yaml` or generated during convergence
and immediately encrypted and committed.

**G2 — convergence.** The metaphor is water in a watershed: a molecule
anywhere in the basin eventually reaches the delta. You do not prove a
watershed works by dropping water at one point; you prove it by
demonstrating convergence from points throughout the region. A rebuild
script that works from bare metal but fails after a partial deployment,
a botched restore, or a provider bug is a rail, not a watershed.

*Single-pass is the metric.* One successful rebuild proves only that
that run missed the hidden assumptions. A failure on any attempt is
diagnostic: it identifies a dependency not in the repo or a transition
not handled. The temptation to re-run a failed rebuild is the trap —
success on the second attempt is not a fix, it is evidence of a defect
(a non-determinism or ordering dependency that will resurface). If the
process is truly idempotent, one pass is sufficient.

*Equivalent mechanisms.* The system admits multiple convergence
mechanisms — the CI pipeline for ongoing changes, and
`rebuild-cluster.sh`/`safe-apply.sh` from the workstation for disaster
recovery and initial setup. All must produce equivalent cluster state
from the same commit; otherwise cluster state becomes a function of
which mechanism last ran rather than of the desired state. Likewise,
when the desired-state representation evolves to require new derived
state — a schema change, a new persistent dependency — the convergence
mechanism carries that transition from any prior valid state. "The
change works once the operator runs the backfill" describes an
incomplete change.

*The full-rebuild target.* After physically assembling hardware and
installing Proxmox on each node's boot drive, the operator provides
their SOPS age private key and runs `framework/scripts/rebuild-cluster.sh`.
Everything else — state initialization, image builds, deployment, DNS,
certificates, Vault initialization, verification — is automated.

*Precious data.* Runtime data (database contents, Vault Raft storage,
application state — "Category 3") is the one thing the commit does not
determine, so its protection is a chosen trade-off: backup cadence,
replication interval, and redundancy are configured costs, and the loss
window they buy is explicit. Recovery of precious data is bounded only
by physical survival — if the bits exist on any disk, backup, or
replica, the framework must not be the reason they cannot be recovered.
Where recovery requires an irreversible judgment — which backup to
restore, whether an empty data disk is acceptable on first deploy — the
judgment belongs to the operator and enters the mechanism as data (a
restore pin file, an explicit allow-list such as
`FIRST_DEPLOY_ALLOW_VMIDS`), never as a manual procedure. "Operator
decision required" must never degenerate into "operator procedure
required."

**G3 — failures stop the line.** This is the Andon principle from the
Toyota Production System applied to infrastructure: stopping the line
to fix a quality problem is expensive in the moment and cheaper than
letting the defect propagate. The temptation is stronger in software
because an engineer who can SSH in and fix anything feels productive —
they are actually degrading convergence by adding a hidden dependency,
creating an untested starting state, and hiding the gap. Every rebuild
failure that occurs in practice produces two artifacts: a structural
fix, and a DR test case that starts from the degraded state the failure
revealed (`framework/dr-tests/DR-REGISTRY.md` is the inventory).
Passing a DR test once proves little; passing it longitudinally across
code changes, operator errors, and infrastructure failures is what
demonstrates control. The incident exception is deliberately narrow: a
manual fix during an active incident is explicitly temporary, requires
a blocking issue before the incident closes, and the incident is not
resolved until the structural fix is deployed and the manual
intervention is no longer needed.

**G4 — enforcement over discipline.** The absence of enforcement and
the absence of verification look identical to the operator until
something goes wrong.

*Fail, don't skip.* When a safety check cannot determine whether the
system is safe — a prerequisite missing, a tool failed, the check
itself errored — the check fails with an explanation, and the operator
decides. A SKIP that hides an undeterminable safety problem is a false
pass. The one legitimate SKIP is genuine non-applicability: a drift
check that requires build artifacts may skip in a pre-build pipeline
stage. The distinction: SKIP when the check is not yet applicable; FAIL
when something went wrong.

*No standing overrides.* A flag like `--skip-reboot` or
`--allow-empty-vdb` on a mechanism that enforces a structural invariant
is not an emergency tool; it is a standing weakening. The reasoning is
empirical: the override accumulates use (every hard moment is a reason
to reach for it), it prevents the mechanism from building the track
record that would prove the override unnecessary, and by the time it
could be removed, removal feels like a regression. The Sprint 027 vdb
readiness gate began as defense-in-depth and became the reason
orchestration confidence couldn't grow; deleting it (Sprint 031)
required explicit architectural commitment. Not every flag is an
override: `--dry-run` reinforces correctness; scope flags
(`rebuild-cluster.sh --scope`) constrain *what* is acted on, not *how*;
parameter flags (timeouts, log levels) operate within the correctness
model. Forbidden are flags that suspend an invariant the operation
otherwise enforces. A bypass is acceptable only as a transition tool
with a specific sunset — an issue number, a successor mechanism — never
an aspirational one ("when we're sure it works").

*Deployed and verified.* A mechanism that should apply universally is
adopted structurally (assertion, gate, drift check), because the gap
between "merged" and "deployed everywhere" widens without active
enforcement. And a property is verified only across the conditions
actually exercised: a test passing under design-center conditions does
not verify the property under the conditions where it must hold.

**G5 — boundaries.** Each layer of the stack — Proxmox, OpenTofu,
NixOS, application — has exactly one controlling authority; ambiguity
about what manages what is where drift breeds. Dev/prod separation is
by construction (VLANs, split DNS, distinct Vaults and policies), so
that accidental cross-environment access is structurally impossible
rather than discouraged. The framework/site split serves a different
value — reusability: a new operator clones the repository, provides
their own `site/` configuration (domain, IPs, hardware), and gets a
working platform. Site-specific values never appear in `framework/`.
And framework features that consume site resources, call external APIs,
run scheduled jobs, or take destructive action default to off: the
framework provides capability; the site grants consent by setting the
enable flag in `site/config.yaml`.

**G6 — environment ignorance.** No environment name, FQDN, IP address,
or conditional environment logic appears in any VM image, so the same
artifact is what was tested wherever it lands. The current binding
mechanism: environment is determined by network placement at VM
creation (VLAN → DHCP search domain), and role by boot metadata
(cloud-init). Those binding times and mechanisms are commitments, not
values — what is goal-level is that the image itself never knows.
Credentials follow the same principle, generated in place rather than
transported: TLS private keys are generated on the VM that uses them.
(The original goal also committed to VMs authenticating to Vault by TLS
certificate; that mechanism is deferred indefinitely — ACME-issued
certificates carry an empty CN, which Vault's cert auth cannot accept;
see section 9.3 — and its future is a commitment question, not a goal.)

**G7 — files in, instructions out.** A well-structured YAML file with
inline comments teaches the operator the system as they use it; a
wizard collects values and hides the system. When the operator needs to
change something, they go to the file — not to a program they ran once.
Generators (`new-site.sh`, `enable-app.sh`) produce starting points and
never touch existing content: re-running one must preserve every
operator edit (a generator that overwrites operator edits is a
data-loss bug), and the framework must work identically whether a file
was generated or written by hand — a generator that has never been run
is not a blocker. Comments belong at the point of action, adjacent to
the values they constrain. In the other direction, automation output
carries a duty of care: when a script prints "run this command," the
operator will run it, so automation must never print a dangerous raw
operation. A guard that detects control-plane image drift directs the
operator to `rebuild-cluster.sh --scope control-plane` (which handles
backup pinning, preboot restore, and convergence), not to a raw
`tofu apply -target=...` that destroys data silently. The same duty
applies to error messages and diagnostics.

### 2.2 Appendix: Mapping from the Original 21 Goals

References of the form "Goal N" / "Design Goal N" elsewhere in this
document, in `.claude/rules/`, and in sprint and research documents use
the original numbering below.

| Old | Original title | Now in | Notes |
|-----|----------------|--------|-------|
| 1 | Convergence, not just rebuildability | G2 | Watershed framing kept in 2.1 |
| 2 | Single-pass convergence is the quality metric | G2 | The "single pass" clause |
| 3 | Rebuild failures are diagnostic, not operational | G3 | Incident exception preserved verbatim in intent |
| 4 | Configuration is software | G1 | |
| 5 | Rebuildability over repair | G2, G1 | Full-rebuild target in G2; age-key-as-encryption-root in G1 interpretation. The "two-path deployment model" is reclassified as a *consequence* of G1's one-way-flow clause (see 2.1); the mechanism itself is a commitment the roadmap may re-examine (operator decision, 2026-07-04) |
| 6 | Minimal hidden state | G1 | |
| 7 | DFMEA defines the test surface | G3 | The "rim" is reframed as a frontier with no fixed limit; growing it is the developer's mission (operator decision, 2026-07-04) |
| 8 | Clear ownership boundaries | G5 | |
| 9 | Strong dev/prod separation | G5 | |
| 10 | Unknowable state must fail, not pass | G4 | FAIL/SKIP distinction preserved in 2.1 |
| 11 | Automation output is operator instruction | G7 | |
| 12 | Human-auditable configuration | G1 | "Reviewable and explainable" clause |
| 13 | Reusable framework, site-specific configuration | G5 | Default-off consent clause preserved |
| 14 | Files are the operator interface | G7 | Generator invariants preserved in 2.1 |
| 15 | Early binding of environment, late binding of role | G6 | Binding times reclassified as mechanism, not value (operator decision, 2026-07-04) |
| 16 | Stateless identity where possible | G6 | Vault TLS-cert auth dropped from goals — deferred upstream (section 9.3); the generated-in-place value is preserved (operator decision, 2026-07-04) |
| 17 | Configuration constraints are software | G1 | The "relationships between configuration values" clause; journal-overlay example kept in 2.1 |
| 18 | The convergence mechanism handles all transitions | G2 | |
| 19 | All convergence paths produce equivalent end states | G2 | |
| 20 | No standing overrides | G4 | Flag-classification guidance preserved in 2.1 |
| 21 | Commitments must be enforced, not assumed | G4 | |
...
**vda / vdb:** The two virtual disks attached to each VM. vda is the root/OS disk (Category 1, disposable, replaced on image update). vdb is the data disk (Category 2 and/or 3, may contain precious state). vdb's data carries forward across VM recreation via two mechanisms. The default path (Sprint 031) destroys the old vdb zvol with the VM and restores vdb from a pinned PBS backup before the new VM boots (`restore-before-start.sh`). A dev-only opt-in path (Sprint 044) parks the old vdb zvol out of the `vm-<vmid>-*` namespace before destruction and adopts it back onto the new VM before first boot, eliminating the PBS round-trip (see `.claude/rules/replication.md` § "vdb Park/Adopt Bridge Namespace"). Promoting the park/adopt path from dev-only opt-in to the default flow is tracked as #478 (see `docs/research/RESEARCH-004-vdb-survives-recreation.md`). See section 11.0 for the complete four-component VM model (vda, CIDATA, vdb, off-cluster storage) and their lifecycles across all operations.

**Filtered image sources (`sharedSrc` / `mkRoleSrc`):** The per-role filtered source trees used as input to NixOS image derivations, created via `builtins.path` in `flake.nix` (replacing the former single `nixSrc`). Each role's source is a shared base (`framework/nix/`, `framework/step-ca/`, `flake.lock`, shared hook scripts) plus only that role's own paths (`site/nix/hosts/<role>.nix`, `framework/catalog/<role>/`, `site/apps/<role>/`) and any declared cross-tree exceptions. Changes to non-build files (CI config, documentation, operational scripts) do not change image hashes; a change to one role's source rehashes only that role's image. See section 11.1.2.
...
- ZFS replication between nodes via Proxmox (per-VM cadence: 1m for
  backup/prod/shared defaults, 24h for dev defaults, explicit overrides
  where ratified)
- ZFS scrubs scheduled weekly, SMART monitoring enabled

**Replication Strategy** (cadence-driven since Sprint 048; see § 11.4.0):
- Every enabled VM is replicated unless explicitly opted out with
  `replicate: false`.
- Defaults: `backup: true` -> 1m; env in `{prod, shared}` -> 1m;
  env `dev` -> 24h. Explicit overrides use cadence strings such as
  `replicate: "24h"` and must be ratified by YAML comments.
- Policy is derived by `framework/scripts/list-replicated-vmids.sh`,
  applied by `configure-replication.sh`, and delivered to nodes as
  `/etc/repl-policy.vmids` with `CADENCE_MAP` so consumers can enforce
  cadence-aware checks.
...
**Trade-offs accepted:**
- No seamless sub-second live migration (Ceph's primary advantage). The actual planned-maintenance and unplanned-failure downtime characteristics of this architecture are detailed in section 6.4.
- Unplanned node failure: writes may be lost up to the VM's declared
  replication cadence. 1m jobs retain the minute-class RPO; 24h jobs accept
  daily staleness by policy so they can still restart from a replica.
...
### 6.4 HA Paths and Maintenance

Two distinct HA paths exist in this architecture. They differ in what
triggers them, what mechanism PVE uses to recover, and what operator-
visible downtime they produce. Conflating them produces the
architecture-vs-implementation gap documented in §6.4.4.

#### 6.4.1 Unplanned-failure path (fence-and-recover)

**Trigger:** A node becomes unreachable for longer than the corosync
membership timeout. Causes include power loss, kernel panic, hardware
failure, severed network link, watchdog-triggered self-fence.

**Mechanism:** Surviving CRM master observes the membership change,
waits for `fence_delay` (default 60s), declares the node fenced, and
issues recovery. Every enabled VM is expected to start on a surviving
node from its most recent local ZFS replica unless that VM has an explicit
`replicate: false` override. The replica's freshness is bounded by the
VM's declared cadence (1m for backup/prod/shared defaults, 24h for dev
defaults unless overridden). If HA lands a replicated VM in `error`, the
operator uses the §6A/§6B ladder in
`.claude/rules/storage-failure-fence.md`; `recreate-derivable-vm.sh` is
only for explicit opt-out VMs. The VM boots fresh; QEMU process and guest
OS state do not transfer.

**Measured downtime:** ~150–160 seconds per VM, end-to-end (power
loss → service responding). Breakdown:
- ~70s for corosync to declare the node `unknown`
- 60s `fence_delay` to confirm the fence
- ~20s CRM cycle to issue recovery
- VM boot time on destination (varies by VM)

**Data loss bound:** Up to the VM's declared replication cadence of
post-replication writes on the data disk. OS disks are derivable from Git
(rebuilt fresh on boot via NixOS image), but they are still replicated so
HA has a local zvol to boot from after node failure.

**Reliability:** Verified empirically on 2026-05-22 — abrupt power
removal from pve01 produced clean recovery of three VMs across the
two surviving nodes, no ZFS corruption, all services responding
within ~160s.

This path works today, requires no operator intervention, and is the
load-bearing primitive of the cluster's "HA against unplanned
failure" promise.

#### 6.4.2 Planned-maintenance path (drain and rejoin)

**Trigger:** Operator initiates `ha-manager crm-command
node-maintenance enable <node>`, typically via
`framework/scripts/reboot-node-rolling.sh` or an equivalent
shutdown script.

**Intended mechanism (target state):** HA live-migrates each VM off
the target node to a surviving node. Memory state, open TCP
connections, and guest-visible time advance through the migration
without interruption. Per-VM downtime is sub-second (memory
transfer + final pause-and-resume). Operator does hardware work;
on rejoin, `rebalance-cluster.sh` migrates VMs back via the same
live mechanism.

**Current delivered mechanism:** Live migration is currently blocked
for every framework VM by issue #336 (the bpg/proxmox provider's
auto-created cloud-init CDROM drive triggers Proxmox's "cannot
migrate local cdrom" pre-flight refusal). When `ha-manager migrate`
is invoked against a VM the pre-flight check refuses, HA falls back
to cold migration: stop on source, `pvesm export | pvesm import` the
zvols, start on destination, update config to point at the imported
zvol. Per-VM downtime in cold-fallback: ~10–30 seconds.

**Current delivered downtime per VM:** ~10–30 seconds via cold-
fallback. This is meaningfully better than the unplanned-failure
path (~150s) because no fence_delay is involved, but worse than the
target state (~sub-second) because memory state is lost.

The framework does not currently set `shutdown_policy=migrate` in
`/etc/pve/datacenter.cfg`. With the default `conditional` policy on
poweroff, a bare `shutdown -h now` does not invoke the migrate path
at all — it queues local stops, which deadlock against `vmstore-
data.mount` unmount (qemu still holding zvol references), producing
the stuck-shutdown failure mode observed on 2026-05-22.

#### 6.4.3 Downtime summary

| Scenario | Mechanism | Per-VM downtime | Status |
|---|---|---|---|
| Unplanned node failure | Fence + cold start from replica | ~150s | Working |
| Planned maintenance (current) | HA migrate, cold-fallback | ~10–30s | Working (interim) |
| Planned maintenance (target) | HA migrate, live | ~sub-second | Blocked by #336 |
| `shutdown -h now` (current default) | Local stop, deadlock against mount | Stuck shutdown | Broken |
| `shutdown -h now` (with `shutdown_policy=migrate`) | HA migrate, cold-fallback | ~10–30s | Available after framework fix |

#### 6.4.4 Architecture-vs-implementation gap

The cluster's central architectural promise is HA during planned
maintenance — the operator must be able to take a node down for
hardware work without service interruption. This promise is not
currently delivered in its target form because of #336.

The gap is a Goal 21 instance: the architecture commits to a
property (zero-downtime planned maintenance) that no mechanism
enforces and no validation catches when broken. Specifically:

- The framework provides `reboot-node-rolling.sh` for rolling reboots
  but the script's success depends on live migration working, which
  it doesn't.
- `validate.sh` does not include a live-migration smoke test;
  consequently the broken state has persisted undetected since #336
  was filed on 2026-05-14.
- No framework script exists for graceful node shutdown (as
  distinct from reboot).
- The framework does not set `shutdown_policy=migrate`, leaving the
  PVE default `conditional` policy in effect — which means
  `shutdown -h now` doesn't even attempt the cold-fallback migrate
  path.

Closing the gap requires four pieces:

1. Fix #336 (live migration works for cloud-init VMs)
2. Set `shutdown_policy=migrate` in datacenter.cfg via the framework
3. Add a live-migration smoke test to `validate.sh` so future
   regressions are caught
4. Write `framework/scripts/shutdown-node-graceful.sh` as a symmetric
   counterpart to `reboot-node-rolling.sh`

Pieces 2-4 are small (each is /mycofu-fix scope). Piece 1 is sprint
scope; empirical research has identified candidate fix paths
(renaming the cloud-init drive at provision time to bypass PVE's
name-based refusal, or patching the pre-flight check to permit
replicated cdrom drives). Pre-planning verification on bfnet will
choose the path before /sprint-plan.

#### 6.4.5 Goal 21 commitment

Once the four pieces above land, the architecture commits to:
- Live migration is a verified property of every framework VM
  (validated by `validate.sh`)
- `shutdown_policy=migrate` is set as a framework invariant
  (validated by site-config check)
- Graceful node shutdown for hardware maintenance is a documented
  operator workflow with a framework-provided script

Until these commitments are mechanism-enforced, the interim
operator guidance is: use `ha-manager crm-command node-maintenance
enable <node>` followed by `qm list` polling for empty drain before
issuing `shutdown -h now`. The result is ~10-30s downtime per VM
via cold-fallback migration, not zero-downtime — which is the
honest current capability of the system.

### 6.5 Time Service (NTP)
...
- Enables planned-maintenance windows with limited downtime per VM (see §6.4 for the current cluster's HA paths and per-VM downtime; DNS redundancy ensures the *service* survives a single-VM gap regardless of per-VM mechanism)
...
**Renewal invocation contract:** The framework renews certificates through
`certbot-renew.service`, whose generated script passes the current NixOS
generation's `--manual-auth-hook` and `--manual-cleanup-hook` values on the
`certbot renew` CLI. Persisted `renewal.conf` hook values on vdb are advisory:
they may still contain generation-specific `/nix/store` paths from an older
system, but they are not authoritative for framework renewals. A bare manual
`certbot renew` bypasses this contract and can still read stale persisted hook
fields; operational checks and DR tests must exercise the framework service
path.

**Backup trust marker contract:** `backup-now.sh` records certbot
renewability evidence in each restore pin as capture-time metadata:
`trust=trusted|untrusted|unknown`, `days_remaining`, and `near_expiry`.
`trusted` answers only "was this backup's cert lineage renewable when the
backup was captured?" It must not be interpreted as "is this candidate
renewable now?" Recovery-quality pin selection requires both facts: the D3
trust marker for capture-time history, and #450's future selection-time
prod dry-run (`certbot renew --dry-run --cert-name <name>`) for current
renewability before a pin is certified for recovery.
...
step-ca supports a static, operator-provided root CA. The root CA
certificate at `framework/step-ca/root-ca.crt` is a public certificate —
safe to publish. The corresponding root CA private key at
`framework/step-ca/root-ca.key` IS a secret: anyone holding it can mint
TLS certificates trusted by Mycofu dev VMs. The key is retained in the
private GitLab repo and excluded from the GitHub publish path by
`framework/scripts/publish-filter.sh` (see the `PUBLISH_EXCLUDED_PATHS`
array and the `publish_filter_path_allowed` case). An attacker holding this key can mint TLS certificates for any hostname
that dev clients would trust; combined with a DNS or routing position
on the dev VLAN, this enables full man-in-the-middle of dev-VLAN TLS.
Because dev VMs sit on an isolated VLAN and prod/public clients do not
trust this CA, the blast radius is bounded to the dev VLAN — but that is
defense in depth, not a substitute for keeping the key private. Prior versions of this document called the key "NOT a secret"
and the publish filter did not exclude it; the key was mirrored to the
public GitHub repo from April 2026 until the publish-filter exclusion
landed (see issue #364), so the current in-repo key must be rotated as
a separate operator-scheduled follow-up. The static root means:
...
**Rate limit protection:** Vault-backed `cert-restore` in
`framework/nix/modules/certbot.nix` restores certificate lineages on every
boot when a Vault token and complete stored lineage are available and the
certificate has more than 30 days remaining, avoiding a new ACME request.
For a rebuild session expected to issue certificates, the operator sets
`acme: staging` in `site/config.yaml`. `--override-branch-check` only
bypasses branch safety and never changes a VM's ACME server.
...
**4. Explicit ACME mode in persistent configuration.** The operator selects
`acme: staging` in `site/config.yaml` for rebuild sessions expected to issue
certificates. The configured ACME URL is delivered via CIDATA at boot, so
the choice persists across VM recreations. Rebuilds and pipeline deploys
use this configured mode; `--override-branch-check` only bypasses branch
safety and never selects staging.
...
prevents rate limit consumption on rebuild. Layer 4 (explicit ACME mode)
lets the operator select staging when a rebuild is expected to issue
certificates instead of reusing an existing lineage.
...
- During planned maintenance, Vault is unavailable for ~10–30 seconds via the cluster's current cold-fallback migration path (see §6.4 for the full HA-path analysis). When #336 is resolved and live migration is delivered, this drops to ~sub-second memory-transfer time. The window is short in either case relative to the periodic-query model that VMs use against Vault, and vault-agent caching means most VMs are unaffected during the gap.
...
# build-all-images.sh internally invokes build-image.sh per role, which
# uploads each built image to all Proxmox nodes — no separate step.
...
| **vdb** (data disk) | Application state, databases, Vault Raft, certificates | The running application | `tofu apply` (creates empty zvol) | Default path (Sprint 031): destroyed with the VM on every recreation, then vdb restored from a pinned PBS backup before boot. Dev-only opt-in (Sprint 044): parked out of the `vm-<vmid>-*` namespace and adopted directly, no PBS round-trip. Promoting park/adopt to the default is tracked as #478 (RESEARCH-004). Data is only lost by explicit operator action (`reset-cluster.sh`, `tofu destroy` without a valid pin). |
...
|-----------|-----|--------|-----|-------------|
| **Image update (factory)** (new commit, data-plane) | Destroyed and rebuilt from new image | Regenerated (may be identical) | Default (Sprint 031): destroyed with the VM, restored from PBS before boot. Dev-only opt-in (Sprint 044): parked and adopted directly across recreation. Promoting park/adopt to the default is pending #478. | State updated by tofu |
...
| **Config change** (CIDATA edit, data-plane) | Destroyed and rebuilt | Regenerated with new values | Default (Sprint 031): destroyed with the VM, restored from PBS before boot. Dev-only opt-in (Sprint 044): parked and adopted directly across recreation. Promoting park/adopt to the default is pending #478. | State updated by tofu |
...
3. **vdb data outlives the VM.** When an image update recreates a framework
   VM, the vdb data carries forward: new software, old data. This is the
   normal upgrade path. Applications are responsible for forward migration
   (database schema upgrades, etc.) when they encounter older data. For
   vendor appliances, vdb is backed up alongside vda as part of the
   whole-VM PBS backup.

   **Mechanism (default vs dev-only opt-in):** Two mechanisms carry the
   data forward. The default path (Sprint 031) destroys the old vdb zvol
   with the VM; before the new VM boots, `restore-before-start.sh`
   restores vdb from a pinned PBS backup. From the application's
   perspective the data continues, but the underlying zvol is new. The
   dev-only opt-in path (Sprint 044) parks the old vdb zvol out of the
   `vm-<vmid>-*` namespace before destruction and adopts it back onto
   the new VM before first boot, eliminating the PBS round-trip and any
   restore window (see `.claude/rules/replication.md` § "vdb Park/Adopt
   Bridge Namespace"). Promoting the park/adopt path from dev-only
   opt-in to the default flow is tracked as #478 (zfs-rename-park
   bridge — see `docs/research/RESEARCH-004-vdb-survives-recreation.md`).
...
| GitLab | Write-once | `configure-gitlab.sh` — writes SOPS only on first password capture. If SOPS entry exists, uses it; after a fresh-DB rebuild where `/var/lib/gitlab/initial_root_password` still authenticates, resets DB to match SOPS. Rotation uses `rotate-gitlab-root-password.sh` so the driver can authenticate with the escrowed old password. |
...
| Value | Path on VM | Prod VLAN 10 | Dev VLAN 60 |
|-------|-----------|-------------|-------------|
...
**Filtered image sources (per-role):**
...
The fix is filtered source trees created via `builtins.path`, partitioned per
role so that a change to one role's source rehashes only that role's image:

```nix
# Shared base: rehashes every role when changed.
sharedSrc = mkFilteredSrc "nix-source-shared" [] (_: false);

# Per-role source: the shared base plus only that role's own paths.
mkRoleSrc    = role: mkFilteredSrc "nix-source-${role}" (roleWantedPaths role) ...;
sourceForRole = role: mkRoleSrc role;
```

The shared base contains `framework/nix/`, `framework/step-ca/`, `flake.nix`,
`flake.lock`, and the shared certbot hook scripts. Each role's source additionally
includes its name-derived paths (`site/nix/hosts/<role>.nix`,
`framework/catalog/<role>/`, `site/apps/<role>/`) plus any non-name-derived
cross-tree inputs declared in `extraRoleSubtrees` (e.g. influxdb's
`cluster-dashboard`, grafana's `benchmarks/grafana/`, hil-boot's HIL fixtures).
All image derivation entry points reference `${sourceForRole role}/...` — never
`./...` or `self` — so a file not in a role's filtered source literally does not
exist for that role's build. This is fail-closed: a required file missing from a
role's source produces a hard build failure (via path-literal coercion or a NixOS
module assertion), not silent use of stale content.

Two properties matter, and both are verified in both directions:

- **No over-inclusion / bypass:** image code must not reference `self` or bare
  top-level `./framework` / `./site` paths (which resolve against the unfiltered
  source and reintroduce global hash churn). Host-tool packages under
  `framework/nix/pkgs/` are excluded from every image source and are the one
  sanctioned `self` reference, since they are not image inputs.
- **No under-isolation:** a change to one role's source must not rehash other
  roles, and an unrelated new directory must not leak into a role's tree
  (directory inclusion is filtered to ancestors of wanted paths, with an
  `ensureSlash` boundary rule so `framework/nix` cannot match `framework/nixfoo`).

**Maintenance requirement:** Name-derived role paths are included automatically.
When adding a non-name-derived cross-tree input that a role references, add it to
`extraRoleSubtrees.<role>` in `flake.nix`. The pipeline validates both directions
via a static lint (`check-source-filter.sh`) and nix-level checks
(`source-filter-check.nix` for bypass/over-inclusion, `per-role-isolation-check.nix`
for per-role membership and isolation). See `.claude/rules/nixos.md` for the full
invariant, the "Adding a Cross-Tree Dependency" procedure, and verification
commands.
...
Uploads are append-only: `upload-image.sh` never deletes. Image reclaim
runs as a separate, single-writer, off-hot-path job (`reclaim-images.sh`)
that removes images for the same role that are no longer referenced by
the current `image-versions.auto.tfvars`. This keeps upload fast and
race-free while still cleaning up old versions after a role's nix
configuration changes. (`upload-image.sh` retains a `--prune` flag as a
deprecated no-op for backwards compatibility with older call sites.)
...
- **ZFS replication:** Both volumes replicate to other nodes for every enabled
  VM unless explicitly opted out. Cadence, not state class, bounds failover
  staleness (see § 11.4.0).
...
- Examples: Tailscale node identity (generated at first tailnet join, stored in Vault, restored on subsequent boots so the VM maintains a stable Tailscale identity across rebuilds); cert-restore-managed TLS certificates (generated by certbot on first issuance, stored in Vault, restored on subsequent boots to preserve Let's Encrypt rate-limit quota — see §8.3 Category 4 Pattern #2)
...
- The cluster's TLS certificates are also a Cat 4 secret managed by cert-restore — generated by certbot on first issuance, persisted in Vault, and restored on VM recreation rather than re-issued. The reason this mechanism exists is severity: re-issuance would consume Let's Encrypt rate-limit quota; preserving the cert in Vault avoids that consumption. The mechanism is the same shape as Tailscale identity; the operational consequence of failing to preserve is more severe.
- This distinction determines what PBS backs up. Replication/HA is now
  cadence-driven (Sprint 048; see § 11.4.0): the taxonomy decides backup
  scope, while per-VM cadence decides replica freshness for node failure.
...
**Vault contains:**
- Category 4: VM-produced secrets (Tailscale node identity, cert-restore-managed TLS certificates, etc.)
...
| Vault | NixOS + Vault | Policies (from Git) | Raft storage, leases, dynamic secrets | Cluster's TLS cert lineage (via cert-restore) | Yes |
| Home Assistant | HAOS | YAML config (from Git) | Entity/device registries, history DB | TLS cert (fetched at boot via cert-restore) | Yes |
| InfluxDB | NixOS + InfluxDB | Schema (from Git) | Time series data | TLS cert (fetched at boot via cert-restore) | Yes |
| GitLab (Tailscale) | NixOS + GitLab | Config (from Git) | Repos, DB, CI history | Tailscale node identity + TLS cert (cert-restore) | Yes (for Cat 3) |
...
**Decision:** ZFS local storage with replication (enables HA and planned migration) — with **universal per-VM cadence policy** as of Sprint 048.

**HA strategy:** All VMs are HA-enabled. Every enabled VM is replicated at a declared cadence unless it is explicitly opted out with `replicate: false`. Backup-backed, prod, and shared VMs default to 1m; dev VMs default to 24h. A node failure recovers every affected VM from a replica at that cadence; explicit opt-out VMs are the only ones that route to `framework/scripts/recreate-derivable-vm.sh`.

#### 11.4.0 Replication policy (Sprint 048)

Every VM has a derived replication cadence:

```
cadence(vm) = explicit replicate cadence   (if set on vms.<name> or applications.<app>)
           else 1m                         (if backup: true)
           else 1m                         (if env in {prod, shared})
           else 24h                        (if env == dev)
```

`replicate: false` is an explicit opt-out, not a default class. Override
ratification is comment-based: Sprint 048 Q-B case law in
`.claude/rules/design-taste.md` rejected metadata blocks such as
`replicate_override:`; a YAML comment above the `replicate:` line carries
the human justification.

The policy is enumerated by **one authority** —
`framework/scripts/list-replicated-vmids.sh` — and consumed by
`configure-replication.sh`, `repl-health.sh`, and `validate.sh`.
`configure-replication.sh` delivers `/etc/repl-policy.vmids` to each node
with `POLICY_ON_VMIDS`, `POLICY_OFF_VMIDS`, `CADENCE_MAP`, and
`POLICY_GEN`. Consumers use `CADENCE_MAP` to distinguish 1m strictness from
24h async jobs.

**Cadence bounds staleness.** The per-VM cadence and node artifact are the
primary knob. 1m jobs retain STRICT repl-health behavior, including fail-count
and never-run checks. 24h jobs use seed-progress WARN semantics during first
sync; once seeded, their acceptable stale window follows the cadence watchdog
rules rather than a minute-class freshness contract.

**The failover manifest is derived, not committed.** Q5/Q-C case law in
`.claude/rules/design-taste.md` rejected a committed failover-manifest doc and
drift ratchet because no automation may read it. The failover set is derived
at read time from `site/config.yaml` + `site/applications.yaml` by
`list-replicated-vmids.sh`. Visibility comes from consumer logs and the
`/etc/repl-policy.vmids` node artifact. A small `--render` inspection shim is
allowed if it remains a thin view over the same helper output.

**Recreate tooling is override-only.** `framework/scripts/recreate-derivable-vm.sh`
is no longer a default failover path for dev or derivable VMs. It is reserved
for VMs explicitly opted out with `replicate: false`; all other VM recovery
uses HA restart-from-replica plus the §6A/§6B ladder if HA needs operator
recovery.
...
The mechanism, downtime, and current limitations are detailed in §6.4
(HA Paths and Maintenance). In brief: the target mechanism is live
migration with sub-second downtime; the current delivered mechanism is
cold-fallback via HA migrate (~10–30 seconds per VM) because issue
#336 blocks live migration for cloud-init VMs. The closure-of-gap path
is in §6.4.4.

**Unplanned failover (node failure):**
1. Proxmox detects node failure via fencing/watchdog
2. Surviving CRM master selects a node with the ZFS replica and issues
   recovery
3. VM starts on that node from the replica
4. Total failover time: ~150 seconds measured (2026-05-22 power-loss
   test on 3 VMs, full breakdown in §6.4.1)
5. Data loss: up to the VM's declared replication cadence of writes on vdb
   (1m for strict jobs; 24h for daily-cadence jobs)
...
|----------|----------------------|-----------------|
| vda (OS disk zvol) | ZFS replication (per-VM cadence) | Automatic — Proxmox replication jobs |
| vdb (data disk zvol) | ZFS replication (per-VM cadence) | Automatic — Proxmox replication jobs |
...
- ZFS replication keeps replicas current to each VM's declared cadence
...
- One authority derives per-VM cadence; no committed failover manifest exists
...
│   │   └── checks/
│   │       ├── source-filter-check.nix  # Canary: image source filter not bypassed
│   │       └── per-role-isolation-check.nix  # Canary: per-role source isolation holds
│   ├── tofu/
│   │   ├── modules/
...
│   │   ├── check-source-filter.sh # Lint: verify image source filter integrity
...
**Why `site/` cannot be in `.gitignore`:** The Nix flake uses `self` as its source tree, which includes only git-tracked files. The per-role filtered sources (see section 11.1.2) further restrict this to nix-relevant files. `site/nix/hosts/*.nix` and `site/apps/*` are nix build inputs — if `site/` were gitignored, these files would be invisible to `self` and every image build would fail. The `.gitignore` must only target specific generated artifacts within `site/`, never the directory itself.
...
|-------|------|-----------------|---------------|
| VMs | `--vms` | All VMs, HA resources, replication jobs, OpenTofu state | ZFS pools, Proxmox cluster, **CIDATA snippets** (#347 — managed by tofu, re-uploaded on apply), NAS, workstation |
...
| Vault | 1 minute | ~15s (VM crash), <2 min (node failure) | ZFS replication (1m cadence). VM crash: HA restarts on same node (~10s) + auto-unseal (~5s). Node failure: HA migrates to survivor + auto-unseal. |
| Home Assistant | 1 minute | <2 minutes | ZFS replication (1m cadence). Proxmox HA restart (highest priority). |
| Databases (InfluxDB, etc.) | Declared cadence for node failure; 24 hours for PBS | <2 min (ZFS), <30 min (PBS restore) | ZFS replication for node failure. PBS for data corruption requiring point-in-time restore. |
| CI/CD | 24h cadence | <2 minutes after HA recovery; Mac fallback available immediately | ZFS replication at 24h for node failure. No precious state. |
| Gatus (monitoring) | 1 minute | <2 minutes | ZFS replication at prod default cadence. Sentinel on NAS covers the gap. |
...
The estimated ~45–90 second HA failover window referenced in earlier
parts of this document refers to the corosync detection + boot
portion of failover. The 2026-05-22 power-loss test (§6.4.1) measured
~150–160 seconds end-to-end including fence_delay; the ~120s
historical measurement above (7 VMs) excluded the fence_delay wait
or used a shorter configured value. See §6.4 for the full HA-paths
analysis with current numbers. Individual VM crash recovery (same-
node restart) remains much faster (~10–15 seconds).

**RPO note:** ZFS RPO for unplanned node failure is bounded by each VM's declared replication cadence. 1m jobs lose at most minute-class writes; 24h jobs intentionally accept daily staleness so they can still restart from a replica. For data corruption scenarios, RPO is bounded by PBS backup frequency (daily), meaning up to 24 hours of data loss if the corruption isn't caught before the next backup overwrites the good one. PBS retention policies (keeping multiple daily snapshots) mitigate this — you can restore from any retained snapshot, not just the most recent.
...
- Node evacuation (prod): planned migration of all VMs off one node (simulates rolling Proxmox update), verify all services continue operating from other nodes within expected downtime per §6.4 (currently ~10–30s per VM via cold-fallback; ~sub-second once #336 is resolved).
...
|-------|-------------------------|
| `--vms` | 0 VMs on any node. 0 HA resources. 0 replication jobs. 0 VM zvols on any node (no orphans from held snapshots). 0 replication snapshots. CIDATA snippets retained (tofu re-uploads on next apply, #347). No `pbs-nas` storage entry. No backup jobs. |
...
| Vault data | Raft snapshots → PBS | Daily | `vault-raft-snapshot` systemd timer on the Vault VM writes `/var/lib/vault/raft-snapshot.gz` before PBS backup (see "Vault backup mechanics" below in §16.3) |
...
The mechanism:
1. A `vault-raft-snapshot` systemd timer on the Vault VM runs daily (before the PBS backup window)
2. The service reads the root token from vdb and issues an authenticated `POST` to Vault's `/v1/sys/storage/raft/snapshot` HTTP API via `curl`, writing the snapshot body to `/var/lib/vault/raft-snapshot.gz`
...
5. On restore: if the live Raft data is consistent, Vault recovers normally. If not, the snapshot file provides a known-good fallback: `POST /v1/sys/storage/raft/snapshot-force` (or the `vault operator raft snapshot restore` CLI variant) reloads Raft from the snapshot

The snapshot service runs locally on the Vault VM using the HTTP API (per `.claude/rules/vault.md`: "Use the Vault HTTP API via curl, not the vault CLI, in scripts that run on NixOS VMs") — it authenticates with the Vault root token stored on vdb and has direct access to the Vault API via localhost, so no network dependency and no additional TLS setup.
...
- Pre-sync 24h-cadence jobs on the target node before maintenance when their
  daily delta matters
- Drain VMs through the HA-aware rolling reboot wrapper
- All VMs are HA-enabled; replicated VMs restart on survivors from their
  declared-cadence replicas. Only explicit `replicate: false` overrides use
  `framework/scripts/recreate-derivable-vm.sh`.
...
2. Evacuate: migrate all VMs off the target node (Proxmox live migration when available — currently cold-fallback per §6.4 due to issue #336)
...
| Redundant DNS per environment (dns1, dns2) | Single DNS | DNS failure is catastrophic, cost is trivial (~256MB RAM per VM), enables planned-maintenance windows with limited per-VM downtime (see §6.4) |
...
| Universal replication with per-VM cadence | Uniform 1m replication of all VMs; explicit opt-out/recreate | Sprint 048: every enabled VM gets replica-based failover unless explicitly opted out. Cadence controls cost and staleness: backup/prod/shared default to 1m, dev defaults to 24h, and overrides require YAML comments. See § 11.4.0. |
| Three-category data taxonomy | Backup everything / backup nothing | Derivable (vda, from Git), configuration (vdb subset, from Git), precious state (vdb subset, needs PBS). Determines PBS scope; replication/HA is now cadence-driven per § 11.4.0. |
...
| Per-role filtered nix sources for image builds | Use full flake source (`self`), or one shared filtered source | Without filtering, every tracked file is a nix derivation input — changing `.gitlab-ci.yml` or `CLAUDE.md` recreates every VM. A single shared filter fixes that but still rehashes every image whenever any one role's source changes. Per-role `builtins.path` sources (shared base + role-owned paths) make a one-role change rehash only that role's image. Missing required entries cause hard build failures (fail-closed); a static lint plus nix canary/isolation checks verify both bypass and per-role isolation. See section 11.1.2. |
...
19a. **Source filter integrity** - Static lint (`check-source-filter.sh`) and nix checks (`source-filter-check.nix`, `per-role-isolation-check.nix`) verify the filtered image sources are not bypassed and that per-role isolation holds in both directions
