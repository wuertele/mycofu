...
you can rebuild everything from a single config file and a single key.
...
a YAML config file and a rebuild script. The individual tools exist. The
...
operator's interface is a config file and a handful of scripts. The
complexity doesn't disappear — it moves into code that's tested once and
runs reliably thereafter.

You provide a config file describing your hardware, your network, and your
domain. The framework handles DNS, TLS certificates, secrets, VM images,
and orchestration. One config file. One key. One command to rebuild from
scratch.
...
- A VLAN-capable gateway/router
- A managed switch
- An off-cluster NAS (for state storage and backups)

**Configuration:**
- A registered domain name (required for TLS certificates)
- Your network details (subnets, VLANs, IPs)
- An SSH key

**Workstation:**
- macOS or Linux (both first-class)
- OpenTofu, SOPS, and Nix installed
...
**site/** is yours — generated by `new-site.sh` from the template, tracked
on your private remote only. The public repo's `.gitignore` excludes `site/`
entirely. `config.yaml` is read by scripts, tofu, and nix at build time.
...
Your interface is `config.yaml` and `rebuild-cluster.sh`. The architecture
doc explains every decision for when you want to understand — or for the rare
case where you need to debug something.
