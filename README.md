...
flowchart TB
    ...
    subgraph CLUSTER["Proxmox Cluster"]
        ...
        subgraph DEV["Development · VLAN 60"]
            dns1d["dns1-dev"]
            dns2d["dns2-dev"]
            vaultd["vault-dev"]
            pebble["pebble"]
...
a running cluster. The [Getting Started Guide](GETTING-STARTED.md)
...
Your private GitLab repo carries the `dev` and `prod` branches that include
the `site/` directory, configuration, and secrets. Every passing merge to
`prod` publishes a filtered framework-only commit back to GitHub `main`, so
the public branch is a mirror of prod-validated framework code and never
contains `site/`. See [architecture.md](architecture.md) for the full
versioning model.
...
on the NAS before recreated backup-backed VMs start. The rebuild verifies
that the restore succeeded before taking new backups — preventing the most
dangerous failure mode: overwriting a good backup with empty state from a
failed restore.
...
| Production recovery from PBS backups | `rebuild-cluster.sh` preboot restore path (`restore-before-start.sh`) | Full recovery, GitLab history and Vault state intact | — |
...
| [framework/docs/CONTRIBUTING.md](framework/docs/CONTRIBUTING.md) | External contributor | What belongs in a PR and how GitHub contributions flow back to prod |
...
**Contributing:** [framework/docs/CONTRIBUTING.md](framework/docs/CONTRIBUTING.md)
describes the public GitHub mirror, what can be proposed in a PR, and how
accepted GitHub changes are cherry-picked through the private `dev` and `prod`
branches before they are re-published.
